...
function captureStdoutStderr(): { restore: () => { stdout: string; stderr: string } } {
  ...
  console.log = (...args: unknown[]) => {
    outBuf += args.map(stringify).join(" ") + "\n";
  };
  console.error = (...args: unknown[]) => {
    errBuf += args.map(stringify).join(" ") + "\n";
  };
  ...
  test("--limit caps replay count", async () => {
    await withTmp(async (dir) => {
      const lines =
        [1, 2, 3, 4, 5]
          .map((i) =>
            JSON.stringify(makeCapturedRow({ id: i, query: `q${i}`, retrieved_slugs: ["a"] }))
          )
          .join("\n") + "\n";
      const file = join(dir, "baseline.ndjson");
      writeFileSync(file, lines);
...
describe("gbrain eval replay — failure modes", () => {
  ...
  test("schema_version === 2 (future) is rejected with helpful message", async () => {
    await withTmp(async (dir) => {
      ...
      writeFileSync(
        file,
        '{"schema_version":2,"id":1,"tool_name":"search","query":"q","retrieved_slugs":[],"latency_ms":0}\n'
      );
      const origExit = process.exit;
      let code: number | undefined;
      process.exit = (c?: number) => {
