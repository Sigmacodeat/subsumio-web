 ...
 */
// 2026-09-18 cost-aware routing (owner decision: Opus only where a question
// needs it, Fable never by default). think routes simple/moderate legal
// questions to `reasoning` and only complex ones (subsumption, cross-law) to
// `deep` (think/intent.ts). Per question: planner + completeness check on the
...
const OPENROUTER_TIER_DEFAULTS: Record<ModelTier, string> = {
  ...
  reasoning: "openrouter:anthropic/claude-sonnet-5",
 ...
 */
export const USER_MODEL_CHOICES: Record<string, { native: string; openrouter: string }> = {
  "claude-haiku-4-5": {
