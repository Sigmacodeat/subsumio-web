...
export const POST = createHandler(
  ...
  async (ctx, body, _query, req) => {
    const { slug: slugStr } = await ((req as unknown as { params: Promise<{ slug: string }> })).params;
    ...
    try {
      ...
      console.error("[agent-templates/run] load failed:", err instanceof Error ? err.message : String(err));
    ...
    if (Array.isArray(fm.force_specialists)) supervisorData.force_specialists = fm.force_specialists;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return Response.json(
          payload.error ? payload : { error: "run_failed" },
      ...
      console.error("[agent-templates/run] submit failed:", err instanceof Error ? err.message : String(err));
