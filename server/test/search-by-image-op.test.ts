...
const PNG_BYTES = Buffer.from([
  0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 0, 0, 0, 13, 73, 72, 68, 82, 0, 0, 0, 1, 0, 0, 0,
  1, 8, 6, 0, 0, 0, 31, 21, 196, 137, 0, 0, 0, 12, 73, 68, 65, 84, 8, 87, 99, 248, 207, 192, 0, 0,
  0, 3, 0, 1, 90, 12, 105, 240, 0, 0, 0, 0, 73, 69, 78, 68, 174, 66, 96, 130,
]);
...
describe("search_by_image op — D18 remote image_path ban", () => {
  ...
  test("accepts image_path when ctx.remote=false (local CLI)", async () => {
    const path = join(tmpRoot, "test.png");
    writeFileSync(path, PNG_BYTES);
    const results = await op().handler({ engine, remote: false } as any, { image_path: path });
...
describe("search_by_image op — input validation", () => {
  test("rejects missing all three inputs", async () => {
    const err = await op()
      .handler({ engine, remote: false } as any, {})
      .catch((e: any) => e as Error);
    expect(err).toBeInstanceOf(Error);
      ...
      .handler(
        {
          engine,
          remote: true,
          auth: { token: "t", clientId: "client_a", scopes: ["read"] },
        } as any,
        { image_data: PNG_BYTES.toString("base64") }
      )
      .catch((e: any) => e as Error);
