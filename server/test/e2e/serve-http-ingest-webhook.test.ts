...
describeE2E("serve-http POST /ingest webhook (v0.38)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!ready) {
      throw new Error(
        "Webhook E2E server failed to start within 15s.\nstderr: " + stderr.slice(-500)
      );
    }
  }, 30_000);
  ...
  test("text/plain accepted (in the v1 allowlist)", async () => {
    const token = await mintToken("read write");
    const res = await postIngest(token, "text/plain", `plain text webhook ${Date.now()}`);
    expect([200, 202]).toContain(res.status);
  ...
  test("text/html accepted (in the v1 allowlist)", async () => {
    const token = await mintToken("read write");
    const res = await postIngest(token, "text/html", `<p>html webhook ${Date.now()}</p>`);
    expect([200, 202]).toContain(res.status);
  });
  ...
  test("X-Gbrain-Content-Type header overrides request Content-Type", async () => {
    ...
    const res = await postIngest(token, "application/octet-stream", "# override via header", {
  ...
  test("X-Gbrain-Slug header is accepted (job receives the slug hint)", async () => {
    ...
    const res = await postIngest(token, "text/markdown", "# slug header test", {
      "X-Gbrain-Slug": slug,
    });
    expect([200, 202]).toContain(res.status);
  ...
  test("X-Gbrain-Source-Id header is accepted", async () => {
    const token = await mintToken("read write");
    const res = await postIngest(token, "text/markdown", "# source-id header test", {
  ...
  test("X-Gbrain-Source-Uri header is accepted", async () => {
    const token = await mintToken("read write");
    const res = await postIngest(token, "text/markdown", "# source-uri header test", {
  ...
  test("different content from same client → different job_id", async () => {
    const token = await mintToken("read write");
    const first = await postIngest(token, "text/markdown", `# distinct A ${Date.now()}`);
    const second = await postIngest(token, "text/markdown", `# distinct B ${Date.now()}`);
    const firstBody = (await first.json()) as { job_id?: number | string };
    const secondBody = (await second.json()) as { job_id?: number | string };
