...
describeFn("v0.40.4 — graph_signals eval gates (longmemeval-mini A/B)", () => {
  ...
  test("Gate 1 (QUALITY): no statistically significant regression in wrong direction", () => {
    ...
    if (process.env.GBRAIN_EVAL_VERBOSE) {
      console.log(
        `Gate 1: observedDelta=${result.observedDelta.toFixed(3)}, p=${result.pValue.toFixed(3)}`
      );
    }
...
describe("pairedBootstrapPValue — pure-function tests", () => {
  ...
  test("determinism: same seed → same p-value", () => {
    const pairs = Array(20)
      .fill(0)
      .map((_, i) => ({ on: i % 3 === 0 ? 1 : 0, off: i % 4 === 0 ? 1 : 0 }));
    let s1 = 12345;
    const r1 = pairedBootstrapPValue(pairs, 1000, () => {
      s1 = (s1 * 1664525 + 1013904223) >>> 0;
      return s1 / 0xffffffff;
    });
    let s2 = 12345;
    const r2 = pairedBootstrapPValue(pairs, 1000, () => {
