...
describe("extractTitleFromFilename", () => {
  test("strips date prefix and cleans up", () => {
    expect(extractTitleFromFilename("2010-04-13 Apr 13 founders mtg.md")).toBe(
      "Apr 13 founders mtg"
    );
  });
...
describe("inferFrontmatter", () => {
  ...
  test("Apple Notes/YC: adds yc tag", () => {
    const result = inferFrontmatter("Apple Notes/YC/2022-08-04 Project 1783Y.md", "Some content");
    expect(result.type).toBe("apple-note");
  ...
  test("people/ directory: falls back to filename when no heading", () => {
    const result = inferFrontmatter("people/john-doe.md", "Some text without a heading");
    expect(result.type).toBe("person");
    expect(result.title).toBe("John Doe");
  });

  test("personal/therapy: infers therapy-session type with date", () => {
    const result = inferFrontmatter("personal/therapy/jan/2024-01-30.md", "Session notes...");
  ...
  test("docs/projects: infers project type", () => {
    const result = inferFrontmatter("docs/projects/eva-brain.md", "# Eva Brain\n\nProject notes.");
  ...
  test("unknown directory: catch-all remains note when explicitly used by callers", () => {
    const result = inferFrontmatter("random/some-file.md", "# My Random Notes\n\nStuff here");
