    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        JSON.stringify({
          ...
          acknowledged_at: "2025-02-01T00:00:00Z",
        }) +
        "\n"
    );
    const rows = loadSyncFailures();
...
describe("#1 severity — auto_skipped stays visible (WARN)", () => {
  test("decideSyncFailureSeverity branches", async () => {
    ...
    const tenSkipped = Array.from({ length: 10 }, (_, i) => ({
      ...base,
      path: `s${i}.md`,
      state: "auto_skipped" as const,
      ts: "2026-06-06T23:00:00Z",
    }));
...
describe("decideGateAction — full branch table", () => {
  test("all branches", async () => {
    const { decideGateAction } = await L();
    const mk = (over: Partial<Parameters<typeof decideGateAction>[0]>) =>
      decideGateAction({
        fileFailures: [],
        sentinels: [],
        attemptsByPath: new Map(),
    ...
    expect(mk({ fileFailures: [{ path: "a" }], attemptsByPath: new Map([["a", 1]]) }).action).toBe(
      "block"
    ); // fresh
...
describe("#5 + #6 applySyncFailureGate orchestration", () => {
  test("advance_then_autoskip after threshold; advance runs before auto-skip mark", async () => {
    ...
    const run = () =>
      applySyncFailureGate({
        sourceId: "s",
        failedFiles: [{ path: "poison.md", error: "YAML parse failed" }],
        succeededPaths: [],
        commit: "c",
        skipFailed: false,
        advance: async () => {
  ...
  test("a throwing advance() marks nothing auto_skipped (atomicity)", async () => {
    ...
    await expect(
      applySyncFailureGate({
        sourceId: "s",
        failedFiles: [{ path: "poison.md", error: "e" }],
        succeededPaths: [],
        commit: "c",
        skipFailed: false,
        advance: async () => {
  ...
  test("sentinel hard-blocks even with skipFailed; no advance", async () => {
    ...
    const r = await applySyncFailureGate({
      sourceId: "s",
      failedFiles: [{ path: "<head>", error: "history rewrite" }],
      succeededPaths: [],
      commit: "c",
      skipFailed: true,
      advance: async () => {
