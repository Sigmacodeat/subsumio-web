...
describe("idempotencyKey", () => {
  ...
  test("different source produces different key", () => {
    expect(idempotencyKey("A", "sync", { x: 1 })).not.toBe(idempotencyKey("B", "sync", { x: 1 }));
  });

  test("different job produces different key", () => {
    expect(idempotencyKey("default", "sync", { x: 1 })).not.toBe(
      idempotencyKey("default", "embed", { x: 1 })
    );
  });

  test("different params produce different key", () => {
    expect(idempotencyKey("default", "sync", { x: 1 })).not.toBe(
      idempotencyKey("default", "sync", { x: 2 })
    );
  });
});
