...
describe("Cathedral II Layer 3 — searchKeyword external contract", () => {
  ...
  beforeAll(async () => {
    ...
    await engine.upsertChunks("guides/refactor-large-fns", [
      {
        chunk_index: 0,
        chunk_text: "First, refactor the function into smaller units.",
        chunk_source: "compiled_truth",
      },
      {
        chunk_index: 1,
        chunk_text: "Then refactor further using extract-method patterns.",
        chunk_source: "compiled_truth",
      },
    ]);
    ...
    await engine.upsertChunks("guides/refactor-patterns", [
      {
        chunk_index: 0,
        chunk_text: "The strangler-fig refactor is the safest approach.",
        chunk_source: "compiled_truth",
      },
      {
        chunk_index: 1,
        chunk_text: "Refactor incrementally; never boil the ocean at once.",
        chunk_source: "compiled_truth",
      },
    ]);
    ...
    await engine.upsertChunks("guides/unrelated", [
      {
        chunk_index: 0,
        chunk_text: "Ship to production on a Tuesday, never a Friday.",
        chunk_source: "compiled_truth",
      },
    ]);
  });
...
describe("Cathedral II Layer 3 — searchKeywordChunks (internal primitive)", () => {
  ...
  beforeAll(async () => {
    ...
    await engine.upsertChunks("guides/multi-chunk", [
      {
        chunk_index: 0,
        chunk_text: "refactor is a core engineering practice.",
        chunk_source: "compiled_truth",
      },
      {
        chunk_index: 1,
        chunk_text: "refactor safely using characterization tests.",
        chunk_source: "compiled_truth",
      },
      {
        chunk_index: 2,
        chunk_text: "refactor tools can automate common patterns.",
        chunk_source: "compiled_truth",
      },
    ]);
  });
...
describe("Cathedral II Layer 3 — doc-comment weight precedence (A4 foundation)", () => {
  ...
  beforeAll(async () => {
    ...
    await engine.upsertChunks("pages/alpha", [
      {
        chunk_index: 0,
        chunk_text: "Some boilerplate text without the term.",
        chunk_source: "compiled_truth",
      },
    ]);
    ...
    await engine.upsertChunks("pages/beta", [
      {
        chunk_index: 0,
        chunk_text: "The hexagon term appears only inside body text here.",
        chunk_source: "compiled_truth",
      },
    ]);
