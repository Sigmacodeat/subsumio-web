...
describe("operations contract — every op has scope + correct mutability shape", () => {
  ...
  test('every mutating op has a write-class scope (not "read")', () => {
    const WRITE_CLASS_SCOPES = new Set(["write", "admin", "sources_admin", "users_admin", "agent"]);
    for (const op of operations) {
      if (op.mutating === true) {
        expect(
  ...
  test("scope is one of the documented enum values", () => {
    ...
    for (const op of operations) {
      expect(KNOWN_SCOPES.has(op.scope!), `op "${op.name}" has unknown scope "${op.scope}"`).toBe(
        true
      );
    }
  });
