  ...
  test("all six chat-capable providers ship a chat touchpoint with supports_subagent_loop", () => {
    ...
    for (const id of expected) {
      ...
      expect(
        ...
        `${id} should support subagent loop`
      ).toBe(true);
    }
  });
...
describe("chat touchpoint — gateway config plumbing", () => {
  ...
  test("chat_fallback_chain plumbed and retrievable", () => {
    configureGateway({
      chat_fallback_chain: ["anthropic:claude-opus-4-7", "deepseek:deepseek-chat"],
      env: {},
    });
    expect(getChatFallbackChain()).toEqual(["anthropic:claude-opus-4-7", "deepseek:deepseek-chat"]);
  });
