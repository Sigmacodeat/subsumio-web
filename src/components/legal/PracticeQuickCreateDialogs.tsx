      ...
      <DialogContent className="max-w-xl">
        <form onSubmit={submit}>
          ...
          <div className="grid gap-4 py-5">
            ...
            <div className="grid gap-1.5"><Label htmlFor="practice-case">{t("practice.case_optional")}</Label><select id="practice-case" value={caseSlug} onChange={(e) => setCaseSlug(e.target.value)} className="h-10 rounded-md border border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] px-3 text-sm"><option value="">{t("practice.no_case")}</option>{cases.map((item) => <option key={item.slug} value={item.slug}>{item.title}</option>)}</select></div>
