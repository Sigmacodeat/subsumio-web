...
describeE2E("E2E: runCycle against real Postgres", () => {
  ...
  test("dry-run full cycle: zero DB writes + zero filesystem changes", async () => {
    ...
    const beforeSync = await conn.unsafe(`SELECT value FROM config WHERE key = 'sync.last_commit'`);
    ...
    const afterSync = await conn.unsafe(`SELECT value FROM config WHERE key = 'sync.last_commit'`);
  ...
  test("live cycle: pages get synced + chunks created + cycle lock cleaned up", async () => {
    ...
    const sync = await conn.unsafe(`SELECT value FROM config WHERE key = 'sync.last_commit'`);
