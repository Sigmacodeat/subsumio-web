...
function parseRedlineResponse(text: string): RedlineClause[] {
  ...
  for (const line of lines) {
    ...
    if (/^(Risk|Risiko):/i.test(trimmed)) {
      ...
      current.risk =
        level.includes("high") || level.includes("hoch")
          ? "high"
          : level.includes("low") || level.includes("niedrig")
            ? "low"
            : "medium";
      section = "";
      continue;
...
export function ContractRedlineViewer({
  ...
  useEffect(() => {
    ...
    function handleKey(e: KeyboardEvent) {
      if (e.key === "ArrowDown" || e.key === "j") {
        ...
        e.preventDefault();
        setClauses((prev) =>
          prev.map((c, i) => (i === activeClause ? { ...c, accepted: !c.accepted } : c))
        );
      }
    }
  ...
  const reviewedCount = useMemo(
    () => clauses.filter((c) => c.accepted !== undefined).length,
    [clauses]
  );
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-black/60 backdrop-blur-sm">
      ...
      <div className="flex items-center justify-between border-b border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] px-6 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="flex h-8 w-8 items-center justify-center rounded-lg text-[color:var(--ds-text-muted)] transition-[background-color,border-color,color] hover:bg-[color:var(--ds-hover)] hover:text-[color:var(--ds-text)]"
      ...
      {clauses.length > 0 && (
        <div className="flex items-center gap-4 border-b border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] px-6 py-2.5">
          ...
          <button
            onClick={acceptAll}
            className="rounded-lg px-2 py-1 text-xs font-medium text-emerald-600 transition-[background-color,border-color,color] hover:bg-emerald-500/10 hover:text-emerald-700"
          >
            Alle akzeptieren
          </button>
          <button
            onClick={rejectAll}
            className="rounded-lg px-2 py-1 text-xs font-medium text-red-600 transition-[background-color,border-color,color] hover:bg-red-500/10 hover:text-red-700"
          >
            Alle ablehnen
          </button>
        </div>
            ...
            {clauses.map((clause, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {clause.collapsed ? (
                      <ChevronRight size={14} className="text-[color:var(--ds-text-muted)]" />
                    ) : (
                      <ChevronDown size={14} className="text-[color:var(--ds-text-muted)]" />
                    )}
                    ...
                    <Badge
                      variant="default"
                      className={cn("ml-auto text-xs", riskCfg.bg, riskCfg.border, riskCfg.color)}
                    >
                      {riskCfg.label}
                    </Badge>
                  ...
                  {!clause.collapsed && (
                    <>
                      ...
                      <div className="grid grid-cols-1 divide-x divide-[color:var(--ds-border)] md:grid-cols-2">
                      ...
                      {/* Reason */}
                      {clause.reason && (
                        <div className="border-t border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] px-4 py-2.5">
                      ...
                      <div className="flex items-center gap-2 border-t border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] px-4 py-2">
                        <button
                          onClick={() => acceptClause(idx)}
                          className={cn(
                            "flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-medium transition-[background-color,border-color,color]",
                            clause.accepted === true
