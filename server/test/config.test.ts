...
const configSource = readFileSync(new URL("../src/commands/config.ts", import.meta.url), "utf-8");
...
describe("redactConfigValue (v0.36.x #892 — set output regression)", () => {
  ...
  test("redacts postgresql URL passwords regardless of key", () => {
    expect(redactConfigValue("database_url", "postgresql://u:secret@h:5432/d")).toBe(
  ...
  test("non-sensitive values pass through unchanged", () => {
    ...
    expect(redactConfigValue("embedding_model", "voyage:voyage-3-large")).toBe(
...
describe("loadConfig — GBRAIN_MAX_MARKUP_RATIO env (v0.42 #1699)", () => {
  async function withHomeAndEnv(
    ...
    try {
      ...
      writeFileSync(
        join(tmpHome, ".gbrain", "config.json"),
        JSON.stringify({ engine: "pglite", database_path: "/tmp/x" })
      );
  ...
  test("valid ratio in (0,1] is parsed onto content_sanity", async () => {
    await withHomeAndEnv({ GBRAIN_MAX_MARKUP_RATIO: "0.7" }, (cfg) => {
      expect(
        (cfg as { content_sanity?: { max_markup_ratio?: number } }).content_sanity?.max_markup_ratio
      ).toBe(0.7);
    });
  });
