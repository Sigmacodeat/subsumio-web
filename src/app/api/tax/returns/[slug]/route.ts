...
const updateSchema = z.object({
  ...
  assessmentNotice: z.string().nullable().optional(),
...
export const GET = createHandler(
  ...
  async (ctx, _body, _query, req) => {
    ...
    if (!page) return apiError("not_found", "Tax return not found", 404);
...
export const PATCH = createHandler(
  {
    ...
    body: updateSchema,
    audit: (ctx) => ({
      action: "tax.return_update" as const,
  ...
  async (ctx, body, _query, req) => {
    ...
    if (!existing) return apiError("not_found", "Tax return not found", 404);
    ...
    const updatedFm: Record<string, unknown> = {
      ...
      ...(body.type !== undefined ? { tax_type: body.type } : {}),
      ...(body.year !== undefined ? { year: body.year } : {}),
      ...(body.status !== undefined ? { status: body.status } : {}),
      ...(body.dueDate !== undefined ? { due_date: body.dueDate } : {}),
      ...(body.submittedDate !== undefined ? { submitted_date: body.submittedDate } : {}),
      ...(body.assessedDate !== undefined ? { assessed_date: body.assessedDate } : {}),
      ...(body.assessmentNotice !== undefined ? { assessment_notice: body.assessmentNotice } : {}),
      ...(body.taxAmount !== undefined ? { tax_amount: body.taxAmount } : {}),
    ...
    const title = `${updatedFm.tax_type ?? fm.tax_type} ${updatedFm.year ?? fm.year} — ${updatedFm.client_name ?? fm.client_name}`;
...
export const DELETE = createHandler(
  {
    ...
    rateTier: "standard",
    audit: (ctx) => ({
      action: "tax.return_delete" as const,
      entityType: "tax_return",
      details: { by: ctx.user.email },
    }),
  },
