...
describe("redactSecretsInText", () => {
  ...
  test("multiple occurrences of the same secret all redacted", () => {
    ...
    expect(out).toBe("first: <REDACTED:database_url>, again: <REDACTED:database_url>, done.");
  });

  test("multiple secrets each independently redacted", () => {
    const text = "connecting to postgresql://x:y@h/d with key sk-ant-test123 done";
    const out = redactSecretsInText(
      text,
      new Map([
    ...
    expect(out).toBe(
      "connecting to <REDACTED:database_url> with key <REDACTED:anthropic_api_key> done"
    );
  });
  ...
  test("regex metacharacters in value are treated as literal", () => {
    ...
    const out = redactSecretsInText(`dump: ${trickyValue} end`, new Map([["foo", trickyValue]]));
