 ...
 */
export async function discoverPoolerUrl(token: string, projectRef: string): Promise<string> {
  const res = await fetch(`https://api.supabase.com/v1/projects/${projectRef}/database`, {
  ...
  if (!res.ok) {
    if (res.status === 401)
      throw new Error(
        "Invalid Supabase access token. Generate one at supabase.com/dashboard/account/tokens"
      );
    if (res.status === 404)
      throw new Error(`Project not found: ${projectRef}. Check the project URL.`);
  ...
  const settingsRes = await fetch(`https://api.supabase.com/v1/projects/${projectRef}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
