...
export async function withMutation(
  ...
  return await withPackLock(packName, opts, async () => {
    ...
    try {
      ...
      const wrapped =
        e instanceof SchemaPackMutationError
          ? e
    ...
    if (!lintReport.ok) {
      ...
      const err = new SchemaPackMutationError(
        "INVALID_RESULT",
        `mutation would produce invalid pack: ${msg}`,
    ...
    try {
      ...
      /* swallow — cache invalidation must not block mutation success */
    }
    if (opts.engine) {
      try {
...
function checkNoReferences(manifest: SchemaPackManifest, typeName: string): void {
  ...
  for (const lt of manifest.link_types) {
    ...
    if (lt.inference?.target_type === typeName)
      refs.push(`link_type ${lt.name}.inference.target_type`);
  }
...
export async function addTypeToPack(
  packName: string,
  opts: AddTypeOpts,
  ...
  return withMutation(
    ...
    "add_type",
    { type: opts.name, prefix: opts.prefix }
  );
}

export async function removeTypeFromPack(
  packName: string,
  name: string,
  mutateOpts: MutateOpts = {}
): Promise<MutateResult> {
...
export async function updateTypeOnPack(
  packName: string,
  opts: UpdateTypeOpts,
  mutateOpts: MutateOpts = {}
): Promise<MutateResult> {
...
export async function addAliasToType(
  packName: string,
  typeName: string,
  alias: string,
  mutateOpts: MutateOpts = {}
...
export async function removeAliasFromType(
  packName: string,
  typeName: string,
  alias: string,
  mutateOpts: MutateOpts = {}
...
export async function addPrefixToType(
  packName: string,
  typeName: string,
  prefix: string,
  mutateOpts: MutateOpts = {}
  ...
  return withMutation(
    ...
    "add_prefix",
    { type: typeName, prefix }
  );
}

export async function removePrefixFromType(
  packName: string,
  typeName: string,
  prefix: string,
  mutateOpts: MutateOpts = {}
  ...
  return withMutation(
    ...
    (m) => {
      const t = findType(m, typeName);
      if (!t.path_prefixes.includes(prefix)) return m;
      const next: PackPageType = {
        ...t,
        path_prefixes: t.path_prefixes.filter((p) => p !== prefix),
      };
      return { ...m, page_types: m.page_types.map((pt) => (pt.name === typeName ? next : pt)) };
    },
    "remove_prefix",
...
export async function addLinkTypeToPack(
  packName: string,
  opts: AddLinkTypeOpts,
  mutateOpts: MutateOpts = {}
): Promise<MutateResult> {
...
export async function removeLinkTypeFromPack(
  packName: string,
  linkName: string,
  mutateOpts: MutateOpts = {}
): Promise<MutateResult> {
...
export async function setExtractableOnType(
  packName: string,
  typeName: string,
  value: boolean,
  mutateOpts: MutateOpts = {}
): Promise<MutateResult> {
  return updateTypeOnPack(
    packName,
    { name: typeName, patch: { extractable: value } },
    { ...mutateOpts }
  );
}

export async function setExpertRoutingOnType(
  packName: string,
  typeName: string,
  value: boolean,
  mutateOpts: MutateOpts = {}
  ...
  return updateTypeOnPack(
    packName,
    { name: typeName, patch: { expert_routing: value } },
    { ...mutateOpts }
  );
}
