...
describe("self-consumption guard (v0.23.2 marker-based)", () => {
  test("REGRESSION: catches actual reverseWriteSlugs output from a real Page", () => {
    ...
    const page = {
      ...
      compiled_truth:
        "I learned something about [Alice](people/alice). No own-slug citation in body.",
      timeline: "",
      frontmatter: {},
    };
  ...
  function mockClient(captured: { model?: string }): JudgeClient {
    return {
      create: async (p: any) => {
        ...
        return {
          content: [{ type: "text", text: '{"worth_processing": true, "reasons": ["test"]}' }],
        } as any;
      },
    };
  }
...
describe("judgeSignificance — UTF-16 safety (v0.41.13)", () => {
  ...
  function makeCapturingClient(): {
    client: JudgeClient;
    captured: { userMessage: string | null };
  } {
    ...
    const client: JudgeClient = {
      create: async (p: any) => {
        ...
        return {
          content: [{ type: "text", text: '{"worth_processing": false, "reasons": ["stub"]}' }],
        } as any;
      },
    };
   ...
   */
  function scanForUnpairedSurrogates(
    s: string
  ): { index: number; kind: "lone-high" | "lone-low" } | null {
  ...
  function makeLongTranscript(
    content: string
  ): import("../src/core/cycle/transcript-discovery.ts").DiscoveredTranscript {
    return {
