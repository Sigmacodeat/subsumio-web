...
describe("inferTypeAndSubtypeFromPack", () => {
  it("returns concept for missing path", () => {
    expect(
      inferTypeAndSubtypeFromPack(undefined, {
  ...
  it("returns concept fallback when no prefix matches", () => {
    expect(
      inferTypeAndSubtypeFromPack("foo/bar.md", {
        page_types: [{ name: "media", path_prefixes: ["/media/"] }],
      })
    ).toEqual({ type: "concept" });
  });
  ...
  it("matches prefix → returns canonical type", () => {
    expect(
      inferTypeAndSubtypeFromPack("media/x.md", {
        page_types: [{ name: "media", path_prefixes: ["/media/"] }],
      })
    ).toEqual({ type: "media" });
  });
  ...
  it("fires subtype rule from path_pattern (D5)", () => {
    expect(
      inferTypeAndSubtypeFromPack("videos/x.md", {
        page_types: [
          {
            name: "media",
  ...
  it("fires subtype rule from frontmatter (D5)", () => {
          ...
          page_types: [
            {
              ...
              subtypes: [
                ...
                {
                  name: "single",
                  when: { frontmatter_field: "thread_length", frontmatter_value: 1 },
                },
              ],
            },
          ],
    ...
    ).toEqual({ type: "tweet", subtype: "bundle" });
  });
  ...
  it("first-prefix-wins ordering", () => {
    expect(
      inferTypeAndSubtypeFromPack("wiki/concepts/foo.md", {
        page_types: [
  ...
  it("malformed regex in path_pattern is silently skipped", () => {
    expect(
      inferTypeAndSubtypeFromPack("foo/x.md", {
        page_types: [
          {
            name: "media",
            path_prefixes: ["/foo/"],
