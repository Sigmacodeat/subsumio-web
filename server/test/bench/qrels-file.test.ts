...
describe("qrels-file: parser", () => {
  test("parses the existing legacy fixture shape (relevant_slugs + first_relevant_slug)", () => {
    ...
    expect(parsed.queries[0]!.expected_top1).toEqual({
      source_id: "default",
      slug: "people/alice",
    });
  });
...
describe("qrels-file: math", () => {
  ...
  test("DEFAULT_QRELS_THRESHOLDS shape and values", () => {
    expect(DEFAULT_QRELS_THRESHOLDS.recall_at_k).toBe(0.7);
    expect(DEFAULT_QRELS_THRESHOLDS.first_relevant_hit).toBe(0.6);
    expect(DEFAULT_QRELS_THRESHOLDS.expected_top1).toBe(0.5);
