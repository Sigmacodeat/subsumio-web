...
describe("v0.41.31 — willEmbedSynchronously (embed-mode resolver)", () => {
  ...
  test("v2 off → inline (legacy synchronous embed)", () => {
    expect(willEmbedSynchronously({ v2Enabled: false, serialFlag: false, noEmbed: false })).toBe(
      "inline"
    );
  });
  test("v2 on + parallel → deferred (backfill jobs)", () => {
    expect(willEmbedSynchronously({ v2Enabled: true, serialFlag: false, noEmbed: false })).toBe(
      "deferred"
    );
  });
  test("v2 on + --serial → inline", () => {
    expect(willEmbedSynchronously({ v2Enabled: true, serialFlag: true, noEmbed: false })).toBe(
      "inline"
    );
  });
  test("--no-embed forces deferred regardless of v2/serial", () => {
    expect(willEmbedSynchronously({ v2Enabled: false, serialFlag: false, noEmbed: true })).toBe(
      "deferred"
    );
    expect(willEmbedSynchronously({ v2Enabled: true, serialFlag: true, noEmbed: true })).toBe(
      "deferred"
    );
  });
});
