...
describe("file-resolver", () => {
  ...
  test("resolves via .redirect breadcrumb", async () => {
    ...
    writeFileSync(
      join(brainRoot, "people/redirected.json.redirect"),
  ...
  test("throws when redirect exists but no storage backend", async () => {
    writeFileSync(
      join(brainRoot, "people/no-storage.json.redirect"),
      "moved_to: supabase\nbucket: test\npath: test.json\nmoved_at: 2026-04-09\noriginal_hash: sha256:abc\n"
    );
  ...
  test("blocks resolveFile path traversal at root level", async () => {
    await expect(resolveFile("../../etc/passwd", brainRoot, storage)).rejects.toThrow(
  ...
  test("blocks .supabase marker with traversal prefix", async () => {
    ...
    writeFileSync(
      join(subDir, ".supabase"),
      "synced_at: 2026-04-12\nbucket: brain-files\nprefix: ../../etc/\nfile_count: 1\n"
    );
    await expect(resolveFile("poisoned/secret.json", brainRoot, storage)).rejects.toThrow(
      "marker prefix contains path traversal"
    );
  });
  ...
  test("blocks .supabase marker with absolute path prefix", async () => {
    ...
    await expect(resolveFile("abs/passwd", brainRoot, storage)).rejects.toThrow(
      "marker prefix contains path traversal"
    );
  });
...
describe("parseMarker", () => {
  ...
  test("parses .supabase marker YAML", () => {
    ...
    writeFileSync(
      path,
      "synced_at: 2026-04-09T14:58:00Z\nbucket: brain-files\nprefix: people/.raw/\nfile_count: 484\n"
    );
