...
describe("parseConversationMessages", () => {
  ...
  test("ignores leading orphan lines (no anchor message yet)", () => {
    const body = [
      "orphan one",
      "orphan two",
      fmt("Alice Example", "2024-03-15", "9:00 AM", "real"),
    ].join("\n");
    const msgs = parseConversationMessages(body);
    expect(msgs).toHaveLength(1);
...
describe("extractConversationFactsFingerprint (Eng-v2 A3)", () => {
  test("same sourceId yields same fingerprint", () => {
    expect(extractConversationFactsFingerprint({ sourceId: "default" })).toBe(
      extractConversationFactsFingerprint({ sourceId: "default" })
    );
  });

  test("different sourceId yields different fingerprint", () => {
    expect(extractConversationFactsFingerprint({ sourceId: "a" })).not.toBe(
      extractConversationFactsFingerprint({ sourceId: "b" })
    );
  });
});
...
describe("checkpoint entry encoding", () => {
  ...
  test("slug with forward slashes survives encoding (no pipe collision)", () => {
    const entry = encodeCheckpointEntry(
      "src-a",
      "conversations/group/2024/march/team-x",
      "2024-03-16T08:05:00Z"
    );
    const decoded = decodeCheckpointEntry(entry);
...
const SAMPLE_BODY = [
  fmt(
    ...
    "2024-03-15",
    "9:00 AM",
    "Hi, I just signed the offer letter for Acme Corp."
  ),
...
describe("runExtractConversationFactsCore", () => {
  ...
  beforeEach(async () => {
    ...
    await engine.executeRaw(
      `DELETE FROM facts WHERE source LIKE 'cli:extract-conversation-facts%'`
    );
    await engine.executeRaw(`DELETE FROM op_checkpoints WHERE op = 'extract-conversation-facts'`);
    await engine.executeRaw(
      `DELETE FROM pages WHERE slug LIKE 'conversations/%' OR slug LIKE 'people/alice%'`
    );
