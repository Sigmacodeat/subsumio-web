...
Integrationen mit Code-Beleg: DocuSign, E-Mail-Import als Datei-Upload (.eml/.msg — KEIN IMAP-Postfachabruf, am 17.09. im Code geprüft), WhatsApp Business, Word-Add-in (Projekt `word-addin/` mit Manifest), SSO/SAML über WorkOS (Enterprise). DocuSign: `src/lib/docusign.ts` + Routen. OHNE Beleg: webERV-Anbindung, RA-Micro-Import, BMD/RZL-Export — melden.
