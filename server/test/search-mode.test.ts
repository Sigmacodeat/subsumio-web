...
describe("knobsHash determinism + cross-mode separation (CDX-4)", () => {
  ...
  test("T1 (codex): floor_ratio set vs unset produces DIFFERENT hashes (cache contamination prevention)", () => {
    ...
    const withFloor = knobsHash(
      resolveSearchMode({ mode: "balanced", perCall: { floor_ratio: 0.85 } })
    );
  ...
  test("T1 (codex): different floor_ratio values produce different hashes", () => {
    ...
    const b = knobsHash(resolveSearchMode({ mode: "balanced", perCall: { floor_ratio: 0.9 } }));
    expect(a).not.toBe(b);
  });
...
describe("loadOverridesFromConfig flat-map parser", () => {
  ...
  test("numeric keys parse and clamp", () => {
    expect(
      loadOverridesFromConfig({ "search.cache.similarity_threshold": "0.95" })
        .cache_similarity_threshold
    ).toBe(0.95);
    expect(loadOverridesFromConfig({ "search.cache.ttl_seconds": "7200" }).cache_ttl_seconds).toBe(
      7200
    );
  ...
  test("similarity_threshold rejects values outside (0, 1]", () => {
    expect(
      loadOverridesFromConfig({ "search.cache.similarity_threshold": "1.5" })
        .cache_similarity_threshold
    ).toBeUndefined();
    expect(
      loadOverridesFromConfig({ "search.cache.similarity_threshold": "0" })
        .cache_similarity_threshold
    ).toBeUndefined();
    expect(
      loadOverridesFromConfig({ "search.cache.similarity_threshold": "-0.1" })
        .cache_similarity_threshold
    ).toBeUndefined();
  });
...
describe("v0.40.4 — graph_signals knob", () => {
  ...
  test("knobsHash distinct for graph_signals=true vs =false", () => {
    ...
    const off = knobsHash(
      resolveSearchMode({ mode: "balanced", perCall: { graph_signals: false } })
    );
...
describe("v0.42.3.0 — autocut knobs", () => {
  ...
  test("resolveSearchMode threads autocut: per-call > config > bundle", () => {
    // per-call wins
    expect(resolveSearchMode({ mode: "balanced", perCall: { autocut: false } }).autocut).toBe(
      false
    );
    // config override wins over bundle
    expect(resolveSearchMode({ mode: "balanced", overrides: { autocut: false } }).autocut).toBe(
      false
    );
    ...
    expect(
      resolveSearchMode({
        mode: "balanced",
        overrides: { autocut: false },
        perCall: { autocut: true },
      }).autocut
    ).toBe(true);
    // jump knob threads too
    expect(
      resolveSearchMode({ mode: "balanced", perCall: { autocut_jump: 0.5 } }).autocut_jump
    ).toBe(0.5);
  });
...
describe("v0.43 — relational recall knobs", () => {
  ...
  test("per-call relationalRetrieval:false overrides bundle/config true", () => {
    ...
    expect(
      resolveSearchMode({ mode: "balanced", perCall: { relationalRetrieval: false } })
        .relationalRetrieval
    ).toBe(false);
    ...
    expect(
      resolveSearchMode({
        mode: "conservative",
        overrides: { relationalRetrieval: true },
        perCall: { relationalRetrieval: false },
      }).relationalRetrieval
    ).toBe(false);
    // config override alone flips conservative on
    expect(
      resolveSearchMode({ mode: "conservative", overrides: { relationalRetrieval: true } })
        .relationalRetrieval
    ).toBe(true);
  });
  ...
  test("loadOverridesFromConfig reads search.relational_retrieval(+_depth)", () => {
    const ov = loadOverridesFromConfig({
      ...
      "search.relational_retrieval_depth": "3",
    });
    ...
    expect(
      loadOverridesFromConfig({ "search.relational_retrieval_depth": "9" })
