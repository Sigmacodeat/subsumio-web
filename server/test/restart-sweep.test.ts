...
async function loadDetector(): Promise<any> {
  ...
  const m = md.match(/<!--\s*restart-sweep:script\s*-->\s*\n```javascript\s*\n([\s\S]+?)\n```/);
  if (!m) {
    throw new Error(
  ...
  test("filters Telegram group sessions for the configured group", async () => {
    await withEnv(
      ...
      async () => {
        ...
        expect(filtered.every((s: any) => s.key.includes("telegram:group:-1001234567890"))).toBe(
          true
        );
      }
    );
  ...
  test("returns empty Map + warns on corrupt JSON (D4)", async () => {
    ...
    await withEnv({ GBRAIN_HOME: stateRoot, OPENCLAW_TELEGRAM_GROUP: "-100" }, async () => {
      ...
      try {
        ...
        expect(map.size).toBe(0);
        expect(
          warnings.some((w) => w.includes("Failed to load") || w.includes("alerted.json"))
        ).toBe(true);
      } finally {
...
test("saveAlerted writes via tmp+rename (atomic, no leftover .tmp)", async () => {
  ...
  await withEnv({ GBRAIN_HOME: stateRoot, OPENCLAW_TELEGRAM_GROUP: "-100" }, async () => {
    ...
    d.alerted = new Map([
      [
        "session-1",
        { lastAlertedAt: new Date().toISOString(), restartTime: new Date().toISOString() },
      ],
    ]);
    await d.saveAlerted();
...
describe("cooldown layer (C1)", () => {
  ...
  test("cooldown suppresses re-alert within 6h even when restartTime is unstable (the C1 bug)", async () => {
    ...
    await withEnv({ GBRAIN_HOME: stateRoot, OPENCLAW_TELEGRAM_GROUP: "-100" }, async () => {
      ...
      d.alerted = new Map([[sessionKey, { lastAlertedAt: oneHourAgo, restartTime: "restart-A" }]]);
