 ...
 */
export const GET = createHandler({ action: "platform.operator" }, async (_ctx, _body, _query, req) => {
  ...
  if (day && !/^\d{4}-\d{2}-\d{2}$/.test(day)) return apiError("invalid_day", "Tag als JJJJ-MM-TT", 400);
