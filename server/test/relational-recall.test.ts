...
beforeAll(async () => {
  ...
  await eng.putPage("companies/widget-co", {
    type: "company",
    title: "Widget Co",
    compiled_truth: "A payments company.",
    timeline: "",
  });
  ...
  await eng.upsertChunks("people/alice-example", [
    {
      chunk_index: 0,
      chunk_text: "Alice is a seed-stage investor based in Lisbon.",
...
describe("buildRelationalArm", () => {
  ...
  test("non-relational query is a pure no-op", async () => {
    ...
    const list = await buildRelationalArm(eng, "summary of the payments roadmap", {
      onMeta: (m) => {
        meta.fired = m.fired;
      },
    });
