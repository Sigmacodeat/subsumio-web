...
import { resolvePhantomCanonical, findPrefixCandidates } from "../src/core/entities/resolve.ts";
import {
...
function withTempDirs<T>(
  fn: (dirs: { brainDir: string; auditDir: string }) => Promise<T>
): Promise<T> {
...
describe("stripFenceAndFrontmatterAndLeadingH1 (A3 + codex #2 body-shape gate)", () => {
  ...
  test("only an H1 + facts fence → empty residue (phantom with facts)", () => {
    ...
    const body = FACT_FENCE(`| 1 | A | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`);
...
` +
      FACT_FENCE(
        `| 1 | I prefer async | preference | 1.0 | private | medium | 2026-01-01 |  | OH |  |`
      );
    const residue = stripFenceAndFrontmatterAndLeadingH1(body);
...
` +
  ...
  test("codex #3/#12: lossless metadata preservation + dedup-guard on canonical", async () => {
    await withTempDirs(async ({ brainDir }) => {
      ...
      const facts2 = await engine.executeRaw<{
        valid_until: Date | null;
        visibility: string;
        notability: string;
        confidence: number;
      ...
      const canonicalKeyed = facts2.filter(
        (r) => r.source_markdown_slug === "people/alice-example"
      );
  ...
  test("codex #4 idempotency: second call after success is a clean no-op", async () => {
    await withTempDirs(async ({ brainDir }) => {
      ...
      const second = await engine.migrateFactsToCanonical(
        "alice",
  ...
  test("logPhantomEvent writes JSONL line; readRecent surfaces it", async () => {
    await withTempDirs(async () => {
      ...
      logPhantomEvent({
        outcome: "redirected",
        phantom_slug: "alice",
        canonical_slug: "people/alice-example",
        fact_count: 3,
        source_id: "default",
      });
      const events = readRecentPhantomEvents();
