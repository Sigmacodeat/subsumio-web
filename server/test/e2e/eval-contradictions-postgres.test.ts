...
describe("E2E: P2 persistent cache with real now()", () => {
  ...
  test("different prompt_version is a different cache key (Codex fix)", async () => {
    ...
    const wrong = await engine.getContradictionCacheEntry({
      chunk_a_hash: buildCacheKey({
        textA: "shared-a",
        textB: "shared-b",
        modelId: "haiku-pg-test",
      }).chunk_a_hash,
      chunk_b_hash: buildCacheKey({
        textA: "shared-a",
        textB: "shared-b",
        modelId: "haiku-pg-test",
      }).chunk_b_hash,
...
describe("E2E: find_contradictions MCP op on Postgres", () => {
  ...
  test("returns latest run findings with slug+severity filters", async () => {
        ...
        per_query: [
          {
            ...
            contradictions: [
              ...
              },
              {
                ...
                b: {
                  slug: "people/alice-smith-example",
                  chunk_id: 4,
                  take_id: null,
                  source_tier: "curated",
                  holder: null,
    ...
    const highOnly = (await op.handler(ctx, { severity: "high" })) as {
