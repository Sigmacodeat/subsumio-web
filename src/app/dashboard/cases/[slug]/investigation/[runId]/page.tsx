...
function ContradictionsTab({
  ...
  if (contradictions.length === 0) {
    return (
      <EmptyState
        icon={Check}
        title="Keine Widersprüche gefunden"
        description="Die extrahierten Sachverhaltsbehauptungen enthalten keine erkennbaren Widersprüche oder wurden bereits geprüft."
      />
    );
  }
          ...
          {contradictions.map((c) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "w-full rounded-lg border p-3 text-left transition-[background-color,border-color,color,box-shadow,transform,opacity] duration-[var(--ds-duration-normal)] focus-visible:ring-2 focus-visible:ring-[color:var(--ds-ring)] focus-visible:ring-offset-2 focus-visible:ring-offset-[color:var(--ds-surface)] active:scale-[0.99] motion-reduce:transition-none",
                  isSelected
...
function GapsTab({ result }: { result: CaseInvestigationResult }) {
  if (result.evidence_gaps.length === 0) {
    return <EmptyState icon={Check} title="Keine Beweislücken erkannt" />;
  }
