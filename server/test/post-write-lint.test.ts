...
async function reset(): Promise<void> {
  await engine.executeRaw(
    "TRUNCATE pages, links, content_chunks, timeline_entries, tags, raw_data, page_versions, ingest_log RESTART IDENTITY CASCADE"
  );
...
describe("runPostWriteLint", () => {
  ...
  test("flag disabled → returns ran=false, no findings", async () => {
    await engine.putPage("people/x", {
      type: "person",
      title: "X",
      compiled_truth: "X has a bare factual paragraph without a citation.",
      frontmatter: {},
    });
  ...
  test("validate:false frontmatter → skipped (grandfather)", async () => {
    await engine.putPage("people/old", {
      type: "person",
      title: "Old",
      compiled_truth: "Lots of factual paragraphs without citations.",
  ...
  test("forces run even when flag is off", async () => {
    await engine.putPage("people/y", {
      type: "person",
      title: "Y",
      compiled_truth: "Y raised money [Source: X, 2026-04-18](https://x.com/y).",
      frontmatter: {},
    });
  ...
  test("flag on + bad page → findings include citation error", async () => {
    ...
    await engine.putPage("people/bad", {
      type: "person",
      title: "Bad",
      compiled_truth: "Bad raised $5M in Series A from Sequoia without citation.",
      frontmatter: {},
    });
    const r = await runPostWriteLint(engine, "people/bad", { noLog: true });
    expect(r.ran).toBe(true);
    expect(r.findings.length).toBeGreaterThan(0);
    const citationError = r.findings.find(
      (f) => f.validator === "citation" && f.severity === "error"
    );
