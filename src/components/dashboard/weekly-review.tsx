...
export function WeeklyReview() {
  ...
  return (
    <section
      className="rounded-xl border border-[color:var(--brand-primary)]/25 bg-[color:var(--brand-glow)] p-4"
      aria-labelledby="weekly-review-title"
    >
      <div className="flex items-start gap-3">
        <ClipboardCheck
          className="mt-0.5 text-[color:var(--brand-primary)]"
          size={20}
          aria-hidden="true"
        />
        <div className="flex-1">
          <h2 id="weekly-review-title" className="font-semibold text-[color:var(--ds-text)]">
            {t("weekly.title")}
          </h2>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {ITEMS.map((item) => (
              <label
                key={item}
                className="flex cursor-pointer items-center gap-2 rounded-lg bg-[color:var(--ds-surface)] px-3 py-2 text-sm"
              >
                <input
                  type="checkbox"
        ...
        <button
          ...
          className="rounded-md p-1 text-[color:var(--ds-text-muted)] hover:bg-[color:var(--ds-hover)]"
        >
          <X size={16} />
        </button>
      </div>
