...
describe("redactPgUrl", () => {
  ...
  test("strips userinfo from postgres:// URL", () => {
    expect(redactPgUrl("postgres://user:pass@host:5432/db")).toBe("postgres://***@host:5432/db");
  });

  test("preserves URL without userinfo", () => {
    expect(redactPgUrl("postgresql://host:5432/db")).toBe("postgresql://host:5432/db");
  });
  ...
  test("handles user-only (no password)", () => {
    expect(redactPgUrl("postgresql://user@host:5432/db")).toBe("postgresql://***@host:5432/db");
  });
...
describe("redactDeep", () => {
  ...
  test("redacts URLs inside arrays", () => {
    ...
    expect(redactDeep(input)).toEqual(["postgresql://***@host/db", "safe string"]);
  });
  ...
  test("handles nested objects", () => {
    const input = {
      config: { primary: "postgresql://u:p@h/d", secondary: { url: "postgres://u:p@h2/d" } },
    };
    const out = redactDeep(input);
