...
describe("JSONL key contract", () => {
  test("every line carries question_id + hypothesis at minimum", async () => {
    ...
    try {
      await runEvalLongMemEval(
        [FIXTURE_PATH, "--keyword-only", "--retrieval-only", "--limit", "3", "--output", outPath],
        { engine: sharedEngine }
      );
  ...
  test("all questions already done -> early return, no client calls", async () => {
    ...
    try {
      ...
      writeFileSync(
        outPath,
        fixture
          .map((q) => JSON.stringify({ question_id: q.question_id, hypothesis: "done" }))
          .join("\n") + "\n",
        "utf8"
      );
...
describe("runEvalLongMemEval --by-type (v0.40.1.0 Track D / T1+T2)", () => {
  ...
  test("--by-type emits a final by_type_summary line; absent when flag not set", async () => {
    ...
    try {
      ...
      const withLines = readFileSync(withFlag, "utf8")
        .split("\n")
        .filter((l) => l.length > 0);
      ...
      const withoutLines = readFileSync(withoutFlag, "utf8")
        .split("\n")
        .filter((l) => l.length > 0);
  ...
  test("resume-replace: prior by_type_summary at the tail is REPLACED, not appended", async () => {
    ...
    try {
      ...
      const firstLines = readFileSync(outPath, "utf8")
        .split("\n")
        .filter((l) => l.length > 0);
      const firstSummaryCount = firstLines.filter((l) => {
        try {
          return JSON.parse(l).kind === "by_type_summary";
        } catch {
          return false;
        }
      }).length;
      ...
      const secondLines = readFileSync(outPath, "utf8")
        .split("\n")
        .filter((l) => l.length > 0);
