...
function fullScores(
  values: Partial<Record<(typeof RUBRIC_DIMENSIONS)[number], number>>
): ParsedModelResult {
      ...
      slots: [
        ok(
          ...
          fullScores({
            accuracy: 8,
            attribution: 8,
            weight_calibration: 8,
            kind_classification: 8,
            signal_density: 8,
          })
        ),
        ok(
...
describe("aggregate — FAIL branches", () => {
  test("one dim mean < 7 → FAIL", () => {
    const r = aggregate({
      slots: [ok("a", fullScores({ accuracy: 5 })), ok("b", fullScores({ accuracy: 6 }))],
    });
...
describe("aggregate — INCONCLUSIVE branches", () => {
  test("< 2 models contribute → INCONCLUSIVE", () => {
    const r = aggregate({
      slots: [ok("a", fullScores({})), fail("b", "timeout"), fail("c", "parse_failed")],
    });
...
describe("aggregate — codex review #5: strict required-dim enforcement", () => {
  test("model missing one dim → contribution dropped, treated as failure", () => {
    ...
    const r = aggregate({
      slots: [ok("a", fullScores({})), ok("b", fullScores({})), ok("c", partial)],
    });
  ...
  test("two models missing dims → only 1 contributing → INCONCLUSIVE", () => {
    ...
    const r = aggregate({
      slots: [ok("a", fullScores({})), ok("b", partial1), ok("c", partial2)],
    });
    expect(r.verdict).toBe("inconclusive");
    expect(r.successes).toBe(1);
    expect(r.failures).toBe(2);
