...
beforeEach(async () => {
  ...
  await engine.putPage(
    "legal/statutes/at/abgb/p-1295",
    {
      type: "law",
      title: "§ 1295 ABGB",
      compiled_truth: "Schadenersatz aus Verschulden.",
      frontmatter: {},
    },
    { sourceId: "law-at" }
  );
  await engine.putPage(
    "akten/mandant-x",
    {
      type: "note",
      title: "Mandant X",
      compiled_truth: "firm-only content",
      frontmatter: {},
    },
    { sourceId: "firm-a" }
  );
...
describe("dispatchToolCall allowedSources federates reads to the shared statute source", () => {
  test("tenant WITH the law-at federation can read a statute page", async () => {
    const page = parse(
      await call(
        "get_page",
        { slug: "legal/statutes/at/abgb/p-1295" },
        {
          sourceId: "firm-a",
