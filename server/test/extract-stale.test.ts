...
describe("engine: stale-page extraction methods", () => {
  ...
  test("countStalePagesForExtraction: version arm flags pre-version stamps", async () => {
    ...
    await engine.executeRaw(
      `UPDATE pages SET updated_at = '2000-01-01T00:00:00Z' WHERE slug = 'people/alice'`
    );
  ...
  test("countStalePagesForExtraction: edited-since arm (CDX-1)", async () => {
    ...
    await engine.executeRaw(
      `UPDATE pages SET updated_at = '2099-01-01T00:00:00Z' WHERE slug = 'people/alice'`
    );
  ...
  test("listStalePagesForExtraction: returns content columns + keyset paginates", async () => {
    ...
    const batch2 = await engine.listStalePagesForExtraction({
      batchSize: 10,
      afterPageId: batch1[0].id,
    });
...
describe("gbrain extract --stale", () => {
  test("extracts typed edges + stamps every processed page (incl. zero-link)", async () => {
    ...
    await engine.putPage(
      "companies/acme",
      companyPage("Acme", "[Alice](people/alice) is the CEO of [Acme](companies/acme).")
    );
  ...
  test("idempotent: second run finds 0 stale and creates no new links", async () => {
    ...
    await engine.putPage(
      "companies/acme",
      companyPage("Acme", "[Alice](people/alice) advises [Acme](companies/acme).")
    );
  ...
  test("--dry-run reports count and writes nothing", async () => {
    ...
    await engine.putPage(
      "companies/acme",
      companyPage("Acme", "[Alice](people/alice) joined [Acme](companies/acme).")
    );
  ...
  test("CDX-4 (D2): a link-flush throw aborts the sweep and leaves pages UNSTAMPED", async () => {
    ...
    await engine.putPage(
      "companies/acme",
      companyPage("Acme", "[Alice](people/alice) founded [Acme](companies/acme).")
    );
    ...
    let threw = false;
    (engine as unknown as { addLinksBatch: unknown }).addLinksBatch = async () => {
      throw new Error("__flush_boom__");
    };
    try {
      await runExtract(engine, ["--stale"]);
    } catch (e) {
      if ((e as Error).message === "__flush_boom__") threw = true;
      else throw e;
    } finally {
    ...
    await runExtract(engine, ["--stale"]);
    expect(
      (await engine.getLinks("companies/acme")).some((l) => l.to_slug === "people/alice")
    ).toBe(true);
    expect(await stampOf("companies/acme")).not.toBeNull();
    expect(
      await engine.countStalePagesForExtraction({ versionTs: LINK_EXTRACTOR_VERSION_TS })
  ...
  test("D4 race: a concurrent edit landing during the sweep is NOT masked", async () => {
    ...
    await engine.putPage(
      "companies/acme",
      companyPage("Acme", "[Alice](people/alice) backs [Acme](companies/acme).")
    );
    // Anchor acme's updated_at in the past so the read value is well-defined.
    await engine.executeRaw(
      `UPDATE pages SET updated_at = now() - interval '3 hours' WHERE slug = 'companies/acme'`
    );
    ...
    (engine as unknown as { markPagesExtractedBatch: unknown }).markPagesExtractedBatch = async (
      ...
      if (!hooked) {
        hooked = true;
        await engine.executeRaw(
          `UPDATE pages SET updated_at = now() - interval '1 hour' WHERE slug = 'companies/acme'`
        );
      }
    ...
    try {
      ...
      (engine as unknown as { markPagesExtractedBatch: unknown }).markPagesExtractedBatch =
        origStamp;
    }
