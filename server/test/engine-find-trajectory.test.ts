...
async function insertTyped(args: {
  ...
  await engine.executeRaw(`INSERT INTO sources (id, name) VALUES ($1, $1) ON CONFLICT DO NOTHING`, [
    sid,
  ]);
  const r = await engine.executeRaw<{ id: number }>(
    ...
    [
      sid,
      args.entity_slug,
      args.text ?? `${args.metric} ${args.value}`,
...
describe("findTrajectory — chronological ordering (R3)", () => {
  test("returns points in (valid_from ASC, id ASC) order regardless of insert order", async () => {
    ...
    const idJul = await insertTyped({
      entity_slug: "traj-order",
      metric: "mrr",
      value: 150000,
      valid_from: new Date("2026-07-08"),
    });
    const idJan = await insertTyped({
      entity_slug: "traj-order",
      metric: "mrr",
      value: 50000,
      valid_from: new Date("2026-01-15"),
    });
    const idApr = await insertTyped({
      entity_slug: "traj-order",
      metric: "mrr",
      value: 200000,
      valid_from: new Date("2026-04-12"),
    });
...
describe("findTrajectory — source scoping (D-CDX-6)", () => {
  test("scalar sourceId returns only that source", async () => {
    await insertTyped({
      source_id: "traj-src-A",
      entity_slug: "traj-srcscope",
      metric: "mrr",
      value: 50000,
      valid_from: new Date("2026-01-15"),
    });
    await insertTyped({
      source_id: "traj-src-B",
      entity_slug: "traj-srcscope",
      metric: "mrr",
      value: 99999,
      valid_from: new Date("2026-01-15"),
    ...
    const pointsA = await engine.findTrajectory({
      entitySlug: "traj-srcscope",
      sourceId: "traj-src-A",
    });
    expect(pointsA.length).toBe(1);
    expect(pointsA[0].value).toBe(50000);

    const pointsB = await engine.findTrajectory({
      entitySlug: "traj-srcscope",
      sourceId: "traj-src-B",
    });
