...
describe("v0.32.4 — sync_freshness check", () => {
  ...
  test('last_sync_at IS NULL → fail with "never been synced"', async () => {
    ...
    const result = await checkSyncFreshness(
      makeStubEngine([{ id: "wiki", name: "", local_path: "/tmp/wiki", last_sync_at: null }])
    );
    expect(result.status).toBe("fail");
      ...
      makeStubEngine([
        {
          ...
          name: "",
          local_path: "/tmp/wiki",
          last_sync_at: agoMs(4 * 24 * 60 * 60 * 1000),
        },
      ])
    );
    expect(result.status).toBe("fail");
      ...
      makeStubEngine([
        {
          ...
          name: "",
          local_path: "/tmp/wiki",
          last_sync_at: new Date(nowMs - 72 * 60 * 60 * 1000),
        },
      ]),
      { nowMs }
    );
      ...
      makeStubEngine([
        {
          ...
          name: "",
          local_path: "/tmp/wiki",
          last_sync_at: new Date(nowMs - 24 * 60 * 60 * 1000),
        },
      ]),
      { nowMs }
    );
      ...
      makeStubEngine([
        {
          ...
          name: "",
          local_path: "/tmp/wiki",
          last_sync_at: new Date(Date.now() + 10 * 60 * 1000),
        },
      ])
    );
    expect(result.status).toBe("warn");
        ...
        makeStubEngine([
          {
            ...
            name: "",
            local_path: "/tmp/wiki",
            last_sync_at: agoMs(7 * 60 * 60 * 1000),
          },
        ])
      );
      expect(result.status).toBe("fail");
      ...
      makeStubEngine([
        {
          id: "unchanged",
          name: "",
          local_path: "/tmp/unchanged",
          last_sync_at: agoMs(40 * 60 * 60 * 1000),
          last_commit: "frozen-sha",
          chunker_version: currentChunkerVersion,
        },
        {
          id: "recent",
          name: "",
          local_path: "/tmp/recent",
...
describe("v0.40.4 — graph_signals_coverage check", () => {
  ...
  beforeEach(async () => {
    ...
    await engine.executeRaw(
      `DELETE FROM config WHERE key IN ('search.graph_signals', 'search.mode')`
    );
  });
...
describe("issue #972 — link_resolution_opportunity check", () => {
  ...
  test("no bare wikilinks → ok", async () => {
    await engine.putPage("people/alice", {
      type: "person",
      title: "Alice",
      compiled_truth: "Met [Bob](people/bob).",
      timeline: "",
    });
  ...
  test("≥5 would-resolve AND ≥20% ratio → warn with paste-ready hint", async () => {
    ...
    await engine.putPage("concepts/wiki-index", {
      ...
      title: "Wiki",
      compiled_truth:
        "See [[struktura]], [[eeva]], [[Fast-Weigh]], [[rosa]], [[dragon]] for context.",
      timeline: "",
    });
    const check = await checkLinkResolutionOpportunity(engine);
    expect(check.status).toBe("warn");
    ...
    expect(check.message).toContain("gbrain config set link_resolution.global_basename true");
  });
  ...
  test("counts slugified-only matches (shared matcher, codex #972 DRY)", async () => {
    ...
    await engine.putPage("concepts/x", {
      type: "concept",
      title: "X",
      compiled_truth: "See [[Fast Weigh]] for context.",
      timeline: "",
    });
    const check = await checkLinkResolutionOpportunity(engine);
  ...
  test("check is wired into runDoctor AND doctorReportRemote (source-grep)", async () => {
    ...
    expect(source).toMatch(
      /ORDER BY id DESC LIMIT \$\{?SAMPLE_LIMIT\}?|ORDER BY id DESC LIMIT 1000/
    );
