...
describe("readinessHint", () => {
  test("not_built / indexing / unknown produce a hint; ready does not", () => {
    expect(
      readinessHint({ status: "not_built", ready: false, has_code: false, pending_edges: false })
    ).toContain("not built");
    expect(
      readinessHint({ status: "indexing", ready: false, has_code: true, pending_edges: true })
    ).toContain("still building");
    expect(
      readinessHint({ status: "unknown", ready: false, has_code: false, pending_edges: false })
    ).toContain("unavailable");
    expect(
      readinessHint({ status: "ready", ready: true, has_code: true, pending_edges: false })
    ).toBeNull();
  });
});
