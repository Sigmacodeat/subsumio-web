...
import { loadTranscriptRows, renderTranscript } from "../src/core/minions/transcript.ts";
...
beforeEach(async () => {
  ...
  const j = await queue.add("subagent", { prompt: "hi" }, {}, { allowProtectedSubmit: true });
  jobId = j.id;
});
