...
function auditReportPath(): string {
  return join(migrationsDir(), "v0.22.4-audit.json");
}
...
async function phaseBAudit(
  opts: OrchestratorOpts
): Promise<{ phase: OrchestratorPhaseResult; report: AuditReport | null }> {
  if (opts.dryRun)
    return { phase: { name: "audit", status: "skipped", detail: "dry-run" }, report: null };
  try {
