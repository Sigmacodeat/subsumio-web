...
// Top 30 nach Stars
const byStars = [...data].sort((a, b) => b.stargazers_count - a.stargazers_count).slice(0, 30);
console.log("\nTop 30 nach Stars:");
...
for (const f of renamed
  ...
  .slice(0, 30)) {
  console.log(
    `  ${f.full_name}: ${f.stargazers_count} stars, pushed ${f.pushed_at.slice(0, 10)}, ${f.description?.slice(0, 60) || "-"}`
  );
}
...
for (const f of withIssues.slice(0, 20)) {
  console.log(
    `  ${f.full_name}: ${f.open_issues_count} issues, ${f.stargazers_count} stars, pushed ${f.pushed_at.slice(0, 10)}`
  );
}

// Größte Repos (size in KB)
const bySize = [...data].sort((a, b) => b.size - a.size).slice(0, 20);
console.log("\nGrößte Repos (KB):");
