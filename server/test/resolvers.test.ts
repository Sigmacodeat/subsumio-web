...
import {
  ...
  checkDnsRebinding,
} from "../src/core/resolvers/builtin/url-reachable.ts";
import {
  xHandleToTweetResolver,
  computeBackoffMs,
} from "../src/core/resolvers/builtin/x-api/handle-to-tweet.ts";
...
describe("x_handle_to_tweet resolver", () => {
      ...
      new Response(
        JSON.stringify({
          data: [
            {
              id: "123",
              text: "talking about building gbrain today",
  ...
  test("single weak-match → confidence in 0.5-0.8 review range", async () => {
    ...
    globalThis.fetch = (async () =>
      new Response(
        JSON.stringify({
          data: [
            { id: "1", text: "something unrelated entirely", created_at: "2026-04-18T00:00:00Z" },
          ],
        }),
        { status: 200, headers: { "content-type": "application/json" } }
      )) as unknown as typeof fetch;
  ...
  test("many ambiguous candidates → confidence < 0.5 (skip bucket)", async () => {
    ...
    const r = await xHandleToTweetResolver.resolve({
      input: {
        handle: "garrytan",
        keywords: "completely different signal words unlikely to match",
      },
      context: makeCtx(),
    });
  ...
  test("500 → ResolverError(upstream) with body snippet", async () => {
    process.env.X_API_BEARER_TOKEN = "fake";
    globalThis.fetch = (async () =>
      new Response("internal err", { status: 500 })) as unknown as typeof fetch;
    try {
      await xHandleToTweetResolver.resolve({
        input: { handle: "garrytan" },
        context: makeCtx(),
...
describe("builtin resolver schemas — structural array-items guard", () => {
  ...
  for (const { name, resolver } of builtins) {
    test(`${name}: inputSchema has no bare arrays`, () => {
      const violations = findArrayWithoutItems(resolver.inputSchema as SchemaNode | undefined, [
        name,
        "inputSchema",
      ]);
    ...
    test(`${name}: outputSchema has no bare arrays`, () => {
      const violations = findArrayWithoutItems(resolver.outputSchema as SchemaNode | undefined, [
        name,
        "outputSchema",
      ]);
      expect(violations).toEqual([]);
  ...
  test("x_handle_to_tweet candidates declares full item shape with required + additionalProperties:false", () => {
    ...
    expect(Object.keys(props).sort()).toEqual(["created_at", "score", "text", "tweet_id", "url"]);
