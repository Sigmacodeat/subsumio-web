...
export function buildLearningEntry(event: IncorrectResolutionEvent): GstackLearningEntry {
  ...
  const tagSuffix =
    event.activeBiasTags && event.activeBiasTags.length > 0 ? `:${event.activeBiasTags[0]}` : "";
  ...
  const tagTail =
    event.activeBiasTags && event.activeBiasTags.length > 0
...
export async function writeIncorrectResolution(
  ...
  try {
    ...
    const reason =
      error.includes("not on PATH") || error.includes("NOT_FOUND")
        ? "binary_missing"
        : "write_failed";
