...
function parseArgs(argv: string[]): ParsedArgs {
  const out: ParsedArgs = { fixturePath: "", jurisdiction: "de", append: false, llmRerank: false };
...
function extractClaims(answer: string): string[] {
  ...
  return sentences.filter((s) =>
    /(?:§|Art\.|Abs\.|BGB|ABGB|StGB|ZPO|HGB|GmbHG|InsO|UWG|BauGB|DSG|VwGO|AO|ArbVG|EheG|IO|AVG|GewO|Anspruch|Recht|Pflicht|Vertrag|Schadensersatz|Gewährleistung|Verjährung|nichtig|wirksam|zulässig|strafbar|schuldig)/i.test(s)
  );
}
...
function isClaimGrounded(claim: string, context: string): boolean {
  const claimTerms = claim
    .toLowerCase()
    .match(/[\p{L}]{4,}/gu)
    ?.filter((t) => !["der", "die", "das", "ein", "eine", "ist", "wird", "wurde", "hat", "haben", "nach", "nachstehend", "gemäß", "aufgrund", "hinsichtlich", "bezüglich"].includes(t))
    ?? [];
...
async function main() {
  ...
  process.stderr.write(`[subsumption] loaded ${cases.length} cases (jurisdiction=${opts.jurisdiction})\n`);
  if (opts.llmRerank) {
  ...
  const lawPrefix = opts.jurisdiction === "at"
    ? `legal/statutes/at/`
    : `legal/statutes/de/`;
