...
describe("withTimeout", () => {
  ...
  test("rejects with OperationTimeoutError when the promise exceeds the deadline", async () => {
    ...
    let caught: unknown;
    try {
      await withTimeout(slow, 30, "slow-resolve");
    } catch (e) {
      caught = e;
    }
  ...
  test("propagates underlying rejection (not OperationTimeoutError)", async () => {
    ...
    let caught: unknown;
    try {
      await withTimeout(Promise.reject(sentinel), 1000, "rejecting");
    } catch (e) {
      caught = e;
    }
  ...
  test("zero deadline rejects immediately", async () => {
    let caught: unknown;
    try {
      await withTimeout(Promise.resolve("would-have-resolved"), 0, "zero-deadline");
    } catch (e) {
      caught = e;
    }
    expect(caught).toBeInstanceOf(OperationTimeoutError);
  ...
  test("negative deadline rejects immediately", async () => {
    let caught: unknown;
    try {
      await withTimeout(Promise.resolve("would-have-resolved"), -100, "neg-deadline");
    } catch (e) {
      caught = e;
    }
    expect(caught).toBeInstanceOf(OperationTimeoutError);
