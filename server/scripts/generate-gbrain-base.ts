...
ok(
  `loaded gbrain-base v${manifest.version}; ${manifest.page_types.length} page types, ${manifest.link_types.length} link verbs`
);
...
if (missing.length > 0) {
  ...
  fail(
    `gbrain-base.yaml is missing ${missing.length} page_types from ALL_PAGE_TYPES: ${missing.join(", ")}`
  );
}
if (extra.length > 0) {
  ...
  console.warn(
    "[gbrain-base-codegen] WARN: extra types in gbrain-base.yaml not in ALL_PAGE_TYPES:",
    extra
  );
}
...
if (reloadCount !== manifest.page_types.length) {
  fail(
    `re-load produced different page_type count: ${reloadCount} vs ${manifest.page_types.length}`
  );
}
