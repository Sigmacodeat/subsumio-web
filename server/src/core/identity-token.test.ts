...
import { createIdentityToken, verifyIdentityToken, filterResultsByMatterScope, type MatterScope } from './identity-token.ts';
...
describe('identity-token', () => {
  describe('createIdentityToken + verifyIdentityToken', () => {
    ...
    test('round-trips matterScope "all"', () => {
      const token = createIdentityToken(
        { sourceId: 'tenant-1', matterScope: 'all' },
        SECRET,
      );
      const payload = verifyIdentityToken(token, SECRET);
      expect(payload).not.toBeNull();
    ...
    test('rejects tampered payload', () => {
      ...
      const tamperedPayload = Buffer.from(
        JSON.stringify({ sourceId: 'tenant-1', matterScope: 'all', exp: Math.floor(Date.now() / 1000) + 999 }),
      ).toString('base64url');
    ...
    test('rejects token with missing required fields', () => {
      ...
      const payload = JSON.stringify({ matterScope: 'all', exp: Math.floor(Date.now() / 1000) + 300 });
