...
describe("isCrashExit — branch matrix", () => {
  ...
  test("graceful_shutdown is not a crash", () => {
    expect(
      isCrashExit(evt("worker_exited", { likely_cause: "graceful_shutdown", signal: "SIGTERM" }))
    ).toBe(false);
  });

  // Case 3: code=1 from the worker process — a real runtime error.
  test("runtime_error is a crash", () => {
    expect(isCrashExit(evt("worker_exited", { likely_cause: "runtime_error", code: 1 }))).toBe(
      true
    );
  });
  ...
  test("oom_or_external_kill is a crash", () => {
    expect(
      isCrashExit(evt("worker_exited", { likely_cause: "oom_or_external_kill", signal: "SIGKILL" }))
    ).toBe(true);
  });
  ...
  test("unrecognized future likely_cause is a crash (denylist regression guard)", () => {
    expect(isCrashExit(evt("worker_exited", { likely_cause: "future_value_not_known" }))).toBe(
      true
    );
  });
...
describe("summarizeCrashes — aggregation", () => {
  ...
  test("legacy entry with null code counts as crash in legacy bucket", () => {
    const summary = summarizeCrashes([evt("worker_exited", { code: null })]);
