...
describe("IPv6 detection", () => {
  test("detects db.xxx.supabase.co as direct (IPv6)", () => {
    expect(
      isSupabaseDirectUrl("postgresql://postgres:pw@db.rqfedtbs.supabase.co:5432/postgres")
    ).toBe(true);
  });

  test("detects .supabase.co:5432 as direct (IPv6)", () => {
    expect(
      isSupabaseDirectUrl("postgresql://postgres:pw@something.supabase.co:5432/postgres")
    ).toBe(true);
  });
...
describe("smart URL parsing covers all Supabase formats", () => {
  test("dashboard URL with settings path", () => {
    expect(
      extractProjectRef("https://supabase.com/dashboard/project/abcdefghijklmnop/settings/database")
    ).toBe("abcdefghijklmnop");
  });

  test("dashboard URL with just project", () => {
    expect(extractProjectRef("https://supabase.com/dashboard/project/abcdefghijklmnop")).toBe(
      "abcdefghijklmnop"
    );
  });
  ...
  test("pooler URL with region", () => {
    expect(
      extractProjectRef(
        "postgresql://postgres.abcdefghijklmnop:mypassword@aws-1-us-east-1.pooler.supabase.com:6543/postgres"
      )
    ).toBe("abcdefghijklmnop");
  });
  ...
  test("project URL with path", () => {
    expect(extractProjectRef("https://abcdefghijklmnop.supabase.co/rest/v1/")).toBe(
      "abcdefghijklmnop"
    );
  });
  ...
  test("non-supabase postgres URL returns null", () => {
    expect(
      extractProjectRef("postgresql://user:pass@my-rds-instance.amazonaws.com:5432/mydb")
    ).toBeNull();
  });
});
