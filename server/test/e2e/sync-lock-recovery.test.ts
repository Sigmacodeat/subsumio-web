...
beforeEach(async () => {
  ...
  for (let i = 0; i < 5; i++) {
    writeFileSync(
      join(repoDir, "people", `alice-example-${i}.md`),
      [
        "---",
        "type: person",
  ...
  const eng = getEngine();
  try {
    await (eng as any).sql`DELETE FROM gbrain_cycle_locks WHERE id LIKE 'gbrain-sync:%'`;
  } catch {
    /* */
  }
});
...
describeE2E("v0.41.6.0 — sync lock recovery scenarios", () => {
  ...
  test("--break-lock + --all is accepted and iterates sources (self-heal, no longer refused)", () => {
    ...
    expect(out).toMatch(
      /No active sources to break-lock against|is not held \(nothing to break\)|Broke lock/i
    );
  });
  ...
  test("SIGTERM during sync releases the lock within 3s", async () => {
    ...
    const eng = getEngine();
    const sigtermProc = spawn(
      CLI[0],
      [...CLI.slice(1), "sync", "--repo", repoDir, "--full", "--yes", "--no-embed"],
      {
        env: {
          ...process.env,
          GBRAIN_HOME: tmpHome,
    ...
    for (let i = 0; i < 50; i++) {
      const snap = await inspectLock(eng, "gbrain-sync:default");
      if (snap && snap.holder_pid === sigtermProc.pid) {
        lockSeen = true;
        break;
      }
