...
export function loadScenario(name: string, root?: string): ScenarioConfig {
  ...
  try {
    ...
    throw new Error(
      `scenario ${JSON.stringify(name)}: malformed scenario.json (${e instanceof Error ? e.message : e})`
    );
  }
  ...
  if (
    !Array.isArray(cfg.expected_phases) ||
    !cfg.expected_phases.every((x) => typeof x === "string")
  ) {
