...
import { loadCheckpoint } from "../../src/core/brainstorm/checkpoint.ts";
...
async function seedSmallBrain(): Promise<void> {
  ...
  const farSlugs = ["concepts/decay-a", "concepts/decay-b", "people/founder-a", "people/founder-b"];
...
function makeChatFnMixed(failOnCrossCallN: number) {
  ...
  const fn = async (opts: ChatOpts): Promise<ChatResult> => {
    ...
    if (crossCalls === failOnCrossCallN) {
      throw new BudgetExhausted(`synthetic mid-run crash on cross call ${crossCalls}`, {
        reason: "cost",
        spent: 1.5,
        cap: 1.0,
      });
...
describe("brainstorm --resume (TX3 load-bearing)", () => {
  ...
  test("second run with resumeRunId merges pre-crash ideas with post-resume ideas (TX3 contract)", async () => {
    ...
    const preCrashIdeaTexts = cpBefore.completed_crosses.flatMap((cc) =>
      cc.ideas.map((i) => i.text)
    );
