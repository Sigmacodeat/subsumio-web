...
function writeBrainPage(month: string, rawJson: string): number {
  ...
  const tweets = (data.data || []).sort((a: any, b: any) =>
...
function cleanup(month: string): void {
  try {
    unlinkSync(pagePath(month));
  } catch {}
  try {
    unlinkSync(rawPath(month));
  } catch {}
  try {
    execSync(
      `git checkout -- media/x/garrytan/${month}.md 2>/dev/null; git clean -f media/x/garrytan/${month}.md 2>/dev/null`,
      {
        cwd: BRAIN_PATH,
        stdio: "pipe",
      }
...
4. Report tweet count`;
  ...
  return new Promise((resolve) => {
    const proc = spawn(
      "openclaw",
      ["agent", "--agent", "main", "--local", "--message", task, "--timeout", "60"],
      { env: process.env }
    );
...
describe("Tweet Ingestion: Minions vs OpenClaw", () => {
  ...
  test.skipIf(!hasDB || !hasX || !hasBrain)(
    ...
    async () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        results.push(result);
        console.log(
          `  Minions run ${i + 1}: ${result.wallMs}ms ${result.ok ? "✅" : "❌"} ${result.reply || result.error}`
        );
      }
  ...
  test.skipIf(!hasX || !hasBrain)(
    ...
    async () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        results.push(result);
        console.log(
          `  OpenClaw run ${i + 1}: ${result.wallMs}ms ${result.ok ? "✅" : "❌"} ${result.reply || result.error}`
        );
      }
  ...
  test.skipIf(!hasDB || !hasX || !hasBrain)("Summary comparison", async () => {
    ...
    console.log(
      "Environment: " + (process.env.RENDER ? "Render" : process.env.FLY_APP_NAME ? "Fly" : "local")
    );
    console.log(
      "Brain size: " +
        (existsSync(BRAIN_PATH)
          ? execSync(`find ${BRAIN_PATH} -name "*.md" | wc -l`, { encoding: "utf-8" }).trim() +
            " pages"
          : "unknown")
    );
  });
