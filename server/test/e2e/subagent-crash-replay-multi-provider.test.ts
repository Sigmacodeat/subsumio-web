...
const PROVIDER_MATRIX: ProviderShape[] = [
  {
    ...
    finalResponse: {
      ...
      blocks: [
        { type: "text", text: "anthropic resumed: search result was helpful" },
      ] as ChatBlock[],
      stopReason: "end",
      usage: {
        input_tokens: 50,
  ...
  },
  {
    ...
    finalResponse: {
      ...
      blocks: [
        { type: "text", text: "openai resumed: synthesized answer from prior tool result" },
      ] as ChatBlock[],
      stopReason: "end",
      usage: {
        input_tokens: 45,
  ...
  },
  {
    ...
    finalResponse: {
      ...
      blocks: [
        { type: "text", text: "gemini resumed: 1M-context replay went fine" },
      ] as ChatBlock[],
      stopReason: "end",
  ...
  },
  {
    ...
    finalResponse: {
      ...
      blocks: [
        { type: "text", text: "openrouter resumed: proxied claude response" },
      ] as ChatBlock[],
      stopReason: "end",
  ...
  },
  {
    ...
    finalResponse: {
      ...
      blocks: [
        { type: "text", text: "deepseek resumed: openai-compatible chat works" },
      ] as ChatBlock[],
      stopReason: "end",
  ...
  return makeSubagentHandler({
    ...
    makeAnthropic: () =>
      ({
        messages: {
          create: async () => {
            throw new Error("legacy path should not be invoked");
          },
        },
      }) as any,
  });
...
async function seedCrashedState(
  ...
  await engine.executeRaw(
    ...
    [
      jobId,
      JSON.stringify(
        shape === "v1" ? [{ type: "text", text: prompt }] : [{ type: "text", text: prompt }]
      ),
    ]
  );
...
async function makeCrashedCtx(
  jobId: number,
  prompt: string,
  modelId: string
): Promise<MinionJobContext> {
    ...
    it("prior failed tool replays as is_error result, loop completes", async () => {
      ...
      await engine.executeRaw(
        ...
        [
          jobId,
          JSON.stringify([
            { type: "tool-call", toolCallId: "tc-failed", toolName: "search", input: {} },
          ]),
        ]
      );
      await engine.executeRaw(
      ...
      __setChatTransportForTests(
        async () =>
          ({
            ...
            blocks: [
              { type: "text", text: "I see search failed (rate limited). Aborting." },
            ] as ChatBlock[],
            stopReason: "end",
            usage: {
              input_tokens: 30,
