...
describe('bi-temporal links', () => {
  ...
  test('supersedeLink creates new version and marks old as expired', async () => {
    ...
    const result = await engine.supersedeLink('people/alice', 'companies/acme', 'works_at', 'Left Acme, joined NovaMind as CTO', 'manual');
  ...
  test('supersedeLink returns null when no current link exists', async () => {
    ...
    const result = await engine.supersedeLink('people/alice', 'companies/acme', 'works_at', 'New context');
