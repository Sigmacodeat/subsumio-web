...
Some content.`;
  ...
  test("infers type from wiki subdirectory paths", () => {
    ...
    expect(parseMarkdown("", "tech/wiki/hardware/h100-vs-gb200-training-benchmarks.md").type).toBe(
      "hardware"
    );
    expect(parseMarkdown("", "tech/wiki/architecture/kb-infrastructure.md").type).toBe(
      "architecture"
    );
    expect(
      parseMarkdown("", "finance/wiki/analysis/polymarket-bot-automation-thesis.md").type
    ).toBe("analysis");
    expect(parseMarkdown("", "personal/wiki/concepts/career-regrets-2026-framework.md").type).toBe(
      "concept"
    );
  });
