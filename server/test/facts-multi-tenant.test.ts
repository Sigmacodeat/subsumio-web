...
beforeAll(async () => {
  ...
  await engine.executeRaw(
    `INSERT INTO sources (id, name, config) VALUES ('alpha', 'Alpha', '{}'::jsonb) ON CONFLICT DO NOTHING`
  );
  await engine.executeRaw(
    `INSERT INTO sources (id, name, config) VALUES ('beta',  'Beta',  '{}'::jsonb) ON CONFLICT DO NOTHING`
  );
});
...
describe("cross-source isolation", () => {
  ...
  test("CASCADE delete on sources cleans up facts", async () => {
    await engine.executeRaw(
      `INSERT INTO sources (id, name, config) VALUES ('ephemeral', 'Eph', '{}'::jsonb)`
    );
    await engine.insertFact(
