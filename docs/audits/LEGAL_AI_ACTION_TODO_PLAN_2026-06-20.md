...
| P0-SECR-002   | fertig — signed identity token + engine-side matter-scope enforcement, inkl. Fail-Closed- und Leak-Guard-Nachschärfung | Engine-side permission filter: `identity-token.ts` (HMAC-SHA256 signed token, 5min TTL, 13 Tests), `operations.ts` (`matterScopeFilter`, `isSlugInMatterScope` auf `AuthInfo` + `OperationContext`), `dispatch.ts` (matterScope in DispatchOpts + buildOperationContext), `web-api.ts` (`verifiedMatterScope` ersetzt den alten self-asserted Header-Ansatz), `mcp/http-transport.ts` (legacy-Bearer-Token-Pfad: `matterScope` aus `access_tokens.permissions.matter_scope`, server-seitig aus der DB, nicht client-eingegeben — schließt den MCP-Bypass), `src/lib/engine.ts` (`engineHeadersForBrainWithMatterScope` erzeugt das signierte Token), `legal-chat/actions.ts` (sendet `ctx.sender.matterScope` an allen 3 relevanten Stellen). **Nachgeschärft in dieser Session:** (1) Env-Var-Mismatch aus dem Rebrand gefixt — `mountWebApi` las `GBRAIN_WEB_API_KEY`/`SIGMABRAIN_WEB_API_KEY` statt des dokumentierten `SUBSUMIO_WEB_API_KEY`; ohne Fix war der gesamte Engine-API-Key-Schutz wirkungslos UND die HMAC-Verifikation lief nie auf den richtigen Secret. (2) Fail-Closed bei ungültigem Token: `verifiedMatterScope` warf vorher bei abgelaufenem/manipuliertem Token NICHT, sondern fiel leise auf `'all'` zurück — jetzt 403 (`identity_token_invalid`), nur ein wirklich fehlender Header bleibt `'all'` (Altbestand-Caller). (3) Leak-Guard: `get_page`s Matter-Scope-Ablehnung warf einen eigenen `matter_scope_denied`-Code mit Slug im Klartext — jetzt identisch zu `page_not_found`, damit eine Ablehnung von "existiert nicht" ununterscheidbar bleibt (gleicher Vertrag wie `resolveAuthorizedCase`/`caseLookupHelp` in der App-Schicht). Neue Tests: `server/test/matter-scope-enforcement.test.ts` (7, inkl. Leak-Guard-Gleichheitstest). Server-`tsc --noEmit` 0 Fehler, `bun test` grün. **Offen:** kein CLI-Befehl, um `matter_scope` auf einem Legacy-Bearer-Token operator-seitig zu setzen (nur direktes DB-Update); `AuthInfo.matterScope`-Dokumentation war veraltet (jetzt korrigiert: zwei Quellen, nicht nur das Identity-Token). |
...
| P31-SB-001    | fertig — temporal memory, connector coverage, entity resolution, engine-side permission filter                         | Matter Context Graph: matter-context.ts (53 tests). Engine: matter-scope.ts (temporal memory, connector coverage, entity resolution) + REST endpoints. Permission filter: identity-token.ts, operations.ts, dispatch.ts, http-transport.ts, engine.ts, actions.ts. Tests: identity-token.test.ts (13), verify 30/30, vitest 4651/4651.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
