...
beforeEach(() => {
  ...
  writeFileSync(
    join(brainRoot, "topics", "real-estate.md"),
    "We were discussing the warehouse-lease offer and the inspection timeline."
  );
  writeFileSync(
    join(brainRoot, "topics", "yc-batch.md"),
    "Talking through the W26 batch interview schedule."
  );
  // A file with PII to verify scrubbing (shapes built at runtime, see above).
  writeFileSync(
    join(brainRoot, "topics", "with-pii.md"),
    `Call me at ${FAKE_PHONE} or ${FAKE_EMAIL} about the deal.`
  );
