...
describe("checkBatchRetryHealth — ok states", () => {
  ...
  test("only successful retries = ok with recovery count", async () => {
    await withEnv({ GBRAIN_AUDIT_DIR: tmpDir }, async () => {
      const now = new Date();
      writeEvent(now, {
        site: "extract.links_inc",
        batch_size: 100,
        attempt: 1,
        outcome: "success",
        delay_ms: 1000,
        error_message_summary: "blip",
      });
      writeEvent(now, {
        site: "extract.links_inc",
        batch_size: 100,
