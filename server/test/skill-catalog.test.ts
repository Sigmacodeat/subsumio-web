...
describe("buildSkillCatalog", () => {
  ...
  test("D7 usable/unavailable split honors caller scope", () => {
    const read = buildSkillCatalog(ctx(true, ["read"]), FIXTURE, "config").skills.find(
      (s) => s.name === "brain-ops"
    )!;
    ...
    const admin = buildSkillCatalog(ctx(true, ["admin"]), FIXTURE, "config").skills.find(
      (s) => s.name === "brain-ops"
    )!;
  ...
  test("local caller (remote=false) can use any real op", () => {
    const local = buildSkillCatalog(ctx(false), FIXTURE, "config").skills.find(
      (s) => s.name === "brain-ops"
    )!;
