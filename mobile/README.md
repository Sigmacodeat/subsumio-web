...
> die Web-App unter subsum.io läuft, ist der Store-Weg frei.
...
- **App-ID:** `io.subsum.app` (in `capacitor.config.ts`; vor dem ersten
  `cap add` final entscheiden — spätere Änderung ist mühsam).
- **Server-URL:** Produktion lädt `https://subsum.io`. Für Gerätetests gegen
...
| Feature      | Plugin                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Status / Hinweis                                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| Push         | `@capacitor/push-notifications`                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ✅ Verdrahtet (`useNativeFeatures` → `/api/push/register`). APNs-Key (Apple) + FCM-Projekt (Google) im Deployment fehlen. |
| Biometrie    | `capacitor-native-biometric`                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | ✅ Verdrahtet (`biometricAuth` in `mobile-bridge.ts`).                                                                    |
| Share-Target | Android: `ACTION_SEND`-Intent-Filter in `AndroidManifest.xml` + `MainActivity.handleShareIntent` → `/mobile/share` (Text → Brain-Notiz, Datei → `/api/upload` in Akte via `@capacitor/filesystem`). **iOS:** Share-Extension-Target in Xcode anlegen (`File → New → Target → Share Extension`, App Group `group.io.subsum.app` aktivieren, Extension schreibt Shared-Defaults → Haupt-App liest via `appUrlOpen`/App Group). Das Web-Backend (`/mobile/share`) ist für beide Plattformen fertig. |

## Store-Submission-Checkliste

**Apple App Store:**

1. App Store Connect: App anlegen (Bundle-ID `io.subsum.app`).
