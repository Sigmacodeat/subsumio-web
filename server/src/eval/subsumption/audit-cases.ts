...
interface AuditReport {
  ...
  by_jurisdiction: Record<
    string,
    { total: number; valid: number; corrected: number; removed: number; disputed: number }
  >;
...
const KNOWN_AT_CORRECTIONS: Record<
  string,
  {
    status: CaseStatus;
    issues: string[];
    corrections?: Partial<
      Pick<SubsumptionCase, "expected_law" | "expected_section" | "expected_conclusion">
    >;
    notes: string;
  }
> = {
  ...
  "sub-at-035": {
    ...
    corrections: {
      expected_section: "§ 55",
      expected_conclusion:
        "Nach § 55 EheG ist eine Scheidung nach Aufhebung der häuslichen Gemeinschaft möglich, wenn die Wiederherstellung der Lebensgemeinschaft nicht erwartet werden kann.",
    },
    notes:
      "Juristische Freigabe erforderlich. § 43 EheG betrifft Wiederverheiratung nach Todeserklärung.",
  },
  ...
  "sub-at-031": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 145 BAO kann A Wiedereinsetzung in den vorigen Stand beantragen, wenn er ohne sein Verschulden an der Einhaltung einer Frist gehindert war.",
    },
  ...
  "sub-at-019": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 128 UGB haften die Gesellschafter einer OHG persönlich und unbeschränkt für die Gesellschaftsschulden.",
    },
    notes:
      "Juristische Freigabe erforderlich. § 105 UGB definiert die OHG, § 128 UGB regelt die Haftung.",
  },
  ...
  "sub-at-038": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 1167 ABGB kann A Gewährleistung geltend machen: Nachbesserung, Preisminderung oder Wandlung bei mangelhaftem Werk.",
    },
    notes:
      "Juristische Freigabe erforderlich. § 1166 definiert den Werkvertrag, § 1167 regelt die Gewährleistung.",
  },
  ...
  "sub-at-040": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 390 ABGB muss A den Fund unverzüglich dem Verlierer oder der zuständigen Behörde anzeigen und die Sache verwahren.",
    },
    notes:
      "Juristische Freigabe erforderlich. § 399 regelt die Teilung eines Schatzes, nicht die Finderpflichten.",
  },
  ...
  "sub-at-041": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 236 ZPO muss A eine Klageschrift beim zuständigen Gericht einreichen, die den Streitgegenstand und den Klageantrag enthält.",
    },
  ...
  "sub-at-036": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 761 ABGB haben die Kinder einen Pflichtteilsanspruch, auch wenn sie im Testament enterbt wurden.",
    },
    notes:
      "DISPUTED: § 762 im Pflichtteil-Kontext, aber regelt Bedingungen. Juristische Freigabe erforderlich.",
  },
  ...
  "sub-at-025": {
    ...
    corrections: {
      ...
      expected_conclusion:
        "Nach § 291 ZPO benötigt A einen Vollstreckungstitel (z.B. ein Urteil) mit Exekutionsklausel, um die Zwangsvollstreckung durchzuführen.",
    },
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--output-md" && i + 1 < args.length) {
      out.outputMd = args[++i];
      continue;
    }
    if (a === "--output-fixtures" && i + 1 < args.length) {
      out.outputFixtures = args[++i];
      continue;
    }
    if (a === "--help" || a === "-h") {
...
function buildCorpusIndex(): CorpusIndex {
  ...
  const corpusRoot = process.env.SUBSUMIO_LAW_CORPUS_DIR ?? join(REPO_ROOT, "law-corpus");
...
function conclusionReferencesSection(conclusion: string, sectionStr: string): boolean {
  ...
  return (
    c.includes(sectionStr.toLowerCase()) ||
...
function auditCase(c: SubsumptionCase, corpus: CorpusIndex): AuditEntry {
  ...
  if (
    c.expected_section &&
    !conclusionReferencesSection(c.expected_conclusion, c.expected_section)
  ) {
  ...
  if (!c.expected_conclusion || c.expected_conclusion.length < 10)
    issues.push("expected_conclusion missing or too short");
  if (!c.expected_keywords || c.expected_keywords.length === 0)
    issues.push("expected_keywords is empty");
...
function generateMarkdownReport(report: AuditReport, duplicates: string[]): string {
  ...
  for (const id of report.known_at_errors) {
    ...
    if (e.corrections) {
      ...
      if (e.corrections.expected_law)
        lines.push(`  - expected_law → ${e.corrections.expected_law}`);
      if (e.corrections.expected_section)
        lines.push(`  - expected_section → ${e.corrections.expected_section}`);
      if (e.corrections.expected_conclusion)
        lines.push(`  - expected_conclusion → ${e.corrections.expected_conclusion}`);
    }
  ...
  for (const e of report.cases) {
    ...
    lines.push(
      `| ${e.case_id} | ${e.jurisdiction} | ${e.expected_law} | ${e.expected_section} | ${e.status} | ${issueCount} |`
    );
  }
  lines.push("");
  ...
  for (const e of report.cases) {
    ...
    if (e.corrections) {
      ...
      if (e.corrections.expected_section)
        lines.push(`  - section → ${e.corrections.expected_section}`);
      if (e.corrections.expected_conclusion)
        lines.push(`  - conclusion → ${e.corrections.expected_conclusion}`);
    }
    lines.push(`- **Notes:** ${e.notes}`);
    lines.push("");
  }
...
function main() {
  ...
  if (opts.outputFixtures) {
    ...
    for (const e of entries) {
      ...
      if (e.corrections?.expected_section)
        corrected.expected_section = e.corrections.expected_section;
      if (e.corrections?.expected_conclusion)
        corrected.expected_conclusion = e.corrections.expected_conclusion;
      lines.push(JSON.stringify(corrected));
    }
    writeFileSync(opts.outputFixtures, lines.join("\n") + "\n");
    process.stderr.write(
      `[audit] Corrected fixtures written to ${opts.outputFixtures} (${lines.length} cases)\n`
    );
  }
  ...
  for (const s of ["valid", "corrected", "removed", "disputed"] as CaseStatus[]) {
    process.stderr.write(
      `  ${s}: ${byStatus[s]} (${((byStatus[s] / entries.length) * 100).toFixed(1)}%)\n`
    );
  }
  for (const [jur, stats] of Object.entries(byJurisdiction)) {
    process.stderr.write(
      `  ${jur}: valid=${stats.valid} corrected=${stats.corrected} disputed=${stats.disputed} removed=${stats.removed}\n`
    );
  }
}
