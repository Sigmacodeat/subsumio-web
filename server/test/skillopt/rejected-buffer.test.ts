...
describe("rejectedKey", () => {
  ...
  test("different edits → different key", () => {
    const text = "v1";
    expect(rejectedKey(text, [{ op: "add", anchor: "A", content: "1" }])).not.toBe(
      rejectedKey(text, [{ op: "add", anchor: "B", content: "1" }])
    );
  });
});
...
describe("loadRejectedBuffer + saveRejectedBuffer", () => {
  ...
  test("round-trips a single entry", () => {
    const entry = makeRejectedEntry(
      "v1",
      [{ op: "add", anchor: "A", content: "x" }],
  ...
  test("LRU cap evicts oldest entries beyond REJECTED_BUFFER_CAP", () => {
    ...
    for (let i = 0; i < REJECTED_BUFFER_CAP + 20; i++) {
      const e = makeRejectedEntry(
        `v${i}`,
        [{ op: "add", anchor: "A", content: String(i) }],
        "reason"
      );
