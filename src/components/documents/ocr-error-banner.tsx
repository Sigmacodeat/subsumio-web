  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {isErrorState && (
          <button
            onClick={handleRetry}
            disabled={retrying}
            className="mt-2 flex items-center gap-1.5 rounded-lg border border-red-500/20 bg-red-500/5 px-2.5 py-1 text-xs font-medium text-red-600 transition-[background-color,border-color,color] hover:bg-red-500/10 active:scale-[0.97] disabled:opacity-50 motion-reduce:transition-none"
          >
      ...
      <button
        ...
        className="shrink-0 text-[color:var(--ds-text-muted)] transition-[background-color,border-color,color] hover:text-[color:var(--ds-text)] active:scale-[0.97] motion-reduce:transition-none"
        aria-label="Dismiss"
      >
        <X size={14} />
      </button>
