...
function writeAcme(body: string): void {
  writeFileSync(
    join(repoPath, "companies/acme.md"),
    ["---", "type: company", "title: Acme", "---", "", body].join("\n")
  );
}

async function stampOf(slug: string): Promise<string | null> {
  const rows = await engine.executeRaw<{ links_extracted_at: string | null }>(
    `SELECT links_extracted_at FROM pages WHERE slug = $1 AND source_id = 'default'`,
    [slug]
  );
...
describe("#1696 — inline sync extract stamps links_extracted_at", () => {
  ...
  beforeEach(async () => {
    ...
    writeFileSync(
      ...
      ["---", "type: person", "title: Alice", "---", "", "Alice is a founder."].join("\n")
    );
