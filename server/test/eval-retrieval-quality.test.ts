...
async function seedPage(
  ...
  const ci: ChunkInput[] = chunks.map((text, i) => ({
    chunk_index: i,
    chunk_text: text,
...
beforeAll(async () => {
  ...
  __setEmbedTransportForTests(() => {
    throw new Error("stub: no embed in NamedThingBench test");
  });
  ...
  await seedPage("people/alice-example", "Alice Example", "person", [
...
describe("NamedThingBench gate (hermetic)", () => {
  ...
  test("hard-negative: the named projects do NOT surface for an unrelated query", async () => {
    const rs = await hybridSearch(engine, "quarterly tax filing checklist", {
      limit: 10,
      sourceId: "default",
    });
