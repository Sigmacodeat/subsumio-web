...
describe("runExtractCore — incremental cycle path (#417)", () => {
  ...
  test("6. dryRun: true does not invoke addLinksBatch / addTimelineEntriesBatch", async () => {
    ...
    (engine as unknown as { addLinksBatch: typeof originalAddLinks }).addLinksBatch = async (
      ...args
    ) => {
    ...
    (
      engine as unknown as { addTimelineEntriesBatch: typeof originalAddTimeline }
    ).addTimelineEntriesBatch = async (...args) => {
