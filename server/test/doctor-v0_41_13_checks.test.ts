...
describe("doctor — v0.41.16.0 new checks emit", () => {
  ...
  test("progressive_batch_audit_health shape", () => {
    const env = runDoctor();
    const check = env.checks.find((c) => c.name === "progressive_batch_audit_health");
  ...
  test("conversation_parser_probe_health shape + opt-in hint", () => {
    const env = runDoctor();
    const check = env.checks.find((c) => c.name === "conversation_parser_probe_health");
    expect(check).toBeDefined();
    expect(check!.status).toBe("ok");
    expect(check!.message).toContain("opt-in");
    expect(check!.message).toContain("autopilot.conversation_parser_probe.enabled true");
  });
