...
function makeResult(opts: {
  ...
  const shouldQuarantine =
    !!opts.hard || junk_pattern_matches.length > 0 || literal_substring_matches.length > 0;
    ...
    try {
      await withEnv({ GBRAIN_AUDIT_DIR: dir }, async () => {
        const result = makeResult({
          hard: true,
          pattern: "cloudflare_attention_required",
          bytes: 287,
        });
    ...
    try {
      await withEnv({ GBRAIN_AUDIT_DIR: dir }, async () => {
        logContentSanityAssessment(
          "a",
          "src",
          makeResult({ hard: true, pattern: "access_denied" })
        );
