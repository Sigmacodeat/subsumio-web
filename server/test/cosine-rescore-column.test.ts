...
beforeAll(async () => {
  ...
  const v1536a = new Array(1536)
    .fill(0)
    .map(() => 0.001)
    .join(",");
  const v1536b = new Array(1536)
    .fill(0)
    .map(() => 0.002)
    .join(",");
  const v1024a = new Array(1024)
    .fill(0)
    .map(() => 0.5)
    .join(",");
  const v1024b = new Array(1024)
    .fill(0)
    .map(() => 0.6)
    .join(",");

  await (engine as any).db.query(`UPDATE content_chunks SET embedding = $1::vector WHERE id = $2`, [
    `[${v1536a}]`,
    chunkId,
  ]);
  await (engine as any).db.query(`UPDATE content_chunks SET embedding = $1::vector WHERE id = $2`, [
