...
describe("recipe: zhipu", () => {
  ...
  test("dimsProviderOptions threads dimensions for embedding-3 (Matryoshka)", async () => {
    ...
    expect(dimsProviderOptions("openai-compatible", "embedding-3", 1024)).toEqual({
      openaiCompatible: { dimensions: 1024 },
    });
    expect(dimsProviderOptions("openai-compatible", "embedding-3", 2048)).toEqual({
      openaiCompatible: { dimensions: 2048 },
    });
    // embedding-2 is fixed-dim; no passthrough.
    expect(dimsProviderOptions("openai-compatible", "embedding-2", 1024)).toBeUndefined();
  });
});
