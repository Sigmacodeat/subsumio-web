...
describe("validateAndResolveUrl — static rejections (via isInternalUrl)", () => {
  test("rejects http://127.0.0.1 (loopback)", async () => {
    await expect(validateAndResolveUrl("http://127.0.0.1/img.png")).rejects.toBeInstanceOf(
      SSRFError
    );
  });
  ...
  test("rejects metadata.google.internal hostname", async () => {
    await expect(validateAndResolveUrl("http://metadata.google.internal/")).rejects.toBeInstanceOf(
      SSRFError
    );
  });
