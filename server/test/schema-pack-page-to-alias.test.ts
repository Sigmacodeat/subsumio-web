  ...
  describe("apply", () => {
    it("inserts slug_aliases row + soft-deletes source", async () => {
      await seed(
        ...
        "concept",
        "canonical body that is sufficiently long for any backstop guards we have in the codebase"
      );
      await seed(
        "wiki/concepts/redirect-1",
        "concept-redirect",
    ...
    it("rejects self-references", async () => {
      await seed(
        "wiki/concepts/canonical",
        "concept",
        "canonical body that is sufficiently long for any backstop guards"
      );
      await seed(
        "wiki/concepts/self",
        "concept-redirect",
    ...
    it("is idempotent on UNIQUE conflict (re-run no-op)", async () => {
      ...
      await engine.executeRaw(
        `UPDATE pages SET deleted_at = NULL WHERE slug = 'wiki/concepts/redirect-1'`
      );
