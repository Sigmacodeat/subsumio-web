...
describe("Bug 9 — sync-failures JSONL helpers", () => {
  ...
  test("recordSyncFailures upserts per path, incrementing attempts", async () => {
    const { recordSyncFailures, loadSyncFailures, syncFailuresPath } =
  ...
  test("acknowledgeSyncFailures marks unacked entries, leaves acked alone", async () => {
    const { recordSyncFailures, acknowledgeSyncFailures, loadSyncFailures } =
      await import("../src/core/sync.ts");
  ...
  test("unacknowledgedSyncFailures filters correctly", async () => {
    const { recordSyncFailures, acknowledgeSyncFailures, unacknowledgedSyncFailures } =
      await import("../src/core/sync.ts");
  ...
  test("loadSyncFailures tolerates malformed lines", async () => {
    const { loadSyncFailures, syncFailuresPath, recordSyncFailures } =
      await import("../src/core/sync.ts");
...
describe("Bug 9 — sync.ts CLI flag wiring", () => {
  ...
  test("runSync acks pre-existing unacked failures up-front when --skip-failed is set", async () => {
    ...
    expect(source).toMatch(
      /if \(skipFailed\) \{[\s\S]*?unacknowledgedSyncFailures\(\)[\s\S]*?acknowledgeSyncFailures\(\)/
    );
  });
...
describe("classifyErrorCode — error message to code mapping", () => {
  ...
  test("classifies STATEMENT_TIMEOUT", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("canceling statement due to statement timeout")).toBe(
      "STATEMENT_TIMEOUT"
    );
  });
...
describe("classifyErrorCode — DB vs YAML duplicate-key disambiguation", () => {
  test("Postgres unique-constraint violation classifies as DB_DUPLICATE_KEY", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(
      classifyErrorCode('duplicate key value violates unique constraint "pages_slug_key"')
    ).toBe("DB_DUPLICATE_KEY");
  });

  test("YAML duplicated mapping key still classifies as YAML_DUPLICATE_KEY", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode('YAMLException: duplicated mapping key "title"')).toBe(
      "YAML_DUPLICATE_KEY"
    );
  });
  ...
  test("DB pattern is checked BEFORE YAML so DB errors are not mislabeled", async () => {
    ...
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(
      classifyErrorCode('duplicate key value violates unique constraint on table "pages"')
    ).toBe("DB_DUPLICATE_KEY");
    expect(
      classifyErrorCode('duplicate key value violates unique constraint on table "pages"')
...
describe("classifyErrorCode — canonical message coverage", () => {
  test('MISSING_OPEN matches "File is empty or whitespace-only"', async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(
      classifyErrorCode("File is empty or whitespace-only; expected frontmatter starting with ---")
    ).toBe("MISSING_OPEN");
  });

  test('MISSING_OPEN matches "Frontmatter must start with ---"', async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Frontmatter must start with --- on the first non-empty line")).toBe(
      "MISSING_OPEN"
    );
  });
  ...
  test('MISSING_CLOSE matches "Heading at line N found inside frontmatter"', async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(
      classifyErrorCode("Heading at line 5 found inside frontmatter zone (closing --- comes after)")
    ).toBe("MISSING_CLOSE");
  });
  ...
  test('NULL_BYTES matches "Content contains null bytes"', async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Content contains null bytes (likely binary corruption)")).toBe(
      "NULL_BYTES"
    );
  });
  ...
  test('NESTED_QUOTES matches "Nested double quotes"', async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Nested double quotes in YAML value at line 3")).toBe("NESTED_QUOTES");
  });
});
...
describe("v0.41.6.0 D2 — embedding error classification", () => {
  test("EMBEDDING_NO_CREDS matches native-openai verbatim throw", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("OpenAI embedding requires OPENAI_API_KEY.")).toBe(
      "EMBEDDING_NO_CREDS"
    );
  });
  ...
  test("EMBEDDING_NO_CREDS matches native-google verbatim throw", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Google embedding requires GOOGLE_GENERATIVE_AI_API_KEY.")).toBe(
      "EMBEDDING_NO_CREDS"
    );
  });
  ...
  test("EMBEDDING_NO_CREDS matches Voyage AI openai-compat shape", async () => {
    ...
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Voyage AI embedding requires VOYAGE_API_KEY.")).toBe(
      "EMBEDDING_NO_CREDS"
    );
  });
  ...
  test("EMBEDDING_NO_CREDS matches ZeroEntropy openai-compat shape", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("ZeroEntropy embedding requires ZEROENTROPY_API_KEY.")).toBe(
      "EMBEDDING_NO_CREDS"
    );
  });
  ...
  test("EMBEDDING_NO_CREDS matches DeepSeek openai-compat shape", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("DeepSeek embedding requires DEEPSEEK_API_KEY.")).toBe(
      "EMBEDDING_NO_CREDS"
    );
  });
  ...
  test("EMBEDDING_NO_CREDS matches the literal token (back-compat)", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("EMBEDDING_NO_CREDS — VOYAGE_API_KEY missing")).toBe(
      "EMBEDDING_NO_CREDS"
    );
  });
  ...
  test("EMBEDDING_NO_TOUCHPOINT matches anthropic-as-embed-provider misconfig", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(
      classifyErrorCode("Anthropic has no embedding model. Use openai or google for embeddings.")
    ).toBe("EMBEDDING_NO_TOUCHPOINT");
  });

  test("EMBEDDING_RATE_LIMIT matches HTTP 429 phrasing", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Request failed with status 429: rate limit exceeded")).toBe(
  ...
  test('EMBEDDING_RATE_LIMIT matches OpenAI "too many requests" phrasing', async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("OpenAIRateLimitError: too many requests in 1m")).toBe(
      "EMBEDDING_RATE_LIMIT"
    );
  });
  ...
  test("EMBEDDING_QUOTA matches Anthropic credit-balance message", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode("Your credit balance is too low to continue")).toBe("EMBEDDING_QUOTA");
  });
  ...
  test("SLUG_MISMATCH still classifies correctly", async () => {
    const { classifyErrorCode } = await import("../src/core/sync.ts");
    expect(classifyErrorCode('Frontmatter slug "x" does not match path-derived slug "y"')).toBe(
      "SLUG_MISMATCH"
    );
  });
