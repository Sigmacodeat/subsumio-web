...
let syncCalls: Array<{
  ...
  noPull: boolean | undefined;
  noExtract: boolean | undefined;
...
mock.module("../../src/commands/extract.ts", () => ({
  runExtractCore: async (_engine: any, opts: any) => {
    ...
    return {
      ...
      pages_processed: opts.slugs?.length ?? 5,
    };
  },
  walkMarkdownFiles: () => [],
...
describe("runCycle — yieldBetweenPhases hook", () => {
  ...
  test("hook is called between every phase", async () => {
    ...
    // v0.43: 26 phases (+4 legal phases). v0.44: legal_commentary_synthesis
    // added to CyclePhase but not ALL_PHASES (opt-in).
    // v0.45: 29 phases (+engram_maturation, +reconsolidation_sweep,
    // +legal_commentary_synthesis added to ALL_PHASES).
  ...
  test("hook exceptions do not abort the cycle", async () => {
    ...
    // v0.43: 26 phases (+4 legal phases). v0.44: 27 (+legal_commentary_synthesis).
    // v0.45: 29 phases (+engram_maturation, +reconsolidation_sweep,
    // +legal_commentary_synthesis added to ALL_PHASES).
