...
describe("checkFirmRetention", () => {
  it("clear when only open matters and expired receipts exist", async () => {
    byType.set("legal_case", [{ slug: "a", frontmatter: { status: "open" } }]);
    byType.set("invoice", [
  ...
  it("retained for a closed matter and a receipt in its period", async () => {
    ...
    expect(r).toEqual({ status: "retained", cases: ["closed"], receipts: 1, until: "2032-12-31" });
