...
describe("v0.41 T5: parseAtomsResponse", () => {
  ...
  test("clamps virality_score to [0, 100]", () => {
    expect(
      parseAtomsResponse(`[{"title":"a","atom_type":"insight","body":"b","virality_score":150}]`)[0]
        .virality_score
    ).toBeUndefined();
    expect(
      parseAtomsResponse(`[{"title":"a","atom_type":"insight","body":"b","virality_score":-5}]`)[0]
        .virality_score
    ).toBeUndefined();
    expect(
      parseAtomsResponse(`[{"title":"a","atom_type":"insight","body":"b","virality_score":75}]`)[0]
        .virality_score
    ).toBe(75);
  });
});
...
describe("v0.41 T5: runPhaseExtractAtoms via stubbed chat", () => {
  ...
  test("extracts atoms from transcript via stub chat", async () => {
    ...
    const result = await runPhaseExtractAtoms(engine, {
      _transcripts: [
        { filePath: "/fake/meeting.txt", content: "content", contentHash: "abc123def" },
      ],
