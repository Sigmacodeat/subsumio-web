...
describe("isConnectionError", () => {
  ...
  it("detects too many connections", () => {
    expect(isConnectionError(new Error('FATAL: too many connections for role "postgres"'))).toBe(
      true
    );
  });
  ...
  it("does not match constraint violations", () => {
    expect(isConnectionError(new Error("duplicate key value violates unique constraint"))).toBe(
      false
    );
  });
});
...
describe("Eng-review D3 — executeRaw has no per-call retry wrapper", () => {
  it("PostgresEngine.executeRaw is a single-statement passthrough (no try/catch on connection errors)", () => {
    ...
    const fnMatch = src.match(
      /async executeRaw<T = Record<string, unknown>>\(\s*sql: string,\s*params\?: unknown\[\][^)]*\):\s*Promise<T\[\]>\s*\{([\s\S]*?)\n  \}/
    );
    ...
    const directMatch = src.match(
      /async executeRawDirect<T = Record<string, unknown>>\(\s*sql: string,\s*params\?: unknown\[\][^)]*\):\s*Promise<T\[\]>\s*\{([\s\S]*?)\n  \}/
    );
    ...
    const helperMatch = src.match(
      /private runUnsafe<T>\(\s*conn:[^)]*\):\s*Promise<T\[\]>\s*\{([\s\S]*?)\n  \}/
    );
