...
describe("parseMarkdown validation surface", () => {
  ...
  describe("YAML_PARSE", () => {
    test("malformed YAML inside frontmatter triggers error", () => {
      ...
      const hasParse = parsed.errors!.some(
        (e) => e.code === "YAML_PARSE" || e.code === "NESTED_QUOTES"
      );
    ...
    test("parity: validator per-value safeLoad agrees with gray-matter full-document parse", () => {
      const cases: { md: string; shouldFlag: boolean; label: string }[] = [
        // Valid: gray-matter parses cleanly, validator should NOT flag.
        {
          md: `${fence}\ntype: concept\ntags: ["yc", "w2025"]\n${fence}\n\nbody`,
          shouldFlag: false,
          label: "JSON-style array (valid YAML)",
        },
        {
          md: `${fence}\ntype: concept\ntags: ['yc', 'w2025']\n${fence}\n\nbody`,
          shouldFlag: false,
          label: "single-quoted array",
        },
        {
          md: `${fence}\ntype: concept\ntitle: 'a: "b" "c"'\n${fence}\n\nbody`,
          shouldFlag: false,
          label: "single-quoted scalar with literal inner quotes",
        },
        {
          md: `${fence}\ntype: concept\ntitle: ok\n${fence}\n\nbody`,
          shouldFlag: false,
      ...
      for (const c of cases) {
        ...
        if (c.shouldFlag) {
          expect(
            errors.length,
            `[${c.label}] expected at least one NESTED_QUOTES or YAML_PARSE error`
          ).toBeGreaterThan(0);
        } else {
          expect(
            errors.length,
            `[${c.label}] expected no NESTED_QUOTES/YAML_PARSE errors but got ${JSON.stringify(errors)}`
          ).toBe(0);
        }
      }
    });
