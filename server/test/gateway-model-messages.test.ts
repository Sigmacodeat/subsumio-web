...
describe("toModelMessages — v6 ModelMessage shape", () => {
  ...
  test("assistant text block maps to {type:text,text}", () => {
    const msgs: ChatMessage[] = [{ role: "assistant", content: [{ type: "text", text: "hi" }] }];
    ...
    const msgs: ChatMessage[] = [
      {
        role: "user",
        content: [
          { type: "tool-result", toolCallId: "c1", toolName: "search", output: { hits: 0 } },
        ],
      },
    ];
    expect(toModelMessages(msgs)).toEqual([
        ...
        content: [
          {
            ...
            toolCallId: "c1",
            toolName: "search",
            output: { type: "error-text", value: '{"msg":"boom"}' },
          },
        ],
      },
    ]);
