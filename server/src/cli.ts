...
const CLI_ONLY_SELF_HELP = new Set([
  ...
  "connect",
  // v0.44.0: legal brain has its own HELP constant with subcommands.
  "legal",
]);
...
async function main() {
  ...
  if (command === "search" && ["modes", "stats", "tune", "diagnose"].includes(subArgs[0] ?? "")) {
    ...
    try {
      ...
      if (e instanceof OperationTimeoutError) {
        console.error(`${e.label} timed out.`);
        process.exit(124);
      }
      throw e;
    }
  ...
  const queryHasAlt =
    op.name === "query" && typeof params.image === "string" && params.image.length > 0;
    ...
    if (e instanceof RemoteMcpError) {
      ...
      switch (e.reason) {
        ...
        case "auth_after_refresh":
          console.error(
            "OAuth auth failed after token refresh. Credentials may have been revoked."
          );
          console.error("Run `gbrain remote doctor` to confirm.");
          break;
        case "network":
          if (e.detail?.kind === "timeout") {
            const hint = cliOpts.timeoutMs
              ? ""
              : ` (default ${defaultTimeoutMs}ms; pass --timeout=Ns to override)`;
...
async function fetchIdentity(cfg: GBrainConfig, signal: AbortSignal): Promise<BrainIdentity> {
...
function formatResult(opName: string, result: unknown): string {
  switch (opName) {
    ...
    case "get_timeline": {
      ...
      return (
        entries
          .map((e) => `${e.date}  ${e.summary}${e.source ? ` [${e.source}]` : ""}`)
          .join("\n") + "\n"
      );
    }
    case "get_versions": {
...
const THIN_CLIENT_REFUSE_HINTS: Record<string, string> = {
  ...
  embed:
    "embed runs on the host as part of the autopilot cycle. `gbrain remote ping` triggers a full cycle including embed.",
  extract:
    "extract runs on the host. Use `gbrain remote ping` to trigger a cycle including extract.",
  "extract-conversation-facts":
    "extract-conversation-facts runs on the host (requires local engine + chat gateway). Run on the host machine.",
  enrich:
    "enrich runs on the host (requires local engine + chat gateway for grounded synthesis). Run on the host machine.",
  ...
  dream:
    "dream runs the autopilot cycle on the host. `gbrain remote ping` queues one. (Native `gbrain dream` thin-client routing planned for v0.31.2.)",
  orphans:
    "orphans needs the host's brain. Run on the host or use the `find_orphans` MCP tool from your agent.",
  transcripts:
    "transcripts is server-private (raw chat exports stay on the host). Read transcripts on the host machine.",
  storage: "storage operates on the local repo on disk. Run on the host.",
  takes:
    "takes mutate subcommands edit local .md files; routing the read subcommands lands in v0.31.x. For now: use `takes_list` and `takes_search` MCP tools from your agent, or run on the host.",
  sources:
    "sources commands manage local DB + config rows. Per-subcommand thin-client routing lands in v0.31.x. For now: use `sources_list` / `sources_status` MCP tools, or run on the host.",
  // v0.32 audit additions
  pages:
    "`pages purge-deleted` is admin+localOnly (hard-deletes from the local DB). Run on the host.",
  files:
    "`files list` and `files url` MCP ops are localOnly (paths live on the host filesystem). Use `gbrain files` on the host machine.",
  eval: "`eval` export/prune/replay touch the local engine and have no MCP equivalents. Run `gbrain eval` on the host.",
  "code-def":
    "`code-def` needs symbol-aware lookup that has no MCP op yet. Run on the host or use `search` from your agent with a symbol-shaped query.",
...
async function handleCliOnly(command: string, args: string[]) {
  ...
  if (command === "legal") {
    const { runLegalCli } = await import("./commands/legal.ts");
    await runLegalCli(args);
    return;
  }
  if (command === "upgrade") {
  ...
  if (command === "doctor") {
    ...
    if (args.includes("--remediation-plan")) {
      ...
      try {
        await runRemediationPlan(eng, args);
      } finally {
  ...
  const readOnlyDefaultTimeoutMs =
    command === "search"
      ? 30_000
      : command === "sources" && (args[0] === "list" || args[0] === undefined)
        ? 10_000
        : null;
  ...
  try {
    switch (command) {
      ...
      case "extract-conversation-facts": {
        const { runExtractConversationFacts } =
          await import("./commands/extract-conversation-facts.ts");
      ...
      case "reindex": {
        if (args.includes("--multimodal")) {
          ...
          const workers =
            workersValIdx > 0 && workersValIdx < args.length
          ...
          if (args.includes("--json")) {
            ...
            console.log(
              `reindex --multimodal: ${result.reembedded} re-embedded, ${result.failed} failed, ${result.pending_after} pending. est. cost: $${result.cost_usd_estimate.toFixed(2)}`
            );
          }
          break;
        }
      ...
      case "notability-eval": {
        ...
        try {
          repoPath =
            (flags.repo as string) || (await engine.getConfig("sync.repo_path")) || undefined;
        } catch {
          /* engine may not be connected for help */
        }
 ...
 */
async function dispatchReadOnlyCommand(
  engine: BrainEngine,
  command: string,
  args: string[]
): Promise<void> {
...
async function connectEngine(opts?: { probeOnly?: boolean }): Promise<BrainEngine> {
  ...
  const noRetry =
    process.argv.includes("--no-retry-connect") || process.env.GBRAIN_NO_RETRY_CONNECT === "1";
