...
| P0-SECR-002   | Identitäts-Fundament fertig (Matter-Scope-Enforcement offen) | Spec: `docs/designs/PAKET_33_PROAKTIVER_KI_SEKRETAER_SPEC.md`. DB-gestützte WhatsApp-Identität ersetzt die env-only-Bindung: `src/lib/whatsapp/types.ts` (`WhatsAppIdentity`: id, orgId, phoneHash, matterScope, status, verifiedAt), `src/lib/whatsapp/identity-store.ts` (File-/Postgres-Dual-Adapter nach `api-key-store.ts`-Muster, Tabelle `subsumio_whatsapp_identities`, Lookup per `phoneHash`), `src/lib/whatsapp/identity.ts` (`resolveSenderIdentity` — **Prod ohne env-Fallback (Leak-Gate)**, dev mit env-Fallback für DX; `identityCanAccessMatter`). Webhook `src/app/api/whatsapp/webhook/route.ts` nutzt `resolveSenderIdentity`; abgewiesene Absender werden via `whatsapp.sender_denied` nur per Phone-Hash auditiert. Tests: `identity.test.ts` (10, inkl. LEAK-GUARD prod-kein-Fallback + revoked/suspended/unknown→deny) + `identity-store.test.ts` (6, CRUD + Tenant-Isolation) = 16 grün. Verifiziert mit `npx vitest run` (Node 20; **Achtung:** lokaler Default-Node ist v16, vitest 3.x braucht Node ≥18) und `bun run typecheck` (0 Fehler). **Offen:** Bindung jedes `brain_query`/`search`-Intents an den Matter Context API Contract (Paket 31, noch nicht gebaut) — `matterScope` wird bereits durchgereicht, aber noch nicht gegen Engine-seitige Permission-Filter erzwungen. |
...
- `P0-SEC-003`: Idempotency für alle Webhooks und Workflow-Steps durchsetzen (`src/lib/idempotency.ts`). **Status: Webhooks fertig** — Stripe (eigene Events-Tabelle), DocuSign (`createIdempotencyStore`), WhatsApp (Message-Dedup) und Resend (`ON CONFLICT (provider_id)`) sind idempotent; `/api/webhook/incoming` von in-memory auf den durablen Postgres-backed Store umgestellt. Tests: `src/lib/idempotency.test.ts` (4). Workflow-Step-Idempotency folgt mit Paket 3 (Workflow Engine).
...
- `P1-SECR-001`: WhatsApp als dritten Kanal an den Notification-Event-Bus (Paket 29) hängen; `cron/deadline-reminders` (heute nur E-Mail) und `cron/deadlines` auf den gemeinsamen Bus konsolidieren.
- `P0-SECR-002`: Permission-aware WhatsApp-Sekretär — Nummer ↔ Identität ↔ Rolle/Org/Matter/Ethical-Wall binden; jeder `brain_query`/`search`-Intent über den Matter Context API Contract (Paket 31), Cross-Tenant-/Cross-Matter-Leak-Tests, Source Leakage Rate = 0. **Status: Identitäts-Fundament fertig** — DB-gestützte `WhatsAppIdentity` + `resolveSenderIdentity` (Prod ohne env-Fallback, Leak-Guard-Test grün), Webhook umgestellt, Denial-Audit, 16 Tests. Offen: Matter-Scope-Enforcement gegen Paket 31 (noch nicht gebaut).
...
- `P1-SECR-004`: Proaktives Tagesbriefing (Fristen/Approvals/neue Dokumente/Konflikte) gegroundet aus Matter Context (Paket 31) mit Citation Gate (Paket 1), pro Anwalt konfigurierbar. **Status: MVP fertig** — Fristen-Briefing (`buildDailyBriefing`) + Cron um 06:30, Versand über das Outbound-Gate, dedup pro Tag, kein Spam bei leerem Tag; 6 Tests. Offen: Superbrain-Groundung + Approvals/Dokumente/Konflikte + Event-Bus.
- `P1-SECR-005`: Approval-/Aktions-Rückkanal über WhatsApp (Quick-Reply/Flows) an Paket 3 Approval Gates und Paket 22 Filing binden; jede Aktion auditiert.
- `P1-SECR-006`: Sekretär-Eval-Gate (Proactive Precision/Anti-Spam, Leak-Rate = 0, Consent-Compliance = 100%, Template-Window-Verletzungen = 0) in die Eval-Pipeline (Paket 14/31) hängen. **Status: fertig** — `computeSecretaryMetrics` + admin Read-API `/api/legal/secretary-metrics` (`gatePass`-Signal), datenbasierte Consent-Compliance via `hadConsent`-Audit; 6 Tests. Offen: Feedback-Capture-Kanal für Proactive Precision.
