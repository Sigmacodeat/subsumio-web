...
async function extractPdf(buf: Buffer): Promise<ExtractedDocument> {
  ...
  if (totalPages > 0 && cleaned.length < totalPages * PDF_SPARSE_TEXT_CHARS_PER_PAGE) {
    ...
    if (ocrText) {
      warnings.push(
        `pdf_ocr_fallback: OCR extracted ${ocrText.length} chars from ${totalPages} page(s).`
      );
      return {
...
async function tryOcrFallback(pdfBuf: Buffer, totalPages: number): Promise<string> {
  ...
  const capped = maxPages > 0 && totalPages > maxPages;
  const pagesToConvert = capped ? maxPages : totalPages;
  ...
  try {
    if (capped) {
      // Convert only the first `maxPages` pages, one at a time (1-indexed).
      images = [];
      for (let pageNo = 1; pageNo <= pagesToConvert; pageNo++) {
        images.push(await convert(pageNo));
      }
    } else {
      images = await convert.bulk(-1);
    }
  } catch {
    // pdf2pic failed — likely GraphicsMagick/Ghostscript not installed.
    return "";
  }
  ...
  if (capped) {
    pageTexts.push(
      `> ⚠️ OCR auf die ersten ${maxPages} von ${totalPages} Seiten begrenzt ` +
        `(GBRAIN_OCR_MAX_PAGES). Restliche Seiten sind nicht durchsuchbar.`
    );
  }
...
async function extractEml(buf: Buffer): Promise<ExtractedDocument> {
  ...
  if (attachments.length > 0) {
    warnings.push(
      `eml: ${attachments.length} attachment(s) not extracted (${attachments.join(", ")}) — import them as their own files.`
    );
  }
