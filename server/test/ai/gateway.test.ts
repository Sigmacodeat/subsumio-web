...
describe("Voyage flexible-dim runtime validation", () => {
  test("rejects 1536 (the default that bites Voyage-first users) with AIConfigError", () => {
    expect(() => dimsProviderOptions("openai-compatible", "voyage-4-large", 1536)).toThrow(
      AIConfigError
    );
    expect(() => dimsProviderOptions("openai-compatible", "voyage-4-large", 1536)).toThrow(
  ...
  test("rejects 3072 with AIConfigError", () => {
    expect(() => dimsProviderOptions("openai-compatible", "voyage-3-large", 3072)).toThrow(
      AIConfigError
    );
  });
  ...
  test("checks every returned vector dimension, not just the first one", async () => {
    ...
    try {
      ...
      await expect(embed(["first", "second"])).rejects.toThrow(
        "returned 768 but schema expects 1536"
      );
    } finally {
      globalThis.fetch = originalFetch;
    }
