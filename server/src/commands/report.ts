...
export async function runReport(args: string[]) {
  ...
  if (reportType && !/^[a-z0-9][a-z0-9-]*$/.test(reportType)) {
    console.error(
      'Report type must be lowercase alphanumeric with hyphens only (e.g., "enrichment-sweep")'
    );
    process.exit(1);
  }
  ...
  if (!reportType) {
    console.error(
      'Usage: gbrain report --type <name> --title "..." --content "..." [--dir <brain>]'
    );
    ...
    console.error("");
    console.error(
      "  Common types: enrichment-sweep, meeting-sync, maintenance, backlink-check, lint"
    );
