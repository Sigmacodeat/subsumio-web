...
function seedPack(
  packName: string,
  format: "json" | "yaml",
  initial?: Partial<SchemaPackManifest>
): string {
  ...
  const body =
    format === "json"
      ...
      : (require("../src/core/schema-pack/mutate.ts").emitYaml?.(manifest) ??
        buildSimpleYaml(manifest));
...
describe("locateMutablePackFile — bundled guard", () => {
  it("rejects gbrain-base with PACK_READONLY + fork hint", () => {
    ...
    try {
      locateMutablePackFile("gbrain-base");
    } catch (e) {
  ...
  it("rejects gbrain-recommended with PACK_READONLY", () => {
    try {
      locateMutablePackFile("gbrain-recommended");
    } catch (e) {
  ...
  it("rejects gbrain-base-v2 with PACK_READONLY (bundled guard)", () => {
    try {
      locateMutablePackFile("gbrain-base-v2");
    } catch (e) {
      expect((e as SchemaPackMutationError).code).toBe("PACK_READONLY");
    }
  });
...
describe("addTypeToPack", () => {
  it("appends a new type to JSON pack and writes atomically", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      ...
      const result = await addTypeToPack(
        "mine",
        {
          name: "researcher",
          primitive: "entity",
    ...
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      ...
      await expect(
        addTypeToPack(
          ...
          {
            name: "bad",
            primitive: "invalid_primitive" as never,
            prefix: "x/",
          } as never,
          { lockDir }
        )
      ).rejects.toMatchObject({ code: "INVALID_PRIMITIVE" });
    });
  });
...
describe("removeTypeFromPack", () => {
  ...
  it("TYPE_NOT_FOUND when type does not exist", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      seedPack("mine", "json");
      await expect(removeTypeFromPack("mine", "ghost", { lockDir })).rejects.toMatchObject({
        code: "TYPE_NOT_FOUND",
      });
    });
  });
  ...
  it("CODEX C14: refuses removal when another type aliases the target", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      ...
      await expect(removeTypeFromPack("mine", "person", { lockDir })).rejects.toMatchObject({
...
describe("updateTypeOnPack", () => {
  ...
  it("name field on patch is ignored (name is identity)", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      const path = seedPack("mine", "json");
      await updateTypeOnPack(
        "mine",
        { name: "person", patch: { name: "renamed", extractable: true } as never },
        { lockDir }
      );
      const after = loadPackFromFile(path);
  ...
  it("TYPE_NOT_FOUND on patch target missing", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      seedPack("mine", "json");
      await expect(
        updateTypeOnPack("mine", { name: "ghost", patch: { extractable: true } }, { lockDir })
...
describe("addPrefixToType / removePrefixFromType", () => {
  it("addPrefix appends", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      ...
      const after = loadPackFromFile(path);
      expect(after.page_types.find((t) => t.name === "person")!.path_prefixes).toEqual([
        "people/",
        "people-archive/",
      ]);
    });
        ...
        page_types: [
          {
            name: "person",
            primitive: "entity",
            path_prefixes: ["people/", "people-archive/"],
            aliases: [],
            extractable: false,
            expert_routing: false,
          },
...
describe("addLinkTypeToPack / removeLinkTypeFromPack", () => {
  ...
  it("addLinkType rejects duplicate name", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      ...
      await expect(
        addLinkTypeToPack("mine", { name: "attended" }, { lockDir })
  ...
  it("removeLinkType refuses when frontmatter_links references it", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      ...
      await expect(removeLinkTypeFromPack("mine", "attended", { lockDir })).rejects.toMatchObject({
        code: "STILL_REFERENCED",
      });
    });
  });
...
describe("setExtractableOnType / setExpertRoutingOnType", () => {
  ...
  it("TYPE_NOT_FOUND on missing type", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_AUDIT_DIR: auditDir }, async () => {
      seedPack("mine", "json");
      await expect(setExtractableOnType("mine", "ghost", true, { lockDir })).rejects.toMatchObject({
        code: "TYPE_NOT_FOUND",
      });
    });
  });
