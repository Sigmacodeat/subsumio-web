  ...
  test("Postgres with an active build returns the row", async () => {
    const fakeEngine = {
      ...
      executeRaw: async () => [
        {
          pid: 12345,
          query: "CREATE INDEX CONCURRENTLY idx_chunks_embedding ON ...",
          application_name: "gbrain",
        },
      ],
    } as never;
    const r = await checkActiveBuild(fakeEngine, "idx_chunks_embedding");
...
describe("isSupabaseAutoMaintenance", () => {
  test('true for application_name containing "supabase"', () => {
    expect(isSupabaseAutoMaintenance({ active: true, application_name: "supabase-cron" })).toBe(
      true
    );
    expect(isSupabaseAutoMaintenance({ active: true, application_name: "postgres-meta" })).toBe(
      true
    );
  });

  test("false for gbrain", () => {
    expect(isSupabaseAutoMaintenance({ active: true, application_name: "gbrain-worker" })).toBe(
      false
    );
  });
...
describe("dropAndRebuild — A3 atomic-swap", () => {
      ...
      executeRaw: async (sql: string) => {
        if (sql.includes("pg_stat_activity")) {
          return [
            {
              pid: 555,
              query: "CREATE INDEX idx_chunks_embedding...",
  ...
  test("temp name format: <name>_rebuild_<unix-ms>", async () => {
    ...
    const fakeEngine = {
      ...
      withReservedConnection: async (fn: any) =>
        fn({
          executeRaw: async (sql: string) => {
            executedSql = sql;
            return [];
