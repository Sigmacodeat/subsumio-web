...
beforeAll(() => {
  ...
  writeFileSync(
    join(root, "people", "pedro.raw", "source.md"),
  ...
  writeFileSync(
    join(root, "people", "tools", "node_modules", "inner", "a.md"),
    "---\ntitle: nope\n---\n"
  );
...
describe("walkDir (brain-writer.ts) — descent-time pruning", () => {
  ...
  test("regression: pre-v0.38.2.0 walker would have descended into node_modules and stat'd every entry", () => {
    ...
    const vendor = descents.filter(
      (d) => /\/(node_modules|\.git|\.obsidian|ops)(\/|$)/.test(d) || /\.raw$/.test(d)
    );
