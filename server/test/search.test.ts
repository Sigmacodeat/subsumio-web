...
import { DEFAULT_RECENCY_DECAY, DEFAULT_FALLBACK } from "../src/core/search/recency-decay.ts";
...
describe("computeFloorThreshold", () => {
  ...
  test("T1a: all-NaN scores leave gate disabled", () => {
    const results: SearchResult[] = [makeResult({ score: NaN }), makeResult({ score: NaN })];
    expect(computeFloorThreshold(results, 0.85)).toBe(Number.NEGATIVE_INFINITY);
  });
...
describe("applyRecencyBoost — floor gate (T6 IRON RULE)", () => {
  ...
  test("weak result gated out from recency boost", () => {
    ...
    applyRecencyBoost(results, dates, "on", DEFAULT_RECENCY_DECAY, DEFAULT_FALLBACK, now, 0.85);
  ...
  test("floorThreshold undefined preserves prior behavior", () => {
    ...
    applyRecencyBoost(results, dates, "on", DEFAULT_RECENCY_DECAY, DEFAULT_FALLBACK, now);
