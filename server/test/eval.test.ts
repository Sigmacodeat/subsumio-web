...
import { precisionAtK, recallAtK, mrr, ndcgAtK, parseQrels } from "../src/core/search/eval.ts";
...
describe("parseQrels", () => {
  test("parses inline JSON array", () => {
    const input = JSON.stringify([{ query: "foo", relevant: ["a", "b"] }]);
  ...
  test("preserves grades when present", () => {
    const input = JSON.stringify([{ query: "baz", relevant: ["a"], grades: { a: 3, b: 1 } }]);
    const result = parseQrels(input);
