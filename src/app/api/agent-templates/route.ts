...
const createSchema = z.object({
  ...
  })).max(10, "too_many_steps").optional().default([]),
...
export const GET = createHandler(
  ...
  async (ctx, _body, query, _req) => {
    try {
      ...
      console.error("[agent-templates] list failed:", err instanceof Error ? err.message : String(err));
      return apiError("engine_unreachable", "Agent-Templates nicht abrufbar", 503);
    }
  },
);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return Response.json(
          payload.error ? payload : { error: "create_failed" },
      ...
      console.error("[agent-templates] create failed:", err instanceof Error ? err.message : String(err));
