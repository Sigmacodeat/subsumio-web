...
describe("embeddingProviderConfigured (recipe-aware helper)", () => {
  ...
  test("hosted provider configured iff its required key resolves", () => {
    expect(
      embeddingProviderConfigured("openai:text-embedding-3-small", (k) => k === "OPENAI_API_KEY")
    ).toBe(true);
  ...
  test("behavior-change: voyage judged by VOYAGE_API_KEY, not an OpenAI key", () => {
    ...
    expect(embeddingProviderConfigured("voyage:voyage-3", (k) => k === "VOYAGE_API_KEY")).toBe(
      true
    );
    expect(embeddingProviderConfigured("voyage:voyage-3", (k) => k === "OPENAI_API_KEY")).toBe(
      false
    );
  });
...
describe("computeRecommendations", () => {
  ...
  test("D9: different sources produce different idempotency keys", () => {
    const health = makeHealth({ missing_embeddings: 50 });
    const a = computeRecommendations(health, {
      repoPath: "/brain",
      embeddingProviderConfigured: true,
      sourceId: "A",
    });
    const b = computeRecommendations(health, {
      repoPath: "/brain",
      embeddingProviderConfigured: true,
      sourceId: "B",
    });
...
describe("classifyChecks (D13)", () => {
  ...
  test("human_only: orphan_pages (archive is product judgment)", () => {
    const result = classifyChecks([{ name: "orphan_pages", status: "warn" }], {
      repoPath: "/brain",
    });
