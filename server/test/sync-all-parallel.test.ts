...
describe("resolveParallelism", () => {
  test("PGLite always serial regardless of source count or flags", () => {
    ...
    expect(resolveParallelism({ sourceCount: 10, engineKind: "pglite", explicitParallel: 8 })).toBe(
      1
    );
  ...
  test("explicit --parallel wins and is clamped to sourceCount", () => {
    expect(
      resolveParallelism({ sourceCount: 4, engineKind: "postgres", explicitParallel: 2 })
    ).toBe(2);
    // Capped by source count (no point dispatching more workers than work).
    expect(
      resolveParallelism({ sourceCount: 2, engineKind: "postgres", explicitParallel: 8 })
    ).toBe(2);
    // Floor of 1.
    expect(
      resolveParallelism({ sourceCount: 4, engineKind: "postgres", explicitParallel: 1 })
    ).toBe(1);
  });
  ...
  test("single-source --all short-circuits to serial (no fan-out value)", () => {
    ...
    expect(
      resolveParallelism({ sourceCount: 1, engineKind: "postgres", explicitParallel: 8 })
    ).toBe(1);
  });
...
describe("buildSyncStatusReport", () => {
  ...
  test("count-query failure propagates (Q2 sub-fix — no silent zero-counts)", async () => {
    ...
    const sources = [{ id: "a", name: "a", local_path: "/tmp/a", config: {} }];
    const engine = {
      kind: "postgres",
      executeRaw: async (sql: string) => {
        if (/FROM sources WHERE id = ANY/i.test(sql)) {
    ...
    await expect(buildSyncStatusReport(engine, sources)).rejects.toThrow(/statement timeout/);
  });
