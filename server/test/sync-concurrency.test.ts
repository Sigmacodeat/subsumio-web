...
describe("autoConcurrency", () => {
  ...
  test("Postgres + auto path: above threshold parallel", () => {
    expect(autoConcurrency(engineOfKind("postgres"), AUTO_CONCURRENCY_FILE_THRESHOLD + 1)).toBe(
      DEFAULT_PARALLEL_WORKERS
    );
  ...
  test("full-sync large marker fires parallel for Postgres", () => {
    expect(autoConcurrency(engineOfKind("postgres"), Number.MAX_SAFE_INTEGER)).toBe(
      DEFAULT_PARALLEL_WORKERS
    );
  });
});
