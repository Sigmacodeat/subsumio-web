...
async function cmdList(args: string[]): Promise<void> {
  ...
  if (costFilter) {
    if (costFilter !== "free" && costFilter !== "rate-limited" && costFilter !== "paid") {
      console.error(
        `Invalid --cost value: ${costFilter}. Must be one of: free, rate-limited, paid.`
      );
      process.exit(1);
    }
...
function printTable(summaries: ResolverSummary[]): void {
  ...
  for (const r of rows) {
    console.log(
      `${pad(r.id, widths.id)}  ${pad(r.cost, widths.cost)}  ${pad(r.backend, widths.backend)}  ${r.description}`
    );
  }
