...
export function Accordion({ children, defaultValue, value, onValueChange, className }: AccordionProps) {
  ...
  return (
    <AccordionContext.Provider value={{ value: currentValue, onValueChange: handleChange }}>
      <div className={cn("divide-y divide-[color:var(--ds-border)] rounded-lg border border-[color:var(--ds-border)]", className)}>
        {children}
      </div>
...
export function AccordionTrigger({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center justify-between py-4 px-4 text-left text-sm font-medium transition-all hover:bg-[color:var(--ds-surface-2)]",
        className
      )}
    >
      {children}
...
export function AccordionContent({ children, className }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div className={cn("overflow-hidden px-4 pb-4 text-sm text-[color:var(--ds-text-muted)]", className)}>
      {children}
    </div>
  );
}
