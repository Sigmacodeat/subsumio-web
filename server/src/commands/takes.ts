...
async function cmdList(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const t of takes) {
    ...
    console.log(
      `#${t.row_num} [${t.kind} • ${t.holder} • w=${w}${since ? ` • ${since}` : ""}]${tag}\n  ${t.claim}${src}\n`
    );
  }
}
...
async function cmdSearch(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const h of hits) {
    ...
    console.log(
      `${h.page_slug}#${h.row_num} [${h.kind} • ${h.holder} • w=${Number(h.weight).toFixed(2)} • s=${score}]\n  ${h.claim}\n`
    );
  }
}
...
async function cmdAdd(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug) {
    console.error(
      'Usage: gbrain takes add <slug> --claim "..." --kind <k> --who <h> [--weight 0.5] [--source "..."] [--since YYYY-MM]'
    );
    process.exit(1);
  }
  const claim = flagValue(args, "--claim");
  if (!claim) {
    console.error("Missing --claim");
    process.exit(1);
  }
  const kind = ensureKind(flagValue(args, "--kind"));
  const holder = flagValue(args, "--who");
  if (!holder) {
    console.error("Missing --who");
    process.exit(1);
  }
...
async function cmdUpdate(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr) {
    console.error(
      'Usage: gbrain takes update <slug> --row N [--weight 0.7] [--source "..."] [--since YYYY-MM]'
    );
    process.exit(1);
  }
  ...
  await withPageLock(slug, async () => {
    ...
    if (!target) {
      console.warn(
        `[takes update] DB updated but row #${rowNum} not in markdown fence on disk; markdown may be out of sync. Run 'gbrain extract takes --slugs ${slug}' to reconcile.`
      );
      return;
    }
    ...
    const { renderTakesFence, TAKES_FENCE_BEGIN, TAKES_FENCE_END } =
      await import("../core/takes-fence.ts");
...
async function cmdSupersede(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!slug || !rowNumStr) {
    console.error(
      'Usage: gbrain takes supersede <slug> --row N --claim "..." [--kind k] [--who h] [--weight 0.5] [--source "..."]'
    );
    process.exit(1);
  }
  const rowNum = parseInt(rowNumStr, 10);
  ...
  await withPageLock(slug, async () => {
    ...
    if (parseTakesFence(body).takes.find((t) => t.rowNum === rowNum)) {
      ...
      console.warn(
        `[takes supersede] DB updated but markdown lacks row #${rowNum}; only DB written.`
      );
    }
...
async function cmdResolve(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (qualityStr) {
    if (
      qualityStr !== "correct" &&
      qualityStr !== "incorrect" &&
      qualityStr !== "partial" &&
      qualityStr !== "unresolvable"
    ) {
      console.error(
        `Invalid --quality "${qualityStr}". Expected: correct, incorrect, partial, unresolvable.`
      );
      process.exit(1);
    }
    quality = qualityStr;
    ...
    console.error(
      "[deprecated] --outcome is the v0.28 alias for --quality. Prefer --quality correct|incorrect|partial in new scripts."
    );
  }
  ...
  await withPageLock(slug, async () => {
    ...
    if (!body) {
      console.warn(
        `[takes resolve] markdown file not found at ${path}; DB updated but on-disk page absent.`
      );
      return;
    }
    const { parseTakesFence, renderTakesFence, TAKES_FENCE_BEGIN, TAKES_FENCE_END } =
      await import("../core/takes-fence.ts");
    const parsed = parseTakesFence(body);
    const target = parsed.takes.find((t) => t.rowNum === rowNum);
    if (!target) {
      console.warn(
        `[takes resolve] DB updated but row #${rowNum} not in markdown fence; run 'gbrain extract takes --slugs ${slug}' to reconcile.`
      );
      return;
    }
    ...
    const finalQuality =
      quality ?? (outcome === true ? "correct" : outcome === false ? "incorrect" : undefined);
    if (!finalQuality) return; // unreachable — covered by earlier validation
    const finalOutcome =
      finalQuality === "partial" ? undefined : finalQuality === "correct" ? true : false;
    const updated = {
      ...target,
  ...
  const finalQuality =
    quality ?? (outcome === true ? "correct" : outcome === false ? "incorrect" : "unknown");
...
async function cmdScorecard(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  console.log(
    `  Brier:             ${fmt(card.brier, 4)}   (correct ∨ incorrect only; lower is better; 0.25 = always-50% baseline)`
  );
  console.log(`  partial_rate:      ${fmt(card.partial_rate)}`);
  if (
    unresolvableCount > 0 ||
    (card.unresolvable_rate !== undefined && card.unresolvable_rate !== null)
  ) {
    console.log(
      `  unresolvable_rate: ${fmt(card.unresolvable_rate)}   (unresolvable / (resolved + unresolvable); high = weak evidence retrieval)`
    );
  }
  if (card.partial_rate !== null && card.partial_rate > PARTIAL_RATE_WARNING_THRESHOLD) {
    console.log("");
    console.log(
      `  [!] partial_rate is high (>${(PARTIAL_RATE_WARNING_THRESHOLD * 100).toFixed(0)}%) — calibration may be optimistic.`
    );
    console.log(
      `      Hedged bets escape the Brier denominator. Resolve them more decisively if the data supports it.`
    );
  }
  if (
    card.unresolvable_rate !== null &&
    card.unresolvable_rate !== undefined &&
    card.unresolvable_rate > 0.3
  ) {
    console.log("");
    console.log(
      `  [!] unresolvable_rate is high (>${(0.3 * 100).toFixed(0)}%) — most grade attempts are running into evidence gaps.`
    );
    console.log(
      `      The judge is working; retrieval isn't producing enough context to decide. Look at evidence-retrieval coverage, not prediction accuracy.`
    );
  }
...
async function cmdCalibration(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const b of buckets) {
    ...
    const delta =
      b.observed !== null && b.predicted !== null
...
`);
  ...
  switch (sub) {
    case "search":
      return cmdSearch(engine, rest);
    case "add":
      return cmdAdd(engine, rest);
    case "update":
      return cmdUpdate(engine, rest);
    case "supersede":
      return cmdSupersede(engine, rest);
    case "resolve":
      return cmdResolve(engine, rest);
    case "scorecard":
      return cmdScorecard(engine, rest);
    case "calibration":
      return cmdCalibration(engine, rest);
    case "revisit":
      return cmdRevisit(engine, rest);
    case "extract":
      return cmdExtract(engine, rest);
    default:
...
async function cmdRevisit(_engine: BrainEngine, rest: string[]): Promise<void> {
  ...
  if (!repoPath) {
    process.stderr.write(
      "No brain repo configured. Run `gbrain config set sync.repo_path /path/to/brain`.\n"
    );
    process.exit(1);
  }
