...
describe("decideAction", () => {
  ...
  test("D8: patch drift → noop", () => {
    expect(decideAction({ ...baseInput, localVersion: "0.32.0", remoteVersion: "0.32.1" })).toEqual(
      { kind: "noop" }
    );
  });

  test("D8: minor drift → prompt", () => {
    expect(decideAction({ ...baseInput, localVersion: "0.31.0", remoteVersion: "0.32.0" })).toEqual(
      { kind: "prompt", level: "minor" }
    );
  });

  test("D8: major drift → prompt", () => {
    expect(decideAction({ ...baseInput, localVersion: "0.31.0", remoteVersion: "1.0.0" })).toEqual({
      kind: "prompt",
      level: "major",
    });
  });
  ...
  test("new bump after decline → re-prompt", () => {
    ...
    expect(decideAction({ ...baseInput, state, remoteVersion: "0.33.0" })).toEqual({
      kind: "prompt",
      level: "minor",
    });
  });
...
describe("maybePromptForUpgrade orchestrator", () => {
  ...
  test('prompt → "y" → upgrade runs → NOT advanced → state=failed, exit 1', async () => {
    await withEnv({ GBRAIN_HOME: tmpHome }, async () => {
      _setPromptReaderForTest(async () => "y");
      _setUpgradeRunnerForTest(() => {
        /* swallow, simulate exit 0 with no advance */
      });
