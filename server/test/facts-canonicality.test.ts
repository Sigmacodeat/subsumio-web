...
beforeAll(async () => {
  ...
  await engine.executeRaw(
    `INSERT INTO pages (slug, type, title) VALUES ('people/alice-example', 'person', 'Alice Example') ON CONFLICT DO NOTHING`
  );
  await engine.executeRaw(
    `INSERT INTO pages (slug, type, title) VALUES ('companies/anthropic', 'company', 'Anthropic') ON CONFLICT DO NOTHING`
  );
});
