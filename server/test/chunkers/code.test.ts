...
$$ LANGUAGE SQL;`;
    ...
    expect(result.length).toBeGreaterThanOrEqual(1);
    const c = result.find(
      (c) => c.metadata.symbolName === "get_user_by_email_long_function_name_here_for_no_merge"
    );
...
SELECT * FROM long_mixed_table_name_for_no_merge_with_dml_below_it WHERE id = 1 ORDER BY x;`;
    ...
    const ddl = namedChunks.find(
      (c) => c.metadata.symbolName === "long_mixed_table_name_for_no_merge_with_dml_below_it"
    );
