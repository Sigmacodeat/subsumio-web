...
describe("IngestionDaemon — registration", () => {
  ...
  test("register rejects duplicate id", () => {
    ...
    expect(() => daemon.register({ source: makeControllableSource("dup").source })).toThrow(
      /duplicate source id/
    );
  });
  ...
  test("register rejects after start()", async () => {
    ...
    await daemon.start();
    expect(() => daemon.register({ source: makeControllableSource("b").source })).toThrow(
      /after daemon has started/
    );
    await daemon.stop();
  });
});
...
describe("IngestionDaemon — dispatch pipeline", () => {
  ...
  test("dispatcher failure is logged but does not crash the daemon", async () => {
    ...
    await flushMicrotasks();
    expect(calls).toBe(1);
    expect(messages.some((m) => m.level === "warn" && m.msg.includes("dispatch failed"))).toBe(
      true
    );
    expect(daemon.running).toBe(true);
    await daemon.stop();
  ...
  test("dispatcher throwing is caught and logged as error", async () => {
    ...
    push(makeEvent());
    await flushMicrotasks();
    expect(messages.some((m) => m.level === "error" && m.msg.includes("rogue dispatcher"))).toBe(
      true
    );
    expect(daemon.running).toBe(true);
    await daemon.stop();
...
describe("IngestionDaemon — config passthrough", () => {
  ...
  test("source logger is wrapped with per-source prefix", async () => {
    ...
    expect(
      messages.some(
        (m) => m.level === "info" && m.msg === "[ingestion:named-src] hello from inside"
      )
    ).toBe(true);
    await daemon.stop();
  });
