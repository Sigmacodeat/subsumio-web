...
function parseRedlines(v: unknown): Redline[] {
  ...
  for (const raw of v) {
    ...
    const change_type =
      typeof o.change_type === "string" && valid.has(o.change_type as RedlineChange)
...
export async function redlineContract(
  ...
  for (const r of redlines) {
    const dev = playbookDeviations.find(
      (d) =>
        d.matched && d.clause_type && r.reason.toLowerCase().includes(d.clause_type.toLowerCase())
    );
