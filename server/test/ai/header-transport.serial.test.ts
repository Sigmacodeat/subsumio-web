...
let lastEmbedRequest: { url: string; headers: Record<string, string>; body: string | null } | null =
  null;
...
let lastChatRequest: { url: string; headers: Record<string, string>; body: string | null } | null =
  null;
...
describe("transport-level header sweep (v0.37.2.0)", () => {
  ...
  test("3. rerank — manual HTTP path applies Authorization + default_headers", async () => {
    ...
    expect(h["authorization"], "rerank: Authorization Bearer must be present").toBe(
      "Bearer sk-rerank-fake"
    );
