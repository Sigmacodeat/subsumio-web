...
describe("validateShellJobParams — existing param shape checks", () => {
  test("cmd XOR argv: both → reject", () => {
    expect(() =>
      validateShellJobParams({ cmd: "echo", argv: ["echo"], cwd: "/tmp" }, { config: fakeCfg })
    ).toThrow(UnrecoverableError);
  });
  test("cmd XOR argv: neither → reject", () => {
    expect(() => validateShellJobParams({ cwd: "/tmp" }, { config: fakeCfg })).toThrow(
      UnrecoverableError
    );
  });
  test("cwd must be absolute", () => {
    expect(() =>
      validateShellJobParams({ cmd: "echo", cwd: "relative/path" }, { config: fakeCfg })
    ).toThrow(/absolute path/);
  });
  test("cwd required", () => {
    expect(() => validateShellJobParams({ cmd: "echo", cwd: "" }, { config: fakeCfg })).toThrow(
      /cwd/
    );
  });
  test("env must be object of string values", () => {
    expect(() =>
      validateShellJobParams(
        { cmd: "echo", cwd: "/tmp", env: "oops" as unknown as Record<string, string> },
        { config: fakeCfg }
      )
    ).toThrow(/env/);
    expect(() =>
      validateShellJobParams(
        { cmd: "echo", cwd: "/tmp", env: { K: 1 as unknown as string } },
        { config: fakeCfg }
      )
    ).toThrow(/string/);
  });
...
describe("inherit — free-form config-key names (v0.36.5.0)", () => {
    ...
    const p = validateShellJobParams(
      {
        cmd: "echo",
        cwd: "/tmp",
        inherit: ["database_url", "anthropic_api_key", "openai_api_key"],
      },
      { config: fakeCfg }
    );
  ...
  test("inherit empty string rejected", () => {
    expect(() =>
      validateShellJobParams({ cmd: "echo", cwd: "/tmp", inherit: [""] }, { config: fakeCfg })
    ).toThrow(/non-empty strings/);
  });
});
...
describe("inherit — snake_case shape guard (prototype-pollution defense)", () => {
  ...
  test("digits-after-letter allowed (matches regex)", () => {
    ...
    expect(() =>
      validateShellJobParams({ cmd: "echo", cwd: "/tmp", inherit: ["field2"] }, { config: fakeCfg })
...
describe("redact_secrets shape check", () => {
  ...
  test("redact_secrets: undefined is fine (default)", () => {
    const p = validateShellJobParams({ cmd: "echo", cwd: "/tmp" }, { config: fakeCfg });
