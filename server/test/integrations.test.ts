...
describe("executeHealthCheck", () => {
  test("env_exists returns ok when env var is set", async () => {
    ...
    const result = await executeHealthCheck(
      { type: "env_exists", name: "TEST_HC_VAR", label: "Test" },
      "test-id",
      true
    );
  ...
  test("env_exists returns fail when env var is missing", async () => {
    ...
    const result = await executeHealthCheck(
      { type: "env_exists", name: "TEST_HC_MISSING" },
      "test-id",
      true
    );
  ...
  test("command returns ok for exit 0", async () => {
    const result = await executeHealthCheck(
      { type: "command", argv: ["true"], label: "true cmd" },
      "test-id",
      true
    );
    expect(result.status).toBe("ok");
  ...
  test("command returns fail for exit 1", async () => {
    const result = await executeHealthCheck(
      { type: "command", argv: ["false"], label: "false cmd" },
      "test-id",
      true
    );
    expect(result.status).toBe("fail");
  ...
  test("command health_check is blocked for non-embedded recipes", async () => {
    const result = await executeHealthCheck(
      { type: "command", argv: ["true"], label: "true" },
      "test-id",
      false
    );
    expect(result.status).toBe("blocked");
...
describe("parseOctet", () => {
  test("parses plain decimal", () => {
    expect(parseOctet("80")).toBe(80);
  });
  test("parses hex (0x prefix)", () => {
    expect(parseOctet("0x50")).toBe(80);
  });
  test("parses hex (uppercase)", () => {
    expect(parseOctet("0X7F")).toBe(127);
  });
  test("parses octal (leading zero)", () => {
    expect(parseOctet("0177")).toBe(127);
  });
  test("zero is decimal zero", () => {
    expect(parseOctet("0")).toBe(0);
  });
  test("rejects empty", () => {
    expect(Number.isNaN(parseOctet(""))).toBe(true);
  });
  test("rejects non-numeric", () => {
    expect(Number.isNaN(parseOctet("foo"))).toBe(true);
  });
  test("rejects invalid octal (8/9)", () => {
    expect(Number.isNaN(parseOctet("089"))).toBe(true);
  });
});

describe("hostnameToOctets", () => {
  test("dotted decimal", () => {
    expect(hostnameToOctets("127.0.0.1")).toEqual([127, 0, 0, 1]);
  });
  test("single decimal integer", () => {
    expect(hostnameToOctets("2130706433")).toEqual([127, 0, 0, 1]);
  });
  test("hex integer", () => {
    expect(hostnameToOctets("0x7f000001")).toEqual([127, 0, 0, 1]);
  });
  test("dotted mixed radix", () => {
    expect(hostnameToOctets("0x7f.0.0.1")).toEqual([127, 0, 0, 1]);
  });
  test("dotted octal", () => {
    expect(hostnameToOctets("0177.0.0.1")).toEqual([127, 0, 0, 1]);
  });
  test("non-IP hostname returns null", () => {
    expect(hostnameToOctets("api.example.com")).toBe(null);
  });
  test("too many parts returns null", () => {
    expect(hostnameToOctets("1.2.3.4.5")).toBe(null);
  });
  test("octet out of range returns null", () => {
    expect(hostnameToOctets("256.0.0.1")).toBe(null);
  });
});
...
describe("isPrivateIpv4", () => {
  ...
  test("loopback 127.255.255.255", () => {
    expect(isPrivateIpv4([127, 255, 255, 255])).toBe(true);
  });
  test("RFC1918 10.0.0.1", () => {
    expect(isPrivateIpv4([10, 0, 0, 1])).toBe(true);
  });
  test("RFC1918 172.16.0.1", () => {
    expect(isPrivateIpv4([172, 16, 0, 1])).toBe(true);
  });
  test("RFC1918 172.31.255.255", () => {
    expect(isPrivateIpv4([172, 31, 255, 255])).toBe(true);
  });
  test("172.15 is NOT RFC1918", () => {
    expect(isPrivateIpv4([172, 15, 0, 1])).toBe(false);
  });
  test("172.32 is NOT RFC1918", () => {
    expect(isPrivateIpv4([172, 32, 0, 1])).toBe(false);
  });
  test("RFC1918 192.168.1.1", () => {
    expect(isPrivateIpv4([192, 168, 1, 1])).toBe(true);
  });
  test("link-local 169.254.169.254 (AWS metadata)", () => {
    expect(isPrivateIpv4([169, 254, 169, 254])).toBe(true);
  });
  test("CGNAT 100.64.0.1", () => {
    expect(isPrivateIpv4([100, 64, 0, 1])).toBe(true);
  });
  test("CGNAT 100.127.255.255", () => {
    expect(isPrivateIpv4([100, 127, 255, 255])).toBe(true);
  });
  test("100.63 is NOT CGNAT", () => {
    expect(isPrivateIpv4([100, 63, 0, 1])).toBe(false);
  });
  test("100.128 is NOT CGNAT", () => {
    expect(isPrivateIpv4([100, 128, 0, 1])).toBe(false);
  });
  test("unspecified 0.0.0.0", () => {
...
describe("isInternalUrl", () => {
  ...
  test("blocks ftp:// scheme", () => {
    expect(isInternalUrl("ftp://internal.corp/")).toBe(true);
  });
  test("blocks javascript: scheme", () => {
    expect(isInternalUrl("javascript:alert(1)")).toBe(true);
  });
  test("blocks blob: scheme", () => {
    expect(isInternalUrl("blob:http://evil.com/abc")).toBe(true);
  });
  // Blocked — malformed
  test("blocks malformed URL (fail-closed)", () => {
  ...
  test("allows URL with port", () => {
    expect(isInternalUrl("https://example.com:8443/x")).toBe(false);
  });
  ...
  test("blocks trailing-dot numeric 127.0.0.1.", () => {
    expect(isInternalUrl("http://127.0.0.1./")).toBe(true);
  });
});
