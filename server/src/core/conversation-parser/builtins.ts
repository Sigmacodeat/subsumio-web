...
export const BUILTIN_PATTERNS: readonly PatternEntry[] = [
...

  {
    ...
    test_negative: ["[18:37] Alice: no date prefix", "3/15/24, 6:37 PM - Alice: US locale"],
...

  {
    ...
    regex: /^(\d{1,2})\/(\d{1,2})\/(\d{2,4}),\s+(\d{1,2}):(\d{2})\s*(AM|PM)\s+-\s+(.+?):\s+(.*)$/,
    captures: {
...

  {
    ...
    regex: /^\[(\d{1,2})\/(\d{1,2})\/(\d{4})\s+(\d{1,2}):(\d{2})\s*(AM|PM)\]\s+(.+)$/,
    captures: {
    ...
    test_positive: ["[03/15/2024 6:37 PM] Alice Example", "[01/01/2024 12:00 AM] Bob Example"],
    test_negative: ["[15/03/24, 18:37:00] Alice: WhatsApp shape", "Alice — Today at 18:37"],
    source_doc: "DiscordChatExporter (Tyrrrz/DiscordChatExporter) TXT export shape",
  },
...

  {
    ...
    regex: /^(.+?)\s+\((\d{4}-\d{2}-\d{2})\s+(\d{1,2}):(\d{2}):(\d{2})\s+UTC\):\s+(.*)$/,
    captures: {
      speaker_group: 1,
      date_group: 2,
      hour_group: 3,
...

  {
    ...
    regex:
      /^([\p{L}\p{N}][\p{L}\p{N}\s.'-]*?)\s+—\s+(?:Today|Yesterday)\s+at\s+(\d{1,2}):(\d{2})\s*(AM|PM)?\s*$/u,
    captures: {
      speaker_group: 1,
      hour_group: 2,
      minute_group: 3,
    ...
    test_positive: ["Alice Example — Today at 6:37 PM", "Bob Example — Yesterday at 12:00 AM"],
    test_negative: ["Alice Example, 3/15/2024 6:37 PM: hello", "[03/15/2024 6:37 PM] Alice"],
...

  {
    ...
    regex: /^\[(\d{1,2}):(\d{2})\]\s+(@[\p{L}\p{N}_.-]+:[\p{L}\p{N}.-]+):\s+(.*)$/u,
    captures: {
      speaker_group: 3,
      hour_group: 1,
      minute_group: 2,
    ...
    test_positive: ["[18:37] @alice:matrix.org: hello", "[06:00] @bob:example.org: morning"],
    test_negative: ["[18:37] Alice: matrix without @", "**[18:37] G T:** telegram bracket"],
...

  {
    ...
    test_negative: ["<-- alice has joined #channel", "Alice: not irc format"],
    source_doc: "IRC default log format (irssi /SET autolog, weechat /SET logger.format)",
  },
...
function validateAllBuiltins(): void {
  ...
  for (const entry of BUILTIN_PATTERNS) {
    if (ids.has(entry.id)) {
      throw new Error(`[conversation-parser] duplicate built-in PatternEntry id: ${entry.id}`);
    }
    ids.add(entry.id);
  }
}
