...
`;
      ...
      expect(page).not.toBeNull();
      const marker = (page!.frontmatter as Record<string, unknown>).quarantine as Record<
        string,
        unknown
      >;
  ...
  test.each([
    ...
    "v0.41.13 patterns still fire: title %j → quarantined (%s)",
    async (title, _expectedPattern) => {
      await withIsolatedHome(async () => {
        const content = `---
title: '${title}'
type: note
...
scraper junk body`;
        const result = await importFromContent(
          engine,
          "test/v04113-" + title.toLowerCase().replace(/[^a-z]/g, "-"),
          content,
          { noEmbed: true }
        );
        expect(result.quarantined).toBe(true);
...
A legitimate essay about handling access-denied errors in your app.`;
      const result = await importFromContent(engine, "test/v04113-essay", content, {
        noEmbed: true,
      });
      expect(result.status).toBe("imported");
...
describe("importFromContent — markup-heavy FLAG (Q1=A: warn, stay searchable)", () => {
  test("markup-heavy page lands, KEEPS chunks, carries content_flag (not quarantined)", async () => {
    await withIsolatedHome(async () => {
      ...
      expect(getContentFlag(page!.frontmatter as Record<string, unknown>)?.reason).toBe(
        "markup_heavy"
      );
      expect(isQuarantined(page!.frontmatter as Record<string, unknown>)).toBe(false);
    });
...
A perfectly normal note with real prose and nothing wrong with it.`;
      const result = await importFromContent(engine, "notes/planted", content, {
        noEmbed: true,
        remote: true,
      });
      expect(result.quarantined).toBeUndefined();
