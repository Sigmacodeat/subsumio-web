...
export function buildGatewayConfig(c: GBrainConfig): AIGatewayConfig {
  ...
  if (process.env.LLAMA_SERVER_BASE_URL)
    envBaseUrls["llama-server"] = process.env.LLAMA_SERVER_BASE_URL;
  ...
  // on different ports.
  if (process.env.LLAMA_SERVER_RERANKER_BASE_URL)
    envBaseUrls["llama-server-reranker"] = process.env.LLAMA_SERVER_RERANKER_BASE_URL;
