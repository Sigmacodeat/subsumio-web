...
describe("snapshotIndexes diff", () => {
  test("matching snapshots produce a clean diff", () => {
    const pg = snap([
      { name: "idx_foo", table: "foo", columns: "CREATE INDEX idx_foo ON foo (id)" },
    ]);
    const pglite = snap([
      { name: "idx_foo", table: "foo", columns: "CREATE INDEX idx_foo ON foo (id)" },
    ]);
  ...
  test("pg-only index surfaces in pgOnly", () => {
    const pg = snap([
      { name: "idx_only_pg", table: "foo", columns: "CREATE INDEX idx_only_pg ON foo (id)" },
    ]);
    const pglite = snap([]);
    const d = diffIndexSnapshots(pg, pglite);
  ...
  test("pglite-only index surfaces in pgliteOnly", () => {
    const pg = snap([]);
    const pglite = snap([
      { name: "idx_pl_only", table: "foo", columns: "CREATE INDEX idx_pl_only ON foo (id)" },
    ]);
    const d = diffIndexSnapshots(pg, pglite);
    ...
    const pg = snap([
      {
        name: "idx_u",
        table: "foo",
        columns: "CREATE UNIQUE INDEX idx_u ON foo (id)",
        isUnique: true,
      },
    ]);
    const pglite = snap([
    ...
    const pg = snap([
      {
        name: "idx_p",
        table: "foo",
        columns: "CREATE INDEX idx_p ON foo (id) WHERE id IS NULL",
        isPartial: true,
      },
    ]);
    const pglite = snap([
  ...
  test("formatter produces a readable failure message", () => {
    const pg = snap([
      { name: "idx_missing_pl", table: "foo", columns: "CREATE INDEX idx_missing_pl ON foo (id)" },
    ]);
    const pglite = snap([]);
    const d = diffIndexSnapshots(pg, pglite);
