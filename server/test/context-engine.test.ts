...
describe("gbrain-context engine", () => {
  ...
  it("injects upcoming events within 4-hour window", async () => {
    ...
    tmpDir = makeWorkspace({
      ...
      calendar: {
        ...
        events: [
          {
            summary: "Office Hours — Batch W26",
            start: soon,
            end: new Date(new Date(soon).getTime() + 30 * 60 * 1000).toISOString(),
          },
          {
            summary: "GP Lunch",
            start: later,
            end: new Date(new Date(later).getTime() + 60 * 60 * 1000).toISOString(),
          },
          {
            summary: "Evening dinner",
            start: tooFar,
            end: new Date(new Date(tooFar).getTime() + 2 * 60 * 60 * 1000).toISOString(),
          },
        ],
      },
    });
  ...
  it("A4: active flight to a known airport resolves to that timezone", async () => {
    tmpDir = makeWorkspace({
      heartbeat: { garryAwake: true },
      flights: {
        flights: [{ status: "active", flightNumber: "AC8", origin: "SFO", destination: "YYZ" }],
      },
    });
    const engine = createGBrainContextEngine({ workspaceDir: tmpDir });
  ...
  it("L0-A: active flight to an UNKNOWN airport emits NO concrete local time", async () => {
    ...
    tmpDir = makeWorkspace({
      heartbeat: { garryAwake: true },
      flights: {
        flights: [{ status: "active", flightNumber: "AI191", origin: "SFO", destination: "BOM" }],
      },
    });
    const engine = createGBrainContextEngine({ workspaceDir: tmpDir });
