...
function trackCacheWrite(promise: Promise<unknown>): void {
  ...
  promise
    .finally(() => pendingCacheWrites.delete(promise))
    .catch(() => {
      /* swallow */
    });
}
...
const ALIAS_HOP_PRESENT_BOOST = 1.1; // bounded boost when canonical already in results
...
export async function applyAliasHop(
  ...
  try {
    aliasMap = await engine.resolveAliases([qNorm], {
  ...
  for (const ref of ordered) {
    const idx = out.findIndex(
      (r) => r.slug === ref.slug && (r.source_id ?? "default") === ref.source_id
    );
    if (idx >= 0) {
...
export async function hybridSearch(
  ...
  const emitMeta = (meta: HybridSearchMeta): void => {
    ...
    try {
      recordSearchTelemetry(engine, meta, {
        results_count: lastResultsCount,
        rank1_score: lastRank1Score,
      });
    } catch {
  ...
  if (!isAvailable("embedding", providerProbe)) {
    ...
    if (relationalList.length > 0) {
      ...
      noEmbedResults = rrfFusionWeighted(
        [
          { list: keywordResults, k: fk },
          { list: relationalList, k: fk },
        ],
        detailResolved !== "high"
      );
    }
    ...
    const { results: noEmbedBudgeted, meta: noEmbedBudgetMeta } = enforceTokenBudget(
      noEmbedSliced,
      resolvedMode.tokenBudget
    );
    await stampContentFlags(engine, noEmbedBudgeted);
    lastResultsCount = noEmbedBudgeted.length;
    lastRank1Score = noEmbedBudgeted[0]
      ? (noEmbedBudgeted[0].base_score ?? noEmbedBudgeted[0].score)
      : undefined;
    emitMeta({
  ...
  if (unifiedRouting) {
    try {
      ...
      if (!aiIsAvailable("embedding")) {
        throw new Error(
          "gateway not configured for embedding — unified multimodal would also fail"
        );
      }
  ...
  if (unifiedDone) {
    ...
    try {
      ...
      const embeddings = await Promise.all(
        queries.map((q) => embedQueryBounded(q, embedOpts, embedDl))
      );
  ...
  if (vectorLists.length === 0) {
    ...
    const { results: kwBudgeted, meta: kwBudgetMeta } = enforceTokenBudget(
      kwSliced,
      resolvedMode.tokenBudget
    );
...
export async function hybridSearchCached(
  ...
  const cfgCached = mergedCfgCached ??
    (await import("../config.ts")).loadConfig() ?? { engine: "pglite" as const };
