...
async function runList(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const r of rows) {
    ...
    console.log(
      `  ${r.marker === "quarantine" ? "HIDDEN " : "FLAGGED"} ${r.slug}${src}  reason=${r.reason}  at=${r.assessed_at}`
    );
  }
...
async function runClear(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  console.log(
    `Cleared "${slug}".` +
      ...
      (noEmbed
        ? " Embedding skipped (--no-embed); run `gbrain embed --stale` to make it searchable."
        : "")
  );
}
...
async function runScan(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  try {
    ...
    /* fall back to defaults if DB-config lift fails */
  }
  ...
  if (json) {
    console.log(
      JSON.stringify(
        { schema_version: 1, applied: apply, scanned, quarantined, flagged, touched },
        null,
        2
      )
    );
  ...
  console.log(
    `Scanned ${scanned} page(s): ${verb}quarantine ${quarantined}, ${verb}flag ${flagged}.`
  );
