...
describe("extractEntityRefs", () => {
  ...
  test("extracts multiple refs in same content", () => {
    const refs = extractEntityRefs(
      "[Alice](people/alice) and [Bob](people/bob) met at [Acme](companies/acme)."
    );
  ...
  test("skips qualified-syntax tokens (those belong to 2a)", () => {
    ...
    const wrongQualified = refs.filter((r) => r.slug.includes(":") && r.needsResolution === true);
...
describe("extractPageLinks", () => {
  ...
  test("extracts bare slug references in text", async () => {
    const { candidates } = await extractPageLinks(
      "docs/x",
      "See companies/acme for details.",
      {},
      "concept",
      nullResolver
    );
  ...
  test("bare wikilink with single basename match emits one candidate", async () => {
    const resolver: SlugResolver = {
      resolve: async () => null,
      resolveBasenameMatches: async (name) => (name === "struktura" ? ["projects/struktura"] : []),
    };
    const { candidates } = await extractPageLinks(
      "concepts/knowledge-graph",
  ...
  test("opts.skipFrontmatter suppresses the frontmatter pass", async () => {
    ...
    const withFm = await extractPageLinks("docs/x", "plain content", fm, "person", resolver, {
      skipFrontmatter: false,
    });
    const withoutFm = await extractPageLinks("docs/x", "plain content", fm, "person", resolver, {
...
describe("inferLinkType", () => {
  ...
  test("investment narrative variants -> invested_in", () => {
    ...
    expect(inferLinkType("person", "Acme is a portfolio company of Founders Fund.")).toBe(
      "invested_in"
    );
  ...
  test("v0.10.5 works_at: possessive time at", () => {
    expect(inferLinkType("person", "During her time at Goldman, she built the team.")).toBe(
      "works_at"
    );
    expect(inferLinkType("person", "His time at Delta taught him systems thinking.")).toBe(
      "works_at"
    );
  });
  ...
  test("v0.10.5 works_at: tenure/stint/role as", () => {
    expect(inferLinkType("person", "Her tenure as head of engineering was short.")).toBe(
      "works_at"
    );
    expect(inferLinkType("person", "A brief stint as VP of Product.")).toBe("works_at");
    expect(inferLinkType("person", "His role at Delta was to unblock the pipeline team.")).toBe(
      "works_at"
    );
  });
  ...
  test("v0.10.5 works_at: page-role employee prior for ambiguous context", () => {
    ...
    expect(inferLinkType("person", perEdgeContext, globalContext, "companies/delta-3")).toBe(
      "works_at"
    );
  });
  ...
  test("v0.10.5 works_at: page-role CTO-of prior", () => {
    ...
    expect(inferLinkType("person", perEdgeContext, globalContext, "companies/prism-43")).toBe(
      "works_at"
    );
  });
  ...
  test("v0.10.5 advises: prefixed advisor (security advisor to X)", () => {
    ...
    expect(inferLinkType("person", "Product advisor to several early-stage startups.")).toBe(
      "advises"
    );
  });
  ...
  test("v0.10.5 advises: advisory engagement / partnership / contract", () => {
    expect(inferLinkType("person", "Began a formal advisory engagement with Prism.")).toBe(
      "advises"
    );
  ...
  test('v0.10.5 advises: "serves as advisor" page prior', () => {
    ...
    expect(inferLinkType("person", perEdgeContext, globalContext, "companies/acme")).toBe(
      "advises"
    );
  });
  ...
  test('v0.10.5 regression: "backs companies" still resolves to invested_in via partner prior', () => {
    ...
    const globalContext =
      "Wendy is a venture partner who backs companies at the seed stage. Her portfolio is diverse.";
    const perEdgeContext = "Wendy recently discussed Cipher.";
    expect(inferLinkType("person", perEdgeContext, globalContext, "companies/cipher-13")).toBe(
      "invested_in"
    );
  });
...
describe("extractFrontmatterLinks — field-map coverage", () => {
  ...
  test("unresolvable name goes to unresolved list, not candidates", async () => {
    const { candidates, unresolved } = await extractFrontmatterLinks(
      "meetings/x",
      "meeting" as never,
      { attendees: ["Pedro", "Unknown Person"] },
      resolver
    );
    expect(candidates).toHaveLength(1);
  ...
  test("bad types (number, null, empty) skipped silently", async () => {
    const { candidates, unresolved } = await extractFrontmatterLinks(
      "meetings/x",
      "meeting" as never,
      { attendees: [42, null, "", "Pedro", { nothing: true }] },
      resolver
    );
...
describe("makeResolver — fallback chain", () => {
  ...
  test("resolveBasenameMatches: exact tail hit returns the slug", async () => {
    const engine = makeFakeEngineWithSlugs(["projects/struktura", "people/alice"]);
    const r = makeResolver(engine);
  ...
  test("resolveBasenameMatches: multi-match returns ALL hits", async () => {
    ...
    expect(out.sort()).toEqual(["archive/struktura", "notes/struktura", "projects/struktura"]);
  });
  ...
  test("resolveBasenameMatches: index built once, reused across calls", async () => {
    const engine = makeFakeEngineWithSlugs(["projects/struktura", "archive/struktura"]);
    const r = makeResolver(engine);
...
describe("extractEntityRefs — v0.18.0 qualified wikilinks", () => {
  test("[[wiki:topics/ai]] extracts with sourceId=wiki", () => {
    const refs = extractEntityRefs(
      "See [[concepts/ai]] vs [[wiki:concepts/ai]] for wiki-specific take."
    );
  ...
  test("qualified source-id format is validated (must match [a-z0-9-]+ kebab rules)", () => {
    ...
    const refs = extractEntityRefs(
      "Legit: [[yc-media:concepts/seed]] Not legit: [[NotValid:concepts/x]]"
    );
