...
> | ------------------ | -----: | ----------------------------------- |
> | ✅ Fertig          |     75 | Code + Tests verifiziert vorhanden  |
> | 🔶 MVP/Teil-fertig |      0 | Kern-Logik da, Abhängigkeiten offen |
...
>
> _Keine teil-fertigen Tickets mehr — alle 75 Tickets fertig._
>
...
| P0-SEC-001    | fertig                                                                                | `sanitizeUserInput()` greift in `/api/legal/analyze` (Dokumenttext) und `/api/agent-templates/[slug]/run` (User-Input) — die einzigen Web-App-Routen, die selbst Prompts bauen. Restliche Legal-Routen proxien zur Engine (Engine-seitige Sanitization offen). Verifiziert mit `bun run typecheck` + `prompt-sanitizer.test.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| P0-SECR-002   | App-Schicht fertig; Engine-seitiger Ansatz existiert (uncommitted), ist aber unauthentifiziert und unverdrahtet — kein echter Permission-Filter | Spec: `docs/designs/PAKET_33_PROAKTIVER_KI_SEKRETAER_SPEC.md`. DB-gestützte WhatsApp-Identität: `src/lib/whatsapp/types.ts`, `identity-store.ts`, `identity.ts` (`resolveSenderIdentity` — Prod ohne env-Fallback, `identityCanAccessMatter`). `src/lib/legal-chat/actions.ts` erzwingt `matterScope` im Chat-Pfad über `resolveAuthorizedCase()` (einziger Einstieg, ersetzt 11 Direktaufrufe von `resolveCase()`, Denial ununterscheidbar von "nicht gefunden", auditiert per `whatsapp.sender_denied`). 20 Tests grün, `bun run typecheck` 0 Fehler. **Das ist nach wie vor die einzige real wirksame Durchsetzung.** **Engine-Versuch (uncommitted, `server/src/core/matter-scope.ts` + `server/src/commands/web-api.ts`):** `requestMatterScope()` liest die erlaubte Scope aus einem vom Aufrufer selbst gesetzten Header (`x-sigmabrain-matter-scope`) bzw. Body-Feld — kein Bezug zu einer verifizierten Identität, Default bei fehlendem Header ist `'all'`. Jeder Halter des einen geteilten Web-API-Keys kann sich selbst beliebige Scope zuweisen; das ist ein Ehrenwort-Feld, kein Permission-Filter. Nicht in `server/src/core/operations.ts` registriert, nicht in `server/src/mcp/server.ts` verdrahtet (ein direkter MCP-Call umgeht es vollständig), und der WhatsApp-Pfad selbst sendet diesen Header gar nicht (0 Treffer in `src/`) — die Funktion ist von der echten App-Schicht-Durchsetzung komplett entkoppelt. **Offen:** ein echter Engine-seitiger Permission-Filter braucht eine verifizierte Aufrufer-Identität im Engine-Layer (aktuell nur ein einziger geteilter API-Key, keine Per-Identität-Auflösung) — größeres Architektur-Stück, nicht mit dem Header-Ansatz erledigt. |
...
| P31-SB-001    | MVP fertig; Engine-Integration (uncommitted) ist Code-Skelett ohne Verdrahtung | Kanzlei Superbrain / Legal Context Graph MVP: `src/lib/matter-context-types.ts`, `src/lib/matter-context.ts` (`buildMatterContext`, `checkCoverage`, `detectGaps`, `explainRetrieval`, `buildBrainQualitySummary`). API-Routen + UI (`MatterContextPanel`, `BrainQualityPanel`), Query Modes in `/api/think`. Tests: `matter-context.test.ts` (53/53 grün). **Engine-Versuch (uncommitted, `server/src/core/matter-scope.ts`, 288 Zeilen, + REST-Endpunkte in `web-api.ts`):** `markSuperseded`/`markContradiction`/`getTemporalRelations` (Temporal Memory), `getConnectorCoverage` (Connector-Coverage-Matrix), `resolveEntity`/`resolveEntities` existieren als Funktionen und sind über neue REST-Routen (`/api/temporal/supersede`, `/api/temporal/contradict`, `/api/connector-coverage/:caseSlug`, `/api/entity/resolve`) erreichbar — aber: (a) keiner dieser Pfade wird vom eigentlichen Such-/Think-Retrieval aufgerufen (0 Treffer für `filterSuperseded`/`matterScope` in `server/src/core/search/` und `server/src/core/think/`), reine Frontmatter-Manipulation ohne Pipeline-Integration; (b) `resolveEntity` bleibt reine Keyword-Heuristik (`title.includes(mention)` → Score 0.9, sonst 0.5), kein echtes Cross-Type-Resolving; (c) Permission-Filter fehlt wie bei P0-SECR-002 beschrieben — MCP-Exposure kann dadurch nichts erzwingen. Nicht committet. **Offen:** Verdrahtung von Temporal-Filterung in den Retrieval-Pfad, echte Entity-Resolution, Engine-seitiger Permission-Filter, MCP-Operations-Registrierung. |
