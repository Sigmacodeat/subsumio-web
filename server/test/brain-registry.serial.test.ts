...
describe("loadMounts — entry validation", () => {
  ...
  test("resolves paths to absolute form", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [
          { id: "a", path: "/tmp/relative-test", engine: "pglite", database_path: "/tmp/a/.pg" },
        ],
      })
    );
    const mounts = loadMounts(path);
  ...
  test("rejects entry missing id", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [{ path: "/tmp/x", engine: "pglite", database_path: "/tmp/x/.pg" }],
      })
    );
  ...
  test("rejects entry missing path", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [{ id: "no-path", engine: "pglite", database_path: "/tmp/x/.pg" }],
      })
    );
  ...
  test("rejects invalid engine kind", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [{ id: "bad", path: "/tmp/b", engine: "sqlite" }],
      })
    );
  ...
  test("rejects postgres without database_url", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [{ id: "pg-no-url", path: "/tmp/p", engine: "postgres" }],
      })
    );
  ...
  test("rejects pglite without database_path or url", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [{ id: "pgl-no-path", path: "/tmp/p", engine: "pglite" }],
      })
    );
  ...
  test("rejects reserved host id", () => {
    const path = track(
      tempMountsFile({
        version: 1,
        mounts: [{ id: "host", path: "/tmp/h", engine: "pglite", database_path: "/tmp/h/.pg" }],
      })
    );
