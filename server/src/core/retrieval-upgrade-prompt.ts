...
export async function runRetrievalUpgradePrompt(
  ...
  if (!isTTY) {
    writeFn(
      "[ze-switch] non-TTY environment; skipping prompt. Run `gbrain ze-switch` manually when ready."
    );
  ...
  if (normalized === "s") {
    ...
    if (result.status === "applied") {
      writeFn(
        "[ze-switch] Schema rebuilt at 1024d. Run `gbrain embed --stale` to refill embeddings (or wait for autopilot)."
      );
    ...
    if (result.status === "refused" && result.reason === "env_override") {
      ...
      return {
        status: "failed",
        plan,
        reason: "env_override (use --ignore-env-override to apply anyway)",
      };
    }
...
> {
  ...
  if (!snapshotStr) {
    writeFn(
      "[ze-switch --undo] No prior config snapshot found (brain was never switched). Nothing to undo."
    );
  ...
  try {
    ...
    return {
      status: "failed",
      reason: `corrupt snapshot: ${err instanceof Error ? err.message : String(err)}`,
    };
  }
  ...
  if (result.status === "undone") {
    writeFn(
      `[ze-switch --undo] Restored ${snapshot.embedding_model} at ${snapshot.embedding_dimensions}d. Run \`gbrain embed --stale\` to refill embeddings.`
    );
    return result;
  }
  return result;
...
export function formatBanner(plan: RetrievalUpgradeState): string {
  ...
  lines.push(
    `Target:              ${ZE_TARGET_EMBEDDING_MODEL} (${ZE_TARGET_EMBEDDING_DIM}d via Matryoshka)`
  );
  lines.push("");

  // D10: two-line cost split — schema change vs re-embed.
  const dollars =
    plan.est_cost_usd > 0 ? `~$${plan.est_cost_usd.toFixed(2)}` : "estimate unavailable";
  const pages = Math.max(plan.pages_pending_chunker, plan.pages_pending_dim);
  lines.push(
    `Schema change: ~${plan.est_schema_change_seconds}s (drops + recreates embedding column with new index)`
  );
  lines.push(
    `Re-embed:      ~${plan.est_minutes}min and ${dollars} for ${pages.toLocaleString()} pages`
  );
