...
describe("triggerCleanupAndExit", () => {
  ...
  test("callback throw does not block other callbacks (Promise.allSettled)", async () => {
    const fired: string[] = [];
    registerCleanup("throwing", async () => {
      fired.push("throwing");
      throw new Error("boom");
    });
    registerCleanup("quiet", async () => {
      fired.push("quiet");
    });
  ...
  test("deregister-then-trigger: deregistered callback does not fire", async () => {
    const fired: string[] = [];
    const dereg = registerCleanup("dropped", async () => {
      fired.push("dropped");
    });
    registerCleanup("kept", async () => {
      fired.push("kept");
    });
    dereg();
