...
beforeAll(async () => {
  ...
  const brief = [
    ...
    "",
    "1. Run: `" +
      (typeof gbrainBin === "object" ? `${gbrainBin.bin} ${gbrainBin.args.join(" ")}` : gbrainBin) +
      " integrations install agent-voice --target " +
      scratchDir +
      "`",
  ...
  openclawProcess = spawn(
    process.env.OPENCLAW_BIN,
    ["agent", "--local", "--message", `Read ${join(scratchDir, "BRIEF.md")} and execute it.`],
    {
      cwd: scratchDir,
      env: {
        ...process.env,
...
describe.skipIf(!SHOULD_RUN)("voice-full-flow E2E (openclaw + roundtrip)", () => {
  it("openclaw installs agent-voice and starts the server", () => {
    expect(
      existsSync(join(scratchDir, "services/voice-agent/code/server.mjs")),
      "server.mjs missing post-install"
    ).toBe(true);
    expect(
      existsSync(join(scratchDir, "services/voice-agent/.gbrain-source.json")),
      ".gbrain-source.json missing"
    ).toBe(true);

    const manifest = JSON.parse(
      readFileSync(join(scratchDir, "services/voice-agent/.gbrain-source.json"), "utf8")
    );
    ...
    // Persona files exist.
    expect(existsSync(join(scratchDir, "services/voice-agent/code/lib/personas/mars.mjs"))).toBe(
      true
    );
    expect(existsSync(join(scratchDir, "services/voice-agent/code/lib/personas/venus.mjs"))).toBe(
      true
    );
    ...
    if (process.env.AGENT_VOICE_PII_BLOCKLIST) {
      ...
      const grep = spawnSync(
        ...
        [
          ...
          "--include=*.md",
          pattern,
          join(scratchDir, "services/voice-agent"),
        ],
        { encoding: "utf-8" }
      );
  ...
  it(
    "drives a WebRTC roundtrip and asserts audio quality",
    async () => {
      ...
      expect(
        result.audioSendCount,
        "PLUMBING: no audio sent — mic → WebRTC broken"
      ).toBeGreaterThan(0);
      expect(
        result.audioPlayCount,
        "PLUMBING: no audio received — server → WebRTC broken"
      ).toBeGreaterThan(0);
      ...
      // Semantic — soft.
      const judgement = await transcribeAndJudge(result.responseBlob, "What is two plus two?", {
        mimeType: "audio/webm",
        expectedAnswer: "4 or four",
      });
...
if (!SHOULD_RUN) {
  console.log(
    "[full-flow] SKIPPED — set AGENT_VOICE_FULL_E2E=1, OPENAI_API_KEY, ANTHROPIC_API_KEY, OPENCLAW_BIN to run"
  );
}
