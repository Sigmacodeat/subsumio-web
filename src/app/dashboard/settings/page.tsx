...
import { kanzleiSettingsSchema, apiKeysSchema, type KanzleiSettingsFormData, type ApiKeysFormData } from "@/lib/schemas/settings";
import { useMe } from "@/lib/queries/auth";
import { useTeam, useSettingsApiKeys, useSaveSettingsApiKeys, useUpdateTeamRole } from "@/lib/queries/settings";
...
const ALL_TABS: { id: string; labelKey: DashboardKey; icon: typeof Database; allowed: string[] }[] = [
  { id: "brain", labelKey: "settings.tab_brain", icon: Database, allowed: ["admin", "lawyer", "assistant"] },
  ...
  { id: "kanzlei", labelKey: "settings.tab_kanzlei", icon: Briefcase, allowed: ["admin", "lawyer", "assistant"] },
  { id: "team", labelKey: "settings.tab_team", icon: Users, allowed: ["admin"] },
  { id: "scim", labelKey: "settings.tab_scim", icon: Network, allowed: ["admin"] },
  { id: "account", labelKey: "settings.tab_account", icon: Settings, allowed: ["admin", "lawyer", "assistant", "client_viewer"] },
];
  ...
  return (
    <div className="relative flex items-center">
      <input
        ...
        className="w-full bg-[color:var(--ds-surface)] border border-[color:var(--ds-border)] rounded-lg px-3 py-2.5 pr-20 text-sm text-[color:var(--ds-text)] placeholder:text-[color:var(--ds-text-muted)] focus:outline-none focus:border-[color:var(--brand-primary)] font-mono transition-colors"
      />
...
export default function SettingsPage() {
  ...
  const rechtsgebietSaetzeWatch = useWatch({ control: kanzleiForm.control, name: "rechtsgebietSaetze" });
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-6">
      ...
      {/* Tab navigation */}
      <div className="flex items-center gap-1 border-b border-[color:var(--ds-border)] pb-px overflow-x-auto">
      ...
      {activeTab === "brain" && (
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <h2 className="text-base font-semibold text-[color:var(--ds-text)]">{t("settings.brain_config")}</h2>
          </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn("text-sm", engineStatus === "online" ? "text-emerald-600" : "text-amber-600")}>
                    {engineStatus === "checking" ? t("settings.checking") : engineStatus === "online" ? t("settings.connected") : t("settings.not_connected")}
                  </span>
                </div>
      ...
      {activeTab === "api" && (
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <h2 className="text-base font-semibold text-[color:var(--ds-text)]">{t("settings.api_keys")}</h2>
            <p className="text-sm text-[color:var(--ds-text-muted)] mt-1">{t("settings.api_keys_desc")}</p>
          </div>
          <div className="px-6 divide-y divide-[color:var(--ds-border)]">
            <Field label={t("settings.openai_key")} desc={t("settings.openai_key_desc")}>
              <div className="space-y-2">
                <MaskedInput value={apiKeysForm.watch("openaiKey")} placeholder="sk-..." onChange={(v) => apiKeysForm.setValue("openaiKey", v)} />
            ...
            <Field label={t("settings.anthropic_key")} desc={t("settings.anthropic_key_desc")}>
              <div className="space-y-2">
                <MaskedInput value={apiKeysForm.watch("anthropicKey")} placeholder="sk-ant-..." onChange={(v) => apiKeysForm.setValue("anthropicKey", v)} />
            ...
            <Field label={t("settings.zeroentropy_key")} desc={t("settings.zeroentropy_key_desc")}>
              <div className="space-y-2">
                <MaskedInput value={apiKeysForm.watch("zeroEntropyKey")} placeholder="ze-..." onChange={(v) => apiKeysForm.setValue("zeroEntropyKey", v)} />
                <Badge variant="info" className="text-xs">{t("settings.zeroentropy_badge")}</Badge>
              </div>
            </Field>
          </div>
        ...
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-[color:var(--ds-text)]">{t("settings.dream_title")}</h2>
                <p className="text-sm text-[color:var(--ds-text-muted)]">{t("settings.dream_desc")}</p>
              </div>
      ...
      {activeTab === "kanzlei" && (
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <h2 className="text-base font-semibold text-[color:var(--ds-text)]">{t("settings.kanzlei_title")}</h2>
            <p className="text-sm text-[color:var(--ds-text-muted)] mt-1">{t("settings.kanzlei_desc")}</p>
          </div>
          <div className="px-6 divide-y divide-[color:var(--ds-border)]">
            <Field label={t("settings.kanzlei_name")} desc={t("settings.kanzlei_name_desc")}>
              ...
              {kanzleiForm.formState.errors.kanzleiName && (
                <p className="text-xs text-red-600 mt-1">{kanzleiForm.formState.errors.kanzleiName.message}</p>
              )}
            </Field>
            ...
            <Field label={t("settings.anwalt_name")} desc={t("settings.anwalt_name_desc")}>
              ...
              {kanzleiForm.formState.errors.anwaltName && (
                <p className="text-xs text-red-600 mt-1">{kanzleiForm.formState.errors.anwaltName.message}</p>
              )}
            </Field>
            ...
            <Field label={t("settings.kanzlei_address")} desc={t("settings.kanzlei_address_desc")}>
              <textarea
                ...
                rows={3}
                className="w-full bg-[color:var(--ds-surface)] border border-[color:var(--ds-border)] rounded-lg px-3 py-2.5 text-sm text-[color:var(--ds-text)] placeholder:text-[color:var(--ds-text-muted)] focus:outline-none focus:border-[color:var(--brand-primary)]"
              />
            </Field>

            <Field label={t("settings.contact")} desc={t("settings.contact_desc")}>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <Input {...kanzleiForm.register("kanzleiEmail")} placeholder="kanzlei@example.com" />
              ...
              <>
                <Field label={t("settings.hourly_rate")} desc={t("settings.hourly_rate_desc")}>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Input type="number" {...kanzleiForm.register("stundensatz")} placeholder="200" className="w-32" />
                    <span className="text-sm text-[color:var(--ds-text-muted)]">{t("settings.per_hour")}</span>
                    <Input type="number" {...kanzleiForm.register("abrechnungstakt")} placeholder="15" className="w-24 ml-2" />
                    <span className="text-sm text-[color:var(--ds-text-muted)]">{t("settings.billing_increment")}</span>
                  </div>
                </Field>

                <Field label={t("settings.rates_per_area")} desc={t("settings.rates_per_area_desc")}>
                  <div className="space-y-2">
                    {Object.entries(rechtsgebietSaetzeWatch ?? {}).map(([gebiet, satz]) => (
                      <div key={gebiet} className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          value={String(satz)}
                          onChange={(e) => {
                            const updated = { ...(rechtsgebietSaetzeWatch ?? {}), [gebiet]: parseInt(e.target.value, 10) || 0 };
                            kanzleiForm.setValue("rechtsgebietSaetze", updated);
                          }}
                          className="w-24 bg-[color:var(--ds-surface)] border border-[color:var(--ds-border)] rounded-lg px-3 py-1.5 text-sm text-[color:var(--ds-text)] focus:outline-none focus:border-[color:var(--brand-primary)]"
                        />
                        <span className="text-xs text-[color:var(--ds-text-muted)]">{t("settings.per_hour_short")}</span>
                      </div>
                    ))}
                  </div>
                </Field>
            ...
            <Field label={t("settings.payment_terms")} desc={t("settings.payment_terms_desc")}>
              <div className="flex items-center gap-2">
                <Input type="number" {...kanzleiForm.register("zahlungszielTage")} placeholder="14" className="w-24" />
                <span className="text-sm text-[color:var(--ds-text-muted)]">{t("settings.days_net")}</span>
              </div>
            </Field>
            ...
            <Field label={t("settings.datev_consultant")} desc={t("settings.datev_consultant_desc")}>
            ...
            <Field label={t("settings.smtp_server")} desc={t("settings.smtp_server_desc")}>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-2">
                <Input {...kanzleiForm.register("smtpHost")} placeholder="mail.example.com" className="sm:col-span-2" />
                ...
                <label className="flex items-center gap-2 text-sm text-[color:var(--ds-text-muted)]">
                  <input type="checkbox" {...kanzleiForm.register("smtpSecure")} className="accent-[var(--brand-primary)]" />
                  {t("settings.tls")}
                </label>
              </div>
            </Field>
            ...
            <Field label={t("settings.smtp_password")} desc={t("settings.smtp_password_desc")}>
              <Input type="password" {...kanzleiForm.register("smtpPassword")} placeholder="••••••" />
            </Field>
          ...
          <div className="p-6 border-t border-[color:var(--ds-border)]">
            {kanzleiSaveError && (
              <p className="text-sm text-red-600 mb-3">{t("settings.save_fail")} {kanzleiSaveError}</p>
            )}
      ...
      {activeTab === "team" && (
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <h2 className="text-base font-semibold text-[color:var(--ds-text)] tracking-tight">{t("settings.team_title")}</h2>
            <p className="text-sm text-[color:var(--ds-text-muted)] mt-1 leading-relaxed">{t("settings.team_desc")}</p>
          </div>
          <div className="px-6 divide-y divide-[color:var(--ds-border)]">
            {teamMembers.length === 0 ? (
              <div className="py-10 text-center">
                ...
                <p className="text-sm text-[color:var(--ds-text-muted)]">{t("settings.team_empty")}</p>
              </div>
            ) : (
              teamMembers.map((member) => (
                <div key={member.id} className="py-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-[color:var(--ds-hover)] border border-[color:var(--ds-border)] flex items-center justify-center shrink-0">
                      <span className="text-xs font-semibold text-[color:var(--ds-text-muted)]">{member.name.charAt(0).toUpperCase()}</span>
                    </div>
                    <div>
                      <div className="text-sm font-medium text-[color:var(--ds-text)]">{member.name}</div>
                      <div className="text-xs text-[color:var(--ds-text-muted)] mt-0.5">{member.email}</div>
                    </div>
                  </div>
                  <select
                    value={member.role}
                    onChange={async (e) => {
                      try {
                        await updateRoleMutation.mutateAsync({ userId: member.id, role: e.target.value });
                    ...
                    className="bg-[color:var(--ds-surface-2)] border border-[color:var(--ds-border)] rounded-lg px-3 py-1.5 text-sm text-[color:var(--ds-text)] focus:outline-none focus:ring-2 focus:ring-[var(--brand-primary)] focus:ring-offset-1 focus:ring-offset-[var(--ds-surface)] focus:border-[color:var(--brand-primary)] transition-all"
                  >
        ...
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-[color:var(--ds-text)] tracking-tight">{t("settings.scim_title")}</h2>
          ...
          <div className="px-6 divide-y divide-[color:var(--ds-border)]">
            <Field label={t("settings.scim_endpoint")} desc={t("settings.scim_endpoint_desc")}>
              <code className="text-xs font-mono text-[color:var(--ds-text-muted)] bg-[color:var(--ds-surface-2)] border border-[color:var(--ds-border)] rounded-lg px-3 py-2 block">
                {typeof window !== "undefined" ? window.location.origin : "https://subsum.eu"}/api/scim
              </code>
            </Field>
      ...
      {activeTab === "account" && (
        <Card>
          <div className="p-6 border-b border-[color:var(--ds-border)]">
            <h2 className="text-base font-semibold text-[color:var(--ds-text)] tracking-tight">{t("settings.account_title")}</h2>
          </div>
          <div className="px-6 divide-y divide-[color:var(--ds-border)]">
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-[color:var(--ds-text-muted)]">{t("settings.usage_pages")}</span>
                    <span className="text-[color:var(--ds-text)] font-mono tabular-nums">0 / 100</span>
                  </div>
                  <div className="h-1.5 bg-[color:var(--ds-border)] rounded-full overflow-hidden">
                    <div className="h-full w-0 brand-bg rounded-full" />
                  </div>
                </div>
                <div>
                  <div className="flex justify-between text-xs mb-1.5">
                    <span className="text-[color:var(--ds-text-muted)]">{t("settings.usage_queries")}</span>
                    <span className="text-[color:var(--ds-text)] font-mono tabular-nums">0 / 50</span>
                  </div>
                  <div className="h-1.5 bg-[color:var(--ds-border)] rounded-full overflow-hidden">
                    <div className="h-full w-0 brand-bg rounded-full" />
                  </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 bg-gradient-to-r from-amber-500/10 to-transparent border border-amber-500/20 rounded-lg px-4 py-3">
                  ...
                  <p className="text-xs text-[color:var(--ds-text-muted)] leading-relaxed">
                    ...
                    {referrals !== null && (
                      <span className="text-amber-600 font-medium"> {t("settings.referral_so_far")} {referrals}.</span>
                    )}
                  </p>
                </div>
                <MaskedInput
                  value={referralUrl}
                  placeholder={t("settings.referral_loading")}
                />
                <Link
                  href="/partners"
                  className="inline-flex items-center gap-1 text-xs brand-text hover:underline"
            ...
            <Field
              ...
              desc={t("settings.data_export_desc")}
            >
