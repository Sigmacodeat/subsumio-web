...
const STALE_TIME_BUDGET_MS = Math.max(
  1000,
  Number(process.env.GBRAIN_EXTRACT_TIME_BUDGET_MS) || 30 * 60 * 1000
);
...
export async function stampExtracted(
  ...
  try {
    ...
    /* best-effort: page stays stale, extract --stale re-sweeps it */
  }
}
...
export function resolveCandidateSources(
  ...
  const fromSourceId = fromSources.includes(pageSourceId)
    ? pageSourceId
    : fromSources.includes("default")
      ? "default"
      : fromSources[0];
...
export function logBatchRetry(
  ...
  console.error(`[${label}] connection blip, retrying ${snapshotLen} rows in 500ms (${msg})`);
}
 ...
 */
export function resolveBasenameMatchesFromSlugs(name: string, allSlugs: Set<string>): string[] {
  ...
  for (const { name, relTarget } of extractMarkdownLinks(scanContent)) {
    ...
    for (const target of resolvedSlugs) {
      ...
      links.push({
        ...
        context: isBasename ? `wikilink (basename match): [${name}]` : `markdown link: [${name}]`,
...
export function extractTimelineFromContent(
  content: string,
  slug: string
...
export async function runExtractCore(
  ...
  const workersResolved = resolveWorkersWithClamp(engine, opts.workers, "extract", 0);
...
export async function runExtract(engine: BrainEngine, args: string[]) {
  ...
  const typeFilter =
    typeIdx >= 0 && typeIdx + 1 < args.length ? (args[typeIdx + 1] as string) : undefined;
  ...
  const workersValIdx =
    workersIdx >= 0 ? workersIdx + 1 : concurrencyIdx >= 0 ? concurrencyIdx + 1 : -1;
  ...
  if (since !== undefined) {
    ...
    if (!Number.isFinite(sinceMs)) {
      console.error(
        `Invalid --since date: "${since}". Must be a parseable date (e.g., "2026-01-15" or full ISO timestamp).`
      );
      process.exit(1);
    }
  }
...
Status (v0.42):
  ...
  if (ner && subcommand === "timeline") {
    console.error(`--ner is a links-pass only; it does not apply to timeline extraction.`);
    process.exit(2);
  }
  ...
  try {
    if (source === "db") {
      ...
      if (fromMeetings) {
        ...
        if (!jsonMode) {
          console.log(
            `Timeline from meetings: ${r.entries_created} entries on ${r.entities_touched} entity pages from ${r.meetings_scanned} meetings`
          );
        }
        ...
        if (ner) {
          ...
          if (r.pack_unavailable && !jsonMode) {
            console.log(
              "Note: no active schema pack with link_types[].inference.regex — NER pass produced 0 links."
            );
          }
          result.links_created += r.created;
...
async function extractForSlugs(
      ...
      try {
        ...
        if (doLinks) {
          ...
          for (const link of links) {
            if (dryRun) {
              if (!jsonMode)
                console.log(`  ${link.from_slug} → ${link.to_slug} (${link.link_type})`);
              linksCreated++;
            } else {
              linkBatch.push(link);
  ...
  if (!jsonMode) {
    ...
    console.log(
      `Incremental extract: ${label} ${linksCreated} link(s), ${timelineCreated} timeline entries from ${pagesProcessed}/${slugs.length} page(s)`
    );
  }
    ...
    try {
      ...
      if (jsonMode) {
        process.stderr.write(
          JSON.stringify({ event: "batch_error", size: snapshot.length, error: msg }) + "\n"
        );
      } else {
    ...
    try {
      ...
      for (const link of await extractLinksFromFile(content, slug + ".md", allSlugs, {
        ...
        try {
          ...
          /* skip */
        } // gbrain-allow-direct-insert: gbrain extract single-row fallback when batch path declines a row
      }
    } catch {
      /* skip */
    }
    ...
    try {
      ...
      for (const entry of extractTimelineFromContent(content, slug)) {
        try {
          ...
          /* skip */
        } // gbrain-allow-direct-insert: gbrain extract single-row fallback for timeline entries
      }
    } catch {
      /* skip */
    }
...
async function extractLinksFromDB(
  ...
  const fullRefsForResolver = sourceIdFilter ? await engine.listAllPageRefs() : allRefs;
    ...
    for (const c of extracted.candidates) {
      ...
      if (dryRunSeen) {
        ...
        if (jsonMode) {
          ...
          console.log(
            `  ${fromSlug} → ${c.targetSlug} (${c.linkType})${c.linkSource === "frontmatter" ? " [fm]" : ""}`
          );
        }
        created++;
      } else {
...
async function extractStaleFromDB(
  ...
  const totalStale = await engine.countStalePagesForExtraction({
  ...
  if (dryRun) {
    if (jsonMode) {
      ...
      console.log(
        `(dry run) ${totalStale} page(s) need link/timeline extraction. Run without --dry-run to extract.`
      );
    }
    ...
    for (const page of rows) {
      ...
      for (const c of extracted.candidates) {
        ...
        linkRows.push({
          ...
          origin_slug: c.originSlug,
          origin_field: c.originField,
          from_source_id: r.fromSourceId,
          to_source_id: r.toSourceId,
  ...
  progress.finish();
  const staleRemaining = await engine.countStalePagesForExtraction({
    sourceId: sourceIdFilter,
    versionTs,
  });
  ...
  if (!jsonMode) {
    console.log(
      `Extract --stale: ${linksCreated} link(s) + ${timelineCreated} timeline entr(ies) from ${pagesProcessed} page(s).`
    );
    if (budgetHit && staleRemaining > 0) {
      console.log(
        `Time budget reached — ${staleRemaining} page(s) still stale. Re-run 'gbrain extract --stale' (or pass --catch-up) to continue.`
      );
    }
  } else {
    process.stdout.write(
...
async function extractMentionsFromDb(
  ...
  const completed = dryRun ? new Set<string>() : new Set(await loadOpCheckpoint(engine, ckptKey));
  const remaining =
    completed.size > 0
  ...
  if (completed.size > 0 && !jsonMode) {
    console.log(
      `[by-mention] resuming: ${completed.size}/${allRefs.length} pages already scanned, ${remaining.length} remaining`
    );
  }
  ...
  async function flushBatch() {
    ...
    try {
      ...
      if (jsonMode) {
        process.stderr.write(
          JSON.stringify({ event: "batch_error", size: batch.length, error: msg }) + "\n"
        );
      } else {
  ...
  async function flushAndCheckpoint(force = false): Promise<void> {
    ...
    if (
      force ||
      unpersistedCount >= PERSIST_EVERY_N ||
      now - sinceLastPersistMs >= PERSIST_EVERY_MS
    ) {
  ...
  if (!jsonMode) {
    const label = dryRun ? "(dry run) would create" : "created";
    console.log(
      `Mentions: ${label} ${created} links from ${processed} pages against gazetteer of ${gazetteer.size} first-token buckets`
    );
  }
  return { created, pages: processed };
}
