...
export async function judgeResponse(question, transcript, opts = {}) {
  ...
  const prompt = [
    ...
    expectedAnswer
      ? `Expected answer contains: "${expectedAnswer}"`
      : "Use your judgment for what counts as a correct answer.",
    "",
  ...
  try {
    ...
    return {
      verdict: "inconclusive",
      reason: `judge returned non-JSON: ${raw.slice(0, 100)}`,
      transcript,
    };
  }
  const verdict =
    parsed.verdict === "pass" ? "pass" : parsed.verdict === "fail" ? "fail" : "inconclusive";
  return {
    verdict,
