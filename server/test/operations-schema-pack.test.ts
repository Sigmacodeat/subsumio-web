...
function ctxOf(
  opts: { remote?: boolean; clientId?: string; sourceId?: string } = {}
): OperationContext {
  return {
    engine,
    config: {},
...
describe("get_active_schema_pack", () => {
  it("returns identity packet for the bundled gbrain-base pack", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: undefined }, async () => {
      const result = (await operationsByName.get_active_schema_pack!.handler(
        ctxOf(),
        {}
...
describe("list_schema_packs", () => {
  it("returns bundled + installed", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir }, async () => {
      seedPack("mine");
      const result = (await operationsByName.list_schema_packs!.handler(ctxOf(), {})) as {
        bundled: string[];
        installed: string[];
      };
...
describe("schema_stats", () => {
  it("returns coverage + per-source breakdown", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: undefined }, async () => {
      ...
      const result = (await operationsByName.schema_stats!.handler(ctxOf(), {})) as Record<
        string,
        unknown
      >;
...
describe("schema_lint", () => {
  it("lints the active pack and returns ok=true for clean pack", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: undefined }, async () => {
      const result = (await operationsByName.schema_lint!.handler(ctxOf(), {})) as Record<
        string,
        unknown
      >;
  ...
  it("returns pack_not_found for unknown pack", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir }, async () => {
      const result = (await operationsByName.schema_lint!.handler(ctxOf(), {
        pack: "nonexistent",
      })) as Record<string, unknown>;
...
describe("schema_graph", () => {
  it("returns nodes + edges from link_types", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: undefined }, async () => {
      const result = (await operationsByName.schema_graph!.handler(ctxOf(), {})) as Record<
        string,
        unknown
      >;
...
describe("schema_explain_type", () => {
  it("returns settings for a known type", async () => {
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: undefined }, async () => {
      const result = (await operationsByName.schema_explain_type!.handler(ctxOf(), {
        type: "person",
      })) as Record<string, unknown>;
...
describe("schema_review_orphans", () => {
  it("returns untyped pages from the DB", async () => {
    ...
    const result = (await operationsByName.schema_review_orphans!.handler(ctxOf(), {})) as Record<
      string,
      unknown
    >;
  ...
  it("respects limit param", async () => {
    ...
    const result = (await operationsByName.schema_review_orphans!.handler(ctxOf(), {
      limit: 2,
    })) as Record<string, unknown>;
      ...
      const result = (await operationsByName.schema_apply_mutations!.handler(ctxOf(), {
        ...
        mutations: [
          ...
          { op: "add_type", name: "company", primitive: "entity", prefix: "companies/" },
          {
            op: "add_link_type",
            name: "works_at",
            inference: { page_type: "researcher", target_type: "company" },
          },
        ],
      })) as Record<string, unknown>;
...
describe("reload_schema_pack", () => {
  it("returns invalidated list", async () => {
    const result = (await operationsByName.reload_schema_pack!.handler(ctxOf(), {})) as {
      invalidated: string[];
    };
  ...
  it("invalidates a specific pack by name", async () => {
    const result = (await operationsByName.reload_schema_pack!.handler(ctxOf(), {
      pack: "foo",
    })) as { invalidated: string[] };
