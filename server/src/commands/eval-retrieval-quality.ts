...
export async function runEvalRetrievalQuality(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!fixture) {
    console.error(
      "Usage: gbrain eval retrieval-quality <fixture.jsonl> [--json] [--source <id>] [--ab-relational]"
    );
    process.exit(2);
  }
  ...
  if (abRelational) {
    ...
    const payload = {
      ...
      graph_relationship: {
        n: rel.on?.n ?? 0,
        recall_at_10: {
          off: rel.off?.recall_at_10 ?? 0,
          on: rel.on?.recall_at_10 ?? 0,
    ...
    if (json) {
      ...
      console.log(
        `  recall@10:  off=${(g.recall_at_10.off * 100).toFixed(0)}%  on=${(g.recall_at_10.on * 100).toFixed(0)}%  Δ=+${(g.recall_at_10.delta * 100).toFixed(0)}pp`
      );
      console.log(
        `  Hit@3:      off=${(g.hit_at_3.off * 100).toFixed(0)}%  on=${(g.hit_at_3.on * 100).toFixed(0)}%`
      );
      console.log(
        `  latency:    off=${payload.latency_ms.off_total}ms  on=${payload.latency_ms.on_total}ms (arm adds ${payload.latency_ms.on_total - payload.latency_ms.off_total}ms over ${payload.graph_relationship.n} queries)`
      );
    }
    process.exit(0);
  }
  ...
  if (json) {
    ...
    console.log(
      `NamedThingBench — ${report.total} queries across ${report.families.length} families\n`
    );
    for (const f of report.families) {
      console.log(
        `  ${f.family.padEnd(22)} n=${f.n}  Hit@1=${(f.hit_at_1 * 100).toFixed(0)}%  Hit@3=${(f.hit_at_3 * 100).toFixed(0)}%  MRR=${f.mrr.toFixed(3)}`
      );
    }
    console.log("");
    if (gate.breaches.length) {
      ...
      for (const b of gate.breaches) {
        console.log(
          `  ✗ ${b.family} ${b.metric}=${(b.got * 100).toFixed(0)}% < floor ${(b.floor * 100).toFixed(0)}%`
        );
      }
    } else {
    ...
    for (const w of gate.warnings) {
      console.log(
        `  ⚠ (warn) ${w.family} ${w.metric}=${(w.got * 100).toFixed(0)}% < ${(w.floor * 100).toFixed(0)}%`
      );
    }
  }
