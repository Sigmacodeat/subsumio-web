...
describe("connect bearer probe E2E (PGLite + real serve --http)", () => {
  ...
  beforeAll(async () => {
    ...
    execFileSync(
      "bun",
      ["run", "src/cli.ts", "init", "--pglite", "--no-embedding", "--non-interactive"],
      {
        cwd: process.cwd(),
        env,
        stdio: "ignore",
    ...
    if (!oauthClientId || !oauthClientSecret)
      throw new Error(`register-client did not yield creds:\n${regOut}`);
  ...
  (HAS_CLAUDE ? test : test.skip)(
    "claude-code --install registers + connects against the live server",
    () => {
      expect(serverReady).toBe(true);
      const claudeHome = mkdtempSync(join(tmpdir(), "gb-claude-"));
      try {
        const r = runConnectCli([MCP_URL, "--token", token, "--install", "--yes"], {
          HOME: claudeHome,
        });
        expect(r.code).toBe(0);
        expect(r.out).toMatch(/Verified/);
        // The real `claude` CLI actually registered the server.
        const got = spawnSync("claude", ["mcp", "get", "gbrain"], {
          encoding: "utf8",
        ...
        try {
          spawnSync("claude", ["mcp", "remove", "gbrain"], {
            env: { ...process.env, HOME: claudeHome },
          });
        } catch {
          /* best-effort */
  ...
  (HAS_CODEX ? test : test.skip)(
    "codex --install registers the env-var bearer against the live server",
    () => {
      expect(serverReady).toBe(true);
      const codexHome = mkdtempSync(join(tmpdir(), "gb-codex-"));
      try {
        const r = runConnectCli(
          [MCP_URL, "--token", token, "--agent", "codex", "--install", "--yes"],
          { CODEX_HOME: codexHome, GBRAIN_REMOTE_TOKEN: token }
        );
        expect(r.code).toBe(0);
        expect(r.out).toMatch(/Verified/);
        ...
        const got = spawnSync("codex", ["mcp", "get", "gbrain"], {
          encoding: "utf8",
  ...
  test("perplexity OAuth: connect --oauth → mint client-credentials token → tool call", async () => {
    ...
    const minted = await mintClientCredentialsToken(
      ...
      oauthClientId,
      oauthClientSecret,
      { scope: "read write", timeoutMs: 10_000 }
    );
    expect(minted.ok).toBe(true);
    if (!minted.ok) return;
    const probed = await probeBrainIdentity(MCP_URL, minted.token.access_token, {
      timeoutMs: 15_000,
    });
