...
describe("embeddingMismatchMessage", () => {
  ...
  test("source: doctor uses a different header than source: init", () => {
    const initMsg = embeddingMismatchMessage({
      currentDims: 1536,
      requestedDims: 768,
      source: "init",
      engineKind: "postgres",
    });
    const doctorMsg = embeddingMismatchMessage({
      currentDims: 1536,
      requestedDims: 768,
      source: "doctor",
      engineKind: "postgres",
...
describe("resolveSchemaEmbeddingDim", () => {
  ...
  test("OpenAI text-3-large rejects 2048 (not in declared dims_options)", () => {
    ...
    expect(got.ok).toBe(false);
    if (!got.ok)
      expect(got.error).toMatch(
        /rejects custom dimensions 2048|does not support custom dimensions/
      );
  });
