...
async function importSource(
  sourceName: string,
  connector: {
    fetchDelta: (cursor?: string) => Promise<{ items: ConnectorItem[]; nextCursor?: string }>;
    toIngestionEvent: (item: ConnectorItem) => Promise<IngestionEvent>;
  },
  engine: BrainEngine,
  cursor?: string
): Promise<ImportStats> {
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    if (sourceName === "openlegaldata")
      cursorState.openlegaldata = nextCursor ?? cursorState.openlegaldata;
  ...
  console.log(
    `[${sourceName}] Fertig: ${imported} importiert, ${skipped} übersprungen, ${errors} Fehler in ${(duration / 1000).toFixed(1)}s`
  );
...
async function main() {
  ...
  if (SOURCE === "all" || SOURCE === "ris-ogd") {
    if (!cursorState.completedSources.includes("ris-ogd")) {
      ...
      const stats = await importSource(
        "ris-ogd",
        conn,
        engine,
        cursorState.risOgd ?? defaultCursor
      );
      results.push(stats);
  ...
  if (SOURCE === "all" || SOURCE === "openlegaldata") {
    if (!cursorState.completedSources.includes("openlegaldata")) {
      ...
      const stats = await importSource(
        ...
        conn,
        engine,
        cursorState.openlegaldata ?? defaultCursor
      );
      results.push(stats);
  ...
  if (SOURCE === "all" || SOURCE === "eur-lex") {
    if (!cursorState.completedSources.includes("eur-lex")) {
      const conn = new EurLexConnector({
        filters: {
          query: "court ruling OR preliminary ruling OR case law",
          max_pages: "5",
      ...
      const stats = await importSource(
        ...
        conn,
        engine,
        cursorState.eurLex ?? defaultCursor
      );
      results.push(stats);
  ...
  for (const r of results) {
    console.log(
      `${r.source.padEnd(20)} | ${r.imported.toString().padStart(5)}/${r.fetched.toString().padStart(5)} | ${r.skipped} skip | ${r.errors} err | ${(r.duration / 1000).toFixed(1)}s`
    );
  }
  console.log("───────────────────────────────────────────────────────────");
  console.log(
    `${"GESAMT".padEnd(20)} | ${totalImported.toString().padStart(5)}/${totalFetched.toString().padStart(5)} | ${totalSkipped} skip | ${totalErrors} err | ${(totalDuration / 1000).toFixed(1)}s`
  );
  console.log("");
