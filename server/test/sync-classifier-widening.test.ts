...
describe("Layer 2 — resolveSlugForPath dispatches by extension (SP-5)", () => {
  test("markdown path → markdown-style slug (slugifyPath)", () => {
    expect(resolveSlugForPath("people/alice-smith.md")).toBe(
      slugifyPath("people/alice-smith.md").toLowerCase()
    );
  });
  ...
  test("code path → code-style slug (slugifyCodePath)", () => {
    ...
    expect(resolveSlugForPath("src/core/sync.ts")).toBe(
      slugifyCodePath("src/core/sync.ts").toLowerCase()
    );
  });

  test("Rust file uses code-slug (previously fell through to markdown, SP-5)", () => {
    expect(resolveSlugForPath("crates/worker/src/main.rs")).toBe(
      slugifyCodePath("crates/worker/src/main.rs").toLowerCase()
    );
  });

  test("repoPrefix is applied before lowercasing", () => {
    expect(resolveSlugForPath("src/foo.ts", "my-repo")).toMatch(/^my-repo\//);
  });
