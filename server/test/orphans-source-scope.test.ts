...
beforeAll(async () => {
  ...
  await engine.addLink(
    ...
    undefined,
    undefined,
    { fromSourceId: "src-b", toSourceId: "default" }
  );
});
...
describe("findOrphanPages — source scoping (A2)", () => {
  ...
  test("unscoped (brain-wide) returns the union", async () => {
    ...
    const slugs = rows.map((r) => r.slug).sort();
    expect(slugs).toEqual(["people/alice-orphan", "people/src-b-linker", "people/zara-orphan"]);
  });
  ...
  test("federated sourceIds scopes to the union of the given sources", async () => {
    ...
    expect(onlyB.map((r) => r.slug).sort()).toEqual(["people/src-b-linker", "people/zara-orphan"]);
  });
});
