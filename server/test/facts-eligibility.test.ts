...
describe("isFactsBackstopEligible — guards", () => {
  test("null parsed → no_parsed_page", () => {
    expect(isFactsBackstopEligible("any/slug", null)).toEqual({
      ok: false,
      reason: "no_parsed_page",
    });
  });

  test("undefined parsed → no_parsed_page", () => {
    expect(isFactsBackstopEligible("any/slug", undefined)).toEqual({
      ok: false,
      reason: "no_parsed_page",
    });
  });
  ...
  test("subagent namespace (wiki/agents/...) is rejected even with eligible type", () => {
    ...
    expect(isFactsBackstopEligible(f.slug, f.parsed)).toEqual({
      ok: false,
      reason: "subagent_namespace",
    });
  });
