...
/** NtryRef/AcctSvcrRef für Deduplizierung; Fallback bleibt der generierte id. */
export function parseCamt053(xml: string): CamtStatement {
  ...
  for (const stmt of stmts) {
    for (const ntry of descendants(stmt, "Ntry")) {
      ...
      const ntryRef = firstText(ntry, "NtryRef") ?? firstText(ntry, "AcctSvcrRef");
      ...
      if (ntryRef) txn.id = `camt-${ntryRef.replace(/[^A-Za-z0-9_-]/g, "-")}`;
