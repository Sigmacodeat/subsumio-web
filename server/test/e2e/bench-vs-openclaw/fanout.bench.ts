...
describeBench("Bench: Fan-out (parent → 10 children in parallel)", () => {
  ...
  test(
    `Minions: ${RUNS} runs × fan-out to ${CHILDREN} children (concurrency=${CHILDREN})`,
    async () => {
      ...
      try {
        ...
        for (let run = 0; run < RUNS; run++) {
          ...
          console.log(`  [minions-fanout] run=${run + 1} ok=${ok}/${CHILDREN} wallMs=${wallMs}`);
        }
  ...
  test(
    `OpenClaw: ${RUNS} runs × ${CHILDREN} parallel --local spawns`,
    async () => {
      const runs: RunResult[] = [];
      ...
      for (let run = 0; run < RUNS; run++) {
        ...
        runs.push({ ok, fail, wallMs });
        console.log(`  [openclaw-fanout] run=${run + 1} ok=${ok}/${CHILDREN} wallMs=${wallMs}`);
      }
