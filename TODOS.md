...
- [ ] **P3 — Element-isolation fallback for batch inserts.** On a non-retryable
      ...
      there is no remaining data-dependent crash for this to catch _today_ — it's
...
OUT — pre-existing, and the ownership fix _reduces_ its window. See plan +
...
- [ ] **P3 — Stale `ConnectionManager` read-pool after an owner `reconnect()`.**
      ...
      the ownership fix makes owner reconnects _rarer_ (the singleton no longer gets
...
- [ ] **P3 — Size the direct session pool for enrich fan-out.** The lock
      ...
      checks out per-statement (not held open), so the risk is bounded by _concurrent
      in-flight heartbeats_, not duration — but under heavy `enrich` fan-out (many
...
- [ ] **T7 (P3): Tier 2 — local thin-client over a bearer token.** `gbrain connect`
      today only wires the MCP _connection_ (Claude Code talks straight to the remote /mcp).
...
**Codex F2 note for the implementation:** Use `Math.floor(50 × 0.95)` (index 47) for p95 or the actual sorted-percentile method, NOT `Math.floor(50 ×
...
key exists, which could false-reject a _usable_ model in some test/unconfigured
