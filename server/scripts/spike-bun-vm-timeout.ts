...
if (outcome === "timeout") {
  ...
  console.log(
    `[spike] wall-clock latency: ~${elapsed.toFixed(0)}ms for a ${TIMEOUT_MS}ms configured timeout.`
  );
  ...
  console.log(
    "[spike] action: swap redos-guard.runRegexBounded for E6 option B (persistent worker pool)."
  );
  process.exit(1);
} else {
  console.log("");
