  ...
  try {
    ...
    try {
      ...
      return {
        name: "verify",
        status: "complete",
        detail: `${REQUIRED_TABLES.length} tables present`,
      };
    } finally {
      try {
...
async function orchestrator(opts: OrchestratorOpts): Promise<OrchestratorResult> {
  ...
  const status: "complete" | "partial" | "failed" = b.status === "failed" ? "partial" : "complete";
...
export const v0_16_0: Migration = {
  ...
  featurePitch: {
    headline:
      "Durable LLM agents land in the brain — survive crashes, sleeps, and worker restarts.",
    description:
