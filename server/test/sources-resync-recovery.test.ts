...
describe("validateRepoState — full state matrix (sync re-clone driver)", () => {
  test("healthy: existing .git + matching origin URL", async () => {
    await withEnv({ GBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      expect(validateRepoState(row.local_path!, "https://github.com/example/repo")).toBe("healthy");
    });
  });
  ...
  test("missing: clone dir was rmd", async () => {
    await withEnv({ GBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      expect(validateRepoState(row.local_path!, "https://github.com/example/repo")).toBe("missing");
    });
  });
  ...
  test("not-a-dir: clone path is a file", async () => {
    await withEnv({ GBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      expect(validateRepoState(row.local_path!, "https://github.com/example/repo")).toBe(
        "not-a-dir"
      );
    });
  });
  ...
  test("no-git: directory exists but no .git/ inside", async () => {
    await withEnv({ GBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      expect(validateRepoState(row.local_path!, "https://github.com/example/repo")).toBe("no-git");
    });
  });
