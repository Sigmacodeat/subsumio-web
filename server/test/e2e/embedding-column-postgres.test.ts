...
if (!dbUrl) {
  ...
  beforeAll(async () => {
    ...
    await engine.executeRaw(
      `UPDATE content_chunks SET embedding_voyage = '${vec1024(0.5)}'::vector WHERE id = ${catId}`
    );
    await engine.executeRaw(
      `UPDATE content_chunks SET embedding_voyage = '${vec1024(0.7)}'::vector WHERE id = ${dogId}`
    );
    await engine.executeRaw(
      `UPDATE content_chunks SET embedding_ze = '${vec2560(0.5)}'::halfvec WHERE id = ${catId}`
    );
    await engine.executeRaw(
      `UPDATE content_chunks SET embedding_ze = '${vec2560(0.7)}'::halfvec WHERE id = ${dogId}`
    );
  });
  ...
  describe("Postgres: HNSW index visible to planner", () => {
    test("pg_indexes shows the hnsw index on embedding_voyage", async () => {
      ...
      const voyageHnsw = rows.find(
        (r) => r.indexname === "idx_chunks_embedding_voyage" && /USING\s+hnsw/i.test(r.indexdef)
      );
  ...
  describe("Postgres: doctor SQL probe for coverage (D14)", () => {
    ...
    test("coverage % drops to 50 after a partial wipe; gate at < 90 fires", async () => {
      ...
      await engine.executeRaw(
        `UPDATE content_chunks SET embedding_voyage = NULL WHERE id = ${dogId}`
      );
      ...
      await engine.executeRaw(
        `UPDATE content_chunks SET embedding_voyage = '${v}'::vector WHERE id = ${dogId}`
      );
    });
  });
}
