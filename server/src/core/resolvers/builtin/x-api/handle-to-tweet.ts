...
export const xHandleToTweetResolver: Resolver<XHandleToTweetInput, XHandleToTweetOutput> = {
  ...
  backend: "x-api-v2",
  description:
    "Find a tweet by handle + keyword hint. Used by integrity to repair bare-tweet citations.",
  inputSchema: {
    type: "object",
    properties: {
  ...
  async resolve(
    req: ResolverRequest<XHandleToTweetInput>
  ): Promise<ResolverResult<XHandleToTweetOutput>> {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES_ON_429; attempt++) {
      try {
        ...
        throw new ResolverError(
          "upstream",
          `x_handle_to_tweet fetch failed: ${errMessage(err)}`,
          "x_handle_to_tweet",
          err
        );
      }
    ...
    if (!resp) {
      throw new ResolverError(
        "upstream",
        `x_handle_to_tweet: no response after retries (${errMessage(lastErr)})`,
        "x_handle_to_tweet"
      );
    }
    ...
    if (resp.status === 401 || resp.status === 403) {
      throw new ResolverError(
        "auth",
        `x_handle_to_tweet: auth failed (HTTP ${resp.status}) — check X_API_BEARER_TOKEN`,
        "x_handle_to_tweet"
      );
    }
    if (resp.status === 429) {
    ...
    if (!resp.ok) {
      ...
      throw new ResolverError(
        "upstream",
        `x_handle_to_tweet: HTTP ${resp.status} — ${body.slice(0, 200)}`,
        "x_handle_to_tweet"
      );
    }
