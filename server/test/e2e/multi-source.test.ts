...
describeE2E("v0.18.0 multi-source — Postgres schema shape (fresh install)", () => {
  ...
  test("sources('default') exists after initSchema + migration chain", async () => {
    const conn = getConn();
    const rows = await conn.unsafe(`SELECT id, name, config FROM sources WHERE id = 'default'`);
...
describeE2E("v0.18.0 multi-source — composite UNIQUE semantics on real Postgres", () => {
  ...
  test("same slug in two sources coexists (REGRESSION GUARD — Codex critical)", async () => {
    ...
    await engine.putPage("topics/ai", {
      type: "concept",
      title: "AI from default",
      compiled_truth: "default source take",
    });
    await conn.unsafe(
...
describeE2E("v0.18.0 multi-source — cascade delete covers every dependent row", () => {
  ...
  test("sources remove cascades to pages + chunks + timeline + links + files", async () => {
    ...
    expect(
      (await conn.unsafe(`SELECT COUNT(*)::int AS n FROM pages WHERE source_id = 'cascadetest'`))[0]
        .n
    ).toBe(2);
    expect(
      (
        await conn.unsafe(
          `SELECT COUNT(*)::int AS n FROM content_chunks WHERE page_id = ${aliceId}`
        )
      )[0].n
    ).toBe(1);
    expect(
      (
        await conn.unsafe(
          `SELECT COUNT(*)::int AS n FROM timeline_entries WHERE page_id = ${aliceId}`
        )
      )[0].n
    ).toBe(1);
    expect(
      (await conn.unsafe(`SELECT COUNT(*)::int AS n FROM links WHERE from_page_id = ${aliceId}`))[0]
        .n
    ).toBe(1);
    expect(
      (await conn.unsafe(`SELECT COUNT(*)::int AS n FROM files WHERE source_id = 'cascadetest'`))[0]
        .n
    ).toBe(1);
...
describeE2E("v0.18.0 multi-source — storage backfill against file_migration_ledger", () => {
  ...
  test("seeded ledger + stub storage: pending → complete end-to-end", async () => {
    ...
    const filesAfter = await conn.unsafe(`SELECT storage_path FROM files WHERE id = ${fileId}`);
...
// own coverage.
describeE2E(
  "v0.18.0 multi-source — addLinksBatch / addTimelineEntriesBatch source-awareness",
  () => {
    beforeAll(async () => {
      await setupDB();
      const conn = getConn();
    ...
    async function seedSameSlugTwoSources() {
      ...
      await engine.putPage("topics/ml", {
        type: "concept",
        title: "ML (default)",
        compiled_truth: "",
        timeline: "",
