...
describe("quarantine marker (hides)", () => {
  test("buildQuarantineMarker shape", () => {
    const m = buildQuarantineMarker("junk_pattern", "cloudflare_ray_id", {
      bytes: 1234,
...
describe("content_flag marker (warns, does NOT hide)", () => {
  test("buildContentFlagMarker shape (markup_heavy)", () => {
    const m = buildContentFlagMarker("markup_heavy", "ratio 0.91", {
      markup_ratio: 0.91,
      now: new Date("2026-06-01T00:00:00Z"),
    });
  ...
  test("getContentFlag returns reason+detail or null", () => {
    expect(getContentFlag({ [CONTENT_FLAG_KEY]: { reason: "oversized", detail: "big" } })).toEqual({
      reason: "oversized",
      detail: "big",
    });
