...
describe("lintContent", () => {
  test('detects LLM preamble "Of course"', () => {
    const content =
      "Of course. Here is a detailed brain page for Jane Doe.\n\n# Jane Doe\n\nContent.";
  ...
  test('detects LLM preamble "I\'ve created"', () => {
    const content =
      "I've created a comprehensive brain page for the company.\n\n# Acme\n\nContent.";
    const issues = lintContent(content, "test.md");
    expect(issues.some((i) => i.rule === "llm-preamble")).toBe(true);
  });
  ...
  test("detects empty sections", () => {
    ...
    const issues = lintContent(content, "test.md");
    expect(
      issues.some((i) => i.rule === "empty-section" && i.message.includes("What They Believe"))
    ).toBe(true);
  });
  ...
  test("detects agent placeholder sections", () => {
    ...
    const issues = lintContent(content, "test.md");
    expect(issues.some((i) => i.rule === "empty-section" && i.message.includes("Summary"))).toBe(
      true
    );
  });
