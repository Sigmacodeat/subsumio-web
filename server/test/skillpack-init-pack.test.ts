...
import { InitScaffoldError, runInitScaffold } from "../src/core/skillpack/init-scaffold.ts";
import { PackPublishError, runPackPublish } from "../src/core/skillpack/pack-publish.ts";
...
describe("runInitScaffold — cathedral default", () => {
  ...
  test("refuses to overwrite existing files", () => {
    ...
    expect(result.filesSkippedExisting.some((p) => p.endsWith("skills/preexist/SKILL.md"))).toBe(
      true
    );
    // And contents preserved.
    expect(require("fs").readFileSync(join(dir, "skills/preexist/SKILL.md"), "utf-8")).toBe(
      "user content"
    );
  });
