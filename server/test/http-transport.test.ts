...
interface FakeEngine {
  ...
  getTags: (slug: string, opts?: Record<string, unknown>) => Promise<string[]>;
...
interface FakeEngineConfig {
  ...
  validTokens?: Map<
    string,
    {
      id: string;
      name: string;
      permissions?: { takes_holders?: string[]; matter_scope?: string[] | "all" };
    }
  >;
...
function makeFakeEngine(cfg: FakeEngineConfig = {}): FakeEngine {
  ...
  const getPage = async (slug: string): Promise<Record<string, unknown> | null> => ({
    id: 1,
    slug,
    title: slug,
    type: "note",
    compiled_truth: "visible page",
    frontmatter: {},
  ...
  return { kind: "postgres", executeRaw, getPage, getTags, sql, audit };
}
...
describe("http-transport: tools/call dispatch", () => {
  let srv: TestServer;
  const TOK = "tok-fix";
  const EMPTY_SCOPE_TOK = "tok-empty-scope";

  beforeAll(async () => {
    srv = await startTest({
      validTokens: new Map([
        [hash(TOK), { id: "tok-fix-id", name: "fix" }],
        [
          hash(EMPTY_SCOPE_TOK),
          {
            id: "tok-empty-scope-id",
            name: "empty-scope",
            permissions: { matter_scope: [] },
          },
        ],
      ]),
    });
  ...
  test("8b. token matter_scope [] is deny-all, not unrestricted", async () => {
    const r = await fetch(`${srv.url}/mcp`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${EMPTY_SCOPE_TOK}`,
        "Content-Type": "application/json",
      },
      body: rpc("tools/call", { name: "get_page", arguments: { slug: "legal/cases/123" } }),
    });
    expect(r.status).toBe(200);
    const body = await r.json();
    expect(body.result.isError).toBe(true);
    expect(body.result.content[0].text).toContain("page_not_found");
  });
...
describe("http-transport: rate limit", () => {
  ...
  test("15. token bucket: refill mechanic over time", async () => {
    ...
    try {
      // Use up 2 tokens
      const ok1 = await fetch(`${srv.url}/mcp`, {
        method: "POST",
        headers: { Authorization: `Bearer ${TOK}`, "Content-Type": "application/json" },
        body: rpc("tools/list"),
      });
      expect(ok1.status).toBe(200);
      const ok2 = await fetch(`${srv.url}/mcp`, {
        method: "POST",
        headers: { Authorization: `Bearer ${TOK}`, "Content-Type": "application/json" },
        body: rpc("tools/list"),
      });
      ...
      // Third should 429
      const blocked = await fetch(`${srv.url}/mcp`, {
        method: "POST",
        headers: { Authorization: `Bearer ${TOK}`, "Content-Type": "application/json" },
        body: rpc("tools/list"),
      });
      ...
      advanceClock(35_000);
      const refilled = await fetch(`${srv.url}/mcp`, {
        method: "POST",
        headers: { Authorization: `Bearer ${TOK}`, "Content-Type": "application/json" },
        body: rpc("tools/list"),
      });
...
describe("http-transport: mcp_request_log audit", () => {
  test("21. successful request → audit row with token_name + operation + status", async () => {
    const TOK = "audit-tok";
    const srv = await startTest({
      validTokens: new Map([[hash(TOK), { id: "a-1", name: "audit-test" }]]),
    });
    try {
      await fetch(`${srv.url}/mcp`, {
        method: "POST",
