...
export async function tryAcquireDbLock(
  ...
  const acquireOnce = async (): Promise<DbLockHandle | null> => {
    ...
    if (engine.kind === "pglite" && maybePGLite.db) {
      ...
      if (rows.length === 0) return null;
      const deregister = registerCleanup(`db-lock:${lockId}`, async () => {
        await db.query(`DELETE FROM gbrain_cycle_locks WHERE id = $1 AND holder_pid = $2`, [
          lockId,
          pid,
        ]);
      });
  ...
  try {
    ...
    if (
      snap &&
      !snap.ttl_expired &&
      isHolderDeadLocally(snap.holder_pid, snap.holder_host, snap.age_ms)
    ) {
...
function rowToLockSnapshot(row: RawLockRow, now: number): LockSnapshot | null {
  ...
  const ttlExpires =
    row.ttl_expires_at instanceof Date ? row.ttl_expires_at : new Date(row.ttl_expires_at);
  const lastRefreshed =
...
async function selectLockRows(
  ...
  if (engine.kind === "postgres" && maybePG.sql) {
    ...
    if (opts.lockId !== undefined) {
      rows =
        await sql`SELECT id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at FROM gbrain_cycle_locks WHERE id = ${opts.lockId}`;
    } else if (opts.staleOnly) {
      rows =
        await sql`SELECT id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at FROM gbrain_cycle_locks WHERE ttl_expires_at < NOW() ORDER BY acquired_at`;
    } else {
      rows =
        await sql`SELECT id, holder_pid, holder_host, acquired_at, ttl_expires_at, last_refreshed_at FROM gbrain_cycle_locks ORDER BY acquired_at`;
    }
  } else if (engine.kind === "pglite" && maybePGLite.db) {
    if (opts.lockId !== undefined) {
      rows = (
        await maybePGLite.db.query(
          `SELECT ${LOCK_SELECT_COLS} FROM gbrain_cycle_locks WHERE id = $1`,
          [opts.lockId]
        )
      ).rows as RawLockRow[];
    } else if (opts.staleOnly) {
      rows = (
        await maybePGLite.db.query(
          `SELECT ${LOCK_SELECT_COLS} FROM gbrain_cycle_locks WHERE ttl_expires_at < NOW() ORDER BY acquired_at`
        )
      ).rows as RawLockRow[];
    } else {
      rows = (
        await maybePGLite.db.query(
          `SELECT ${LOCK_SELECT_COLS} FROM gbrain_cycle_locks ORDER BY acquired_at`
        )
      ).rows as RawLockRow[];
    }
  } else {
