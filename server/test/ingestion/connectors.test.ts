...
import { type IngestionEvent, type IngestionSourceContext } from '../../src/core/ingestion/types.ts';
...
describe('Google OAuth helper', () => {
  ...
  test('exchangeCode throws on error response', async () => {
    ...
    try {
      await expect(
        exchangeCode('bad-code', 'v', 'id', 'secret', 'uri'),
...
describe('GoogleDriveConnector webhook push', () => {
  test('registerWebhook calls changes.watch and persists channel info', async () => {
    ...
    try {
      const { GoogleDriveConnector } = await import('../../src/core/ingestion/connectors/google-drive.ts');
  ...
  test('fetchDelta returns files from list_folder', async () => {
    ...
    (globalThis as any).fetch = async (input: string | URL) => {
      ...
      if (url.includes('list_folder/continue')) {
        return new Response(JSON.stringify({
          entries: [
            { '.tag': 'file', id: 'id1', name: 'report.md', path_lower: '/docs/report.md', path_display: '/docs/report.md', server_modified: '2026-01-15T10:00:00Z', size: 1024, content_hash: 'abc123' },
          ],
          cursor: 'cursor-2',
          has_more: false,
        }), { status: 200 });
      ...
      if (url.includes('list_folder')) {
        return new Response(JSON.stringify({
          entries: [
            ...
            { '.tag': 'file', id: 'id1', name: 'report.md', path_lower: '/docs/report.md', path_display: '/docs/report.md', server_modified: '2026-01-15T10:00:00Z', size: 1024, content_hash: 'abc123' },
            { '.tag': 'file', id: 'id2', name: 'data.json', path_lower: '/data.json', path_display: '/data.json', server_modified: '2026-01-16T12:00:00Z', size: 512 },
          ],
          cursor: 'cursor-1',
          has_more: true,
        }), { status: 200 });
  ...
  test('fetchDelta returns tasks from workspaces', async () => {
    ...
    (globalThis as any).fetch = async (input: string | URL) => {
      ...
      if (url.includes('/workspaces')) {
        return new Response(JSON.stringify({ data: [{ gid: 'ws1', name: 'Engineering' }] }), { status: 200 });
      }
      if (url.includes('/tasks')) {
        return new Response(JSON.stringify({
          data: [
            {
              gid: 'task1', name: 'Fix bug', notes: 'Critical fix needed',
              created_at: '2026-01-01T00:00:00Z', modified_at: '2026-01-15T10:00:00Z',
              completed: false, due_on: '2026-01-20',
...
describe('JiraConnector', () => {
  test('fetchDelta returns issues from JQL search', async () => {
          ...
          issues: [
            {
              ...
              fields: {
                ...
                comment: {
                  comments: [
                    { body: 'Reproduced on staging', author: { displayName: 'Charlie' }, created: '2026-01-10T09:00:00Z' },
                  ],
                },
              },
            },
    ...
    try {
      ...
      const c = new JiraConnector({ api_key: 'alice@example.com:jira-token', base_url: 'https://example.atlassian.net' });
