...
describe("#1309 — overlapping-ingest-roots dedup", () => {
  ...
  test("different frontmatter.id (and therefore different content_hash) imports both, no dedup signal", async () => {
    ...
    const a = makeFile(
      "templates/daily-1.md",
      granolaFrontmatter("granola-uuid-A", "Daily Template")
    );
    const b = makeFile(
      "templates/daily-2.md",
      granolaFrontmatter("granola-uuid-B", "Daily Template")
    );
  ...
  test("--force-rechunk bypasses dedup pre-check", async () => {
    ...
    const second = await importFromFile(engine, b.path, "note.md", {
      noEmbed: true,
      forceRechunk: true,
    });
...
describe("findDuplicatePage engine method", () => {
  ...
  test("matches on frontmatter.id even when content_hash differs", async () => {
    ...
    const r = await engine.findDuplicatePage!("default", {
      hash: "zzz",
      frontmatterId: "external-uuid",
    });
    expect(r?.slug).toBe("alpha/note");
  });
  ...
  test("cross-source isolation: hash match in source B is invisible from source A", async () => {
    await engine.executeRaw(
      `INSERT INTO sources (id, name, local_path) VALUES ('alpha', 'alpha', '/tmp/alpha') ON CONFLICT (id) DO NOTHING`
    );
    await engine.executeRaw(
      `INSERT INTO sources (id, name, local_path) VALUES ('beta', 'beta', '/tmp/beta') ON CONFLICT (id) DO NOTHING`
    );
