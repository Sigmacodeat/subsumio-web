  ...
  test("drops a clause whose excerpt is fabricated, scores overall by worst surviving clause", async () => {
    const llm = stubLLM({
      ...
      clause_risks: [
        {
          clause_type: "Haftung",
          text_excerpt: "Die Haftung des Auftragnehmers ist vollständig ausgeschlossen",
          score: 85,
          issue: "Totalausschluss",
          recommendation: "begrenzen",
          legal_basis: "§ 309 BGB",
        },
        {
          clause_type: "Schiedsgericht",
          text_excerpt: "Schiedsgericht in Genf entscheidet",
          score: 90,
          issue: "erfunden",
          recommendation: "-",
          legal_basis: "",
...
describe("contract-redline grounding", () => {
  test("keeps add-changes and grounded modifies, drops fabricated modify targets", async () => {
    const llm = stubLLM({
      ...
      redlines: [
        {
          original_clause: "Die Haftung des Auftragnehmers ist vollständig ausgeschlossen",
          suggested_text: "Die Haftung ist auf Vorsatz und grobe Fahrlässigkeit begrenzt.",
          change_type: "modify",
          reason: "AGB-fest",
          risk_level: "high",
          legal_basis: "§ 309 BGB",
        ...
        {
          original_clause: "Gerichtsstand ist München",
          suggested_text: "Gerichtsstand ist Wien",
          change_type: "modify",
          reason: "erfunden",
          risk_level: "low",
        },
        {
          original_clause: "",
          suggested_text: "Neue Datenschutzklausel ergänzen.",
          change_type: "add",
          reason: "DSGVO",
          risk_level: "medium",
        },
...
describe("generative modules — shape + attorney flag", () => {
  test("summarize returns word_count + structured fields", async () => {
    const llm = stubLLM({
      ...
      dates: [{ label: "Frist", date: "2026-01-01" }],
      obligations: ["leisten"],
      risks: ["Haftung"],
    });
  ...
  test("memo assembles markdown with all four sections", async () => {
    const llm = stubLLM({
      sections: { sachverhalt: "SV", rechtsfragen: "RF", wuerdigung: "WD", ergebnis: "ERG" },
      statutes: ["§ 1295 ABGB"],
    });
    const res = await generateMemo(engine, {
      question: "Haftet A?",
      facts: "A hat B geschädigt.",
      jurisdiction: "at",
      llm,
    });
  ...
  test("contract-draft prepends the AI-draft banner", async () => {
    const llm = stubLLM({
      title: "NDA",
      contract_markdown: "# NDA\n§ 1 ...",
      clauses: ["Geheimhaltung"],
    });
    const res = await draftContract(engine, {
      type: "NDA",
      jurisdiction: "de",
      parties: { a: "Acme", b: "Widget" },
      llm,
    });
      ...
      findings: [
        {
          ...
          details: "Ausschluss",
          severity: "high",
          page_refs: ["akten/vertrag-acme", "akten/erfunden"],
        },
      ],
      red_flags: ["Haftungsausschluss"],
...
describe("graceful degradation", () => {
  test("no LLM configured → empty result with NO_LLM_AVAILABLE, never throws", async () => {
    const noLLM: LegalLLM = (() => {
      throw new Error("should not be called");
    }) as unknown as LegalLLM;
