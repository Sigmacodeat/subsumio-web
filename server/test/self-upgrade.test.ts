...
describe("decideSelfUpgrade — pure branches", () => {
  ...
  test("patch/micro bump only → not_behind (ignored)", () => {
    expect(
      decideSelfUpgrade(baseInputs({ currentVersion: "0.42.0", latestVersion: "0.42.1" })).action
    ).toBe("not_behind");
    expect(
      decideSelfUpgrade(baseInputs({ currentVersion: "0.42.3.0", latestVersion: "0.42.3.1" }))
        .action
    ).toBe("not_behind");
  });
  ...
  test("minor bump → behind", () => {
    expect(
      decideSelfUpgrade(baseInputs({ currentVersion: "0.42.0", latestVersion: "0.43.0" })).action
    ).toBe("notify");
  });

  test("major bump → behind", () => {
    expect(
      decideSelfUpgrade(baseInputs({ currentVersion: "0.42.0", latestVersion: "1.0.0" })).action
    ).toBe("notify");
  });
...
describe("reconcileBreadcrumb", () => {
  ...
  test("failed dedups into existing failed_versions", () => {
    const r = reconcileBreadcrumb(
      { attempting_version: "0.43.0", failed_versions: ["0.43.0", "0.41.0"] },
      "0.42.0"
    );
