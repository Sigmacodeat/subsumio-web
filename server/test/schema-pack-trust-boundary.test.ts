...
describe("validateSchemaPackTrustGate (T8 D13)", () => {
  ...
  test("MCP caller (remote=true) rejects per-call schema_pack", () => {
    expect(() => validateSchemaPackTrustGate(ctx(true), "malicious-pack")).toThrow(
      SchemaPackTrustGateError
    );
    expect(() => validateSchemaPackTrustGate(ctx(true), "malicious-pack")).toThrow(
      /rejected for remote\/MCP callers/
    );
  });

  test("fail-closed: undefined remote treated as remote", () => {
    expect(() => validateSchemaPackTrustGate(ctx(undefined), "p")).toThrow(
      SchemaPackTrustGateError
    );
  });
  ...
  test("non-string per-call rejects with type error", () => {
    ...
    expect(() => validateSchemaPackTrustGate(ctx(false), { name: "x" })).toThrow(
      /must be a string/
    );
