...
export async function runEvalSchemaAuthoring(argv: string[]): Promise<EvalVerdict> {
  ...
  if (!args.fixture) {
    return {
      ...
      delta: 0,
      reasoning:
        "No fixture brain provided. Pass --fixture <path> pointing at a fixture brain directory (e.g. test/fixtures/schema-authoring/notion-refugee).",
      suggestion_count: 0,
  ...
  return {
    ...
    delta: 0,
    reasoning:
      "Hermetic engine wiring follows the longmemeval pattern; in v0.39.0.0 ship, in-process callers use aggregateVerdict() directly. Full CLI harness lands in v0.39.1.",
    suggestion_count: 0,
    low_confidence_count: 0,
  };
}
