...
describe("checkGradeConfidenceDrift", () => {
  ...
  test("engine throw → warn with diagnostic", async () => {
    const out = await checkGradeConfidenceDrift(
      buildMockEngine({ throwOn: /FROM take_grade_cache/ })
    );
    expect(out.status).toBe("warn");
...
describe("checkVoiceGateHealth", () => {
  ...
  test("low fail rate → ok", async () => {
    const out = await checkVoiceGateHealth(buildMockEngine({ voiceTotal: 10, voiceFailures: 1 }));
    expect(out.status).toBe("ok");
    expect(out.message).toContain("1/10 failed");
  });

  test("30%+ fail rate → warn with rubric-review hint", async () => {
    const out = await checkVoiceGateHealth(buildMockEngine({ voiceTotal: 10, voiceFailures: 4 }));
    expect(out.status).toBe("warn");
  ...
  test("engine throw → warn with diagnostic", async () => {
    const out = await checkVoiceGateHealth(buildMockEngine({ throwOn: /WHERE generated_at/ }));
    expect(out.status).toBe("warn");
