...
describe("data-research", () => {
  ...
  describe("verifyExtraction", () => {
    test("matching fields verify OK", () => {
      const result = verifyExtraction({ mrr: "188K", arr: "2.3M" }, { mrr: "188K", arr: "2.3M" });
      expect(result.verified).toBe(true);
      expect(result.mismatches.length).toBe(0);
    });

    test("mismatched fields are flagged", () => {
      const result = verifyExtraction({ mrr: "188K", arr: "2.3M" }, { mrr: "200K", arr: "2.3M" });
  ...
  describe("isDuplicate", () => {
    ...
    test("new entry is not duplicate", () => {
      const result = isDuplicate(
        existing,
        { date: "2026-04-02", recipient: "Charlie", amount: "$300.00" },
        ["date", "recipient", "amount"]
      );
...
| 2026-04-02 | $200 | Pending |`;
  ...
  describe("computeTotals", () => {
    test("sums numeric columns", () => {
      ...
      expect(totals.amount).toBeCloseTo(300.5, 2);
