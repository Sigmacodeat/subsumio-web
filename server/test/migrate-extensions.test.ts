...
describe("isMigrationIdempotent — D6 default", () => {
  ...
  test("explicit true", () => {
    expect(isMigrationIdempotent({ version: 1, name: "x", sql: "", idempotent: true })).toBe(true);
  });

  test("explicit false opts out (destructive)", () => {
    expect(isMigrationIdempotent({ version: 1, name: "x", sql: "", idempotent: false })).toBe(
      false
    );
  });
...
describe("MigrationRetryExhausted (F2 named-PID UX)", () => {
      ...
      [
        {
          ...
          query_start: "2026-05-08 14:02:00",
          query: "SELECT 1",
        },
      ],
  ...
  test("without a blocker, suggests checking pg_locks + audit log", () => {
    const err = new MigrationRetryExhausted(1, "m", 3, [], new Error("connection refused"));
