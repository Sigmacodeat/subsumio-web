...
function makeMockReporter(): {
  ...
  events: Array<{
    kind: "start" | "tick" | "heartbeat" | "finish" | "child";
    phase?: string;
    note?: string;
    n?: number;
  }>;
  ...
  const reporter: ProgressReporter = {
    start: (phase, _total) => {
      events.push({ kind: "start", phase });
    },
    tick: (n, note) => {
      events.push({ kind: "tick", n, note });
    },
    heartbeat: (note) => {
...
describe("extract_atoms progress wiring (T4)", () => {
  test("phase does NOT call start or finish — cycle.ts owns those", async () => {
    ...
    const validAtomJson = JSON.stringify([{ title: "A", atom_type: "insight", body: "body a" }]);
