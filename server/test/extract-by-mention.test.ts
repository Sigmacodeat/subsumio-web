...
function captureCli(): void {
  ...
  console.log = (msg?: unknown) => {
    stdoutBuffer.push(typeof msg === "string" ? msg : String(msg));
  };
  console.error = (msg?: unknown) => {
    stderrBuffer.push(typeof msg === "string" ? msg : String(msg));
  };
...
describe("gbrain extract links --by-mention — integration", () => {
  ...
  test("2. idempotency — second run produces 0 new links", async () => {
    ...
    await runCli(["links", "--by-mention", "--source", "db"]);
    const firstCount = (
      await engine.executeRaw<{ c: string }>(
  ...
  test("14. empty brain (no entity pages) → no-op with informative message", async () => {
    ...
    await seedContentPage(
      "writing/lonely",
      "Acme Corp Alice Example all mentioned but no entities exist."
    );
    await runCli(["links", "--by-mention", "--source", "db"]);
    const rows = await engine.executeRaw<{ c: string }>(
      `SELECT COUNT(*)::text AS c FROM links WHERE link_source = 'mentions'`,
  ...
  test("5. --source-id scopes page WALK", async () => {
    ...
    await engine.executeRaw(
      `INSERT INTO sources (id, name) VALUES ('team-b', 'Team B') ON CONFLICT (id) DO NOTHING`,
      []
    );
    ...
    await engine.putPage(
      "writing/team-b-post",
      {
        type: "note",
        title: "Team B Post",
        compiled_truth: "Acme Corp mentioned here.",
        timeline: "",
        frontmatter: {},
      },
      { sourceId: "team-b" }
    ...
    await engine.putPage("writing/default-post", {
      type: "note",
      title: "Default Post",
      compiled_truth: "Acme Corp here too.",
      timeline: "",
      frontmatter: {},
    });
  ...
  test("9. cross-source mention suppressed (source isolation)", async () => {
    ...
    await engine.putPage(
      "writing/team-b-post",
      {
        type: "note",
        title: "Team B Post",
        compiled_truth: "Acme Corp mentioned.",
        timeline: "",
        frontmatter: {},
      },
      { sourceId: "team-b" }
