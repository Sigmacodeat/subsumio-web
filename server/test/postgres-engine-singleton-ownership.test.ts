...
const DB_SRC = readFileSync(join(import.meta.dir, "..", "src", "core", "db.ts"), "utf-8");
...
describe("postgres-engine / module-singleton ownership (#1471)", () => {
  ...
  test("db.connect() returns the ownership token (Promise<boolean>), not void", () => {
    const sig = /export async function connect\(config: EngineConfig\): Promise<boolean>/.test(
      DB_SRC
    );
  ...
  test("disconnect() calls db.disconnect() ONLY when this engine owns the singleton", () => {
    ...
    const guarded =
      /if\s*\(\s*this\._ownsModuleSingleton\s*\)\s*\{[\s\S]*?db\.disconnect\s*\(\s*\)/.test(
        disconnect
      );
