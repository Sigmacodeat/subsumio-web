...
} from "../src/core/backfill-base.ts";
...
class FakeEngine {
  ...
  async withReservedConnection<T>(
    fn: (c: { executeRaw: typeof FakeEngine.prototype.executeRaw }) => Promise<T>
  ): Promise<T> {
    this.reservedCalls++;
...
describe("ensureBackfillIndex — P2/X4", () => {
  ...
  test("returns existed: true on PGLite (no CONCURRENTLY)", async () => {
    const engine = { kind: "pglite" as const } as unknown as Parameters<
      typeof ensureBackfillIndex<FakeRow>
    >[0];
    const spec: BackfillSpec<FakeRow> = {
      ...makeSpec(),
      requiredIndex: { name: "test_idx", sql: "CREATE INDEX test_idx ON pages(id)" },
