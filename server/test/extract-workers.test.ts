...
const EXTRACT_SRC = readFileSync(resolve(REPO_ROOT, "src/commands/extract.ts"), "utf-8");
...
describe("extract.ts → workers wiring (T7)", () => {
  ...
  test("imports parseWorkers + resolveWorkersWithClamp", () => {
    expect(EXTRACT_SRC).toMatch(/parseWorkers,\s*resolveWorkersWithClamp/);
  });
  ...
  test("legacy `for (let i = 0; i < files.length; i++)` per-file loops are gone", () => {
    ...
    const serialLoops =
      EXTRACT_SRC.match(/for\s*\(\s*let\s+i\s*=\s*0;\s*i\s*<\s*files\.length/g) ?? [];
  ...
  test("CLI threads workers into runExtractCore call", () => {
    ...
    expect(EXTRACT_SRC).toMatch(/runExtractCore\(engine,\s*\{[\s\S]*?workers,[\s\S]*?\}\)/);
  });
});
