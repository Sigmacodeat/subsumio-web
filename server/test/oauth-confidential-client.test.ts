...
describe("verifyConfidentialClientSecret (#1166)", () => {
  ...
  test('wrong secret → throws "Invalid client" (RFC 6749 opaque error)', async () => {
    const reg = await provider.registerClientManual("test-conf", ["authorization_code"], "read", [
      "https://example.test/cb",
    ]);
    await expect(
  ...
  test("public client (token_endpoint_auth_method=none) refuses confidential path", async () => {
    ...
    await engine.executeRaw(
      ...
      [["https://example.test/cb"], ["authorization_code"]]
    );
  ...
  test("case-insensitive secret? NO — must be exact match", async () => {
    const reg = await provider.registerClientManual("test-case", ["authorization_code"], "read", [
      "https://example.test/cb",
    ]);
  ...
  test('soft-deleted client → throws "Client has been revoked"', async () => {
    const reg = await provider.registerClientManual("to-revoke", ["authorization_code"], "read", [
      "https://example.test/cb",
    ]);
    await engine.executeRaw(`UPDATE oauth_clients SET deleted_at = NOW() WHERE client_id = $1`, [
      reg.clientId,
    ]);
    await expect(
...
describe("confidential-client full flow #1166", () => {
  test("verify-then-exchange refresh token end-to-end", async () => {
    ...
    await engine.executeRaw(`UPDATE oauth_clients SET grant_types = $1 WHERE client_id = $2`, [
      ["client_credentials", "refresh_token"],
      reg.clientId,
    ]);
    const initial = await provider.exchangeClientCredentials(
      reg.clientId,
      reg.clientSecret!,
      "read"
    );
    ...
    await expect(provider.exchangeRefreshToken(client, refreshToken)).rejects.toThrow(/not found/);
  });
});
