  ...
  test("errors propagate (Q2 sub-fix — no silent swallowing of real DB errors)", async () => {
    ...
    const proxyEngine: BrainEngine = {
      ...
      executeRaw: async (sql: string, params?: unknown[]) => {
        ...
        return (engine.executeRaw as (sql: string, params?: unknown[]) => Promise<unknown[]>)(
          sql,
          params
        );
      },
    ...
    await expect(buildSyncStatusReport(proxyEngine, sources)).rejects.toThrow(/count query failed/);
  });
});
