...
describe("isRetryableConnError extension (v0.41.2.1, re-exported from retry.ts)", () => {
  ...
  test('plain Error with "No database connection" message is retryable (literal match)', () => {
    expect(
      isRetryableConnError(new Error("No database connection: connect() has not been called."))
    ).toBe(true);
  });
...
describe("resolveBulkRetryOpts env-override (D3 cherry-pick, codex M-10/M-12)", () => {
  ...
  test("GBRAIN_BULK_MAX_RETRIES=negative: throws with paste-ready hint", () => {
    expect(() => resolveBulkRetryOpts({ GBRAIN_BULK_MAX_RETRIES: "-1" })).toThrow(
      /GBRAIN_BULK_MAX_RETRIES.*>= 0.*Fix: export/
    );
  });

  test("GBRAIN_BULK_MAX_RETRIES=non-int: throws", () => {
    expect(() => resolveBulkRetryOpts({ GBRAIN_BULK_MAX_RETRIES: "3.5" })).toThrow(
      /GBRAIN_BULK_MAX_RETRIES/
    );
    expect(() => resolveBulkRetryOpts({ GBRAIN_BULK_MAX_RETRIES: "foo" })).toThrow(
      /GBRAIN_BULK_MAX_RETRIES/
    );
  });

  test("GBRAIN_BULK_RETRY_BASE_MS=0 or negative: throws (delays must be > 0)", () => {
    expect(() => resolveBulkRetryOpts({ GBRAIN_BULK_RETRY_BASE_MS: "0" })).toThrow(/> 0/);
    expect(() => resolveBulkRetryOpts({ GBRAIN_BULK_RETRY_BASE_MS: "-100" })).toThrow(/> 0/);
  });
