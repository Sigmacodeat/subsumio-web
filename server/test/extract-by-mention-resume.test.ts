...
describe("by-mention checkpoint/resume (T5)", () => {
  ...
  test("gazetteer change invalidates checkpoint — new entity → re-scan", async () => {
    ...
    const oldHash = createHash("sha256")
      .update(["acme corp", "alice example"].sort().join("|"))
      .digest("hex")
      .slice(0, 8);
    const newHash = await expectedGazetteerHash();
    expect(newHash).not.toBe(oldHash);

    const oldFp = mentionsFingerprint({
      source: undefined,
      type: undefined,
      since: undefined,
      gazetteerHash: oldHash,
    });
    const newFp = mentionsFingerprint({
      source: undefined,
      type: undefined,
      since: undefined,
      gazetteerHash: newHash,
    });
