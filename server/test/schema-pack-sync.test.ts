...
async function seedPage(
  slug: string,
  sourcePath: string,
  opts: { type?: string; sourceId?: string; deleted?: boolean } = {}
): Promise<void> {
...
async function getType(slug: string): Promise<string | null> {
  const rows = await engine.executeRaw<{ type: string }>(`SELECT type FROM pages WHERE slug = $1`, [
    slug,
  ]);
    ...
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: "tiny" }, async () => {
      ...
      for (let i = 0; i < 15; i++) {
        await seedPage(
          ...
          `people/p${String(i).padStart(2, "0")}.md`
        );
      }
      const result = await runSyncCore(ctxOf(), { apply: false });
      const personEntry = result.per_prefix.find((p) => p.type === "person")!;
    ...
    await withEnv({ GBRAIN_HOME: tmpDir, GBRAIN_SCHEMA_PACK: "tiny" }, async () => {
      ...
      for (let i = 0; i < 1500; i++) {
        await seedPage(
          ...
          `people/p${String(i).padStart(4, "0")}.md`
        );
      }
