...
describe("computeDomainHint", () => {
  test("companies slug → hiring/market-timing hint", () => {
    expect(computeDomainHint(buildFinding("wiki/companies/a", "wiki/companies/b"))).toMatch(
      /hiring|market-timing/
    );
  });

  test("people slug → founder-behavior hint", () => {
    expect(computeDomainHint(buildFinding("wiki/people/a", "wiki/people/b"))).toMatch(
      /founder-behavior|hiring/
    );
  });

  test("macro slug → macro hint", () => {
    expect(computeDomainHint(buildFinding("wiki/macro/forecast", "wiki/macro/timing"))).toBe(
      "macro"
    );
  });

  test("geography slug → geography hint", () => {
    expect(computeDomainHint(buildFinding("wiki/geography/ny", "wiki/geography/sf"))).toBe(
      "geography"
    );
  });
