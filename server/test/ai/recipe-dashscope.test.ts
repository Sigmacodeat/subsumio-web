...
describe("recipe: dashscope", () => {
  test("registered with expected shape", () => {
    ...
    expect(r!.base_url_default).toBe("https://dashscope-intl.aliyuncs.com/compatible-mode/v1");
  ...
  test('default auth: DASHSCOPE_API_KEY set → "Bearer <key>"', () => {
    const r = getRecipe("dashscope")!;
    const auth = defaultResolveAuth(r, { DASHSCOPE_API_KEY: "sk-dashscope-fake" }, "embedding");
  ...
  test("dimsProviderOptions threads dimensions for text-embedding-v3 (Matryoshka)", async () => {
    ...
    expect(dimsProviderOptions("openai-compatible", "text-embedding-v3", 512)).toEqual({
      openaiCompatible: { dimensions: 512 },
    });
    expect(dimsProviderOptions("openai-compatible", "text-embedding-v3", 1024)).toEqual({
      openaiCompatible: { dimensions: 1024 },
    });
    // text-embedding-v2 is fixed-dim; no passthrough.
    expect(dimsProviderOptions("openai-compatible", "text-embedding-v2", 1024)).toBeUndefined();
  });
});
