...
export async function runSkillOptCommand(
  ...
  try {
    ...
    process.stderr.write(
      `gbrain skillopt: ${err instanceof Error ? err.message : String(err)}\n\n`
    );
  ...
  if (!engine) {
    process.stderr.write(
      "gbrain skillopt: requires a configured brain (engine connection failed)\n"
    );
    process.exit(2);
  }
  ...
  if (!skillsDir) {
    process.stderr.write(
      `gbrain skillopt: cannot find skills directory. Pass --skills-dir <path> or run from a workspace with a skills/ directory.\n`
    );
    process.exit(2);
  }
  ...
  if (parsed.all) {
    try {
      ...
      if (parsed.json) {
        ...
        process.stderr.write(
          `[skillopt --all] Scanned ${result.skills_scanned} skills, ran ${result.skills_run}\n`
        );
        process.stderr.write(
          `[skillopt --all] Accepted: ${result.accepted}, no_improvement: ${result.no_improvement}, errored: ${result.errored}\n`
        );
        process.stderr.write(
          `[skillopt --all] Total cost: $${result.cumulative_cost_usd.toFixed(2)} (cap $${(parsed.brainWideMaxCostUsd ?? 10).toFixed(2)})\n`
        );
      }
      // Exit code: 0 if at least one accepted, 1 if scanned but none accepted, 2 if errored.
      const exitCode =
        result.errored > 0 && result.accepted === 0 ? 2 : result.accepted === 0 ? 1 : 0;
  ...
  if (parsed.targetModelsFleet) {
    try {
      const benchmarkPath =
        parsed.benchmarkPath ?? path.join(skillsDir, parsed.skillName, "skillopt-benchmark.jsonl");
      ...
      if (parsed.json) {
        ...
        for (const p of result.per_model) {
          process.stderr.write(
            `  ${p.target_model}: outcome=${p.outcome} score=${p.best_sel_score.toFixed(3)} cost=$${p.final_cost_usd.toFixed(2)}\n`
          );
        }
        if (result.best_model) {
          process.stderr.write(
            `[skillopt fleet] Best model: ${result.best_model} (score ${result.best_score?.toFixed(3) ?? "0"})\n`
          );
        }
      }
  ...
  try {
    ...
    if (parsed.json) {
      ...
      process.stderr.write(
        `[skillopt] Best sel-score: ${(result.receipt.best_sel_score ?? 0).toFixed(3)}\n`
      );
      process.stderr.write(
        `[skillopt] Final cost: $${(result.receipt.final_cost_usd ?? 0).toFixed(2)}\n`
      );
      if (result.mutatedSkillFile) {
        process.stderr.write(
          `[skillopt] SKILL.md rewritten with ${result.receipt.total_steps ?? 0} optimization steps.\n`
        );
      } else if (result.proposedPath) {
        process.stderr.write(
          `[skillopt] Proposed improvements written to ${result.proposedPath}. Review + copy manually.\n`
        );
      }
    }
...
export function parseFlags(args: string[]): ParsedFlags {
  ...
  while (i < args.length) {
    ...
    if (a === "--bootstrap-from-routing") {
      bootstrapFromRouting = true;
      i += 1;
      continue;
    }
    if (a === "--bootstrap-from-skill") {
      bootstrapFromSkill = true;
      i += 1;
      continue;
    }
    ...
    if (a === "--bootstrap-reviewed") {
      bootstrapReviewed = true;
      i += 1;
      continue;
    }
    if (a === "--epochs") {
      epochs = mustInt(args[++i], "--epochs");
      i += 1;
      continue;
    }
    if (a === "--batch-size") {
      batchSize = mustInt(args[++i], "--batch-size");
      i += 1;
      continue;
    }
    if (a === "--lr") {
      lr = mustInt(args[++i], "--lr");
      i += 1;
      continue;
    }
    if (a === "--lr-schedule") {
    ...
    if (a === "--max-cost-usd") {
      maxCostUsd = mustFloat(args[++i], "--max-cost-usd");
      i += 1;
      continue;
    }
    if (a === "--max-runtime-min") {
      maxRuntimeMin = mustInt(args[++i], "--max-runtime-min");
      i += 1;
      continue;
    }
    if (a === "--force") {
    ...
    if (a === "--brain-wide-max-cost-usd") {
      brainWideMaxCostUsd = mustFloat(args[++i], "--brain-wide-max-cost-usd");
      i += 1;
      continue;
    }
    if (a === "--target-models") {
  ...
  if (!all && !skillName)
    throw new Error("skill name is required (positional arg), or use --all for batch mode");
  ...
  if (all && bootstrapFromRouting) {
    throw new Error(
      `--all and --bootstrap-from-routing are mutually exclusive (run bootstrap per skill)`
    );
  }
  ...
  if (bootstrapFromSkill) {
    if (bootstrapFromRouting)
      throw new Error(`--bootstrap-from-skill and --bootstrap-from-routing are mutually exclusive`);
    if (benchmarkPath)
      throw new Error(`--bootstrap-from-skill and --benchmark are mutually exclusive`);
    if (all)
      throw new Error(
        `--bootstrap-from-skill and --all are mutually exclusive (run bootstrap per skill)`
      );
    if (targetModelsFleet)
      throw new Error(`--bootstrap-from-skill and --target-models are mutually exclusive`);
