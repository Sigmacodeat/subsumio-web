...
export const GET = createHandler(
  ...
  async (ctx, _body, _query, req) => {
    const slug = buildSlug((await ((req as unknown as { params: Promise<{ slug: string }> })).params).slug);
    if (!slug) return apiError("invalid_slug", "Ungültiger Slug", 400);

    try {
      const res = await fetch(`${ENGINE_URL}/api/pages/${encodeURIComponent(slug)}`, { headers: ctx.headers });
      ...
      return Response.json({
        ...
        force_specialists: Array.isArray(fm.force_specialists) ? fm.force_specialists as string[] : undefined,
        skip_critic: Boolean(fm.skip_critic ?? false),
        created_at: String(p.created_at ?? ""),
        updated_at: String(p.updated_at ?? ""),
      });
    } catch (err) {
      console.error("[agent-templates/slug] get failed:", err instanceof Error ? err.message : String(err));
      return apiError("engine_unreachable", "Agent-Template nicht abrufbar", 503);
    }
  },
);
...
export const PATCH = createHandler(
  ...
  async (ctx, body, _query, req) => {
    ...
    if (body.force_specialists !== undefined) frontmatter.force_specialists = body.force_specialists;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return Response.json(
          errPayload.error ? errPayload : { error: "update_failed" },
      ...
      console.error("[agent-templates/slug] patch failed:", err instanceof Error ? err.message : String(err));
...
export const DELETE = createHandler(
  ...
  async (ctx, _body, _query, req) => {
    ...
    try {
      ...
      console.error("[agent-templates/slug] delete failed:", err instanceof Error ? err.message : String(err));
