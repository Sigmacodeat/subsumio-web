...
describe("assessProse", () => {
  test("pure prose → low markup ratio", () => {
    const r = assessProse(
      "This is a normal paragraph of writing with several real sentences in it."
    );
...
describe("issue #1939 — non-string title defensive coercion", () => {
  ...
  test("number title does not throw", () => {
    expect(() => assessContentSanity({ ...base, title: 1458 as unknown as string })).not.toThrow();
  });
