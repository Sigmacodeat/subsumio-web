...
describe("classifyFactsAbsorbError — reason routing", () => {
  ...
  test("network errors → gateway_error", () => {
    expect(classifyFactsAbsorbError(new Error("connect ECONNREFUSED 127.0.0.1:443"))).toBe(
      "gateway_error"
    );
    expect(classifyFactsAbsorbError(new Error("ECONNRESET"))).toBe("gateway_error");
    expect(classifyFactsAbsorbError(new Error("getaddrinfo EAI_AGAIN api.anthropic.com"))).toBe(
      "gateway_error"
    );
  });

  test("JSON parse failures → parse_failure", () => {
    expect(classifyFactsAbsorbError(new Error("JSON.parse: unexpected character"))).toBe(
      "parse_failure"
    );
    expect(
      classifyFactsAbsorbError(new Error("SyntaxError: Unexpected token } in JSON at position 47"))
    ).toBe("parse_failure");
    expect(classifyFactsAbsorbError(new Error("not valid JSON"))).toBe("parse_failure");
  });

  test("queue overflow / shutdown → matching reason", () => {
    expect(classifyFactsAbsorbError(new Error("queue capacity hit; cap reached"))).toBe(
      "queue_overflow"
    );
  ...
  test("embed-specific failure → embed_failure", () => {
    expect(classifyFactsAbsorbError(new Error("embedOne failed: dim mismatch"))).toBe(
      "embed_failure"
    );
  });
...
describe("writeFactsAbsorbLog — ingest_log row shape", () => {
  test("writes a row with stable shape", async () => {
    ...
    const ours = log.find(
      (r) => r.source_ref === "meetings/test-1" && r.source_type === "facts:absorb"
    );
...
describe("writeFactsAbsorbLog — WARN-4 disconnected-engine suppression (CQ1 + CV13)", () => {
  ...
  test('GBrainError problem="No database connection" SUPPRESSED after first occurrence', async () => {
    const { _resetFactsAbsorbDisconnectedFlagForTests } =
      await import("../src/core/facts/absorb-log.ts");
