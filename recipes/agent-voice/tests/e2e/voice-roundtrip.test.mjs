...
import {
  classifyFailure,
  verdictFor,
  preflightOpenAIStatus,
} from "../../code/lib/upstream-classifier.mjs";
...
beforeAll(async () => {
  ...
  if (status.status === "degraded") {
    console.warn(
      `[voice-roundtrip] OpenAI status degraded: ${status.detail} — test will likely soft-fail`
    );
  }
...
describe.skipIf(!SHOULD_RUN)("voice-roundtrip E2E", () => {
  it("completes a WebRTC roundtrip + asserts three-tier audio quality", async () => {
    ...
    expect(
      ...
      "no audio frames received (server → WebRTC broken)"
    ).toBeGreaterThan(0);

    // Tier [b]: NON-SILENT (hard).
    expect(result.responseBlob, "no response Blob captured").not.toBeNull();
    expect(result.responseBlobSize, "response Blob too small (<5KB)").toBeGreaterThanOrEqual(
      5 * 1024
    );
    ...
    if (result.responseBlob && result.responseBlob[0] === 0x52 && result.responseBlob[1] === 0x49) {
      ...
      expect(rms, `PCM RMS variance too low (${rms}) — audio is likely silence`).toBeGreaterThan(
        0.001
      );
    }
