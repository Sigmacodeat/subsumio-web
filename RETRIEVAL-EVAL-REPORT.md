...
- **Warum das kritisch ist:** IVFFlat-Index 28 GB > Docker-RAM 11.67 GB → Index passt nicht in Buffer-Cache → bimodale Latenz (~1s warm, ~20s kalt). Bei kaltem Cache reisst der 15s `statement_timeout` in `searchVector` → Vektor-Arm fällt still auf keyword-only zurück. **Latenz ist Qualität.**
- **Produktionsbug gefunden:** `at-059` (Asylwerber-Query) fällt deterministisch auf keyword-only zurück (1/80 = 1.25% der Queries). In Produktion ohne Embedding-Cache trifft dieselbe Rate echte Nutzer — still, ohne Fehler im Report.
- **Fix:** Hetzner CCX23 (8 vCPU, 32 GB RAM) + HNSW-Index. Löst beides: HNSW baubar **und** 28GB Index passt in 32GB RAM.
...
3. **Fusionsschicht (RRF/Dedup/Boost):** War nicht deterministisch — 7/80 Rangwechsel bei `--reranker none`. **Fix:** `byScoreDescSlugAsc` Tie-Breaker in `hybrid.ts` + `dedup.ts`. Jetzt: 2/80 (nur bei Pos 11-13, nicht H@K-relevant).
4. **PostgreSQL Statement Timeout:** `at-059` fällt deterministisch auf keyword-only zurück (20.6s kalt > 15s Timeout). **Root Cause:** 28GB IVFFlat-Index > 11.67GB Docker-RAM. **Fix:** HNSW + 32GB RAM (Phase 3).

### Verbleibende Non-Determinismus nach Fixes

- **2/80 Rangwechsel** bei Positionen 11-13 (nicht H@K-relevant)
- **1/80 NO-VEC** (`at-059`, deterministisch — PG Statement Timeout)
