...
import type { Policy, StageReport, Verifier } from "../../src/core/progressive-batch/types.ts";
...
describe("resolveStages", () => {
  test("env override wins", async () => {
    await withEnv({ GBRAIN_PROGRESSIVE_BATCH_STAGES: "5,50,500" }, async () => {
      expect(resolveStages({ label: "t", stages: [10, 100, 500] })).toEqual([5, 50, 500]);
    });
  });
  test("policy when env unset", async () => {
    await withEnv({ GBRAIN_PROGRESSIVE_BATCH_STAGES: undefined as unknown as string }, async () => {
      expect(resolveStages({ label: "t", stages: [7, 70, 700] })).toEqual([7, 70, 700]);
    });
  });
...
describe("resolveCostCap (D3 fail-closed)", () => {
  test("opt-out + maxCostUsd returns policy cap", () => {
    expect(
      resolveCostCap({ label: "t", requireBudgetSafetyNet: false, maxCostUsd: 1 }, null)
    ).toEqual({ capUsd: 1, source: "policy" });
  });
  test("opt-out + no cap returns Infinity", () => {
    expect(resolveCostCap({ label: "t", requireBudgetSafetyNet: false }, null)).toEqual({
      capUsd: Infinity,
      source: "uncapped",
    });
  });
