...
describe("_meta injection on dispatch", () => {
  ...
  test("skipped on facts ops themselves (recall, extract_facts, forget_fact)", async () => {
    for (const opName of ["recall", "extract_facts", "forget_fact"]) {
      ...
      const r = await dispatchToolCall(
        engine,
        opName,
        opName === "forget_fact" ? { id: 1 } : opName === "extract_facts" ? { turn_text: "x" } : {},
        {
          remote: false,
          sourceId: "default",
          metaHook: getBrainHotMemoryMeta,
  ...
  test("different allow-lists produce different _meta", async () => {
    ...
    const noListResp = await dispatchToolCall(
      engine,
      "get_stats",
      {},
      {
    ...
    __resetHotMemoryCacheForTests();
    const withListResp = await dispatchToolCall(
      engine,
      "get_stats",
      {},
      {
        remote: true,
        sourceId: "default",
        takesHoldersAllowList: ["world", "self"],
        metaHook: getBrainHotMemoryMeta,
      }
    );
