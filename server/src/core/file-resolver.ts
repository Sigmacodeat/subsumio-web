...
export async function resolveFile(
  ...
  if (existsSync(yamlRedirectPath)) {
    if (!storage)
      throw new Error(`File redirected to storage but no storage backend configured: ${filePath}`);
  ...
  if (existsSync(markerPath)) {
    if (!storage)
      throw new Error(
        `Directory mirrored to storage but no storage backend configured: ${filePath}`
      );
    ...
    if (marker.prefix) {
      if (
        /\.\.[\\/]/.test(marker.prefix) ||
        marker.prefix === ".." ||
        marker.prefix.startsWith("/")
      ) {
        throw new Error(
          `Blocked: .supabase marker prefix contains path traversal: ${marker.prefix}`
        );
      }
    }
