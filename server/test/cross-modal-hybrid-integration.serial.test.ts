...
beforeEach(async () => {
  ...
  globalThis.fetch = (async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (init?.body) {
      try {
        fetchBodiesSeen.push(JSON.parse(init.body as string));
      } catch {
        /* ignore */
      }
    }
...
describe("hybridSearch cross-modal routing (Phase 1 integration)", () => {
  test("explicit crossModal: 'image' calls Voyage multimodal endpoint, NOT OpenAI", async () => {
    ...
    const results = await hybridSearch(engine, "hackathon stuff", {
      crossModal: "image",
      limit: 5,
    });
    ...
    expect(
      fetchUrlsSeen.some((u) => u.includes("api.openai.com") && u.includes("embeddings"))
    ).toBe(false);
  });
