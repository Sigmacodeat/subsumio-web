...
function migrationsDir(): string {
  return gbrainPath("migrations");
}
function pendingHostWorkPath(): string {
  return join(migrationsDir(), "pending-host-work.jsonl");
}
...
async function phaseASchema(
  ...
  try {
    ...
    return {
      name: "schema",
      status: "complete",
      detail: "schema v38 applied; takes + synthesis_evidence present",
    };
  } catch (e) {
...
function phaseCRechunkTodo(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    const entry = {
      ...
      reason:
        "Pages with pre-v0.28 chunks still contain fenced takes content. Re-chunk so the new chunker strip rule is applied (Codex P0 #3 fix).",
      command:
        "gbrain extract takes --rebuild  # forces re-chunk via reimport pipeline; see migration doc for the precise sweep command in your env",
      _key: key,
    };
...
async function orchestrator(opts: OrchestratorOpts): Promise<OrchestratorResult> {
  ...
  const overallStatus: "complete" | "partial" | "failed" = phases.some((p) => p.status === "failed")
    ? "partial"
    : "complete";
