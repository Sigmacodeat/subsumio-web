...
export function resolveWorkerConcurrency(
  ...
  if (!Number.isFinite(parsed) || parsed < 1) {
    const source =
      parseFlag(args, "--concurrency") !== undefined
        ? "--concurrency flag"
...
function formatJob(job: MinionJob): string {
  const dur =
    job.finished_at && job.started_at
      ...
      : "—";
  const stalled =
    job.status === "active" && job.lock_until && job.lock_until < new Date() ? " (stalled?)" : "";
...
function formatJobDetail(job: MinionJob): string {
  ...
  if (job.lock_token)
    lines.push(`  Lock: ${job.lock_token} (until ${job.lock_until?.toISOString()})`);
  if (job.delay_until) lines.push(`  Delayed until: ${job.delay_until.toISOString()}`);
  if (job.parent_job_id)
    lines.push(`  Parent: job #${job.parent_job_id} (on_child_fail: ${job.on_child_fail})`);
...
`);
  ...
  switch (sub) {
    case "submit": {
      ...
      try {
        maxWaiting = parseMaxWaitingFlag(args);
      } catch (e) {
        console.error(`Error: ${e instanceof Error ? e.message : String(e)}`);
        process.exit(1);
      }
      ...
      const backoffType =
        backoffTypeRaw === "fixed" || backoffTypeRaw === "exponential" ? backoffTypeRaw : undefined;
      const backoffDelayRaw = parseFlag(args, "--backoff-delay");
      const backoffDelay =
        backoffDelayRaw !== undefined ? parseInt(backoffDelayRaw, 10) : undefined;
      const backoffJitterRaw = parseFlag(args, "--backoff-jitter");
      const backoffJitter =
        backoffJitterRaw !== undefined ? parseFloat(backoffJitterRaw) : undefined;
      ...
      if (name.trim() === "shell") {
        try {
          const { validateShellJobParams } =
            await import("../core/minions/handlers/shell-validate.ts");
    ...
    case "list": {
      ...
      console.log(
        `  ${"ID".padEnd(6)} ${"Name".padEnd(14)} ${"Status".padEnd(20)} ${"Queue".padEnd(10)} ${"Time".padEnd(8)} Created`
      );
    ...
    case "get": {
      ...
      if (isNaN(id)) {
        console.error("Error: job ID required. Usage: gbrain jobs get <id>");
        process.exit(1);
      }
    ...
    case "prune": {
      ...
      if (isNaN(days) || days <= 0) {
        console.error(
          "Error: --older-than must be a positive number (days). Example: --older-than 30d"
        );
        process.exit(1);
      }
    ...
    case "stats": {
      ...
      if (stats.by_type.length > 0) {
        console.log(
          `  ${"Type".padEnd(14)} ${"Total".padEnd(7)} ${"Done".padEnd(7)} ${"Failed".padEnd(8)} ${"Dead".padEnd(6)} Avg Time`
        );
        for (const t of stats.by_type) {
          const avgTime =
            t.avg_duration_ms != null ? `${(t.avg_duration_ms / 1000).toFixed(1)}s` : "—";
          console.log(
            `  ${t.name.padEnd(14)} ${String(t.total).padEnd(7)} ${String(t.completed).padEnd(7)} ${String(t.failed).padEnd(8)} ${String(t.dead).padEnd(6)} ${avgTime}`
          );
        }
      } else {
      ...
      try {
        ...
        if (liveWorkers.length > 0 || supNice !== null) {
          ...
          if (supNice !== null)
            console.log(`    supervisor (pid ${sup.pid}): ${formatNice(supNice)}`);
          for (const w of liveWorkers) {
            const diverged =
              w.nice_requested !== null && w.nice_now !== null && w.nice_requested !== w.nice_now
                ? `  ⚠ requested ${formatNice(w.nice_requested)}, not applied`
                : "";
            console.log(
              `    worker (pid ${w.pid}, queue ${w.queue}): ${w.nice_now !== null ? formatNice(w.nice_now) : "?"}${diverged}`
            );
          }
        }
      } catch {
      ...
      // operator/agent catches a silent halt in `jobs stats`, not 15h later.
      {
        ...
        const wedged =
          w.active_healthy === 0 && w.waiting > 0 && (mins === null || mins > wedgeMins);
        if (wedged) {
          const since =
            mins === null ? "no completions on record" : `${mins}m since last completion`;
          console.log(
        ...
        try {
          ...
          if (errRows.length === 0) {
            ...
            for (const c of clusters.slice(0, 5)) {
              const sample =
                c.sample_ids.length > 0 ? `  (e.g. \`gbrain jobs get ${c.sample_ids[0]}\`)` : "";
              console.log(`    ${String(c.count).padStart(4)} × ${c.cluster.padEnd(22)}${sample}`);
            }
            if (clusters.length > 5) {
              console.log(
                `    + ${clusters.length - 5} more cluster${clusters.length - 5 === 1 ? "" : "s"}`
              );
            }
          }
        } catch (e) {
          ...
          if (process.env.GBRAIN_DEBUG === "1") {
            console.error(
              `[jobs stats] cluster-errors skipped: ${e instanceof Error ? e.message : String(e)}`
            );
          }
        }
      }
    ...
    case "smoke": {
      ...
      if (final?.status !== "completed") {
        console.error(
          `SMOKE FAIL — job #${job.id} status: ${final?.status ?? "timeout"} (${elapsedSec}s elapsed)`
        );
      ...
      if (sigkillRescue) {
        ...
        try {
          await queue.removeJob(rescueJob.id);
        } catch {
          /* non-fatal cleanup */
        }
      }
      ...
      if (wedgeRescue) {
        const wedgedJob = await queue.add(
          "noop",
          {},
          {
            queue: "smoke",
        ...
        try {
          await queue.removeJob(wedgedJob.id);
        } catch {
          /* non-fatal cleanup */
        }
      }
    ...
    case "work": {
      ...
      if (config?.engine === "pglite") {
        console.error(
          "Error: Worker daemon requires Postgres. PGLite uses an exclusive file lock that blocks other processes."
        );
      ...
      if (healthRaw !== undefined) {
        ...
        if (!Number.isFinite(parsed) || parsed < 0) {
          console.error(
            `Error: --health-interval must be a non-negative integer (ms), got "${healthRaw}"`
          );
          process.exit(1);
        }
      ...
      console.log(
        `Minion worker started (queue: ${queueName}, concurrency: ${concurrency}${watchdogNote}${healthNote}${niceNote})`
      );
      ...
      try {
        ...
        try {
          ...
          console.error("[gbrain jobs work] engine disconnect failed during shutdown:", e);
        }
    ...
    case "supervisor": {
      ...
      if (isStatusCmd) {
        ...
        const maxCrashesEvent =
          events.filter((e) => e.event === "max_crashes_exceeded").pop() ?? null;
        ...
        const supervisorNice =
          running && supervisorPid !== null ? getEffectiveNiceness(supervisorPid) : null;
        ...
        if (jsonMode) {
          ...
          console.log(
            `  Crashes (24h):     ${summary.total} (runtime=${summary.by_cause.runtime_error} oom=${summary.by_cause.oom_or_external_kill} unknown=${summary.by_cause.unknown} legacy=${summary.by_cause.legacy})`
          );
          console.log(`  Clean exits (24h): ${summary.clean_exits}`);
          if (supervisorNice !== null)
            console.log(`  Nice (supervisor): ${formatNice(supervisorNice)}`);
          for (const w of workers) {
            const req =
              w.nice_requested !== null && w.nice !== null && w.nice_requested !== w.nice
                ? ` (requested ${formatNice(w.nice_requested)})`
                : "";
            console.log(
              `  Worker pid ${w.pid} [${w.queue}]: nice ${w.nice !== null ? formatNice(w.nice) : "?"}${req}`
            );
          }
      ...
      if (isStopCmd) {
        ...
        if (jsonMode) console.log(JSON.stringify(payload));
          ...
          console.log(
            stoppedCleanly
              ? `Supervisor ${supervisorPid} stopped.`
              : `Supervisor ${supervisorPid} did not exit within 40s.`
          );
      ...
      if (config?.engine === "pglite") {
        console.error(
          "Error: Supervisor requires Postgres. PGLite uses an exclusive file lock that blocks other processes."
        );
        process.exit(1);
      }
      ...
      if (supHealthRaw !== undefined) {
        ...
        if (!Number.isFinite(parsed) || parsed < 0) {
          console.error(
            `Error: --health-interval must be a non-negative integer (ms), got "${supHealthRaw}"`
          );
          process.exit(1);
        }
        if (parsed > 0 && parsed < 1000) {
      ...
      const allowShellJobs =
        hasFlag(args, "--allow-shell-jobs") || !!process.env.GBRAIN_ALLOW_SHELL_JOBS;
...
export async function registerBuiltinHandlers(
  ...
  worker.register("sync", async (job) => {
    ...
    const concurrencyOverride =
      typeof job.data.concurrency === "number" ? job.data.concurrency : undefined;
      ...
      try {
        ...
        if (await isFederatedV2Enabled(engine)) {
          ...
          const submission = await submitEmbedBackfill(engine, sourceId, {
            reason:
              typeof job.data.embed_reason === "string"
  ...
  worker.register("enrich", async (job) => {
    ...
    const types = Array.isArray(job.data.types) ? (job.data.types as string[]) : undefined;
    ...
    const result = await runEnrichCore(engine, {
      ...
      maxCostUsd: typeof job.data.maxCostUsd === "number" ? job.data.maxCostUsd : undefined,
      minContextChars:
        typeof job.data.minContextChars === "number" ? job.data.minContextChars : undefined,
      thinThreshold:
        typeof job.data.thinThreshold === "number" ? job.data.thinThreshold : undefined,
      reenrichAfterMs:
        typeof job.data.reenrichAfterMs === "number" ? job.data.reenrichAfterMs : undefined,
      dryRun: !!job.data.dryRun,
      force: !!job.data.force,
    });
    return result;
  ...
  worker.register("legal-case-scanner", async (job) => {
    const { legalCaseScannerHandler } =
      await import("../core/minions/handlers/legal-case-scanner.ts");
  ...
  worker.register("extract", async (job) => {
    ...
    const mode =
      typeof job.data.mode === "string" && ["links", "timeline", "all"].includes(job.data.mode)
        ? (job.data.mode as "links" | "timeline" | "all")
        : "all";
    const dir =
      typeof job.data.dir === "string"
        ? job.data.dir
  ...
  {
    const { makeContextualReindexHandler } =
      await import("../core/minions/handlers/contextual-reindex-per-chunk.ts");
  ...
  worker.register("autopilot-cycle", async (job) => {
    ...
    if (rawSourceId !== undefined && rawSourceId !== null) {
      if (typeof rawSourceId !== "string") {
        throw new Error(
          `autopilot-cycle: invalid source_id (not a string): ${JSON.stringify(rawSourceId)}`
        );
      }
      ...
      if (!isValidSourceId(rawSourceId)) {
        ...
        throw new Error(
          `autopilot-cycle: invalid source_id (regex): ${JSON.stringify(rawSourceId)}`
        );
      }
  ...
  // GBRAIN_ALLOW_SHELL_JOBS=1.
  {
    ...
    if (!quiet) {
      if (process.env.GBRAIN_ALLOW_SHELL_JOBS === "1") {
        ...
        process.stderr.write(
          "[minion worker] shell handler registered in guarded mode (set GBRAIN_ALLOW_SHELL_JOBS=1 to execute shell jobs)\n"
        );
      }
    }
  }
  ...
  const { subagentAggregatorHandler } =
    await import("../core/minions/handlers/subagent-aggregator.ts");
  ...
  // v0.44 — Legal Agent Pipeline V2: 6-Layer Map-Reduce pipeline for
  // automated case file processing (ON-Scanner → Entity → Forensic →
  // Damage+Deadline → Drafter → Critic). Submits subagent children
  // with allowProtectedSubmit, so it's gated by PROTECTED_JOB_NAMES.
  ...
  worker.register("integrity", async (job) => {
    ...
    if (typeof job.data.confidence === "number")
      args.push("--confidence", String(job.data.confidence));
    if (job.data.dryRun) args.push("--dry-run");
  ...
  worker.register("purge", async (job) => {
    const scope =
      typeof job.data.scope === "string" && ["pages", "sources", "all"].includes(job.data.scope)
        ? (job.data.scope as "pages" | "sources" | "all")
        : "all";
    const olderThanHours =
      typeof job.data.olderThanHours === "number" ? job.data.olderThanHours : 72;
    const dryRun = !!job.data.dryRun;
    let pagesPurged = 0;
  ...
  worker.register("extract-timeline-from-meetings", async (job) => {
    const { extractTimelineFromMeetings } =
      await import("../core/extract-timeline-from-meetings.ts");
    const data = (job.data ?? {}) as { sourceId?: string };
  ...
  worker.register("skillopt", async (job) => {
    ...
    if (!skillsDir || !skillName || !benchmarkPath) {
      throw new Error(
        `skillopt handler: missing required job.data fields (skills_dir, skill_name, benchmark_path)`
      );
    }
