...
export async function runSearchDiagnose(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  const query = args
    ...
    .find(
      (a, i) =>
        !a.startsWith("--") &&
        args.slice(1)[i - 1] !== "--target" &&
        args.slice(1)[i - 1] !== "--source"
    );

  if (!query || !target) {
    console.error(
      'Usage: gbrain search diagnose "<query>" --target <slug> [--json] [--source <id>]'
    );
    process.exit(2);
  }
  ...
  if (!isAvailable("embedding")) {
    vector = {
      ...
      score: null,
      top_slug: null,
      note: "skipped — no embedding provider configured",
    };
  } else {
    try {
      ...
      vector = {
        ...
        score: null,
        top_slug: null,
        note: `vector probe failed: ${e instanceof Error ? e.message : String(e)}`,
      };
    }
  }
  ...
  console.log(
    `  keyword:  rank=${keyword.rank ?? "absent"}  score=${fmt(keyword.score)}  (top: ${keyword.top_slug ?? "—"})`
  );
  console.log(
    `  vector:   rank=${vector.rank ?? "absent"}  score=${fmt(vector.score)}  (top: ${vector.top_slug ?? "—"})${vector.note ? `  [${vector.note}]` : ""}`
  );
  console.log(
    `  alias:    ${aliasMatch ? "query IS a registered alias of the target" : "no alias match"}`
  );
  console.log(
    `  hybrid:   rank=${hybrid.rank ?? "absent"}  score=${fmt(hybrid.score)}  base=${fmt(hybrid.base_score)}  evidence=${hybrid.evidence ?? "—"}  create_safety=${hybrid.create_safety ?? "—"}`
  );
  console.log(
    `            boosts: title=${hybrid.title_match_boost ?? "—"}  alias_hit=${hybrid.alias_hit}`
  );
  console.log(`\n  VERDICT: ${verdict}`);
}
