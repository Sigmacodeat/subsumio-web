...
const PER_TASK_KEYS: Array<{ key: string; tier: ModelTier; description: string }> = [
  {
    key: "models.dream.synthesize",
    tier: "reasoning",
    description: "Dream synthesis (conversation → brain pages)",
  },
  {
    key: "models.dream.synthesize_verdict",
    tier: "utility",
    description: "Dream synthesis verdict (Haiku judge)",
  },
  {
    key: "models.dream.patterns",
    tier: "reasoning",
    description: "Pattern discovery (cross-take themes)",
  },
  ...
  { key: "models.subagent", tier: "subagent", description: "`gbrain agent run` subagent loop" },
  {
    ...
    tier: "reasoning",
    description: "Real-time facts extraction during sync",
  },
  {
    key: "models.eval.longmemeval",
    tier: "reasoning",
    description: "LongMemEval benchmark answer-gen",
  },
  {
    key: "models.eval.contradictions_judge",
    tier: "utility",
    description: "Contradiction probe judge (v0.34 temporal-aware)",
  },
...
function formatText(report: ModelsReport): string {
  ...
  lines.push("");
  lines.push(
    "Tip: probe reachability with `gbrain models doctor` (opt-in; spends a minimal request per configured chat/embed/rerank surface)."
  );
  return lines.join("\n");
}
...
async function probeEmbeddingConfig(): Promise<ProbeResult> {
  ...
  try {
    ...
    if (providerId === "zeroentropyai" && supportsZeroEntropyDimension(modelId)) {
      if (!isValidZeroEntropyDim(dims)) {
        return {
          ...
          fix: `gbrain config set embedding_dimensions <${ZEROENTROPY_VALID_DIMS.join("|")}>.`,
          elapsed_ms: Date.now() - start,
        };
      }
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    const fix =
      err && typeof err === "object" && "fix" in err ? (err as { fix?: string }).fix : undefined;
    return {
      model: modelStr,
      touchpoint: "embedding_config",
      status: "config",
...
async function probeRerankerConfig(engine: BrainEngine): Promise<ProbeResult> {
  ...
  if (!modelStr) {
    ...
    return {
      ...
      status: "ok",
      message:
        "reranker not configured (set `gbrain config set search.reranker.model <provider:model>` and `search.reranker.enabled true`)",
      elapsed_ms: Date.now() - start,
    };
  }
...
async function probeModel(
  ...
  try {
    ...
    const controller = new AbortController();
    const timeoutId = setTimeout(
      () => controller.abort(new Error("probe timed out after 5s")),
      5000
    );
    try {
      await chat({
...
export async function runModels(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  const sub =
    args[1] === "doctor"
      ? "doctor"
      : args[1] === "help" || args.includes("--help") || args.includes("-h")
        ? "help"
        : "read";
...
`
  ...
  for (const [modelStr, touchpoint] of [
    [chatModel, "chat"],
  ...
  if (json) {
    ...
    for (const r of results) {
      ...
      process.stdout.write(
        `  ${icon} ${r.touchpoint.padEnd(17)} ${r.model.padEnd(50)} ${r.status} (${r.elapsed_ms}ms)\n`
      );
      if (r.status !== "ok") {
