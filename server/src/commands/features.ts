...
function shouldPitch(
  rec: FeatureRecommendation,
  offers: FeatureOffersFile,
...
async function scanFeatures(engine: BrainEngine): Promise<FeatureScanResult> {
  ...
  if (stats.page_count >= 3) {
    ...
    const unconfigured = RECIPE_META.filter((r) => !r.secrets.every((s) => process.env[s]));
...
export async function runFeatures(engine: BrainEngine, args: string[]) {
  ...
  if (jsonMode) {
    ...
    console.log(
      JSON.stringify(
        { ...scan, recommendations: pitchable, auto_fix_results: autoFix ? fixResults : undefined },
        null,
        2
      )
    );
...
export async function featuresTeaserForDoctor(engine: BrainEngine): Promise<string | null> {
  try {
    ...
    const parts: string[] = [];
    if (health.missing_embeddings > 0)
      parts.push(`${health.missing_embeddings} missing embeddings`);
