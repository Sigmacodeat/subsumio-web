...
describe('detectPII', () => {
  ...
  test('returns empty array for clean text', () => {
    expect(detectPII('Dies ist ein allgemeiner Rechtstext ohne personenbezogene Daten.')).toHaveLength(0);
  });
});
...
describe('anonymizeFacts', () => {
  test('replaces all mapped terms', () => {
    const map = new Map([['Alice Müller', '[ENT-01]'], ['Berlin', '[ENT-02]']]);
...
describe('Legal Brain schema compatibility', () => {
  ...
  test('legal-entity list query shape works via frontmatter JSONB', async () => {
    ...
    const rows = await engine.executeRaw<{ slug: string; title: string; frontmatter: Record<string, unknown> }>(`
