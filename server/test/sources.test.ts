...
describe("sources add", () => {
  ...
  test("rejects uppercase / invalid chars in id", async () => {
    const { engine } = makeStub();
    await expect(runSources(engine, ["add", "BadId", "--path", "/tmp/x"])).rejects.toThrow(
      /Invalid source id/
    );
  });
  ...
  test("rejects id longer than 32 chars", async () => {
    ...
    await expect(runSources(engine, ["add", long, "--path", "/tmp/x"])).rejects.toThrow(
      /Invalid source id/
    );
  });
  ...
  test("rejects overlapping paths (per eng review finding 4.1)", async () => {
    ...
    await expect(
      runSources(engine, ["add", "plans", "--path", "/tmp/gstack/plans"])
      ...
      "SELECT id, name, local_path, last_commit, last_sync_at, config, created_at": [
        {
          ...
          last_commit: null,
          last_sync_at: null,
          config: '{"ttl_days":90,"federated":true}',
          created_at: new Date(),
        },
      ],
    });
