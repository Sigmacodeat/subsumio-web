...
import { validateRequest, signupSchema } from "@/lib/api-validation";

export async function POST(req: NextRequest) {
  // Throttle automated account creation per IP.
  const signupMax = process.env.NODE_ENV === "production" ? 5 : 100;
  const ipLimit = await hit(`signup:ip:${clientIp(req.headers)}`, signupMax, 60 * 60_000);
  if (!ipLimit.ok) {
  ...
  const parsed = await validateRequest(req, signupSchema);
  if (!parsed.ok) return parsed.error;
  const { email: rawEmail, password, name: rawName, locale, industry: rawIndustry } = parsed.data;
