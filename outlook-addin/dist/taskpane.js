...
async function loadCurrentMail() {
    try {
        ...
        if (item.body) {
            item.body.getAsync("text", (asyncResult) => {
...
async function importMail() {
    ...
    try {
        ...
        if (data.success && data.matchedCase) {
            ...
            renderCaseSuggestions(data.suggestions);
        }
        else {
...
function renderCaseSuggestions(suggestions) {
...
async function importToSpecificCase(slug) {
    if (!currentMail)
        return;
...
async function runQuery() {
    ...
    try {
        ...
        if (contentType.includes("text/event-stream") && res.body) {
            ...
            if (!result)
                resultEl.textContent = "(Leere Antwort)";
        }
        else {
