...
`;
  ...
  const failedSection = opts.chapterAnalyses
    ...
    .map(
      (a) =>
        `> Chapter ${a.index}: analysis failed (${a.error ?? "unknown error"}). Re-run \`gbrain book-mirror\` to retry; idempotent on the same inputs.`
    )
    .join("\n\n");
...
export async function runBookMirrorCmd(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!/^[a-z0-9][a-z0-9-]*$/i.test(flags.slug)) {
    console.error(
      `gbrain book-mirror: invalid --slug "${flags.slug}". Use kebab-case (a-z, 0-9, hyphens).`
    );
    process.exit(2);
  }
  ...
  for (let i = 0; i < childIds.length; i++) {
    ...
    try {
      ...
      if (job.status === "completed" && job.result && typeof job.result === "object") {
        ...
        process.stderr.write(
          `  chapter ${chapterIndex}: FAILED (job ${childId} status=${job.status})\n`
        );
      }
    } catch (e) {
      const msg =
  ...
  if (completed === 0) {
    console.error(
      `gbrain book-mirror: every chapter failed. Not writing the brain page. Re-run after diagnosing.`
    );
    process.exit(1);
  }
