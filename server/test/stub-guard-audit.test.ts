...
function freshTmpDir(): string {
  const dir = join(
    tmpdir(),
    `stub-guard-audit-test-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
  ...
  it("never throws when audit dir is unwritable", async () => {
    ...
    try {
      await withEnv({ GBRAIN_AUDIT_DIR: blockerFile }, async () => {
        expect(() =>
          logStubGuardEvent({ slug: "bob", source_id: "default", fact_count: 1 })
        ).not.toThrow();
      });
    } finally {
    ...
    try {
      await withEnv({ GBRAIN_AUDIT_DIR: tmpAuditDir }, async () => {
        ...
        writeFileSync(
          ...
          [
            JSON.stringify({
              ts: outsideWindow.toISOString(),
              slug: "too-old",
              source_id: "default",
              fact_count: 1,
            }),
            JSON.stringify({
              ts: earlierInWindow.toISOString(),
              slug: "monday-prev-week",
              source_id: "default",
              fact_count: 1,
            }),
            JSON.stringify({
              ts: prevSunday.toISOString(),
              slug: "sunday-late",
              source_id: "default",
              fact_count: 1,
            }),
        ...
        writeFileSync(
          ...
          [
            JSON.stringify({
              ts: earlyMonday.toISOString(),
              slug: "monday-current-week",
              source_id: "default",
              fact_count: 1,
            }),
          ].join("\n") + "\n"
