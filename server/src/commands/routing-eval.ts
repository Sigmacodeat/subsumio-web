...
function resolveSkillsDir(flags: Flags): {
  dir: string | null;
  source: string | null;
  error: RoutingEvalEnvelope["error"];
  message: string | null;
} {
  if (flags.skillsDir) {
...
export async function runRoutingEvalCli(args: string[]): Promise<void> {
  ...
  const cleanResults = report.missed === 0 && report.ambiguous === 0 && report.falsePositives === 0;
  ...
  if (flags.json) {
    ...
    console.log(`${header} — ${report.totalCases} case(s), ${pct}% top-1 accuracy`);
    ...
    for (const lint of lintIssues) {
      console.log(`  [lint:${lint.reason}] "${lint.fixture.intent}" — ${lint.detail}`);
    }
