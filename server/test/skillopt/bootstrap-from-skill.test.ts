...
describe("runBootstrapFromSkill — happy path", () => {
  test("writes 15 rows + sentinel with deterministic contiguous task_ids", async () => {
    ...
    const res = await runBootstrapFromSkill({
      skillsDir,
      skillName: SKILL,
      optimizerModel: "test:m",
      chatFn,
    });
...
describe("runBootstrapFromSkill — round-trip into the consumer (D4)", () => {
  test("generated file loads + splits 1:1:1 with sel >= 5", async () => {
    ...
    const stripped = raw
      .split("\n")
      .filter((l) => l.trim() !== BOOTSTRAP_PENDING_REVIEW)
      .join("\n");
...
describe("runBootstrapFromSkill — min-2-checks drop (D6) + validateChecks", () => {
  ...
  test("bad-op checks are filtered, dropping the task below the 2-check floor", async () => {
    ...
    const rows = fs
      .readFileSync(outputPath(skillsDir, SKILL), "utf8")
      .split("\n")
...
describe("parseFlags — --bootstrap-from-skill CLI surface", () => {
  ...
  test("--bootstrap-tasks caps at 50", () => {
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--bootstrap-tasks", "99"])).toThrow(
      /max is 50/
    );
  });

  test("--bootstrap-tasks without --bootstrap-from-skill is rejected", () => {
    expect(() => parseFlags([SKILL, "--bootstrap-tasks", "20"])).toThrow(
      /requires --bootstrap-from-skill/
    );
  });

  test("mutual exclusion: routing / benchmark / all / target-models / resume", () => {
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--bootstrap-from-routing"])).toThrow(
      /mutually exclusive/
    );
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--benchmark", "x.jsonl"])).toThrow(
      /mutually exclusive/
    );
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--all"])).toThrow(
      /mutually exclusive/
    );
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--target-models", "a,b"])).toThrow(
      /mutually exclusive/
    );
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--resume", "run-1"])).toThrow(
      /mutually exclusive/
    );
  });

  test("--background is rejected by the unknown-flag guard (pre-existing CLI behavior)", () => {
    expect(() => parseFlags([SKILL, "--bootstrap-from-skill", "--background"])).toThrow(
      /unknown flag/
    );
  });
});
