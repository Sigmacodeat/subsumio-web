...
describe("pathToSlug", () => {
  ...
  test("slugifies spaces to hyphens", () => {
    expect(pathToSlug("Apple Notes/2017-05-03 ohmygreen.md")).toBe(
      "apple-notes/2017-05-03-ohmygreen"
    );
  });
...
describe("buildSyncManifest edge cases", () => {
  ...
  test("handles multiple renames", () => {
    const output = ["R100\told/a.md\tnew/a.md", "R095\told/b.md\tnew/b.md"].join("\n");
    const manifest = buildSyncManifest(output);
...
describe("performSync dry-run never writes", () => {
  ...
  beforeEach(async () => {
    ...
    writeFileSync(
      ...
      ["---", "type: person", "title: Alice", "---", "", "Alice is a person."].join("\n")
    );
    writeFileSync(
      join(repoPath, "people/bob.md"),
      ["---", "type: person", "title: Bob", "---", "", "Bob is another person."].join("\n")
    );
  ...
  test("first sync without origin skips git pull noise and uses local working tree", async () => {
    ...
    expect(
      messages.some((m) => m.includes("No origin remote") && m.includes("skipping git pull"))
    ).toBe(true);
  ...
  test("incremental dry-run does NOT write to DB or advance the bookmark", async () => {
    ...
    writeFileSync(
      ...
      ["---", "type: person", "title: Carol", "---", "", "Carol joins the cast."].join("\n")
    );
  ...
  test("detached HEAD skips git pull and ingests local working-tree files", async () => {
    ...
    writeFileSync(
      join(repoPath, "people/detached-local.md"),
      [
        "---",
        "type: person",
    ...
    expect(errors.join("\n")).toContain(
      `Detached HEAD on ${repoPath}; skipping git pull. Syncing from local working tree.`
    );
  ...
  test("detached HEAD with --no-pull also ingests local working-tree files", async () => {
    ...
    execSync("git checkout --detach HEAD", { cwd: repoPath, stdio: "pipe" });
    writeFileSync(
      join(repoPath, "people/detached-nopull.md"),
      [
        "---",
        "type: person",
...
describe("sync regression — #132 nested transaction deadlock", () => {
  test("src/commands/sync.ts does not wrap the add/modify loop in engine.transaction()", async () => {
    ...
    const inlineIdx = source.indexOf(
      "for (const path of [...filtered.added, ...filtered.modified]"
    );
    const hoistedIdx = source.indexOf(
...
describe("git() helper invocation order (CJK wave v0.32.7)", () => {
  ...
  test("core.quotepath=false is always emitted first", () => {
    ...
    expect(argv).toEqual(["-c", "core.quotepath=false", "-C", "/repo", "diff", "--name-status"]);
  });
  ...
  test("empty args produces a valid invocation", () => {
    ...
    expect(argv).toEqual(["-c", "core.quotepath=false", "-C", "/repo"]);
  });
});
...
describe("#1970: unreachable last_commit bookmark recovery", () => {
  ...
  test("orphan-present (not an ancestor): diffs tree-to-tree, imports only the delta, advances bookmark", async () => {
    ...
    execSync('git add -A && git commit --amend -m "amended with carol"', {
      cwd: repo,
      stdio: "pipe",
    });
    ...
    let isAncestor = true;
    try {
      execSync(`git merge-base --is-ancestor ${orphan} HEAD`, { cwd: repo, stdio: "pipe" });
    } catch {
      isAncestor = false;
    }
    ...
    const { result, out } = await captureLog(() =>
      performSync(engine, { repoPath: repo, ...SYNC_OPTS })
    );
  ...
  test("orphan-absent (object gc'd): falls back to a full reconcile", async () => {
    ...
    await engine.executeRaw(`UPDATE sources SET last_commit = $1 WHERE id = 'default'`, [
  ...
  test("divergence: a file present in the orphan tree but dropped from HEAD is deleted", async () => {
    ...
    execSync('git add -A && git commit --amend -m "drop bob, edit alice"', {
      cwd: repo,
      stdio: "pipe",
    });
  ...
  test("F-A: full reconcile purges stale file-backed pages but spares manual + metafile pages", async () => {
    ...
    await engine.putPage(
      "manual/note",
      {
        type: "note",
        title: "Manual Note",
        compiled_truth: "Hand-authored, not from a file.",
      },
      { sourceId: "default" }
    );
    ...
    await engine.putPage(
      "people/log",
      {
        type: "note",
        title: "Log",
        compiled_truth: "metafile page",
        source_path: "people/log.md",
      },
      { sourceId: "default" }
    );
  ...
  test("convergence: after orphan recovery, a later commit syncs incrementally to up_to_date", async () => {
    ...
    // Orphan + recover.
    writeFileSync(join(repo, "people/bob.md"), personMd("Bob", "Bob is a person."));
    execSync('git add -A && git commit --amend -m "amended with bob"', {
      cwd: repo,
      stdio: "pipe",
    });
