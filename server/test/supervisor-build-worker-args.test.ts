...
describe("buildWorkerArgs", () => {
  test("base args without nice or rss", () => {
    expect(buildWorkerArgs({ concurrency: 2, queue: "default", maxRssMb: 0 })).toEqual([
      "jobs",
      "work",
      "--concurrency",
      "2",
  ...
  test("includes --max-rss when > 0", () => {
    expect(buildWorkerArgs({ concurrency: 4, queue: "q", maxRssMb: 2048 })).toEqual([
      "jobs",
      "work",
      "--concurrency",
      "4",
  ...
  test("appends --nice when nice_requested is set", () => {
    expect(
      buildWorkerArgs({ concurrency: 2, queue: "default", maxRssMb: 0, nice_requested: 10 })
  ...
  test("nice 0 is explicit and still propagates (distinct from inherit)", () => {
    expect(
      buildWorkerArgs({ concurrency: 1, queue: "q", maxRssMb: 0, nice_requested: 0 })
    ).toContain("--nice");
  });

  test("omits --nice when nice_requested is undefined (inherit)", () => {
    expect(buildWorkerArgs({ concurrency: 1, queue: "q", maxRssMb: 0 })).not.toContain("--nice");
  });
});
