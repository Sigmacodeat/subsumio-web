...
function readEvalResults(repoRoot: string, override?: string): ParsedRecord[] {
  const path = override
    ? override.endsWith(".jsonl")
      ? override
      : join(override, "eval-results.jsonl")
 ...
 */
function groupBySuiteAndMode(
  records: ParsedRecord[]
): Record<string, Record<SearchMode, ParsedRecord | null>> {
...
function renderMarkdown(
  grouped: Record<string, Record<SearchMode, ParsedRecord | null>>,
  ...
  lines.push("");
  lines.push(
    "_Auto-generated from `<repo>/.gbrain-evals/eval-results.jsonl`. Industry terms preserved verbatim so users searching the literature find what we report._"
  );
  lines.push("");
  ...
  for (const [suite, modes] of Object.entries(grouped)) {
    ...
    const hasMetrics = present.some(
      (m) => modes[m]?.metrics && Object.keys(modes[m]!.metrics!).length > 0
    );
    if (hasMetrics) {
      ...
      lines.push(
        "_No metric data yet — orchestrator stubs only. Metric population lands in v0.32.4._"
      );
      lines.push("");
    }
  }
...
export async function runEvalCompare(args: string[]): Promise<void> {
  ...
  if (opts.runIds !== "all")
    filtered = filtered.filter((r) => (opts.runIds as string[]).includes(r.run_id));
  if (opts.modes !== "all")
    filtered = filtered.filter((r) => (opts.modes as SearchMode[]).includes(r.mode));
      ...
      JSON.stringify(
        {
          ...
          _meta: {
            ...
            methodology:
              "Paired bootstrap (10,000 resamples) + Bonferroni correction across 3 modes × 4 metrics. See docs/eval/SEARCH_MODE_METHODOLOGY.md.",
          },
        },
        null,
        2
  ...
  if (records.length === 0) {
    process.stdout.write(
      `_No eval-results.jsonl found at <repo>/.gbrain-evals/eval-results.jsonl._\n`
    );
    process.stdout.write(
      `_Run: gbrain eval run-all --modes conservative,balanced,tokenmax --suites longmemeval,replay --seed 42_\n`
    );
    return;
  }
