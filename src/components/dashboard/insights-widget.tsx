      ...
      {!loading && !error && activeInsights.length > 0 && (
        <div className="space-y-2">
          {activeInsights.slice(0, 8).map((insight) => {
            ...
            const content = (
              <div
                className={`group relative flex items-start gap-3 rounded-lg border ${styles.border} ${styles.bg} px-3 py-2.5 transition-[background-color,border-color,color] hover:bg-[color:var(--ds-hover)] motion-reduce:transition-none`}
              >
