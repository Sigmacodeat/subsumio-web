    ...
    try {
      await withEnv({ GBRAIN_AUDIT_DIR: fixture.skillsDir }, async () => {
        ...
        const {
          ...
          revertAllPending,
          loadHistory: lh,
        } = await import("../../src/core/skillopt/version-store.ts");
        ...
        fs.writeFileSync(
          path.join(versionsDir(fixture.skillsDir, SKILL), "v0002_e1_s2.md"),
          "corrupted v2"
        );
        ...
        fs.writeFileSync(
          historyPath(fixture.skillsDir, SKILL),
          JSON.stringify({ schema: 1, rows: history })
        );
