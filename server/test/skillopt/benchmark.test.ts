  ...
  test("parses well-formed JSONL with rule judge", () => {
    const p = writeBench("b.jsonl", [
      JSON.stringify({
        task_id: "t1",
        task: "do X",
        judge: { kind: "rule", checks: [{ op: "max_chars", arg: 4000 }] },
      }),
      JSON.stringify({
        task_id: "t2",
        task: "do Y",
        judge: { kind: "rule", checks: [{ op: "contains", arg: "foo" }] },
      }),
    ]);
  ...
  test("rejects duplicate task_id", () => {
    const p = writeBench("dup.jsonl", [
      JSON.stringify({
        task_id: "t1",
        task: "a",
        judge: { kind: "rule", checks: [{ op: "max_chars", arg: 100 }] },
      }),
      JSON.stringify({
        task_id: "t1",
        task: "b",
  ...
  test("rejects rule check with unknown op", () => {
    const p = writeBench("badop.jsonl", [
      JSON.stringify({
        task_id: "t1",
        task: "a",
        judge: { kind: "rule", checks: [{ op: "frob", arg: 1 }] },
      }),
    ]);
  ...
  test("rejects qrels judge with empty expected_slugs", () => {
    const p = writeBench("noslugs.jsonl", [
      JSON.stringify({
        task_id: "t1",
        task: "a",
        judge: { kind: "qrels", expected_slugs: [], k: 10 },
      }),
    ]);
...
describe("computeBenchmarkSha8", () => {
    ...
    const tasks = [
      {
        task_id: "t1",
        task: "a",
        judge: { kind: "rule" as const, checks: [{ op: "max_chars" as const, arg: 100 }] },
      },
    ];
  ...
  test("reordering tasks produces same hash (sort-stable)", () => {
    ...
    const t2 = {
      task_id: "t2",
      task: "b",
      judge: { kind: "rule" as const, checks: [{ op: "max_chars" as const, arg: 200 }] },
    };
