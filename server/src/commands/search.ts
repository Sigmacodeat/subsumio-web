...
const KNOB_DESCRIPTIONS: Record<keyof ModeBundle, string> = {
  ...
  graph_signals:
    "Selective graph signals: adjacency hub + cross-source hub + session diversification",
  ...
  contextual_retrieval_disabled:
    "Soft kill switch — neutralizes CR wrapping for queries + new embeds",
  // v0.42.3.0 autocut
  autocut:
    "Score-discontinuity result-sizing (cuts at the rerank-score cliff; no-op without a reranker)",
  autocut_jump:
    "Autocut sensitivity: min normalized score gap that counts as a cliff (0..1, 0.20 default)",
...
interface SearchModesReport {
  ...
  resolved: Record<
    keyof ModeBundle,
    { value: unknown; source: string; source_detail: string; description: string }
  >;
...
function formatModesText(report: SearchModesReport): string {
  ...
  lines.push(
    "Search mode (active): " +
      report.active_mode +
      (report.active_mode_valid ? "" : "  (unset — using balanced fallback)")
  );
  lines.push("");
  ...
  for (const mode of SEARCH_MODES) {
    ...
    lines.push(
      `    tokenBudget=${b.tokenBudget ?? "none"} searchLimit=${b.searchLimit} expansion=${b.expansion}`
    );
  }
  lines.push("");
...
async function runModesSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (reset || dryRunSource) {
    ...
    if (dryRunSource && !isSearchMode(dryRunSource)) {
      console.error(
        `Invalid --source value: ${dryRunSource}. Expected one of: ${SEARCH_MODES.join(", ")}`
      );
      process.exit(1);
    }
    const overrides = await engine.listConfigKeys("search.");
    const toRemove = overrides.filter(
      (k) => k !== SEARCH_MODE_KEY && k !== "search.mode_upgrade_notice_shown"
    );
...
async function runStatsSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
      ...
      JSON.stringify(
        {
          ...
          _meta: {
            metric_glossary: {
              cache_hit_rate:
                "cache_hits / (cache_hits + cache_misses) — fraction of searches that reused a recent answer instead of running fresh",
              ...
              total_budget_dropped:
                "sum of results dropped because the call exceeded its tokenBudget",
              graph_signals_enabled:
                "whether graph_signals is on for the active mode (or via search.graph_signals override)",
              graph_signals_failures_count:
                "count of fail-open events in the JSONL audit over the window",
            },
          },
        },
        null,
  ...
  if (stats.avg_rank1_score !== null && stats.rank1_count > 0) {
    ...
    console.log(
      `  Avg rank-1 score:      ${stats.avg_rank1_score.toFixed(3)}  (${stats.rank1_count} samples; <0.6:${d.lt_solid} 0.6-0.85:${d.solid} >=0.85:${d.high})`
    );
    console.log(
...
async function readGraphSignalsStats(
  engine: BrainEngine,
  days: number
): Promise<GraphSignalsStatsSection> {
...
function printGraphSignalsSection(gs: GraphSignalsStatsSection): void {
  ...
  if (!gs.enabled && gs.failures_count > 0) {
    console.log(
      `    note:       failures observed but graph_signals currently off — historical events`
    );
  }
}
...
async function runTuneSubcommand(engine: BrainEngine, args: string[]): Promise<void> {
        ...
        JSON.stringify(
          {
            ...
            recommendations: [],
            message:
              "Not enough search activity in the last 7 days to tune. Run `gbrain search stats` after some real usage.",
          },
          null,
          2
        )
  ...
  if (!resolved.cache_enabled && stats.total_calls > 5) {
    recs.push({
      ...
      suggested: true,
      reason:
        "Cache is disabled but mode bundles enable it by default. Cache is a free win (zero LLM cost, big latency drop on repeat queries).",
