...
describe("extractEntityRefs", () => {
  ...
  test("extracts multiple refs", () => {
    const content =
      "[Alice](../people/alice.md) and [Bob](../people/bob.md) from [Acme](../companies/acme.md).";
...
describe("findBacklinkGaps dedupe (v0.36.x #967 regression)", () => {
  test("a source page mentioning the same target N times yields one gap, not N", async () => {
    ...
    try {
      ...
      const alicePairs = gaps.filter(
        (g) => g.targetPage === "people/alice.md" && g.sourcePage === "meetings/standup.md"
      );
