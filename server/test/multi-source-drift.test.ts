...
function makeTmpRoot(label: string): string {
  const dir = join(
    tmpdir(),
    `gbrain-drift-test-${label}-${Date.now()}-${Math.random().toString(36).slice(2)}`
  );
...
describe("findMisroutedPages — heuristic correctness", () => {
  ...
  test("case 2: multi-source brain, no misroutes → count=0", async () => {
    ...
    await engine.executeRaw(`UPDATE sources SET local_path = $1 WHERE id = $2`, [
      root,
      "src-case2",
    ]);
    ...
    await engine.putPage(
      "people/bob",
      { type: "person", title: "Bob", compiled_truth: "." },
      { sourceId: "src-case2" }
    );
  ...
  test("case 4: healthy same-slug-across-sources is NOT a false positive (OV4 redesign)", async () => {
    ...
    await engine.putPage("topics/widget", {
      type: "concept",
    ...
    await engine.putPage(
      "topics/widget",
      { type: "concept", title: "Src widget", compiled_truth: "." },
      { sourceId: "src-case4" }
    );
