...
export function createBridge(twilioWs, opts) {
    ...
    inProcessor = createTwilioToGeminiProcessor((pcmBase64) => {
      ...
      if (setupDone && geminiWs?.readyState === WebSocket.OPEN) {
        ...
        if (audioFlushes % 50 === 1) {
          console.log(
            `[twilio-venus] Audio: ${audioChunksIn} in, ${audioFlushes} flushes, ${audioChunksOut} out`
          );
        }
      }
    }, 200);
    ...
    geminiWs.on("message", (data) => {
      try {
        ...
        if (msg.toolCallCancellation) {
          console.log(
            `[twilio-venus] Tool call cancelled: ${JSON.stringify(msg.toolCallCancellation?.ids || [])}`
          );
          if (_twilioToolAbort) {
            _twilioToolAbort.abort();
        ...
        if (msg.toolCall) {
          ...
          Promise.all(
            calls.map(async (fc) => {
              ...
              console.log(
                `[twilio-venus] Tool: ${fc.name}(${JSON.stringify(fc.args).slice(0, 80)})`
              );
  ...
  twilioWs.on("message", (raw) => {
    try {
      ...
      if (msg.event === "stop") {
        console.log(
          `[twilio-venus] Call stopped: ${streamSid} (${audioChunksIn} in, ${audioFlushes} flushes, ${audioChunksOut} out)`
        );
  ...
  return {
    ...
    upgrade(newPrompt, newTools, newToolHandler, newVoice) {
      ...
      transcript.push({
        role: "system",
        text: `[${elapsed}s] UPGRADED to full Venus (${newTools.length} tools)`,
        ts: elapsed,
      });
