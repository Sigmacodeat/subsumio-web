...
import { analyzeDocument, groundIssues, type AnalyzeLLM, type DocumentIssue } from '../src/core/legal/analyze-document.ts';
...
describe('groundIssues', () => {
  const issues: DocumentIssue[] = [
    { issue: 'Totalausschluss der Haftung', severity: 'high', quote: 'Die Haftung des Auftragnehmers ist vollständig ausgeschlossen', rationale: '§ 309 Nr. 7 BGB' },
    { issue: 'Erfundene Schiedsklausel', severity: 'critical', quote: 'Alle Streitigkeiten werden durch ein Schiedsgericht in Genf entschieden', rationale: 'halluziniert' },
  ];
  ...
  test('whitespace differences in the quote still match', () => {
    const { grounded } = groundIssues(
      [{ issue: 'Lange Kündigungsfrist', severity: 'medium', quote: 'Kündigungsfrist von   24\n  Monaten', rationale: '§ 309 Nr. 9 BGB' }],
      CONTRACT,
    );
...
describe('analyzeDocument (stub LLM)', () => {
  const stub: AnalyzeLLM = async () => JSON.stringify({
    document_type: 'Dienstleistungsvertrag',
    parties: ['Acme GmbH', 'Widget AG'],
    key_dates: [{ date: 'unbestimmt', what: 'Laufzeit' }],
    issues: [
      { issue: 'Totalausschluss der Haftung', severity: 'high', quote: 'Die Haftung des Auftragnehmers ist vollständig ausgeschlossen', rationale: '§ 309 Nr. 7 BGB — unwirksam' },
      // Fabricated: this text is NOT in the contract → must be dropped.
      { issue: 'Vertragsstrafe 1 Mio EUR', severity: 'critical', quote: 'Es wird eine Vertragsstrafe von 1.000.000 EUR vereinbart', rationale: 'erfunden' },
    ],
  ...
  test('malformed LLM output degrades gracefully, never throws', async () => {
    const a = await analyzeDocument(engine, { slug: 'akten/vertrag-acme', llm: async () => 'not json at all' });
    expect(a.issues).toEqual([]);
    expect(a.warnings).toContain('LLM_OUTPUT_NOT_JSON');
  });

  test('missing page throws a clear error', async () => {
    await expect(analyzeDocument(engine, { slug: 'akten/gibt-es-nicht', llm: stub })).rejects.toThrow(/not found/);
  });
});
