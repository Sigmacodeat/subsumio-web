...
describe("closure (E8): alias graph BFS", () => {
  test("isolated type closure = {self}", () => {
    const m = parseSchemaPackManifest(
      minimalManifest({
        page_types: [
          {
...
describe("per-source CTE builder (D13)", () => {
  ...
  test("SQL-escapes type literals with quotes", () => {
    const result = buildSourceClosureCte([{ source_id: "src", types: ["o'reilly-book"] }]);
...
describe("per-source bindings", () => {
  test("builds bindings from sourcePacks map", () => {
    const m1 = parseSchemaPackManifest(
      minimalManifest({
        name: "pack-a",
        page_types: [
          {
    ...
    const m2 = parseSchemaPackManifest(
      minimalManifest({
        name: "pack-b",
        page_types: [
          {
    ...
    const bindings = buildPerSourceBindings(
      "person",
      new Map([
        ["src-a", m1],
        ["src-b", m2],
...
describe("7-tier resolution (D13)", () => {
  test("default to gbrain-base when nothing set", () => {
    expect(resolveActivePackName({ remote: false })).toEqual({
