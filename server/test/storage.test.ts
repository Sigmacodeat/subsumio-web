...
describe("LocalStorage path traversal", () => {
  test("blocks upload path traversal via ../", async () => {
    ...
    try {
      ...
      await expect(storage.upload("../sibling/file", Buffer.from("x"))).rejects.toThrow(
        "Path traversal blocked"
      );
    } finally {
      rmSync(tmpDir, { recursive: true });
  ...
  test("blocks delete path traversal via ../", async () => {
    ...
    try {
      const storage = new LocalStorage(tmpDir);
      await expect(storage.delete("../../../tmp/important")).rejects.toThrow(
        "Path traversal blocked"
      );
    } finally {
      rmSync(tmpDir, { recursive: true });
...
describe("createStorage", () => {
  ...
  test("throws for unknown backend", async () => {
    expect(createStorage({ backend: "unknown" as any, bucket: "test" })).rejects.toThrow(
