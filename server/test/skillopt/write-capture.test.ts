...
describe("F10 write-capture registry", () => {
  ...
  test("repeated put_page for same slug captures both writes; virtualPages reflects latest", async () => {
    ...
    await reg.handlers
      .get("brain_put_page")!
      .execute({ slug: "dup/slug", content: "first" }, ctrl.signal);
    await reg.handlers
      .get("brain_put_page")!
      .execute({ slug: "dup/slug", content: "second" }, ctrl.signal);
    expect(reg.getWrites().length).toBe(2);
    // virtualPages reflects the LATEST write (matches real put_page upsert semantics).
    expect((reg.getVirtualPages().get("dup/slug")!.input as { content: string }).content).toBe(
      "second"
    );
  });
