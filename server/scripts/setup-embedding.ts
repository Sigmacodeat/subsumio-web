...
async function main() {
  ...
  if (selected === "auto") {
    ...
    selected = detected.config.name.toLowerCase().includes("voyage")
      ? "voyage"
      : detected.config.name.toLowerCase().includes("ollama")
        ? "ollama"
        : "openai";
    key = detected.key;
  }
  ...
  try {
    await engine.executeRaw(
      `UPDATE sources SET config = config || '{"no_embed": false}'::jsonb WHERE id = 'default'`
    );
  } catch {}
  ...
  const proc = Bun.spawn(
    ["bun", "run", "server/src/cli.ts", "embed", "--all", "--batch-size", "50"],
    {
      stdout: "pipe",
      stderr: "pipe",
