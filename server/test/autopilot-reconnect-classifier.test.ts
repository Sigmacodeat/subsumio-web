...
describe("classifyReconnectError (#1162)", () => {
  ...
  test("malformed URL → unrecoverable", () => {
    expect(classifyReconnectError(new Error("Invalid URL: not a postgres connection string"))).toBe(
      "unrecoverable"
    );
  ...
  test("auth failure → unrecoverable (creds don't fix themselves)", () => {
    expect(
      classifyReconnectError(new Error('password authentication failed for user "gbrain"'))
    ).toBe("unrecoverable");
  ...
  test("no brain configured → unrecoverable", () => {
    expect(classifyReconnectError(new Error("No brain configured. Run: gbrain init"))).toBe(
      "unrecoverable"
    );
  });
  ...
  test("network blip → recoverable", () => {
    ...
    expect(classifyReconnectError(new Error("connection terminated unexpectedly"))).toBe(
      "recoverable"
    );
  });
  ...
  test("pool saturated → recoverable", () => {
    ...
    expect(classifyReconnectError(new Error("remaining connection slots are reserved"))).toBe(
      "recoverable"
    );
  });
  ...
  test("case-insensitive match", () => {
    ...
    expect(classifyReconnectError(new Error("Password Authentication FAILED"))).toBe(
      "unrecoverable"
    );
  });
});
