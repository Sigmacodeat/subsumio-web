...
describe("resolveToken", () => {
  test("--token flag wins", () => {
    expect(resolveToken({ tokenFlag: "tok", env: "envtok", mode: "print" })).toEqual({
      kind: "literal",
      token: "tok",
    });
  });
  test("env used when no flag", () => {
    expect(resolveToken({ tokenFlag: null, env: "envtok", mode: "install" })).toEqual({
      kind: "literal",
      token: "envtok",
    });
  });
  test("print mode without token returns placeholder", () => {
    expect(resolveToken({ tokenFlag: null, env: null, mode: "print" })).toEqual({
      kind: "placeholder",
    });
  });
...
describe("argv + command string", () => {
  test("claude argv shape", () => {
    expect(
      buildClaudeMcpAddArgv({ name: "gbrain", url: "https://h/mcp", headerToken: "TOK" })
    ).toEqual([
      "mcp",
      "add",
      "gbrain",
  ...
  test("a token with shell metacharacters cannot trigger command substitution on paste", () => {
    const cmd = cmdString(
      ...
      buildClaudeMcpAddArgv({
        name: "gbrain",
        url: "https://h/mcp",
        headerToken: "gbrain_$(touch /tmp/pwned)`x`",
      })
    );
...
describe("redactToken", () => {
  ...
  test("Bearer scrub catches a non-exact token echo", () => {
    expect(redactToken("add failed near Bearer SOMETHINGELSE", "tok")).toContain(
      `Bearer ${REDACTED}`
    );
  });
});
...
describe("buildConnectBlock", () => {
  test("claude-code with a literal token inlines it + learn instruction", () => {
    ...
    expect(block).toContain(
      "claude mcp add gbrain -t http https://h/mcp -H 'Authorization: Bearer TOK'"
    );
  ...
  test("codex emits the codex command + env-var export, token only in export", () => {
    ...
    expect(block).toContain(
      "codex mcp add gbrain --url https://h/mcp --bearer-token-env-var GBRAIN_REMOTE_TOKEN"
    );
...
describe("OAuth helpers", () => {
  test("issuerFromMcpUrl strips /mcp", () => {
    expect(issuerFromMcpUrl("https://brain.example.com:3131/mcp")).toBe(
      "https://brain.example.com:3131"
    );
...
describe("classifyProbeError", () => {
  ...
  test("unreachable", () => {
    ...
    expect(classifyProbeError("Unable to connect. Is the computer able to access the url?")).toBe(
      "unreachable"
    );
  });
...
describe("probeBrainIdentity (injected deps)", () => {
  ...
  test("isError with a non-auth message → tool_error", async () => {
    const deps: ProbeDeps = {
      connectAndCall: async () => ({
        isError: true,
        content: [{ type: "text", text: "tool blew up: bad arguments" }],
      }),
    };
    const r = await probeBrainIdentity("https://h/mcp", "TOK", { deps });
    expect(r.ok).toBe(false);
...
function installDeps(over: Partial<ConnectDeps> = {}): ConnectDeps {
  return {
    ...
    hasBinary: () => true,
    runBinary: (_binary, argv) =>
      argv[1] === "get" ? { code: 1, stdout: "", stderr: "" } : { code: 0, stdout: "", stderr: "" },
    ...
    registerOAuthClient: () => ({
      ok: true,
      clientId: "gbrain_cl_minted",
      clientSecret: "gbrain_cs_minted",
    }),
    ...over,
  };
}
...
describe("runConnect --install", () => {
  ...
  test("probe failure warns + exits 1 + never echoes the token", async () => {
    const r = await runWithExitCapture(
      ...
      installDeps({
        probe: async () => ({ ok: false, reason: "auth", message: "HTTP 401 for gbrain_secret" }),
      })
    );
...
describe("runConnect print mode", () => {
  test("prints the block to stdout with the literal token", async () => {
    ...
    expect(r.exitCode).toBeUndefined();
    expect(r.out.join("\n")).toContain(
      "claude mcp add gbrain -t http https://brain.example.com/mcp -H 'Authorization: Bearer gbrain_tok'"
    );
  });
  ...
  test("unknown --agent fails fast", async () => {
    const r = await runWithExitCapture(
      ["https://h/mcp", "--token", "t", "--agent", "bogus"],
      installDeps()
    );
    expect(r.exitCode).toBe(1);
  ...
  test("invalid --name fails fast", async () => {
    const r = await runWithExitCapture(
      ["https://h/mcp", "--token", "t", "--name", "Bad Name"],
      installDeps()
    );
    expect(r.exitCode).toBe(1);
  ...
  test("http non-local prints the plaintext-token warning but still proceeds", async () => {
    const r = await runWithExitCapture(
      ["http://brain.example.com/mcp", "--token", "t"],
      installDeps()
    );
    expect(r.exitCode).toBeUndefined();
  ...
  test("invalid --timeout-ms falls back to the default (probe receives it)", async () => {
    ...
    await runWithExitCapture(
      ...
      installDeps({
        probe: async (_u, _t, ms) => {
          seen = ms;
          return { ok: true, identity: "ok" };
        },
      })
    );
  ...
  test("--agent codex print mode emits the codex block", async () => {
    const r = await runWithExitCapture(
      ["https://brain.example.com/mcp", "--token", "gbrain_tok", "--agent", "codex"],
      installDeps()
    );
    expect(r.exitCode).toBeUndefined();
    const out = r.out.join("\n");
    expect(out).toContain(
      "codex mcp add gbrain --url https://brain.example.com/mcp --bearer-token-env-var GBRAIN_REMOTE_TOKEN"
    );
  ...
  test("--agent perplexity print mode emits GUI connector steps", async () => {
    const r = await runWithExitCapture(
      ["https://brain.example.com/mcp", "--token", "gbrain_tok", "--agent", "perplexity"],
      installDeps()
    );
    expect(r.exitCode).toBeUndefined();
