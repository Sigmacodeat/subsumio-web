...
import { BrainWriter, WriteError, type ValidationReport } from "../src/core/output/writer.ts";
...
import { splitParagraphs } from "../src/core/output/validators/citation.ts";
import {
  normalizeToSlug,
  isExternalUrl,
  isNonBrainRef,
...
async function reset(): Promise<void> {
  await engine.executeRaw(
    "TRUNCATE pages, links, content_chunks, timeline_entries, tags, raw_data, page_versions RESTART IDENTITY CASCADE"
  );
}
...
describe("Scaffolder", () => {
  ...
  test("tweetCitation rejects bad date format", () => {
    expect(() =>
      tweetCitation({ handle: "garrytan", tweetId: "1", dateISO: "2026/04/18" })
  ...
  test("sourceCitation with url", () => {
    const r = sourceCitation(
      { source: "perplexity-sonar", fetchedAt: new Date("2026-04-18") },
...
describe("BrainWriter", () => {
  ...
  test("addLink creates forward + back-link", async () => {
    ...
    await writer.transaction(async (tx) => {
      await tx.createEntity({
        desiredSlug: "people/a",
        displayName: "A",
        type: "person",
        compiledTruth: "a",
      });
      await tx.createEntity({
        desiredSlug: "people/b",
        displayName: "B",
        type: "person",
        compiledTruth: "b",
      });
  ...
  test("lint mode does NOT roll back on validator error", async () => {
    ...
    writer.register({
      ...
      async validate({ slug }) {
        return [
          { slug, validator: "synthetic-fail", severity: "error", message: "still writes in lint" },
        ];
      },
    });
    const { result, report } = await writer.transaction(async (tx) => {
  ...
  test("off mode skips validators entirely", async () => {
    ...
    await writer.transaction(async (tx) => {
      await tx.createEntity({
        desiredSlug: "people/no-validator",
        displayName: "x",
        type: "person",
        compiledTruth: "x",
      });
  ...
  test("setCompiledTruth updates existing page", async () => {
    ...
    await writer.transaction(
      async (tx) => tx.setCompiledTruth("people/update", "updated"),
      makeCtx()
    );
  ...
  test("setFrontmatterField merges into existing frontmatter", async () => {
    ...
    await writer.transaction(
      async (tx) => tx.setFrontmatterField("people/fm", "validate", false),
      makeCtx()
    );
...
describe("citation validator", () => {
  ...
  async function run(
    compiled: string,
    slug = "concepts/test"
  ): Promise<ReturnType<typeof citationValidator.validate>> {
...
describe("back-link validator", () => {
  ...
  test("no outbound links → no findings", async () => {
    await engine.putPage("people/isolated", {
      type: "person",
      title: "x",
      compiled_truth: "x",
      frontmatter: {},
