...
describeE2E("E2E: Page CRUD", () => {
  ...
  test("get_page returns correct data for concept", async () => {
    const page = (await callOp("get_page", {
      slug: "concepts/retrieval-augmented-generation",
    })) as any;
  ...
  test("put_page updates existing page", async () => {
    const updated = readFileSync(join(FIXTURES_PATH, "people/sarah-chen.md"), "utf-8").replace(
      "Stanford CS",
      "MIT CS"
    );
...
describeE2E("E2E: Links", () => {
  ...
  test("add_link + get_links + get_backlinks round trip", async () => {
    ...
    expect(
      links.some(
        (l: any) => l.to_slug === "companies/novamind" || l.to_page_slug === "companies/novamind"
      )
    ).toBe(true);
    ...
    expect(
      backlinks.some(
        (l: any) => l.from_slug === "people/sarah-chen" || l.from_page_slug === "people/sarah-chen"
      )
    ).toBe(true);
  }, 30_000);
  ...
  test("remove_link removes the link", async () => {
    ...
    const hasLink = links.some(
      (l: any) => (l.to_slug || l.to_page_slug) === "companies/threshold-ventures"
    );
...
describeE2E("E2E: addLinksBatch (postgres-engine)", () => {
  ...
  test("rows with missing slug silently dropped by JOIN", async () => {
    ...
    const inserted = await engine.addLinksBatch([
      {
        from_slug: "people/does-not-exist",
        to_slug: "companies/novamind",
        link_type: "e2e-batch-missing",
      },
      {
        from_slug: "people/sarah-chen",
        to_slug: "companies/novamind",
        link_type: "e2e-batch-missing",
      },
    ]);
    ...
    const inserted = await engine.addLinksBatch([
      {
        from_slug: "people/sarah-chen",
        to_slug: "companies/novamind",
        link_type: "e2e-batch-half",
      },
      {
        from_slug: "people/sarah-chen",
        to_slug: "people/marcus-reid",
        link_type: "e2e-batch-half",
      },
    ]);
    expect(inserted).toBe(1);
...
describeE2E("E2E: Versions", () => {
  ...
  test("put_page creates version, revert restores", async () => {
    ...
    const modified = readFileSync(join(FIXTURES_PATH, "people/sarah-chen.md"), "utf-8").replace(
      "Sarah Chen",
      "Sarah Chen (Modified)"
    );
    const engine = getEngine();
...
describeE2E("E2E: Schema Idempotency", () => {
  ...
  test("initSchema twice produces no errors and same object count", async () => {
    const conn = getConn();
    const tables1 = await conn.unsafe(
      `SELECT count(*) as n FROM information_schema.tables WHERE table_schema = 'public'`
    );
    const indexes1 = await conn.unsafe(
    ...
    const tables2 = await conn.unsafe(
      `SELECT count(*) as n FROM information_schema.tables WHERE table_schema = 'public'`
    );
    const indexes2 = await conn.unsafe(
      `SELECT count(*) as n FROM pg_indexes WHERE schemaname = 'public'`
    );
...
describeE2E("E2E: RLS Verification", () => {
  ...
  test("GBRAIN:RLS_EXEMPT comment with valid reason exempts a non-RLS public table", async () => {
    ...
    try {
      ...
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
      await conn.unsafe(
        `COMMENT ON TABLE public.${tbl} IS 'GBRAIN:RLS_EXEMPT reason=e2e test fixture, anon-readable ok'`
      );
  ...
  test("Non-exempt unrelated COMMENT on a no-RLS table still fails doctor", async () => {
    ...
    try {
      ...
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
      await conn.unsafe(
        `COMMENT ON TABLE public.${tbl} IS 'Regular docs comment, not an exemption'`
      );
  ...
  test("v24 self-heals when budget_ledger + budget_reservations are missing", async () => {
    ...
    try {
      ...
      if (priorVersion !== null) {
        await conn.unsafe(`UPDATE config SET value = $1 WHERE key = 'version'`, [priorVersion]);
      }
    }
  }, 60_000);
});
