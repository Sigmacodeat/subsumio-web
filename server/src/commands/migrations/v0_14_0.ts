...
function pendingHostWorkPath(): string {
  return join(pendingHostWorkDir(), "pending-host-work.jsonl");
}
      ...
      if (/does not exist|no such table|relation .* does not exist/i.test(msg)) {
        return {
          result: {
            name: "schema",
            status: "skipped",
            detail: "minion_jobs not yet created (fresh install)",
          },
          engine,
        };
      }
...
async function orchestrator(opts: OrchestratorOpts): Promise<OrchestratorResult> {
  ...
  return {
    ...
    status,
    phases,
    pending_host_work: phases.some((p) => p.name === "host-work" && p.status === "complete")
      ? 1
      : 0,
  };
}
