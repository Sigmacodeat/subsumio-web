...
console.error(
  `[persona-routing-eval] running ${fixtures.length} fixtures (limit=${args.limit || "none"})`
);
...
for (let i = 0; i < slice.length; i++) {
  ...
  process.stderr.write(
    `[persona-routing-eval] ${i + 1}/${slice.length}: ${fixture.user_utterance.slice(0, 60)}\n`
  );
...
writeFileSync(
  resolve(outDir, "persona-routing-summary.json"),
