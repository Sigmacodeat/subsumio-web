...
const enginePatchPage = vi.fn(
  async (_headers: Record<string, string>, page: { slug: string; frontmatter?: unknown }) => {
    writes.push(page as never);
    return new Response("{}", { status: 200 });
  }
);
vi.mock("@/lib/engine", () => ({
  ENGINE_URL: "http://engine.test",
  enginePatchPage: (...a: unknown[]) =>
    enginePatchPage(...(a as [Record<string, string>, { slug: string; frontmatter?: unknown }])),
}));
...
describe("POST /api/bea/send — sending state must be stored first", () => {
  it("sends nothing when the 'sending' state cannot be saved", async () => {
    enginePatchPage.mockResolvedValueOnce(new Response("{}", { status: 500 }));
    const res = await send(
      body({ verification_override: { reason: "Schriftsatz vollständig selbst geprüft." } })
    );
    // Whether the manual-export or the transport path is taken, an unsaved
    // package state stops the send.
    expect(res.status).toBe(502);
    expect(["engine_write_failed", "filing_not_saved"]).toContain((await res.json()).code);
  });
});
