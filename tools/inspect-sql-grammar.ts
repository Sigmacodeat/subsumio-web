...
async function main() {
  ...
  for (const fixture of FIXTURES) {
    ...
    for (let i = 0; i < root.namedChildCount; i++) {
      ...
      console.log(
        "    extractSymbolNameGeneric: " + JSON.stringify(extractSymbolNameGeneric(node))
      );
      console.log(
        "    named children: " +
          childTypes.slice(0, 8).join(", ") +
          (childTypes.length > 8 ? ", ... (" + childTypes.length + " total)" : "")
      );
        ...
        if (f)
          console.log(
            '    field "' +
              ...
              f.type +
              " = " +
              JSON.stringify(sanitize(f.text).slice(0, 50))
          );
      }
    }
  }
