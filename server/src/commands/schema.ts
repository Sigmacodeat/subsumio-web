...
export async function runSchema(args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case "detect":
      return runDetectCmd(args.slice(1));
    case "suggest":
      return runSuggestCmd(args.slice(1));
    case "review-candidates":
      return runReviewCandidatesCmd(args.slice(1));
    case "init":
      return runInitCmd(args.slice(1));
    case "fork":
      return runForkCmd(args.slice(1));
    case "edit":
      return runEditCmd(args.slice(1));
    case "diff":
      return runDiffCmd(args.slice(1));
    case "graph":
      return runGraphCmd(args.slice(1));
    case "lint":
      return runLintCmd(args.slice(1));
    case "explain":
      return runExplainCmd(args.slice(1));
    case "review-orphans":
      return runReviewOrphansCmd(args.slice(1));
    case "downgrade":
      return runDowngradeCmd(args.slice(1));
    case "usage":
      return runUsageCmd(args.slice(1));
    case "stats":
      return runStatsCmd(args.slice(1));
    case "sync":
      return runSyncCmd(args.slice(1));
    case "reload":
      return runReloadCmd(args.slice(1));
    case "add-type":
      return runAddTypeCmd(args.slice(1));
    case "remove-type":
      return runRemoveTypeCmd(args.slice(1));
    case "update-type":
      return runUpdateTypeCmd(args.slice(1));
    case "add-alias":
      return runAddAliasCmd(args.slice(1));
    case "remove-alias":
      return runRemoveAliasCmd(args.slice(1));
    case "add-prefix":
      return runAddPrefixCmd(args.slice(1));
    case "remove-prefix":
      return runRemovePrefixCmd(args.slice(1));
    case "add-link-type":
      return runAddLinkTypeCmd(args.slice(1));
    case "remove-link-type":
      return runRemoveLinkTypeCmd(args.slice(1));
    case "set-extractable":
      return runSetExtractableCmd(args.slice(1));
    case "set-expert-routing":
      return runSetExpertRoutingCmd(args.slice(1));
    case "scaffold-extractable":
      return runScaffoldExtractableCmd(args.slice(1));
    case undefined:
    case "--help":
    case "-h":
      return printHelp();
...
async function runShow(args: string[]): Promise<void> {
  ...
  console.log(
    `Enrichable types: ${manifest.enrichable_types.map((e) => e.type).join(", ") || "(none)"}`
  );
}
...
import { runReviewCandidates, runReviewOrphans } from "../core/schema-pack/review.ts";
...
function parseFlags(args: string[]): ParsedFlags {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a.startsWith("--source-id=")) {
      source = a.slice("--source-id=".length);
      continue;
    }
    positional.push(a);
  }
...
async function withConnectedEngine<T>(
...
async function runDetectCmd(args: string[]): Promise<void> {
  ...
  for (const p of result.prefixes) {
    ...
    console.log(
      `  ${p.prefix.padEnd(30)} ${String(p.page_count).padStart(6)} pages → suggest type \`${p.suggested_type}\`${samples}`
    );
  }
  console.log("");
...
async function runReviewCandidatesCmd(args: string[]): Promise<void> {
  ...
  for (const c of result.candidates) {
    console.log(
      `  ${c.prefix.padEnd(30)} ${String(c.page_count).padStart(6)} pages  (suggest \`${c.suggested_type}\`)`
    );
  }
}
...
`;
  ...
  if (json) {
    console.log(
      JSON.stringify({ schema_version: 1, name, path: baseDir, tier: "experimental" }, null, 2)
    );
    return;
  }
  console.log(`(experimental) Scaffolded pack \`${name}\` at ${baseDir}/pack.yaml`);
  console.log(
    `Next: edit pack.yaml, then run \`gbrain schema validate ${name}\` and \`gbrain schema use ${name}\`.`
  );
}
...
async function runForkCmd(args: string[]): Promise<void> {
  ...
  if (json) {
    console.log(
      JSON.stringify({ schema_version: 1, from, to, path: toDir, tier: "experimental" }, null, 2)
    );
    return;
  }
...
async function runEditCmd(args: string[]): Promise<void> {
  ...
  if (json) {
    console.log(
      JSON.stringify({ schema_version: 1, name, path: p, tier: "experimental" }, null, 2)
    );
    return;
  }
      ...
      JSON.stringify(
        {
          ...
          types: pack.manifest.page_types.map((t) => ({
            name: t.name,
...
async function runLintCmd(args: string[]): Promise<void> {
  ...
  if (name) {
    const p = packPathByName(name);
    try {
      pack = p ? loadPackFromFile(p) : null;
    } catch {
      pack = null;
    }
  } else {
...
async function runUsageCmd(args: string[]): Promise<void> {
  const { json, positional } = parseFlags(args);
  const sinceArg = positional.includes("--since")
    ? positional[positional.indexOf("--since") + 1]
    : "30d";
...
function emitMutateResult(
  result: { pack: string; format: string; prev_sha8: string; new_sha8: string },
  json: boolean
): void {
  if (json) {
...
async function runStatsCmd(args: string[]): Promise<void> {
  ...
  await withConnectedEngine(async (engine) => {
    ...
    console.log(
      `Typed: ${result.aggregate.typed_pages} (${(result.aggregate.coverage * 100).toFixed(1)}%)`
    );
    ...
    if (result.per_source.length > 1) {
      ...
      for (const s of result.per_source) {
        console.log(
          `  ${s.source_id.padEnd(20)} total=${s.total_pages} typed=${s.typed_pages} coverage=${(s.coverage * 100).toFixed(1)}%`
        );
      }
    }
...
async function runSyncCmd(args: string[]): Promise<void> {
  ...
  await withConnectedEngine(async (engine) => {
      ...
      onProgress: (info) => {
        if (!json)
          process.stderr.write(
            `  [sync] ${info.type} ${info.prefix} → ${info.appliedSoFar} applied\n`
          );
      },
    });
    if (json) {
    ...
    for (const p of result.per_prefix) {
      ...
      console.log(
        `  ${p.type.padEnd(20)} ${p.prefix.padEnd(30)} would_apply=${p.would_apply} applied=${p.applied}${marker}`
      );
      if (p.sample_slugs.length > 0 && !apply) {
        console.log(
          `    sample: ${p.sample_slugs.slice(0, 3).join(", ")}${p.sample_slugs.length > 3 ? "..." : ""}`
        );
      }
    }
...
function runReloadCmd(args: string[]): void {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (args[i]?.startsWith("--pack=")) {
      packName = args[i]!.slice("--pack=".length);
      break;
    }
  }
...
async function runAddTypeCmd(args: string[]): Promise<void> {
  ...
  const name = positional[0];
  if (!name) {
    console.error("Usage: gbrain schema add-type <name> --primitive <p> --prefix <dir/>");
    process.exit(2);
  }
  ...
  if (!prefix) {
    console.error("--prefix is required (e.g. --prefix people/researchers/)");
    process.exit(2);
  }
  try {
    const result = await addTypeToPack(packName, {
      name,
      primitive: primitive as PackPrimitive,
      prefix,
      extractable,
...
async function runRemoveTypeCmd(args: string[]): Promise<void> {
  ...
  if (!name) {
    console.error("Usage: gbrain schema remove-type <name>");
    process.exit(2);
  }
  try {
    emitMutateResult(await removeTypeFromPack(packName, name), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runUpdateTypeCmd(args: string[]): Promise<void> {
  ...
  if (!name) {
    console.error(
      "Usage: gbrain schema update-type <name> [--extractable BOOL] [--expert BOOL] [--primitive P]"
    );
    process.exit(2);
  }
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--extractable") {
      ...
      if (v === null) {
        console.error("--extractable requires true|false");
        process.exit(2);
      }
      patch.extractable = v;
      ...
      const v = parseBool(args[++i]);
      if (v === null) {
        console.error("--expert requires true|false");
        process.exit(2);
      }
  ...
  try {
    emitMutateResult(await updateTypeOnPack(packName, { name, patch }), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runAddAliasCmd(args: string[]): Promise<void> {
  ...
  if (pos.length < 2) {
    console.error("Usage: gbrain schema add-alias <type> <alias>");
    process.exit(2);
  }
  try {
    emitMutateResult(await addAliasToType(packName, pos[0]!, pos[1]!), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runRemoveAliasCmd(args: string[]): Promise<void> {
  ...
  const pos = args.filter((a) => !a.startsWith("--"));
  if (pos.length < 2) {
    console.error("Usage: gbrain schema remove-alias <type> <alias>");
    process.exit(2);
  }
  try {
    emitMutateResult(await removeAliasFromType(packName, pos[0]!, pos[1]!), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runAddPrefixCmd(args: string[]): Promise<void> {
  ...
  const pos = args.filter((a) => !a.startsWith("--"));
  if (pos.length < 2) {
    console.error("Usage: gbrain schema add-prefix <type> <prefix>");
    process.exit(2);
  }
  try {
    emitMutateResult(await addPrefixToType(packName, pos[0]!, pos[1]!), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runRemovePrefixCmd(args: string[]): Promise<void> {
  ...
  const pos = args.filter((a) => !a.startsWith("--"));
  if (pos.length < 2) {
    console.error("Usage: gbrain schema remove-prefix <type> <prefix>");
    process.exit(2);
  }
  try {
    emitMutateResult(await removePrefixFromType(packName, pos[0]!, pos[1]!), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runAddLinkTypeCmd(args: string[]): Promise<void> {
  ...
  if (!name) {
    console.error(
      "Usage: gbrain schema add-link-type <name> [--inverse <verb>] [--page-type <t>] [--target-type <t>]"
    );
    process.exit(2);
  }
  ...
  const inference =
    pageType || targetType ? { page_type: pageType, target_type: targetType } : undefined;
  try {
...
async function runRemoveLinkTypeCmd(args: string[]): Promise<void> {
  ...
  const name = args.filter((a) => !a.startsWith("--"))[0];
  if (!name) {
    console.error("Usage: gbrain schema remove-link-type <name>");
    process.exit(2);
  }
  try {
    emitMutateResult(await removeLinkTypeFromPack(packName, name), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runSetExtractableCmd(args: string[]): Promise<void> {
  ...
  const pos = args.filter((a) => !a.startsWith("--"));
  if (pos.length < 2) {
    console.error("Usage: gbrain schema set-extractable <type> <true|false>");
    process.exit(2);
  }
  const v = parseBool(pos[1]);
  if (v === null) {
    console.error("Second argument must be true|false");
    process.exit(2);
  }
  try {
    emitMutateResult(await setExtractableOnType(packName, pos[0]!, v), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runSetExpertRoutingCmd(args: string[]): Promise<void> {
  ...
  const pos = args.filter((a) => !a.startsWith("--"));
  if (pos.length < 2) {
    console.error("Usage: gbrain schema set-expert-routing <type> <true|false>");
    process.exit(2);
  }
  const v = parseBool(pos[1]);
  if (v === null) {
  ...
  try {
    emitMutateResult(await setExpertRoutingOnType(packName, pos[0]!, v), json);
  } catch (e) {
    handleMutationError(e);
  }
}
...
async function runScaffoldExtractableCmd(args: string[]): Promise<void> {
  ...
  if (pos.length < 1) {
    console.error(
      "Usage: gbrain schema scaffold-extractable <type> [--pack <name>] [--dims a,b,c] [--force]"
    );
    process.exit(2);
  }
  ...
  try {
    ...
    if (json) {
      ...
      console.log(
        `  2. Replace fixture placeholders in fixtures/extract/${typeName}.jsonl with real cases.`
      );
