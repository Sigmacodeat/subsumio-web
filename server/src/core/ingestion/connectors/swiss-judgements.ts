...
export class SwissJudgementsConnector extends BaseConnector {
      ...
      try {
        ...
        for (const result of data.results) {
          ...
          items.push({
            ...
            keywords: Array.isArray(result.keywords) ? (result.keywords as string[]).map(String) : [],
            text,
          });
        }
        ...
        this._ctx?.logger.warn(`[swiss-judgements] Fetch error: ${err instanceof Error ? err.message : String(err)}`);
        break;
      }
    }
  ...
  private async fetchDecisionText(decisionId: string): Promise<string> {
    try {
      ...
      const erwagungen = Array.isArray(data.erwagungen)
        ? (data.erwagungen as Array<Record<string, unknown>>).map(e => String(e.text ?? '')).join('\n\n')
  ...
  async toIngestionEvent(item: ConnectorItem): Promise<IngestionEvent> {
    ...
    const slugDate = j.decision_date ? j.decision_date.split('T')[0] : new Date().toISOString().split('T')[0];
