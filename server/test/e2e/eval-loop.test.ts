...
import { parseBaselineFile, serializeBaselineFile } from "../../src/core/bench/baseline-file.ts";
...
function withExitCapture<T>(
  fn: () => Promise<T>
): Promise<{ exitCode: number | null; result?: T }> {
...
beforeAll(async () => {
  ...
  await engine.putPage("people/alice-example", {
    ...
    title: "Alice Example",
    compiled_truth:
      "Alice is a fintech founder building payments infrastructure for emerging markets.",
...
describe("e2e LOOP: perturbed baseline → gate FAILS with named breach", () => {
  test("case 2: perturbed retrieved_slugs → jaccard drops → exit 1", async () => {
    ...
    try {
      ...
      const out = await withExitCapture(() => runEvalGate(engine, ["--baseline", baselinePath]));
...
describe("e2e LOOP: round-trip byte stability", () => {
  test("case 4: published baseline parses back byte-identically", async () => {
    const dir = mkdtempSync(join(tmpdir(), "eval-loop-"));
    try {
      const captured = await Promise.all([captureSearch("fintech"), captureSearch("alignment")]);
