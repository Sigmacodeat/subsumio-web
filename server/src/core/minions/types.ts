...
export interface SubagentHandlerData {
  ...
  /**
   * The web caller's document ACL groups (see core/matter-access.ts
   * `jobAclStamp`). "all" only for firm admins; absent on a user or firm job
   * = open pages only; malformed = open pages only.
   */
  _acl_groups?: string[] | "all";
  /**
