...
describe("validateFixturePath — D-EXTRACT-21 strict validation", () => {
  ...
  test("`..` traversal → REJECTED", () => {
    ...
    try {
      ...
      expect(() => validateFixturePath(packRoot, "fixtures/../../escape.jsonl")).toThrow(
        /'\.\.'\s*segment/
      );
    } finally {
      cleanup();
  ...
  test("null byte → REJECTED", () => {
    setup();
    try {
      expect(() => validateFixturePath(packRoot, "fixtures/extract/bad\0name.jsonl")).toThrow(
        /null byte/
      );
    } finally {
      cleanup();
  ...
  test("symlink pointing inside pack root → REJECTED", () => {
    ...
    try {
      ...
      expect(() => validateFixturePath(packRoot, "fixtures/extract/link.jsonl")).toThrow(
        /symbolic link/
      );
    } finally {
      cleanup();
  ...
  test("symlink pointing outside pack root → REJECTED at lstat check", () => {
    ...
    try {
      ...
      try {
        ...
        expect(() => validateFixturePath(packRoot, "fixtures/extract/escape.jsonl")).toThrow(
          /symbolic link/
        );
      } finally {
    ...
    const jsonl =
      [
        ...
        JSON.stringify({
          fixture_id: "a-002",
          page_body: "multi",
          expected_claims: [{ claim: "x" }, { claim: "y" }],
        }),
      ].join("\n") + "\n";
