...
import { writeReceiptToDb, writeReceipt } from "../../src/core/takes-quality-eval/receipt-write.ts";
...
beforeAll(async () => {
  ...
  await engine.executeRaw(
    `DELETE FROM eval_takes_quality_runs WHERE receipt_sha8_corpus LIKE 'e2e%'`
  );
});
  ...
  return {
    ...
    scores: {
      accuracy: {
        ...
        max: 8,
        scores: [8, 8],
        per_model: { "openai:gpt-4o": 8, "anthropic:claude-opus-4-7": 8 },
      },
    },
...
d("v0.32 EXP-5 — eval_takes_quality_runs on real Postgres", () => {
  ...
  test("writeReceiptToDb persists full receipt_json on Postgres", async () => {
    ...
    const json =
      typeof rows[0].receipt_json === "string"
        ? JSON.parse(rows[0].receipt_json)
        : rows[0].receipt_json;
  ...
  test("rubric_version segregation: distinct rubric_sha8 → distinct row (codex review #3)", async () => {
    const engine = getEngine();
    await writeReceiptToDb(
      engine,
      fixture({ corpus: "e2e_003", rubric_version: "v1.0", rubric_sha8: "rrrr1" })
    );
    await writeReceiptToDb(
      engine,
      fixture({ corpus: "e2e_003", rubric_version: "v2.0", rubric_sha8: "rrrr2" })
    );
