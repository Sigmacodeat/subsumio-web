...
describeE2E("E2E: gbrain dream CLI against real Postgres", () => {
  ...
  test("dream --phase orphans only reports orphans + no cycle-lock footprint", async () => {
    const conn = getConn();
    const before = await conn.unsafe(`SELECT COUNT(*)::int AS n FROM gbrain_cycle_locks`);
    ...
    const after = await conn.unsafe(`SELECT COUNT(*)::int AS n FROM gbrain_cycle_locks`);
