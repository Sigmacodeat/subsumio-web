        ...
        {queries.slice(0, 4).map((q, i) => {
          ...
          return (
            <button
              ...
              className="group flex w-full max-w-sm items-center gap-3 rounded-full border border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] px-4 py-2.5 text-left transition-[border-color,background-color] duration-[var(--ds-duration-normal)] hover:border-[color:var(--ds-border-strong)] hover:bg-[color:var(--ds-hover)] focus-visible:ring-2 focus-visible:ring-[color:var(--brand-primary)] focus-visible:outline-none active:scale-[0.98] motion-reduce:transition-none"
            >
              <Icon
                size={15}
                className="shrink-0 text-[color:var(--ds-text-subtle)] transition-[background-color,border-color,color] group-hover:text-[color:var(--brand-primary)] motion-reduce:transition-none"
              />
              <span className="flex-1 truncate text-[13px] leading-snug text-[color:var(--ds-text-muted)] transition-[background-color,border-color,color] group-hover:text-[color:var(--ds-text)] motion-reduce:transition-none">
                {q}
              </span>
              <ArrowUpRight
                size={13}
                className="shrink-0 text-[color:var(--ds-text-subtle)] opacity-0 transition-opacity duration-[var(--ds-duration-normal)] group-hover:opacity-100"
              />
            </button>
          );
        })}
