    ...
    await engine.addTakesBatch([
      {
        page_id: alicePageId,
        row_num: 2,
        claim: "Best technical founder in batch",
        kind: "take",
        holder: "garry",
        weight: 0.9,
      },
...
describe("updateTake", () => {
  ...
  test("TAKE_ROW_NOT_FOUND when row does not exist", async () => {
    await expect(engine.updateTake(alicePageId, 999, { weight: 0.5 })).rejects.toThrow(
      /TAKE_ROW_NOT_FOUND/
    );
  });
});
...
describe("resolveTake + immutability", () => {
    ...
    await engine.addTakesBatch([
      {
        page_id: alicePageId,
        row_num: 10,
        claim: "Series A within 12 months",
        kind: "bet",
        holder: "garry",
        weight: 0.6,
      },
  ...
  test("v0.30.0: contradictory quality + outcome throws TAKE_RESOLUTION_INVALID", async () => {
    ...
    await expect(
      engine.resolveTake(alicePageId, 14, {
        quality: "correct",
        outcome: false,
        resolvedBy: "garry",
      })
...
describe("v0.30.0 getScorecard", () => {
  ...
  test("counts correct/incorrect/partial; accuracy excludes partial", async () => {
    const card = await engine.getScorecard(
      { holder: "garry", domainPrefix: "companies/scorecard-fixture" },
      undefined
    );
  ...
  test("PRIVACY: SQL allow-list filter — hidden-holder rows contribute zero", async () => {
    ...
    const allowedGarry = await engine.getScorecard(
      { domainPrefix: "companies/allowlist-fixture" },
      ["garry"]
    );
  ...
  test("bins resolved bets by stated weight; partial excluded; harj non-allowed contributes zero", async () => {
    ...
    for (const b of garryAll) {
      ...
      if (b.predicted !== null) {
        ...
        expect(b.predicted).toBeLessThan(1.001);
      }
    }
  });
...
describe("synthesis_evidence", () => {
  test("addSynthesisEvidence persists provenance and CASCADE deletes when take is removed", async () => {
    ...
    const inserted = await engine.addSynthesisEvidence([
      {
        synthesis_page_id: synth.id,
        take_page_id: alicePageId,
        take_row_num: 2,
        citation_index: 1,
      },
    ]);
    ...
    await engine.executeRaw(`DELETE FROM takes WHERE page_id = $1 AND row_num = $2`, [
      alicePageId,
      2,
    ]);
