...
const BASE =
  process.env.SUBSUMIO_API_URL || process.env.SIGMABRAIN_API_URL || "http://localhost:3000";
const AUTH_HEADER =
  process.env.SUBSUMIO_API_KEY || process.env.SIGMABRAIN_API_KEY
...
async function testListQueries() {
  ...
  try {
    ...
    if (testCaseSlug && !cases.some((c) => c.slug === testCaseSlug)) {
      fail(
        step,
        `Created case ${testCaseSlug} missing from type=legal_case list — merge update lost the page type`
      );
      return;
    }
    ...
    if (
      ...
      !invoices.some((i) => i.slug === testInvoiceSlug)
    ) {
