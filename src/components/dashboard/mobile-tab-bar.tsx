...
export function MobileTabBar({
  ...
  const navigationSections = [
    ...
    ...section,
    items: section.items.filter((item) => !item.comingSoon && !DE_ONLY_HREFS.has(item.href)),
  }));
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={sheetTransition}
        role="dialog"
        aria-label={createOnly ? t("topbar.quick_create") : t("mobile.more_actions")}
        ...
        <div className="max-h-[85vh] overflow-y-auto rounded-t-2xl border-t border-[color:var(--ds-border)] bg-[color:var(--ds-surface)] pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl">
          ...
          <div className="flex items-center justify-between px-5 pt-1 pb-3">
            <h3 className="text-sm font-semibold text-[color:var(--ds-text)]">
              {createOnly ? t("topbar.quick_create") : t("mobile.more")}
            </h3>
            <button
              onClick={() => setMoreOpen(false)}
              className="flex h-11 w-11 items-center justify-center rounded-lg text-[color:var(--ds-text-muted)] transition-[background-color,color,transform] duration-[var(--ds-duration-normal)] ease-[var(--ds-ease-smooth)] hover:bg-[color:var(--ds-hover)] hover:text-[color:var(--ds-text)] focus-visible:ring-2 focus-visible:ring-[color:var(--brand-primary)] focus-visible:outline-none motion-reduce:transition-none"
          ...
          {/* Action grid */}
          <div className={cn("grid grid-cols-4 gap-1 px-3 pb-3", createOnly && "hidden")}>
            <MoreSheetButton
              icon={PanelRightOpen}
          ...
          <div className="border-b border-[color:var(--ds-border)] px-3 py-3">
            {createOnly && (
              <p className="mb-3 px-1 text-xs text-[color:var(--ds-text-muted)]">
                {t("mobile.quick_create_description")}
              </p>
            )}
          ...
          <div className={cn("max-h-[50vh] overflow-y-auto px-3 py-3", createOnly && "hidden")}>
      ...
      >
        ...
        <div className="relative h-1">
          <div
            className="brand-bg absolute top-0 h-1 transition-[width,left] duration-[var(--ds-duration-normal)] ease-[var(--ds-ease-smooth)] motion-reduce:transition-none"
            style={{
              width: `${100 / 6}%`,
          ...
          {tabs.map((tab) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "flex flex-1 flex-col items-center justify-center gap-0.5 py-2 transition-[color,transform] duration-[var(--ds-duration-normal)] ease-[var(--ds-ease-smooth)] motion-reduce:transition-none",
                  active
                    ? "brand-text"
...
function MoreSheetButton({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      className={cn(
        "flex flex-col items-center gap-1.5 rounded-xl px-2 py-3 transition-[background-color,color,transform] duration-[var(--ds-duration-normal)] ease-[var(--ds-ease-smooth)] motion-reduce:transition-none",
        active
