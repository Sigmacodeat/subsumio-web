...
const DIMENSIONS: Array<
  Omit<RubricDimensionResult, "passed" | "detail" | "fix_hint"> & {
    check: (
      input: RubricInput
    ) => Promise<{ passed: boolean; detail: string; fix_hint: string | null }>;
  }
> = [
  // ============================================================
  ...
  },
  {
    ...
    category: "core",
    description:
      "every listed skill has SKILL.md with valid frontmatter (name, description, triggers)",
    auto_fixable: false,
    check: async (input) => {
      ...
      for (const skillPath of input.manifest.skills) {
        ...
        try {
          ...
          if (missingFields.length > 0) {
            failures.push(
              `${skillPath}/SKILL.md missing frontmatter fields: ${missingFields.join(", ")}`
            );
          }
          if (
      ...
      return failures.length === 0
        ? {
            passed: true,
            detail: `all ${input.manifest.skills.length} skills have valid SKILL.md`,
            fix_hint: null,
          }
        : {
            passed: false,
  ...
  },
  {
    ...
    check: async (input) => {
      ...
      for (const skillPath of input.manifest.skills) {
        ...
        const lines = readFileSync(evalFile, "utf-8")
          .split("\n")
  ...
  },
  {
    ...
    check: async (input) => {
      ...
      for (const skillPath of input.manifest.skills) {
        ...
        try {
          const fm = parseMarkdown(readFileSync(skillMd, "utf-8")).frontmatter as Record<
            string,
            unknown
          >;
      ...
      return conflicts.length === 0
        ...
        : {
            passed: false,
            detail: conflicts
              .map(([t, skills]) => `"${t}" claimed by: ${skills.join(", ")}`)
              .join("; "),
            fix_hint:
  ...
  },
  {
    ...
    check: async (input) => {
      ...
      const versionEntryRe = new RegExp(
        `##\\s+\\[?${input.manifest.version.replace(/\./g, "\\.")}\\]?`
      );
      ...
      return {
        passed: true,
        detail: `CHANGELOG.md references ${input.manifest.version}`,
        fix_hint: null,
      };
    },
  },
    ...
    check: async (input) => {
      if (!input.manifest.unit_tests || input.manifest.unit_tests.length === 0) {
        return {
          ...
          fix_hint:
            'Add `"unit_tests": ["test/**/*.test.ts"]` to skillpack.json and a passing test in test/.',
        };
      }
      ...
      return found > 0
        ...
        : {
            ...
            fix_hint:
              "Create a test/<name>.test.ts (or matching glob) with at least one bun:test case.",
          };
    },
  },
  {
    ...
    check: async (input) => {
      if (!input.manifest.e2e_tests || input.manifest.e2e_tests.length === 0) {
        return {
          ...
          fix_hint:
            'Add `"e2e_tests": ["e2e/**/*.test.ts"]` and at least one integration test that exercises a full user journey.',
        };
      }
    ...
    check: async (input) => {
      if (!input.manifest.llm_evals || input.manifest.llm_evals.length === 0) {
        return {
          ...
          fix_hint:
            'Add `"llm_evals": ["evals/*.judge.json"]` and at least one *.judge.json with >= 3 cases.',
        };
      }
      ...
      if (evalFiles.length === 0) {
        return {
          ...
          fix_hint:
            "Create evals/<name>.judge.json with {task, output, cases:[...]} shape (cross-modal-eval format).",
        };
      }
      ...
      return {
        ...
        fix_hint:
          'Each *.judge.json must have a "cases" array with >= 3 items. Stubs are flagged as theater.',
      };
    },
  },
  {
    ...
    check: async (input) => {
      ...
      if (!path) {
        return {
          ...
          fix_hint:
            'Add `"runbooks": {"bootstrap": "runbooks/bootstrap.md"}` and write the file with post-scaffold steps.',
        };
      }
