...
describe("buildRecallCalibrationFooter — happy path", () => {
  ...
  test("Brier near-baseline range", () => {
    expect(buildRecallCalibrationFooter({ profile: buildProfile({ brier: 0.24 }) })).toContain(
      "(near baseline)"
    );
  });
  ...
  test("Brier strong calibration", () => {
    expect(buildRecallCalibrationFooter({ profile: buildProfile({ brier: 0.08 }) })).toContain(
      "(strong calibration)"
    );
  });
