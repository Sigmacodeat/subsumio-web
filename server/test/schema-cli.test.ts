...
function gbrain(
  ...
  const env = {
    ...
    // .env's DATABASE_URL merges over the fixture config.json in the child —
    // blank it so `engine: "pglite"` fixtures stay on the PGLite path.
    DATABASE_URL: "",
    ...extraEnv,
  };
...
describe("gbrain schema use (Phase C, gap-fill T3)", () => {
  ...
  test("preserves pre-existing config fields when writing schema_pack", () => {
    ...
    writeFileSync(
      cfgPath,
      JSON.stringify({ engine: "pglite", openai_key: "sk-fake" }, null, 2),
      "utf-8"
    );
    const r = gbrain(["schema", "use", "gbrain-base"], { GBRAIN_HOME: home });
    expect(r.code).toBe(0);
  ...
  test("overwrites prior schema_pack value on re-run", () => {
    ...
    writeFileSync(
      cfgPath,
      JSON.stringify({ engine: "pglite", schema_pack: "something-else" }, null, 2),
      "utf-8"
    );
    const r = gbrain(["schema", "use", "gbrain-base"], { GBRAIN_HOME: home });
    expect(r.code).toBe(0);
