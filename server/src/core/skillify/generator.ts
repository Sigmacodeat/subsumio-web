...
export class SkillifyScaffoldError extends Error {
  constructor(
    message: string,
    public code: "invalid_name" | "exists" | "no_resolver" | "write_failed"
  ) {
    super(message);
...
export function planScaffold(opts: ScaffoldOptions): ScaffoldPlan {
  ...
  const resolverFile = findResolverFile(skillsDir) ?? findResolverFile(dirname(skillsDir));
...
export function applyScaffold(plan: ScaffoldPlan): void {
  ...
  if (plan.resolverFile && plan.resolverAppend !== null) {
    const current = existsSync(plan.resolverFile) ? readFileSync(plan.resolverFile, "utf-8") : "";
