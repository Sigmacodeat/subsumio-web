...
describe("extractLinksFromFile — code-fence stripping (codex P2b)", () => {
  test("a bare wikilink inside a code fence does NOT create an FS edge", async () => {
    ...
    const links = await extractLinksFromFile(content, "concepts/x.md", allSlugs, {
      globalBasename: true,
    });
...
describe("resolveBasenameMatchesFromSlugs (pure)", () => {
  test("returns ALL slugs whose tail matches", () => {
    const all = new Set(["projects/struktura", "archive/struktura", "notes/other"]);
  ...
  test("case-insensitive tail match", () => {
    ...
    expect(resolveBasenameMatchesFromSlugs("Fast-Weigh", all)).toEqual(["companies/fast-weigh"]);
    expect(resolveBasenameMatchesFromSlugs("FAST-WEIGH", all)).toEqual(["companies/fast-weigh"]);
  });
...
describe("resolveSlugAll", () => {
  test("ancestor walk wins → single-element array, no basename fallback", () => {
    ...
    // would find others.
    const all = new Set(["notes/struktura", "archive/struktura", "projects/struktura"]);
    const out = resolveSlugAll("notes/sub", "struktura.md", all, { globalBasename: true });
  ...
  test("ancestor walk misses + globalBasename on → all basename matches", () => {
    const all = new Set(["projects/struktura", "archive/struktura"]);
    const out = resolveSlugAll("concepts", "struktura.md", all, { globalBasename: true });
    expect(out.sort()).toEqual(["archive/struktura", "projects/struktura"]);
  });

  test("zero basename matches when globalBasename on returns []", () => {
    const all = new Set(["projects/struktura"]);
    const out = resolveSlugAll("concepts", "phantom.md", all, { globalBasename: true });
    expect(out).toEqual([]);
  });
  ...
  test("strips dirname from relTarget when applying basename lookup", () => {
    ...
    expect(resolveSlugAll("concepts", "notes/struktura.md", all)).toEqual(["notes/struktura"]);
  });
