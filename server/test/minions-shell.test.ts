...
import {
  ...
  resolveAuditDir,
  logShellSubmission,
} from "../src/core/minions/handlers/shell-audit.ts";
...
describe("MinionQueue.add protected-name guard", () => {
  ...
  test('add("shell", ..., opts, {allowProtectedSubmit:true}) succeeds', async () => {
    const job = await queue.add("shell", { cmd: "echo", cwd: "/tmp" }, undefined, {
  ...
  test('add(" shell ", ...) without trusted arg throws (whitespace bypass defense)', async () => {
    expect(queue.add(" shell ", { cmd: "echo", cwd: "/tmp" })).rejects.toThrow(
      /protected job name/
    );
  });
  ...
  test("REGRESSION: trusted flag does NOT bypass empty-name check", async () => {
    expect(queue.add("", {}, undefined, { allowProtectedSubmit: true })).rejects.toThrow(
      /cannot be empty/
    );
  });
});
...
describe("shell handler: abort", () => {
  test("ctx.signal.abort triggers SIGTERM and handler throws aborted", async () => {
    ...
    const promise = shellHandler(makeCtx({ cmd: "sleep 30", cwd: "/tmp" }, { signal: ac.signal }));
  ...
  test("ctx.shutdownSignal.abort also triggers kill", async () => {
    ...
    const promise = shellHandler(
      makeCtx({ cmd: "sleep 30", cwd: "/tmp" }, { shutdownSignal: shutdownCtl.signal })
    );
...
describe("shell-audit: computeAuditFilename", () => {
  test("2027-01-01 is ISO week 53 of 2026", () => {
    expect(computeAuditFilename(new Date("2027-01-01T12:00:00Z"))).toBe(
      "shell-jobs-2026-W53.jsonl"
    );
  });
  test("2026-12-28 (Monday) is ISO week 53 of 2026", () => {
    expect(computeAuditFilename(new Date("2026-12-28T12:00:00Z"))).toBe(
      "shell-jobs-2026-W53.jsonl"
    );
  });
  test("2027-01-04 (Monday) is ISO week 1 of 2027", () => {
    expect(computeAuditFilename(new Date("2027-01-04T12:00:00Z"))).toBe(
      "shell-jobs-2027-W01.jsonl"
    );
  });
