...
describeE2E("sources-remote-mcp E2E (gstack /setup-gbrain Path 4)", () => {
  ...
  beforeAll(async () => {
    ...
    if (!readOnlyClientId || !readOnlySecret)
      throw new Error("Failed to register read-only client");
  ...
  test("sources_add via MCP: clones, INSERTs, returns row with remote_url", async () => {
    ...
    expect(existsSync(join(GBRAIN_HOME, ".gbrain", "clones", "e2e-yc-artifacts", ".git"))).toBe(
      true
    );
  });
