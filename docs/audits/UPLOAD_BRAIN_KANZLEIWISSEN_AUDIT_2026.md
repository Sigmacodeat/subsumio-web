...
| R2  | **Matter-Scope nur in Search/Think**                                      | Anwalt A kann Anwalt Bs Akte über `/api/legal/analyze`, `/api/pages` oder Upload-Response sehen.        | `verifiedMatterScope` wird nur in `app.get("/api/search")` und `app.post("/api/think")` aufgerufen.                                                                                                            |
...
| **Matter-Scope endpoint-übergreifend**   | M       | R2     | Middleware/Wrapper, die `verifiedMatterScope` für alle `/api/*`-Routes erzwingt.                           |
