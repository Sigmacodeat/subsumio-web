...
function getReq() {
  return new NextRequest("http://localhost/api/legal/templates");
}

describe("GET /api/legal/templates", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubGlobal("fetch", fetchMock);
  });

  it("returns the template list inside the success envelope", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify([{ slug: "legal/templates/a", frontmatter: {} }]), {
        status: 200,
      })
    );
  ...
  it("returns 503 on a network failure too", async () => {
