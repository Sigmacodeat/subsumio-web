...
export function validateRegistryCatalog(raw: unknown): RegistryCatalog {
  ...
  if (typeof obj.updated_at !== "string") {
    throw new RegistrySchemaError("registry.json.updated_at must be a string", "invalid_field", {
      field: "updated_at",
    });
  }
  if (!Array.isArray(obj.skillpacks)) {
    throw new RegistrySchemaError("registry.json.skillpacks must be an array", "invalid_field", {
      field: "skillpacks",
    });
  }
...
export function validateRegistryEntry(raw: unknown): RegistryEntry {
  ...
  if (
    typeof e.skills_count !== "number" ||
    !Number.isFinite(e.skills_count) ||
    e.skills_count < 0
  ) {
    throw new RegistrySchemaError(
...
export function validateEndorsementsFile(raw: unknown): EndorsementsFile {
  ...
  for (const [name, record] of Object.entries(obj.endorsements)) {
    if (typeof record !== "object" || record === null || Array.isArray(record)) {
      throw new RegistrySchemaError(`endorsements.${name} must be an object`, "invalid_field", {
        field: name,
      });
    }
