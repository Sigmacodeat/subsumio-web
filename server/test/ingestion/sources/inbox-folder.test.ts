...
describe("InboxFolderSource — startup", () => {
  test("requires inboxDir", () => {
    expect(() => createInboxFolderSource({ inboxDir: "" as never })).toThrow(
      /inboxDir is required/
    );
  });
  ...
  test("warns when inbox dir is world-writable", async () => {
    ...
    stub.fireReady();
    await startPromise;
    expect(harness.logs.some((l) => l.level === "warn" && l.msg.includes("world-writable"))).toBe(
      true
    );
    await harness.stop();
  });
...
describe("InboxFolderSource — file ingestion", () => {
  ...
  test("symlink is rejected and logged", async () => {
    ...
    expect(harness.logs.some((l) => l.level === "warn" && l.msg.includes("rejected symlink"))).toBe(
      true
    );
    await harness.stop();
  });
