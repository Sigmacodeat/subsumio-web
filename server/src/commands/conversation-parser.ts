...
export async function runConversationParser(
  ...
  if (sub === "scan") {
    if (!engine) {
      process.stderr.write("[conversation-parser scan] requires a connected brain.\n");
      process.exit(2);
    }
  ...
  process.stderr.write(`[conversation-parser] unknown subcommand: ${sub}. See --help.\n`);
  process.exit(2);
}
...
function runListBuiltins(json: boolean): void {
  ...
  process.stdout.write(`[conversation-parser] ${BUILTIN_PATTERNS.length} built-in patterns:\n\n`);
...
function runValidate(args: string[], json: boolean): void {
  ...
  if (!file) {
    process.stderr.write("[conversation-parser validate] USAGE: missing file path.\n");
    process.exit(2);
  }
  if (!existsSync(file)) {
    process.stderr.write(`[conversation-parser validate] USAGE: file not found: ${file}\n`);
    process.exit(2);
  }
  ...
  if (json) {
    ...
    process.stdout.write("[conversation-parser validate] " + payload.message + "\n");
  }
}

async function runScan(engine: BrainEngine, args: string[], json: boolean): Promise<void> {
  const slug = args.find((a) => !a.startsWith("--"));
  if (!slug) {
    process.stderr.write("[conversation-parser scan] USAGE: missing page slug.\n");
    process.exit(2);
  }
  const page = await engine.getPage(slug);
  if (!page) {
    process.stderr.write(`[conversation-parser scan] page not found: ${slug}\n`);
    process.exit(2);
  }
  ...
  if (result.messages.length > 0) {
    ...
    for (const m of result.messages.slice(0, 3)) {
      process.stdout.write(`    - ${m.speaker} @ ${m.timestamp}: ${m.text.slice(0, 80)}\n`);
    }
  }
}
