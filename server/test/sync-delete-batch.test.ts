...
async function seedPageWithPath(
  slug: string,
  sourcePath: string,
  sourceId = "default"
): Promise<number> {
...
describe("engine.deletePages (single-batch primitive)", () => {
  ...
  test("returns confirmed-deleted slugs (D6)", async () => {
    ...
    const deleted = await engine.deletePages(["test/dp1", "test/dp2", "test/ghost-never-existed"], {
      sourceId: "default",
    });
...
describe("engine.resolveSlugsByPaths (single-batch primitive)", () => {
  ...
  test("D10 exotic filename fallback substrate: frontmatter-slug rows resolvable", async () => {
    ...
    const m = await engine.resolveSlugsByPaths(["🌟.md", "ทดสอบ.md", "عربي.md"], {
      sourceId: "default",
    });
