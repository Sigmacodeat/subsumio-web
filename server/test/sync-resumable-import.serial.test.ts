...
import { loadOpCheckpoint, recordCompleted, syncFingerprint } from "../src/core/op-checkpoint.ts";
...
describe("#1794 — resumable incremental sync (pinned target)", () => {
  ...
  test("[CRITICAL] killed mid-import banks progress; next run converges", async () => {
    ...
    const poll = setInterval(() => {
      loadOpCheckpoint(engine, { op: "sync", fingerprint: fp })
        .then((paths) => {
          if (paths.length >= 1 && !ac.signal.aborted) ac.abort();
        })
        .catch(() => {});
    }, 1);
    const aborted = await performSync(engine, {
      repoPath,
      noPull: true,
      noEmbed: true,
      signal: ac.signal,
  ...
  test("[Codex #2/#5] blocked sync leaves last_sync_at + last_commit unchanged; good file banked", async () => {
    ...
    const sid = "srcE";
    await engine.executeRaw(`INSERT INTO sources (id, name, local_path) VALUES ($1, $2, $3)`, [
      sid,
      sid,
      repoPath,
    ]);
    ...
    const beforeRows = await engine.executeRaw<{
    ...
    const afterRows = await engine.executeRaw<{
      last_commit: string | null;
      last_sync_at: string | null;
    }>(`SELECT last_commit, last_sync_at FROM sources WHERE id = $1`, [sid]);
  ...
  test("[Codex #3] vanished-on-disk added file is skipped, not a failure", async () => {
    ...
    const c1 = commitPages(
      repoPath,
      { "keep.md": pageMd("Keep"), "gone.md": pageMd("Gone") },
      "two pages"
    );
...
describe("#1794 pure helpers", () => {
  ...
  test("clampWorkersForConnectionBudget — no-op when budget unset", () => {
    expect(clampWorkersForConnectionBudget(4, { parentPool: 10, perWorkerPool: 2 })).toEqual({
      workers: 4,
      clamped: false,
    });
  });
  ...
  test("clampWorkersForConnectionBudget — clamps to fit budget", () => {
    ...
    expect(
      clampWorkersForConnectionBudget(4, { maxConnections: 16, parentPool: 10, perWorkerPool: 2 })
    ).toEqual({ workers: 3, clamped: true });
    // already fits → unchanged.
    expect(
      clampWorkersForConnectionBudget(2, { maxConnections: 16, parentPool: 10, perWorkerPool: 2 })
    ).toEqual({ workers: 2, clamped: false });
    // parent pool eats the whole budget → fall back to serial (1).
    expect(
      clampWorkersForConnectionBudget(4, { maxConnections: 10, parentPool: 10, perWorkerPool: 2 })
