...
async function runTest(args: string[]): Promise<void> {
  ...
  if (modelArg) {
    ...
    try {
      ...
      /* loadConfig throws when no brain configured — first-time install path; the no-config branch above handles it. */
    }
  ...
  if (!gwIsAvailable(tpArg)) {
    console.error(
      `${tpArg[0]?.toUpperCase()}${tpArg.slice(1)} provider not configured or not ready. Run \`gbrain providers list\` to see status.`
    );
    process.exit(1);
  }
  ...
  try {
    if (tpArg === "embedding") {
      ...
      console.log(
        `  ✓ ${ms}ms · model=${result.model} · stop=${result.stopReason} · in=${result.usage.input_tokens}/out=${result.usage.output_tokens} · "${preview}"`
      );
    }
    console.log("\nAll probes green.");
  } catch (e) {
...
async function runExplain(args: string[]): Promise<void> {
  ...
  for (const [k, v] of Object.entries(env_detected))
    console.log(`  ${k.padEnd(32)} ${v ? "✓ set" : "✗ not set"}`);
  console.log(
    `  Ollama @ ${matrix.local_probes.ollama.url}  ${matrix.local_probes.ollama.models_endpoint_valid ? "✓ reachable" : "✗ not detected"}`
  );
  console.log("");
  ...
  for (const o of options.filter((x) => x.touchpoint === "embedding")) {
    ...
    console.log(
      `  ${o.env_ready ? "✓" : "✗"} ${o.id.padEnd(44)} ${dims.padEnd(8)} ${cost.padEnd(10)} ${o.tier}`
    );
  }
  console.log("");
  ...
  for (const o of options.filter((x) => x.touchpoint === "chat")) {
    ...
    const outCost =
      o.cost_per_1m_output_usd !== undefined ? `out $${o.cost_per_1m_output_usd}` : "—";
    console.log(
      `  ${o.env_ready ? "✓" : "✗"} ${o.id.padEnd(44)} ${inCost.padEnd(12)} ${outCost.padEnd(12)} ${o.tier}`
    );
  }
  console.log("");
  ...
  console.log(
    `  gbrain init --embedding-model ${matrix.recommended.split(":")[0]}:${matrix.recommended.split(":").slice(1).join(":")}`
  );
}
...
function prosFor(r: Recipe, touchpoint: TouchpointFilter): string[] {
  ...
  if (touchpoint === "chat") {
    if (r.id === "anthropic")
      out.push("Default subagent driver", "Prompt-cache support", "Strong tool calling");
...
function pickRecommended(
  ...
  if (env.GOOGLE_GENERATIVE_AI_API_KEY) {
    ...
    if (google)
      return {
        id: google.id,
        reason: "GOOGLE_GENERATIVE_AI_API_KEY set — Gemini embedding at 768 dims.",
      };
  }
  ...
  return {
    ...
    reason:
      "No provider env detected. OpenAI is the fastest setup — get a key at https://platform.openai.com/api-keys.",
  };
}
