...
async function seedPerson(slug: string, title: string, topic: string, embeddingIdx: number) {
...
async function seedCompany(slug: string, title: string, topic: string, embeddingIdx: number) {
  await engine.putPage(slug, {
    type: "company",
    title,
...
async function seedConcept(slug: string, title: string, topic: string, embeddingIdx: number) {
  await engine.putPage(slug, {
    type: "concept",
    title,
...
describe("whoknows E2E — quality gate on synthetic fixture", () => {
    ...
    const inlineFixture = [
      {
        ...
        expected: ["wiki/people/example-alice", "wiki/companies/example-fintech-co"],
      },
      {
        query: "crypto investing",
        expected: ["wiki/companies/example-fund", "wiki/people/example-bob"],
      },
      ...
      { query: "distributed systems", expected: ["wiki/people/example-dave"] },
      {
        ...
        expected: ["wiki/companies/example-health-co", "wiki/people/example-eve"],
      },
      {
        query: "developer tools",
        expected: ["wiki/people/example-frank", "wiki/companies/example-devtools-co"],
      },
      { query: "machine learning research", expected: ["wiki/people/example-grace"] },
      {
        query: "climate tech",
        expected: ["wiki/companies/example-climate-co", "wiki/people/example-hank"],
      },
      { query: "enterprise sales", expected: ["wiki/people/example-ivy"] },
      {
        query: "hardware engineering",
        expected: ["wiki/people/example-jake", "wiki/companies/example-hardware-co"],
      },
    ];
  ...
  test("shipped fixture at test/fixtures/whoknows-eval.jsonl loads and parses", () => {
    ...
    const fixture = readFixture(`${process.cwd()}/test/fixtures/whoknows-eval.jsonl`);
