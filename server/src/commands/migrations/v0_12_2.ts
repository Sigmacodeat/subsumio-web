...
function phaseCVerify(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  try {
    ...
    return {
      name: "verify",
      status: "complete",
      detail: parsed.engine ? `engine=${parsed.engine}` : undefined,
    };
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
...
export const v0_12_2: Migration = {
  ...
  featurePitch: {
    headline:
      "Postgres frontmatter queries now work — JSONB double-encode bug fixed and existing rows auto-repaired",
    description:
