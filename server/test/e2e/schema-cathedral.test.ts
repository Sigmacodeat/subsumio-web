...
describe("v0.39 T22a — custom-pack across consumers", () => {
  test("parseMarkdown with custom pack types files correctly", () => {
    ...
    expect(parseMarkdown("x", "Projects/foo.md", { activePack: customPack }).type).toBe(
      "project-x"
    );
    expect(parseMarkdown("x", "Reading/bar.md", { activePack: customPack }).type).toBe(
      "reading-note"
    );
  });
...
describe("v0.39 T22b — federated_read 2-source pack-divergence", () => {
  test("SchemaPackTrustGateError fires when 2 sources resolve different packs", async () => {
    ...
    const { SchemaPackTrustGateError } =
      await import("../../src/core/schema-pack/op-trust-gate.ts");
...
describe("v0.39 T22d — artifact-type routing", () => {
  ...
  test("validateManifestByKind rejects cross-kind manifests", () => {
    expect(() =>
      validateManifestByKind("schemapack", { api_version: "gbrain-skillpack-v1" })
    ).toThrow();
    expect(() =>
      validateManifestByKind("skillpack", { api_version: "gbrain-schema-pack-v1" })
    ).toThrow();
  });
});
