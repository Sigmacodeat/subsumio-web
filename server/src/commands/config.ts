...
export async function runConfig(engine: BrainEngine, args: string[]) {
  ...
  if (action === "get" && key) {
    ...
    if (!forceFlag) {
      ...
      if (!isKnown && !matchesPrefix) {
        console.error(
          `[config] WARN: writing unknown key "${key}" with --force. Nothing in gbrain reads this.`
        );
      }
    }
    ...
    if (key === "embedding_columns") {
      try {
        ...
        if (err instanceof EmbeddingColumnConfigError) {
          ...
          console.error(`[config] embedding_columns rejected: ${(err as Error).message}`);
          console.error(
      ...
      if (mergedCfg) {
        ...
        try {
          ...
          if (total > 0 && pct < 90 && !coverageOverride) {
            ...
            console.error(`[config] Re-run with --coverage-override (or --yes) to proceed anyway:`);
            console.error(
