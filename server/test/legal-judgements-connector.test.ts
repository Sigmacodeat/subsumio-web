      ...
      OgdDocumentReference: [
        {
          Data: {
            Metadaten: {
              Technisch: { ID: 'JJR_19790330_OGH0002_0010OB00010_7900000_005', Applikation: 'Justiz', Organ: 'OGH' },
              Allgemein: {
                Veroeffentlicht: '1997-06-15',
                DokumentUrl: 'https://www.ris.bka.gv.at/Dokument.wxe?Abfrage=Justiz&Dokumentnummer=JJR_X',
              },
              Judikatur: {
...
describe('extractRisReferences', () => {
  ...
  it('wraps a single (non-array) hit into an array', () => {
    const single = {
      OgdSearchResult: {
        OgdDocumentResults: {
          OgdDocumentReference: (RIS_SAMPLE.OgdSearchResult.OgdDocumentResults.OgdDocumentReference)[0],
        },
      },
    };
...
describe('stripHtml', () => {
  it('converts simple HTML to readable text', () => {
    const text = stripHtml('<p>Erster Absatz</p><p>Zweiter &amp; dritter</p><script>alert(1)</script>');
