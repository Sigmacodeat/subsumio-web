...
class CartesiaTtsAdapter extends EventEmitter {
  constructor({
    ...
    voiceId = "794f9389-aac1-45b6-b726-9d9369183238" /* "professional" default */,
    outputFormat = { container: "raw", encoding: "pcm_s16le", sample_rate: 16000 },
  }) {
    super();
    this.apiKey = apiKey;
...
export function createPipeline({
  ...
  if (sttProvider === "deepgram") {
    if (!process.env.DEEPGRAM_API_KEY)
      throw new Error("DEEPGRAM_API_KEY required for Deepgram STT");
    stt = new DeepgramSttAdapter({
      apiKey: process.env.DEEPGRAM_API_KEY,
      vadPreset: vadPresetChosen,
    });
  } else {
  ...
  if (llmProvider === "claude") {
    if (!process.env.ANTHROPIC_API_KEY)
      throw new Error("ANTHROPIC_API_KEY required for Claude LLM");
    llm = new ClaudeStreamAdapter({
  ...
  if (ttsProviderChosen === "cartesia") {
    if (!process.env.CARTESIA_API_KEY)
      throw new Error("CARTESIA_API_KEY required for Cartesia TTS");
    tts = new CartesiaTtsAdapter({ apiKey: process.env.CARTESIA_API_KEY });
  } else if (ttsProviderChosen === "openai") {
    if (!process.env.OPENAI_API_KEY)
      throw new Error("OPENAI_API_KEY required for OpenAI TTS fallback");
