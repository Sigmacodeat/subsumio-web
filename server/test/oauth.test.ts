...
describe("client credentials", () => {
  ...
  beforeAll(async () => {
    ...
    if (!result.clientSecret)
      throw new Error("test bug: expected confidential client to have secret");
  ...
  test("client without CC grant is rejected", async () => {
    ...
    await expect(provider.exchangeClientCredentials(noCC, "any-secret", "read")).rejects.toThrow(
      "not authorized"
    );
  });
...
describe("sweepExpiredTokens", () => {
  test("removes expired tokens", async () => {
    ...
    // Verify they're gone
    const remaining =
      await sql`SELECT count(*)::int as count FROM oauth_tokens WHERE expires_at < 100`;
...
describe("operation scope annotations", () => {
  test("all operations have a scope", () => {
    ...
    for (const op of operations) {
      ...
      expect(["read", "write", "admin", "sources_admin", "users_admin", "agent"]).toContain(
        op.scope
      );
    }
  });
...
describe("F1/F4 cross-client isolation", () => {
  ...
  test("wrong client cannot read another client PKCE challenge", async () => {
    const { clientId: ownerId } = await provider.registerClientManual(
      "challenge-owner-test",
      ["authorization_code"],
      "read",
      ["http://localhost:3000/callback"]
    );
    const { clientId: attackerId } = await provider.registerClientManual(
      "challenge-attacker-test",
      ["authorization_code"],
      "read",
      ["http://localhost:3000/callback"]
    );
    ...
    await expect(provider.challengeForAuthorizationCode!(owner, code)).resolves.toBe(
      "owner-challenge"
    );
  });
...
describe("F7c redirect_uri binding on auth code exchange", () => {
  ...
  test("mismatched redirect_uri rejects", async () => {
    ...
    await expect(
      provider.exchangeAuthorizationCode(client, code, undefined, "https://attacker.example/cb")
    ).rejects.toThrow();
  });
...
describe("v0.41.3 validateTokenEndpointAuthMethod", () => {
  ...
  test("unknown method throws InvalidTokenEndpointAuthMethodError", () => {
    expect(() => validateTokenEndpointAuthMethod("frobnicate")).toThrow(
      InvalidTokenEndpointAuthMethodError
    );
  });
  ...
  test("non-string input throws", () => {
    expect(() => validateTokenEndpointAuthMethod(123 as any)).toThrow(
      InvalidTokenEndpointAuthMethodError
    );
    expect(() => validateTokenEndpointAuthMethod({} as any)).toThrow(
      InvalidTokenEndpointAuthMethodError
    );
    expect(() => validateTokenEndpointAuthMethod([] as any)).toThrow(
      InvalidTokenEndpointAuthMethodError
