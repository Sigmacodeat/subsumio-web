...
function mkReport(
  ...
  return {
    ...
    source_tier_breakdown: {
      ...
      curated_vs_bulk: findings.length,
      bulk_vs_bulk: 0,
      other: 0,
    },
    per_query:
...
describe("M3 find_contradictions MCP op", () => {
  ...
  test("returns findings from latest run", async () => {
        ...
        findings: [
          {
            severity: "high",
            axis: "MRR figure",
            slugA: "companies/acme",
            slugB: "openclaw/chat/x",
          },
    ...
    const op = operationsByName["find_contradictions"];
    const result = (await op.handler(mkCtx(), {})) as {
  ...
  test("severity filter narrows results", async () => {
    ...
    const op = operationsByName["find_contradictions"];
    const result = (await op.handler(mkCtx(), { severity: "high" })) as {
  ...
  test("limit caps result count", async () => {
    ...
    const op = operationsByName["find_contradictions"];
    const result = (await op.handler(mkCtx(), { limit: 5 })) as {
      contradictions: unknown[];
      total_in_run: number;
    };
