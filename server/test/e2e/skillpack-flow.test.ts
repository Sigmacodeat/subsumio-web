...
describe("skillpack flow (E2E)", () => {
  ...
  test("4. reference --apply-clean-hunks applies upstream change to local workspace", () => {
    ...
    const r0 = spawnSync(
      GBRAIN_CMD,
      [...GBRAIN_ARGS, "skillpack", "scaffold", "apply-demo", "--workspace", ws],
      {
        cwd: gbrainRoot,
        encoding: "utf-8",
...
`
  ...
  test("8. harvest --no-lint bypasses the privacy linter", () => {
    ...
    const r = spawnSync(
      GBRAIN_CMD,
      [...GBRAIN_ARGS, "skillpack", "harvest", "bypass-test", "--from", hostRoot, "--no-lint"],
      { cwd: gbrainRoot, encoding: "utf-8", env: { ...process.env, OPENCLAW_WORKSPACE: "" } }
    );
    expect(r.status).toBe(0);
  ...
  test("14. --apply-clean-hunks prints two-way WARNING to STDERR, not stdout", () => {
    ...
    spawnSync(
      GBRAIN_CMD,
      [...GBRAIN_ARGS, "skillpack", "scaffold", "warn-demo", "--workspace", ws],
      {
        cwd: gbrainRoot,
        encoding: "utf-8",
        env: { ...process.env, OPENCLAW_WORKSPACE: "" },
  ...
  test("15. --apply-clean-hunks --json does NOT print the WARNING (machine mode)", () => {
    ...
    spawnSync(
      GBRAIN_CMD,
      [...GBRAIN_ARGS, "skillpack", "scaffold", "json-demo", "--workspace", ws],
      {
        cwd: gbrainRoot,
        encoding: "utf-8",
        env: { ...process.env, OPENCLAW_WORKSPACE: "" },
