...
describe("classifyEvidence precedence", () => {
  test("alias_hit wins over everything", () => {
    expect(
      classifyEvidence(r({ alias_hit: true, title_match_boost: 1.25, base_score: 0.99 }))
    ).toBe("alias_hit");
  });
  test("exact_title_match when title boost fired", () => {
    expect(classifyEvidence(r({ title_match_boost: 1.25, base_score: 0.2 }))).toBe(
      "exact_title_match"
    );
  });
  test("high_vector_match at/above HIGH_MATCH_FLOOR", () => {
    ...
    expect(classifyEvidence(r({ base_score: 0.9 }))).toBe("high_vector_match");
  });
