...
export const POST = createHandler(
  ...
  async (_ctx, body, _query, _req) => {
    ...
    if (!token || !phoneNumberId) {
      return apiError("whatsapp_not_configured", "WhatsApp ist nicht konfiguriert (WHATSAPP_ACCESS_TOKEN oder WHATSAPP_PHONE_NUMBER_ID fehlt)", 503);
    }

    const allowedSenders = loadAllowedSenders();
    if (allowedSenders.length === 0) {
      return apiError("no_allowed_senders", "Keine erlaubten WhatsApp-Teilnehmer konfiguriert", 403);
    }
    ...
    try {
      switch (body.type) {
        ...
        case "template": {
          ...
          return Response.json({ ok: true, type: "template", messageId: result.messageId, sentTo: normalizedTo.slice(-4) });
        }
        case "interactive": {
          if (!body.interactive) return apiError("interactive_required", "Interactive message fehlt", 400);
          const result = await sendWhatsAppInteractive(normalizedTo, body.interactive as never);
          return Response.json({ ok: true, type: "interactive", messageId: result.messageId, sentTo: normalizedTo.slice(-4) });
        }
        case "media": {
          ...
          return Response.json({ ok: true, type: "media", messageId: result.messageId, sentTo: normalizedTo.slice(-4) });
        }
        case "flow": {
          ...
          return Response.json({ ok: true, type: "flow", messageId: result.messageId, sentTo: normalizedTo.slice(-4) });
        }
        default:
      ...
      return apiError("send_failed", err instanceof Error ? err.message : "WhatsApp-Nachricht konnte nicht gesendet werden", 502);
    }
  },
);
