        ...
        {rows.map((row) => {
          ...
          return (
            <div key={row.label}>
              ...
              >
                <div
                  className={`h-full rounded-full transition-[background-color,border-color,color,box-shadow,opacity,transform] duration-[var(--ds-duration-normal)] ease-[cubic-bezier(0.32,0.72,0,1)] motion-reduce:transition-none ${warn ? "bg-[color:var(--ds-warning-solid)]" : "brand-soft"}`}
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
