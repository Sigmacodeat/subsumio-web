...
describe("extractCodeRefs — basic patterns", () => {
  ...
  test("dedups by path", () => {
    const refs = extractCodeRefs("first src/foo.ts mention, second src/foo.ts, third src/foo.ts");
    expect(refs.length).toBe(1);
    expect(refs[0]!.path).toBe("src/foo.ts");
  });

  test("different paths coexist", () => {
    const refs = extractCodeRefs("see src/a.ts and src/b.ts and lib/c.py");
