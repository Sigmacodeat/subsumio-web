...
describe("buildSourceFactorCase", () => {
  ...
  test("sorts prefixes by length descending so longest-match wins", () => {
    ...
    expect(m).toEqual(["LIKE 'media/articles/%'", "LIKE 'media/x/%'", "LIKE 'media/%'"]);
  });
  ...
  test("uses the supplied slug column reference", () => {
    expect(buildSourceFactorCase("slug", { "originals/": 1.5 }, "medium")).toContain(
      "WHEN slug LIKE"
    );
  });
});
...
describe("parseSourceBoostEnv", () => {
  test("parses comma-separated prefix:factor pairs", () => {
    expect(parseSourceBoostEnv("originals/:1.8,openclaw/chat/:0.3")).toEqual({
      "originals/": 1.8,
      "openclaw/chat/": 0.3,
    });
  });
  ...
  test("skips malformed entries", () => {
    expect(parseSourceBoostEnv("bogus,no-colon,originals/:abc,valid/:1.5")).toEqual({
      "valid/": 1.5,
    });
  });
...
describe("parseHardExcludesEnv", () => {
  test("parses comma-separated prefixes", () => {
    expect(parseHardExcludesEnv("test/,scratch/,private/")).toEqual([
      "test/",
      "scratch/",
      "private/",
    ]);
