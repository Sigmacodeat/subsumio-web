...
export default function RagOptimizerDashboardPage() {
  ...
  const { data: history, refetch: refetchHistory } = useQuery<{ runs: Run[] }>({
  ...
  const { data: active, refetch: refetchActive } = useQuery<{ active: Run | null }>({
  ...
  const { data: queue, refetch: refetchQueue } = useQuery<{ queue: QueueItem[] }>({
