...
export type UrlResult = { ok: true; url: string; warning?: string } | { ok: false; error: string };
...
export function normalizeMcpUrl(input: string): UrlResult {
  ...
  if (!raw) {
    return {
      ok: false,
      error: "Missing MCP URL. Usage: gbrain connect <https://host/mcp> --token <bearer>",
    };
  }
  ...
  if (!/^[a-z][a-z0-9+.-]*:\/\//i.test(raw)) {
    ...
    return {
      ok: false,
      error: `Add an explicit scheme, e.g. https://${guess} (a bare host:port is ambiguous).`,
    };
  }
  let u: URL;
  try {
  ...
  if (u.username || u.password) {
    return {
      ok: false,
      error:
        "Remove credentials from the URL (user:pass@host is not supported); pass the token via --token.",
    };
  }
  if (u.search) {
  ...
  if (isLinkLocalOrMetadata(u.hostname)) {
    return {
      ok: false,
      error: `Refusing to target a link-local / cloud-metadata address (${u.hostname}). Point the MCP URL at the brain host's real address.`,
    };
  }
  ...
  if (scheme === "http:" && !isLocal) {
    return {
      ...
      url,
      warning:
        "Warning: http:// sends your bearer token unencrypted. Use https:// unless this is localhost.",
    };
  }
  return { ok: true, url };
}
...
export function validateToken(token: string): TokenValidation {
  ...
  if (/\s/.test(token))
    return {
      ok: false,
      error: "Token contains whitespace (space/tab/newline) — refusing (header-injection risk).",
    };
  if (/[\x00-\x1f\x7f]/.test(token))
    return {
      ok: false,
      error: "Token contains control characters — refusing (header-injection risk).",
    };
  return { ok: true };
}
...
export function buildClaudeMcpAddArgv(p: {
  ...
  return [
    ...
    p.url,
    "-H",
    `Authorization: Bearer ${p.headerToken}`,
  ];
}
...
function claudeBlock(p: { name: string; url: string; token: string | null }): string {
  ...
  if (!p.token)
    lines.push(
      `Replace ${PLACEHOLDER_TOKEN} with a token from \`gbrain auth create "claude-code"\` on the host.`,
      ""
    );
...
function codexBlock(p: { name: string; url: string; token: string | null }): string {
  ...
  const cmd = cmdString(
    "codex",
    buildCodexMcpAddArgv({ name: p.name, url: p.url, envVar: ENV_VAR })
  );
  const lines = [
    "# Paste into Codex:",
    "",
  ...
  if (!p.token)
    lines.push(
      `Replace ${PLACEHOLDER_TOKEN} with a token from \`gbrain auth create "codex"\` on the host.`,
      ""
    );
  lines.push(
...
export function buildConnectBlock(p: {
  agent: AgentId;
  name: string;
  url: string;
  token: string | null;
  ...
  if (p.oauth) {
    ...
    return p.agent === "generic"
      ? genericOAuthBlock({ oauth: p.oauth })
      : perplexityOAuthBlock({ oauth: p.oauth });
  }
  switch (p.agent) {
    case "claude-code":
      return claudeBlock(p);
    case "codex":
      return codexBlock(p);
    case "perplexity":
      return perplexityBearerBlock(p);
    case "generic":
      return genericBearerBlock(p);
  }
}

export function buildJson(p: {
  url: string;
  name: string;
  agent: AgentId;
  token: string | null;
...
function defaultRegisterOAuthClient(name: string, scopes: string): RegisterResult {
  ...
  if (!clientId || !clientSecret) {
    return {
      ok: false,
      message: "could not parse client_id/client_secret from register-client output",
    };
  }
  ...
  for (; i < args.length; i++) {
    ...
    switch (a) {
      ...
      case "--show-token":
        ...
        break;
      case "--token": {
        const v = takeValue("--token");
        if (v !== undefined) out.token = v;
        break;
      }
      case "--client-id": {
        const v = takeValue("--client-id");
        if (v !== undefined) out.clientId = v;
        break;
      }
      case "--client-secret": {
        const v = takeValue("--client-secret");
        if (v !== undefined) out.clientSecret = v;
        break;
      }
      case "--scopes": {
        const v = takeValue("--scopes");
        if (v !== undefined) out.scopes = v;
        break;
      }
      case "--name": {
        const v = takeValue("--name");
...
function resolveOAuthCreds(f: ParsedFlags, url: string, deps: ConnectDeps): OAuthCreds {
  ...
  if (f.clientId || f.clientSecret) {
    fail(
      "--oauth needs BOTH --client-id and --client-secret (or use --register to mint a client)."
    );
  }
  if (f.register) {
    ...
    if (!r.ok) {
      fail(
        `Could not register an OAuth client (run this on the brain host where the DB lives): ${r.message}\n` +
...
export async function runConnect(args: string[], deps: ConnectDeps = defaultDeps): Promise<void> {
  ...
  if (f.oauth) {
    if (!spec.supportsOAuth) {
      fail(
        `--oauth (client credentials) is for connector-style agents (${AGENT_IDS.filter((a) => AGENT_SPECS[a].supportsOAuth).join(", ")}). ${spec.label} uses the bearer path — drop --oauth.`
      );
    }
    if (f.install) {
      fail(
        `--install is not supported with --oauth. ${spec.label} is configured through its UI; this prints the OAuth connector fields to paste.`
      );
    }
  ...
  if (!f.install) {
    if (f.json) {
      console.log(
        JSON.stringify(
          buildJson({ url, name: f.name, agent: f.agent, token, showToken: f.showToken }),
          null,
          2
        )
      );
  ...
  if (!spec.installable) {
    fail(
      `--install supports claude-code and codex. ${spec.label} is set up through its own UI — drop --install to print the setup steps.`
    );
  }
  const binary = spec.binary as string; // 'claude' | 'codex'
  if (!deps.hasBinary(binary)) {
    fail(
      `${spec.label} CLI ('${binary}') not found on PATH. Install ${spec.label}, or drop --install to print the command to run manually.`
    );
  }
  ...
  if (exists && !f.force) {
    fail(
      `An MCP server named '${f.name}' already exists in ${spec.label}. Run '${binary} mcp remove ${f.name}' first, pass --name <other>, or --force to replace it.`
    );
  }
  ...
  if (!f.yes) {
    if (!deps.isTTY()) {
      ...
      fail(
        "--install in a non-interactive shell requires --yes (refusing to register a credential-bearing MCP server without confirmation)."
      );
    }
  ...
  if (exists && f.force) {
    ...
    if (rm.code !== 0) {
      fail(
        `Could not replace existing server '${f.name}': ${redactToken(rm.stderr || rm.stdout, realToken)}`
      );
    }
  ...
  if (add.code !== 0) {
    const note = removedExisting
      ? ` (note: the previous '${f.name}' was already removed — re-run to restore it)`
      : "";
  ...
  if (f.agent === "codex" && deps.env(ENV_VAR) !== realToken) {
    console.error(
      `Codex reads the token from $${ENV_VAR} at runtime. Add this to your shell profile so new sessions can reach the brain:`
    );
