...
// This is a public, unauthenticated, rate-limited endpoint — bound every
// field so a malicious caller can't smuggle an oversized payload (long
// message arrays/strings) into the prompt-building + lead-storage path.
...
const advisorContextSchema = z.object({
  ...
  fields: z
    .object({
      ...
      hosting: z.string().max(60).optional(),
      timeline: z.string().max(60).optional(),
      email: z.string().max(200).optional(),
    })
    .partial()
    .optional(),
  messages: z.array(advisorMessageSchema).max(50).optional(),
...
function extractFields(
  ...
  if (!next.teamSize) {
    const seatMatch = lower.match(
      /(\d{1,4})\s*(seat|seats|user|users|people|employees|brokers|lawyers|consultants|advisors|personen|mitglieder|leute|anwälte|berater|makler|team)/
    );
    if (seatMatch) next.teamSize = seatMatch[1];
    else if (/^\s*25\+\s*$/.test(text)) next.teamSize = "25";
    else if (/^\s*(\d{1,3})\s*[-–]\s*(\d{1,3})\s*$/.test(text))
      next.teamSize = text.match(/\d{1,3}\s*$/)?.[0] ?? text;
    else if (/^\s*\d{1,4}\s*$/.test(text)) next.teamSize = text.trim();
  }
  if (!next.hosting) {
    if (/(self.?host|on.?prem|eigene hardware|own infrastructure|eigene infra)/i.test(text))
      next.hosting = "self-hosted";
  ...
  if (!next.useCase) {
    if (/(search|suche|finden|knowledge|wissen|gedächtnis)/i.test(text))
      next.useCase = "knowledge search";
...
function planFor(
  ...
  if (
    seats >= 25 ||
    hosting === "self-hosted" ||
    /sso|saml|scim|on.?prem|soc|iso|enterprise/.test(text)
  )
    return "enterprise";
...
function scoreLead(
  ...
  const filled = ["industry", "teamSize", "useCase", "hosting", "timeline", "email"].filter(
    (key) => fields[key as FieldKey]
  ).length;
...
function missingQuestion(
  ...
  if (!industry)
    return lang === "de" ? "Welche Branche passt am ehesten?" : "Which industry fits you best?";
  if (!fields.useCase)
    return lang === "de"
      ? "Was ist euer dringendster Schmerz: Suche, Onboarding, Compliance, Fristen oder Dokumentenchaos?"
      : "What is the urgent pain: search, onboarding, compliance, deadlines or document chaos?";
  if (!fields.teamSize)
    return lang === "de"
      ? "Wie groß ist das Team, das mit dem Brain arbeiten soll?"
      : "How many people should work with the brain?";
  if (!fields.hosting)
    return lang === "de"
      ? "Bevorzugt ihr EU-Cloud oder On-Premise?"
      : "Do you prefer EU cloud, self-hosting or on-prem?";
  if (!fields.timeline)
    return lang === "de"
      ? "Geht es um sofort starten, dieses Quartal oder nur Evaluierung?"
      : "Are you looking to start now, this quarter, or just evaluating?";
  return null;
}
...
function chipsFor(
  ...
  if (!industry) return lang === "de" ? ["Kanzlei"] : ["Law firm"];
  if (!fields.useCase)
    return lang === "de"
  ...
  if (!fields.hosting)
    return lang === "de" ? ["EU-Cloud", "On-Premise"] : ["EU Cloud", "Self-hosted", "On-prem"];
  if (!fields.timeline)
    return lang === "de"
      ? ["Sofort", "Dieses Quartal", "Nur evaluieren"]
      : ["Now", "This quarter", "Just evaluating"];
  return lang === "de"
    ? ["Mit dieser Konfiguration starten", "Enterprise sprechen", "Vergleich ansehen"]
    : ["Start with this setup", "Talk enterprise", "See comparison"];
}
...
function answerProductQuestion(lang: Lang, text: string, industry: string | null): string | null {
  ...
  if (/(price|pricing|preis|kosten|plan|tarif)/.test(lower)) {
    return lang === "de"
      ? `Kurz: Open Source ist kostenlos, Pro startet bei ${PRICING.de.tiers[1].price}/Monat, Team bei ${PRICING.de.tiers[2].price}/Monat. Für SSO, On-Premise, 25+ Nutzer oder regulierte Rollouts ist Enterprise richtig.`
  ...
  if (/(security|privacy|gdpr|dsgvo|hosting|self.?host|on.?prem|daten)/.test(lower)) {
    return lang === "de"
      ? "Security-Logik: Public Chat sieht keine Mandantendaten. Für das Produkt selbst habt ihr die Wahl zwischen On-Premise-Betrieb auf eigener Infrastruktur oder gemanagter EU-Cloud. Team-Zugriffe sind pro Nutzer berechtigt."
  ...
  async (_req, validatedBody) => {
      ...
      plan === "enterprise"
        ...
        : plan === "team"
          ...
          : plan === "pro"
            ? "Pro"
            : "Open Source";
    const signupIndustry = industry
      ? `?industry=${encodeURIComponent(industry)}${plan === "team" ? "&plan=team" : ""}`
      : "";
    const ctaHref =
      plan === "enterprise"
    ...
    const reply = [
      ...
      question
        ? question
        : lang === "de"
          ? "Du kannst mit dieser Konfiguration starten oder Enterprise direkt anschreiben."
          : "You can start with this setup or talk to Enterprise directly.",
      privacy,
    ].join("\n\n");
    const transcript = [
      ...(body.messages ?? []),
    ...
    return NextResponse.json({
      ...
      capture: {
        ...
        message: savedLead
          ? lang === "de"
            ? "Gespeichert. Wir melden uns mit Kontext."
            : "Saved. We'll follow up with context."
          : fields.email
            ? lang === "de"
              ? "E-Mail erkannt. Mit Zustimmung kann ich diesen Verlauf ans Team geben."
              : "Email detected. With consent, I can pass this conversation to the team."
            : lang === "de"
              ? "Wenn du möchtest, gib eine E-Mail an und aktiviere die Übergabe."
              : "If you want follow-up, share an email and enable handoff.",
      },
    });
  }
);
