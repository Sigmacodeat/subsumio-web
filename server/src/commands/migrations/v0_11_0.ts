...
async function phaseCMode(opts: OrchestratorOpts): Promise<{
  ...
  if (opts.mode) {
    return {
      phase: { name: "mode", status: "complete", detail: `mode=${opts.mode}` },
      mode: opts.mode,
    };
  }
  ...
  if (existing.minion_mode) {
    return {
      phase: {
        name: "mode",
        status: "complete",
        detail: `mode=${existing.minion_mode} (preserved)`,
      },
      mode: existing.minion_mode,
    };
  }
  ...
  if (opts.yes || !process.stdin.isTTY) {
    console.log(
      "Defaulting minion_mode=pain_triggered (non-interactive). Change with `gbrain config set minion_mode <always|off>`."
    );
    return {
      phase: { name: "mode", status: "complete", detail: "mode=pain_triggered (default)" },
      mode: "pain_triggered",
    };
  }
  ...
  console.log(
    "  [1] always          — route every background agent task through Minions (most durable)"
  );
  console.log(
    "  [2] pain_triggered  — default to native subagents, switch to Minions when pain signals fire (recommended)"
  );
...
function injectAgentsMdMarker(
  path: string,
  opts: OrchestratorOpts
): { injected: boolean; skipReason?: string } {
  ...
  if (content.includes(AGENTS_MD_MARKER))
    return { injected: false, skipReason: "already has marker" };
  ...
  try {
    ...
    if (nowMtime !== beforeMtime) {
      return {
        injected: false,
        skipReason: "file modified between read and write — skipping; re-run to retry",
      };
    }
...
function findCronManifests(opts: OrchestratorOpts): string[] {
  const found: string[] = [];
  for (const scope of hostScopes(opts)) {
    const candidates = [join(scope, "cron", "jobs.json"), join(scope, "cron.json")];
...
function rewriteCronManifest(
  ...
  try {
    ...
    return {
      rewritten: 0,
      todos_emitted: 0,
      skipReason: `malformed JSON: ${e instanceof Error ? e.message : e}`,
    };
  }
  ...
  if (!Array.isArray(entries)) {
    return {
      rewritten: 0,
      todos_emitted: 0,
      skipReason: "no entries array (expected Array or { jobs: [...] })",
    };
  }
  ...
  for (const rawEntry of entries) {
    ...
    const handler =
      (typeof entry.skill === "string" ? entry.skill : undefined) ||
    ...
    try {
      ...
      if (nowMtime !== beforeMtime) {
        return {
          rewritten: 0,
          todos_emitted: 0,
          skipReason: "file modified mid-rewrite — skipping",
        };
      }
      ...
      return {
        rewritten: 0,
        todos_emitted: 0,
        skipReason: `write failed: ${e instanceof Error ? e.message : e}`,
      };
    }
  }
...
export function loadPendingHostWork(): PendingHostWorkEntry[] {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (!trimmed) continue;
    try {
      out.push(JSON.parse(trimmed) as PendingHostWorkEntry);
    } catch {
      /* skip malformed line */
    }
  }
...
async function phaseEHost(opts: OrchestratorOpts): Promise<{
  ...
  return {
    phase: {
      name: "host",
      status: "complete",
      detail: `rewrote ${filesTouched} entries; ${todosEmitted} host-work TODOs emitted`,
    },
...
function phaseFInstall(opts: OrchestratorOpts): OrchestratorPhaseResult {
  ...
  if (opts.noAutopilotInstall)
    return { name: "install", status: "skipped", detail: "--no-autopilot-install" };
  try {
    execSync("gbrain autopilot --install --yes", {
      stdio: "inherit",
      timeout: 60_000,
      env: process.env,
    });
...
async function orchestrator(opts: OrchestratorOpts): Promise<OrchestratorResult> {
  ...
  phases.push({
    name: "record",
    status: opts.dryRun ? "skipped" : "complete",
    detail: `status=${status} (ledger write in runner)`,
  });
  ...
  if (pending_host_work > 0) {
    console.log("");
    console.log(
      `${pending_host_work} host-specific item(s) need your agent's attention before the Minions migration is complete.`
    );
    console.log("");
