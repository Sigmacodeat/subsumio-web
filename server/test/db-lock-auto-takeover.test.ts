...
const EINVAL = () => {
  const e = new Error("weird") as NodeJS.ErrnoException;
  e.code = "EINVAL";
  throw e;
};
const aliveKill = () => {
...
describe("classifyHolderLiveness", () => {
  test("same-host + ESRCH + old → dead_eligible", () => {
    expect(classifyHolderLiveness(123, LOCAL, OLD_MS, { processKill: ESRCH })).toBe(
      "dead_eligible"
    );
  });
  ...
  test("cross-host → cross_host (never probe a remote pid)", () => {
    expect(classifyHolderLiveness(123, "some-other-host", OLD_MS, { processKill: ESRCH })).toBe(
      "cross_host"
    );
  });
