...
describe("resolveSyncHardDeadline", () => {
  test("--no-hard-deadline disables everything (even with --timeout)", () => {
    const r = resolveSyncHardDeadline(["--source", "x", "--timeout", "60", "--no-hard-deadline"], {
      isTty: false,
    });
    expect(r).toBeNull();
  });
  ...
  test("--hard-deadline accepts s/m/h suffix", () => {
    expect(resolveSyncHardDeadline(["--hard-deadline", "2m"], { isTty: true })?.deadlineMs).toBe(
      120_000
    );
  });
  ...
  test("--timeout (single-source) auto-arms the hard backstop", () => {
    const r = resolveSyncHardDeadline(["--source", "briefings", "--timeout", "480"], {
      isTty: false,
    });
  ...
  test("env GBRAIN_SYNC_MAX_RUNTIME_SECONDS sets the deadline", () => {
    const r = resolveSyncHardDeadline([], {
      isTty: true,
      env: { GBRAIN_SYNC_MAX_RUNTIME_SECONDS: "900" },
    });
    expect(r).toEqual({
      deadlineMs: 900_000,
      graceMs: GRACE_MS,
      reason: "env:GBRAIN_SYNC_MAX_RUNTIME_SECONDS",
    });
  });
  ...
  test("env 0 disables (overrides the non-TTY default)", () => {
    const r = resolveSyncHardDeadline([], {
      isTty: false,
      env: { GBRAIN_SYNC_MAX_RUNTIME_SECONDS: "0" },
    });
    expect(r).toBeNull();
  });
