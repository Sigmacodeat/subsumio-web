...
describe("schema-pack vocab extension (D2=B)", () => {
  const vocab: RelationVocab = {
    extraVerbs: [
      { verb: "related to|associated with", linkTypes: ["related_to"], direction: "both" },
    ],
  };

  test("extra verb extends detection", () => {
    expect(parseRelationalQuery("who related to widget-co", vocab)!.linkTypes).toEqual([
      "related_to",
    ]);
  ...
  test("validateVocab throws on unknown link_type", () => {
    expect(() =>
      validateVocab({
        extraVerbs: [{ verb: "pwns", linkTypes: ["not_a_real_edge"], direction: "in" }],
      })
