      ...
      onBatch: ({ batch, lastId, rowsTouched, cumulative }) => {
        ...
        if (batch % 10 === 0) {
          process.stderr.write(
            `  [backfill] batch ${batch} | last_id=${lastId} | examined=${cumulative} | updated_so_far=${totalUpdated}\n`
          );
        }
      },
    });
...
async function orchestrator(opts: OrchestratorOpts): Promise<OrchestratorResult> {
  ...
  const status: "complete" | "partial" | "failed" = c.status === "failed" ? "partial" : "complete";
