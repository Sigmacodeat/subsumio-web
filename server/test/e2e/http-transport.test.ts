...
describeE2E("http-transport E2E (real Postgres)", () => {
  ...
  beforeAll(async () => {
    ...
    await conn.unsafe("INSERT INTO access_tokens (name, token_hash) VALUES ($1, $2)", [
      validTokenName,
  ...
  test("5. last_used_at debounce: two consecutive valid calls → only one UPDATE within 60s", async () => {
    ...
    await conn.unsafe("UPDATE access_tokens SET last_used_at = NULL WHERE name = $1", [
      validTokenName,
    ]);
    ...
    const [row1] = (await conn.unsafe("SELECT last_used_at FROM access_tokens WHERE name = $1", [
      validTokenName,
    ...
    const [row2] = (await conn.unsafe("SELECT last_used_at FROM access_tokens WHERE name = $1", [
      validTokenName,
    ])) as { last_used_at: Date | null }[];
  ...
  test("6. last_used_at debounce: simulating 65s gap → second request DOES update", async () => {
    ...
    const [before] = (await conn.unsafe("SELECT last_used_at FROM access_tokens WHERE name = $1", [
      validTokenName,
    ])) as { last_used_at: Date | null }[];
    ...
    const [after] = (await conn.unsafe("SELECT last_used_at FROM access_tokens WHERE name = $1", [
      validTokenName,
    ])) as { last_used_at: Date | null }[];
  ...
  test("7. mcp_request_log gets a row per request", async () => {
    const conn = getConn();
    const beforeRows = (await conn.unsafe("SELECT count(*)::int AS n FROM mcp_request_log")) as {
      n: number;
    }[];
    ...
    const afterRows = (await conn.unsafe("SELECT count(*)::int AS n FROM mcp_request_log")) as {
      n: number;
    }[];
