...
| 2 | B | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`
    ...
    expect(after2.rows.map((r: { fact: string }) => r.fact)).toEqual(
      after1.rows.map((r: { fact: string }) => r.fact)
    );
...
| 2 | B | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`
    ...
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | A | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`)
    );
  ...
  test("page with no facts fence → DB facts for that page wiped (empty fence reconciles to empty index)", async () => {
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | seeded | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`)
    );
    await runExtractFacts(engine, { slugs: ["people/alice"] });
  ...
  test("walks every brain page when no slugs filter is provided", async () => {
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | A1 | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`)
    );
    await putPage(
      "companies/acme",
      FACT_FENCE(`| 1 | C1 | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`)
    );
...
describe("runExtractFacts — empty-fence guard (Codex R2-#7)", () => {
  test("refuses to run when legacy v0.31 rows are pending the v0_32_2 backfill", async () => {
    ...
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | new fact | fact | 1.0 | world | high | 2026-01-01 |  | s |  |`)
    );
  ...
  test("guard releases when all legacy rows have been backfilled", async () => {
    ...
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | F1 | fact | 1.0 | world | high | 2026-01-01 |  | s |  |`)
    );
  ...
  test("NULL entity_slug legacy rows do NOT trigger the guard (they are structurally unfenceable)", async () => {
    ...
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | F | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`)
    );
...
describe("runExtractFacts — multi-source isolation", () => {
  test("deleteFactsForPage scoping does not affect other sources", async () => {
    ...
    await putPage(
      "people/alice",
      FACT_FENCE(`| 1 | default fact | fact | 1.0 | world | medium | 2026-01-01 |  | s |  |`)
    );
...
describe("runExtractFacts — empty-slugs guard (v0.36.x #1096 regression)", () => {
  test("slugs:[] returns immediately without a full-brain walk", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await putPage(
        `people/seed-${i}`,
        FACT_FENCE(`| 1 | Seed ${i} | fact | 1.0 | world | high | 2017-01-01 |  | seed |  |`)
      );
    }
  ...
  test("slugs:undefined still triggers full-brain walk (regression guard for the other side of the bug)", async () => {
    await putPage(
      "people/unscoped-walk",
      FACT_FENCE(`| 1 | Unscoped fact | fact | 1.0 | world | high | 2017-01-01 |  | seed |  |`)
    );
  ...
  test('slugs:["a"] walks just the one slug, no full-brain fallback', async () => {
    await putPage(
      "people/just-this-one",
      FACT_FENCE(`| 1 | Just one fact | fact | 1.0 | world | high | 2017-01-01 |  | seed |  |`)
    );
    await putPage(
      "people/sibling",
      FACT_FENCE(`| 1 | Sibling fact | fact | 1.0 | world | high | 2017-01-01 |  | seed |  |`)
    );
