...
test.describe("CLM Flow: Drafting", () => {
  ...
  test("generate contract draft via AI", async ({ page }) => {
    ...
    // Select template
    const templateButton = page.getByRole("button", { name: /service agreement|dienstleistungsvertrag/i });
  ...
  test("extract obligations from contract", async ({ page }) => {
    ...
    const createRes = await page.context().request.post("/api/pages", {
      data: {
        ...
        type: "contract",
        content: "Party A shall pay Party B €10,000 by 2026-12-31. Contract renews automatically on 2027-01-01.",
        frontmatter: {
          parties: { a: "Party A", b: "Party B" },
          contract_type: "service",
          status: "signed",
    ...
    const extractButton = page.getByRole("button", { name: /extract obligations|verpflichtungen extrahieren/i });
  ...
  test("complete CLM flow from intake to renewal", async ({ page }) => {
    ...
    const createRes = await page.context().request.post("/api/pages", {
      data: {
        ...
        type: "contract",
        content: "Service agreement between Party A and Party B. Payment due 2026-12-31. Renews 2027-01-01.",
        frontmatter: {
          parties: { a: "Party A", b: "Party B" },
          contract_type: "service",
          status: "draft",
