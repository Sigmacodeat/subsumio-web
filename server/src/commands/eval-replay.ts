...
function summarize(results: RowResult[]): ReplaySummary {
  ...
  const meanJaccard =
    eligible.length === 0 ? 0 : eligible.reduce((a, r) => a + r.jaccard, 0) / eligible.length;
  const top1Rate =
    eligible.length === 0 ? 0 : eligible.filter((r) => r.top1Match).length / eligible.length;
  ...
  const over2x = eligible.filter((r) => {
    ...
    return (
      captured &&
      captured.current_latency_ms > 2 * (captured.current_latency_ms - captured.latency_delta_ms)
    );
  }).length;
...
function printHumanSummary(
  ...
  console.log(
    `Replayed ${eligible} of ${total} captured queries (${summary.rows_skipped} skipped, ${summary.rows_errored} errored)`
  );
  ...
  console.log(
    `Mean latency Δ:    ${sign}${summary.mean_latency_delta_ms.toFixed(0)}ms (current vs captured)`
  );
...
export async function runEvalReplay(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!opts.json) {
    ...
    try {
      ...
      const cappedCount =
        opts.limit && opts.limit > 0 ? Math.min(allRows.length, opts.limit) : allRows.length;
      console.error(
      ...
      /* swallow; replayCore will throw with the same message */
    }
  }
