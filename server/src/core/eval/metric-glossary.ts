...
export const METRIC_GLOSSARY: Readonly<Record<string, Readonly<MetricGlossEntry>>> = Object.freeze({
  ...
  "precision@k": Object.freeze({
    ...
    eli10:
      "Of the top k results the engine returned, what fraction were actually relevant? High precision means few junk results in the top of the list.",
  ...
  "recall@k": Object.freeze({
    ...
    eli10:
      "Of all the relevant results that exist in the brain, what fraction did the engine find in its top k? High recall means few missed answers.",
    range:
      "0..1, higher is better. R@10 = 0.81 means out of every 100 questions, the right answer was in the top 10 for 81 of them.",
  }),
  mrr: Object.freeze({
    industry_term: "Mean Reciprocal Rank (MRR)",
    eli10:
      "On average, how far down the list is the FIRST relevant result? An MRR of 1.0 means the first hit is always right; an MRR of 0.5 means it's typically at rank 2.",
    range:
      "0..1, higher is better. Computed as the average of 1/rank-of-first-relevant-result across all test queries.",
  }),
  "ndcg@k": Object.freeze({
    industry_term: "Normalized Discounted Cumulative Gain at k (nDCG@k)",
    eli10:
      "Like precision@k, but the engine gets MORE credit for putting good results near the top than near rank k. A perfect ordering scores 1.0; a totally random ordering scores near 0.",
    range:
      '0..1, higher is better. nDCG@10 above 0.65 is the common "ship it" threshold for hybrid retrieval on technical corpora.',
  }),
  ...
  "hit@1": Object.freeze({
    ...
    eli10:
      'Fraction of queries where the right page is the very first result. NamedThingBench hard-gates title-substring Hit@1 >= 0.95 and alias Hit@1 >= 0.98 — a query that is a page\'s name or title phrase should land it at rank 1, not "somewhere in the top 10".',
    range: "0..1, higher is better.",
  }),
  "hit@3": Object.freeze({
    industry_term: "Hit rate at 3 (Hit@3)",
    eli10:
      "Fraction of queries where the right page is in the top 3 results. NamedThingBench requires the multi-chunk-dilution family to hit 1.0 — a page with one strong chunk among many weak ones must never be buried.",
    range: "0..1, higher is better.",
  }),
  avg_rank1_score: Object.freeze({
    industry_term: "Average rank-1 match score",
    eli10:
      "The mean base (pre-boost) retrieval score of the TOP result across recent searches, from `gbrain search stats`. It is NOT a labeled accuracy number — it is a drift signal: if this trends DOWN over time, retrieval quality is regressing (the early warning that would have caught the duplicate-page incident before a human did).",
    range:
      "0..1. Watch the trend, not the absolute value; pair with the <0.6 / 0.6-0.85 / >=0.85 bucket counts for shape.",
  }),
  create_safety: Object.freeze({
    industry_term: "Create-safety hint (evidence contract)",
    eli10:
      "A result's answer to \"is this page already in the brain — safe to NOT write a new one?\" Derived from the strongest evidence, NOT a raw score: exists (alias_hit / exact_title_match / high_vector_match — do not duplicate), probable (solid keyword match — prefer updating), unknown (weak match — look closer). An agent keys its don't-duplicate decision off this, which is what prevents the incident's duplicate-stub class.",
  ...
  "jaccard@k": Object.freeze({
    ...
    eli10:
      "How much do two result lists overlap? Compare the top k slugs from the captured baseline against the current run; Jaccard@10 = 1.0 means perfect agreement, 0.0 means zero overlap.",
    range:
      "0..1, higher = more stable. Below 0.5 on a stable corpus means retrieval changed significantly.",
  }),
  top1_stability: Object.freeze({
    industry_term: "Top-1 stability rate",
    eli10:
      "Fraction of queries where the #1 result is the same between two runs. The most aggressive stability check — small ranking shifts that don't change the top answer don't hurt it.",
    range:
      "0..1, higher = more stable. Above 0.85 typically means safe-to-merge for retrieval changes.",
  }),
  ...
  p_value: Object.freeze({
    ...
    eli10:
      "How likely the observed difference between two modes is just noise. Lower = stronger evidence the difference is real. We compute paired bootstrap with 10,000 resamples and Bonferroni correction across the 12 comparisons (3 modes × 4 metrics).",
    range:
      '0..1, lower = stronger signal. Below 0.05 is the common "statistically significant" threshold; below 0.01 is strong evidence.',
  }),
  confidence_interval: Object.freeze({
    industry_term: "95% Confidence Interval (CI)",
    eli10:
      "The range we're 95% sure the true value falls inside, given the sample we measured. Narrower CI = more reliable estimate. Computed via bootstrap resampling.",
    range:
      "Two-tuple [low, high]. If 0 is inside the CI for a Δ, the difference isn't statistically significant.",
  }),
  ...
  cache_hit_rate: Object.freeze({
    ...
    eli10:
      "Fraction of searches that reused a recent cached answer instead of running fresh. Higher hit rate = lower latency + lower LLM spend, but stale results may slip through if the threshold is too loose.",
    range:
      "0..1, higher generally better. 0.7-0.9 is the sweet spot for a busy brain; above 0.9 may indicate the similarity threshold is too loose.",
  }),
  avg_results: Object.freeze({
    industry_term: "Average results returned",
    eli10:
      "Mean number of search-result rows the engine returned per call. Should be near the active mode's searchLimit unless the brain is small or the budget is dropping results.",
  ...
  avg_tokens: Object.freeze({
    ...
    eli10:
      "Estimated tokens (chars / 4) in the chunk text returned per search call. The direct measure of how much context an agent loop is paying for each search.",
    range:
      "0..tokenBudget. Approximates OpenAI tiktoken count for English; off by ~5-10% for Anthropic and worse for non-English.",
  }),
  cost_per_query_usd: Object.freeze({
    industry_term: "Cost per query (USD)",
    eli10:
      "Sum of LLM + embedding API charges for one search call. Includes Haiku expansion call (tokenmax mode only) + embedding cost + downstream answer-model cost if measured.",
    range:
      "0..unbounded. Conservative mode is typically <\\$0.001 per call; tokenmax with answer-gen can exceed \\$0.01.",
  }),
  p99_latency_ms: Object.freeze({
    industry_term: "p99 latency (ms)",
    eli10:
      "99th percentile wall-clock time per search call. The latency that 1% of users see — long-tail experience, not the average.",
    range:
      "0..unbounded. Warm-cache hits should be <50ms; tokenmax with expansion can exceed 200ms due to the Haiku call.",
  }),
  ...
  "autocut.signal": Object.freeze({
    ...
    eli10:
      "Which signal autocut used to size the result set. 'rerank' means it found a real score cliff in the cross-encoder rerank scores and cut there; 'none' means no trustworthy cliff (no reranker, <2 scored results, or the gap was too small) so it returned the full list.",
    range:
      "'rerank' | 'none'. 'none' is not a failure — it means autocut declined to cut because the signal didn't justify it.",
  }),
  "autocut.gap_ratio": Object.freeze({
    industry_term: "Autocut gap ratio",
    eli10:
      "The size of the largest score drop autocut found, as a fraction of the top result's score. A gap of 0.40 means the score fell by 40% of the top score at the steepest point. Autocut cuts there only when this clears the sensitivity threshold (autocut_jump, default 0.20).",
    range:
      "0..1, higher = a sharper cliff (more confident cut). Below the autocut_jump threshold → no cut.",
  }),
});
