...
async function seedAll9Clusters() {
  const seeds: Array<{ slug: string; type: string; body?: string; fm?: Record<string, unknown> }> =
    [
      ...
      {
        slug: "atoms/partner-1",
        type: "atom-partner-link",
  ...
  for (const s of seeds) {
    await engine.putPage(s.slug, {
      ...
      compiled_truth:
        s.body ??
        "page body that is sufficiently long to pass any minimum-length backstop guards in the codebase",
      timeline: "",
      frontmatter: s.fm ?? {},
