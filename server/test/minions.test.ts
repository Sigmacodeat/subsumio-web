...
describe("MinionQueue: Dependencies", () => {
  ...
  test("child fail → continue policy", async () => {
    ...
    // Mark child as dead
    await engine.executeRaw("UPDATE minion_jobs SET status = 'dead' WHERE id = $1", [child.id]);
  ...
  test("child fail → remove_dep", async () => {
    ...
    const child = await queue.add(
      "sync",
      {},
      { parent_job_id: parent.id, on_child_fail: "remove_dep" }
    );
...
describe("MinionQueue: Depth tracking", () => {
  ...
  test("depth exceeding maxSpawnDepth rejected", async () => {
    ...
    await expect(tightQueue.add("d", {}, { parent_job_id: c2.id })).rejects.toThrow(
  ...
  test("per-submit max_spawn_depth override works", async () => {
    ...
    await expect(
      queue.add("b", {}, { parent_job_id: root.id, max_spawn_depth: 0 })
...
describe("MinionQueue: max_children", () => {
  ...
  test("max_children=N rejects N+1th submit", async () => {
    ...
    await expect(queue.add("c", {}, { parent_job_id: parent.id })).rejects.toThrow(
      /already has 2 live children \(max_children=2\)/
    );
  });
...
describe("MinionQueue: child_done", () => {
  ...
  test("child_done NOT posted if parent already terminal", async () => {
    ...
    await engine.executeRaw("UPDATE minion_jobs SET status = 'cancelled' WHERE id = $1", [
      parent.id,
    ]);
    await queue.claim("tok1", 30000, "default", ["research"]);
    await queue.completeJob(child.id, "tok1", { ok: true });
  ...
  test("readChildCompletions returns only child_done messages", async () => {
    ...
    expect(completions.map((c) => c.child_id).sort((a, b) => a - b)).toEqual(
...
describe("resolveWorkerConcurrency (v0.19.1 H3): clamp + validation", () => {
  ...
  test("flag-unset env=8 → 8", () => {
    expect(
      resolveWorkerConcurrency([], { GBRAIN_WORKER_CONCURRENCY: "8" } as NodeJS.ProcessEnv)
    ).toBe(8);
  });
  test("flag=2 env=8 → 2 (flag wins)", () => {
    expect(
      resolveWorkerConcurrency(["--concurrency", "2"], {
        GBRAIN_WORKER_CONCURRENCY: "8",
  ...
  test('garbage env "foo" → clamped to 1 (H3)', () => {
    expect(
      resolveWorkerConcurrency([], { GBRAIN_WORKER_CONCURRENCY: "foo" } as NodeJS.ProcessEnv)
    ).toBe(1);
  });
  test("env=0 → clamped to 1 (H3 — prevents silent wedge)", () => {
    expect(
      resolveWorkerConcurrency([], { GBRAIN_WORKER_CONCURRENCY: "0" } as NodeJS.ProcessEnv)
    ).toBe(1);
  });
  test("env=-5 → clamped to 1 (H3)", () => {
    expect(
      resolveWorkerConcurrency([], { GBRAIN_WORKER_CONCURRENCY: "-5" } as NodeJS.ProcessEnv)
    ).toBe(1);
  });
});
...
describe("MinionWorker: --max-rss watchdog", () => {
  ...
  test("per-job check: handler bumps RSS, post-job check trips, sibling aborts", async () => {
    ...
    worker.register("slow", async (job) => {
      ...
      job.signal.addEventListener("abort", () => {
        sibling2Aborted = true;
      });
      await new Promise<void>((resolve) => {
        const t = setInterval(() => {
          if (job.signal.aborted) {
            clearInterval(t);
            sibling2Resolved = true;
            resolve();
          }
        }, 20);
      });
  ...
  test("shutdownAbort fires (closes shell-handler zombie gap)", async () => {
    ...
    worker.register("observer", async (job) => {
      ...
      job.shutdownSignal.addEventListener("abort", () => {
        shutdownSignalFired = true;
      });
      await new Promise<void>((resolve) => {
        const t = setInterval(() => {
          if (job.signal.aborted) {
...
describe("connectWithRetry / isRetryableDbConnectError", () => {
  test("isRetryableDbConnectError matches transient patterns", async () => {
    ...
    expect(
      isRetryableDbConnectError(new Error("password authentication failed for user postgres"))
    ).toBe(true);
    ...
    get(target, prop, receiver) {
      if (prop === "executeRaw") {
        return async (sql: string, params?: unknown[]): Promise<unknown[]> => {
          ...
          if (
            overrides.countWaiting &&
            /count\(\*\).*minion_jobs.*WHERE\s+status\s*=\s*'waiting'/is.test(sql)
          ) {
          ...
          return (
            target as unknown as { executeRaw: (s: string, p?: unknown[]) => Promise<unknown[]> }
  ...
  test("inFlight > 0 blocks stall detection (long-running legitimate job)", async () => {
    ...
    const fakeInFlight = (
      worker as unknown as {
        inFlight: Map<
          number,
          { lockTimer: NodeJS.Timeout; abort: AbortController; promise: Promise<void> }
        >;
      }
    ).inFlight;
    const fakeAbort = new AbortController();
    const fakePromise = new Promise<void>(() => {
      /* never resolves */
    });
