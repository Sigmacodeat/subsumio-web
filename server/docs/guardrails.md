...
prompt-injection detector, a PII scrubber — that wants to _observe_ content at
those boundaries.
...
- **Observe-only.** `runGuardrails()` returns `void`. Callers never branch on a
  provider verdict. A guardrail registered through this interface _cannot_
...
- **Content boundaries.** Hooks pass only the ingest/user-facing payload — the
  ...
  input. They never pass system prompts, full chat history, tool _output_, LLM
...
- **Async fan-out.** If you don't want to block ingest on your classifier,
  enqueue inside `classify` and return immediately. The seam awaits _your_
...
Because the verdict is ignored by GBrain, "shadow mode" requires _no_ special
