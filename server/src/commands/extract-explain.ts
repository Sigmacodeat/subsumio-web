...
export async function runExtractExplain(engine: BrainEngine, args: string[]): Promise<void> {
  const json = args.includes("--json");
  // --explain consumes the NEXT positional arg as the kind.
  const explainIdx = args.indexOf("--explain");
  const kindArg =
    explainIdx >= 0 && explainIdx + 1 < args.length
      ? args[explainIdx + 1]
  ...
  if (!spec && !cyclePhaseKinds.has(kindArg)) {
    ...
    if (json) {
      ...
      console.log("");
      console.log(
        `To declare it: gbrain schema scaffold-extractable ${kindArg} --pack ${pack.manifest.name}`
      );
    }
    return;
  }
  ...
  console.log(`Active pack: ${pack.manifest.name}`);
  console.log(
    `Status: ${spec ? "pack-declared extractable" : "built-in cycle phase (no pack declaration)"}`
  );
  console.log("");
  if (spec) {
    console.log("ExtractableSpec:");
    console.log(
      `  prompt_template: ${spec.prompt_template ?? "(none)"} ${promptExists ? "✓" : "(missing)"}`
    );
    console.log(
      `  fixture_corpus:  ${spec.fixture_corpus ?? "(none)"} ${fixtureExists ? "✓" : "(missing)"}`
    );
