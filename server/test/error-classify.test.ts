...
describe("classifyJobError", () => {
  ...
  test("prompt_too_long — Anthropic 400", () => {
    expect(classifyJobError("400 prompt is too long: 1707509 tokens > 1000000 maximum")).toBe(
      "prompt_too_long"
    );
  ...
  test("tool_unavailable — registry config", () => {
    expect(classifyJobError('tool "ghost" is not in the registry for this subagent')).toBe(
      "tool_unavailable"
    );
  ...
  test("tool_permission — capability decision", () => {
    expect(classifyJobError('tool "put_page" permission denied: slug outside namespace')).toBe(
      "tool_permission"
    );
  ...
  test("tool_schema_mismatch — bad args (self-fixable)", () => {
    expect(classifyJobError('invalid input: missing required field "slug"')).toBe(
      "tool_schema_mismatch"
    );
    expect(classifyJobError("tool_use validation failed: param X required")).toBe(
      "tool_schema_mismatch"
    );
  ...
  test("tool_crash — real impl bug (NOT self-fixable)", () => {
    ...
    expect(classifyJobError("tool.execute error: Cannot read property of undefined")).toBe(
      "tool_crash"
    );
  });
